...
class Extractor:
    ...
    def summation(A, tree): B = tree; vars = set(); C = set(); D = A.collect_variables_include(find_from(B.children, 'summation_sub_assignee')); C.update(D.vars); E = A.collect_variables_include(
        B.children); vars.update(E.vars); F = A.collect_variables_include(B.children[-1]); C.update(F.vars); B.children.pop(-1); A.inferred = infer_variable_grouping(vars, C); A.format_vars(B)
