...
class Extractor:
    ...
    def summation(A, tree): B = tree; vars = set(); C = set(); D = A.collect_variables_include(find_from(B.children[2], 'summation_sub_assignee')); C.update(D.vars); E = A.collect_variables_include(B.children[2]); vars.update(
        E.vars); F = A.collect_variables_include(B.children[4]); vars.update(F.vars); G = A.collect_variables_include(B.children[5]); vars.update(G.vars); B.children.pop(6); A.inferred = infer_variable_grouping(vars, C); A.format_vars(B)
