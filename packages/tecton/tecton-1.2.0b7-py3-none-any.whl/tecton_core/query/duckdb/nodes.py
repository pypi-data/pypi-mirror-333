...
from tecton_core.query.node_interface import JoinKeyPartitioning
...
class IcebergOfflineStoreScanNode(OfflineStoreScanNode):
    ...
    def to_arrow_reader(self, context: ExecutionContext) -> "pyarrow.RecordBatchReader":
        ...
        if (
            ...
            s3_options = get_s3_options_for_fd(self.feature_definition_wrapper, self.offline_store_options_providers)
        reader = IcebergReader(fdw=self.feature_definition_wrapper, s3_options=s3_options)
        exec_context = ExecutionContext(self.offline_store_options_providers)
        join_keys_table = self.entity_filter.to_arrow_reader(exec_context).read_all() if self.entity_filter else None
        dataset = reader.read(time_limits=self.partition_time_filter, join_keys_table=join_keys_table)
