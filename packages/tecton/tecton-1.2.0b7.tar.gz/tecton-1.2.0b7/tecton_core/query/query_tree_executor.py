...
class QueryTreeOutput:
    output: Optional[pyarrow.Table] = None
    ...
    def result_df(self) -> pandas.DataFrame:
        ...
        if contains_map_type:
            ...
            return self.output.to_pandas(maps_as_pydicts="strict")

        return self.output.to_pandas()

    @property
    def result_table(self) -> "pyarrow.RecordBatchReader":
        batches = self.output.to_batches()
        return pyarrow.RecordBatchReader.from_batches(self.output.schema, batches)
...
def _execute_stage(
    ...
    if isinstance(compute, ArrowCompute):
        reader = compute.run(output_node_ref, input_nodes_ref, input_data=inputs, context=context, monitor=None)
    elif isinstance(compute, SQLCompute):
        with compute:
            for input_node_ref, input_table in zip(input_nodes_ref, inputs):
                assert isinstance(input_node_ref.node, StagingNode)
...
def convert_plan_to_ray_dag(node_ref: NodeRef, context: ExecutionContext, **kwargs: Any) -> RayDataset:
    ...
    def find_inputs(input_: NodeRef, output: StagingNode) -> Iterable[NodeRef]:
        if (
            ...
            and (input_.node.dialect != output.dialect or input_.node.checkpoint)
        ):
    ...
    child_ray_datasets = [convert_plan_to_ray_dag(input_node, context, **kwargs) for input_node in inputs]
    ...
    if not child_ray_datasets:
        ...
        return RayDataset.from_partition_generator(_partition_generator, SinglePartition())
        ...
        return input_ray_dataset.map(_map_fn)
    ...
    return left.co_group(others, _co_group_fn)
...
class QueryTreeExecutor:
    ...
    def exec_qt(self, logical_plan: NodeRef) -> QueryTreeOutput:
        ...
        # TODO: Should merge partitions once we have something other than a SinglePartition

        return QueryTreeOutput(output=outputs[0])
