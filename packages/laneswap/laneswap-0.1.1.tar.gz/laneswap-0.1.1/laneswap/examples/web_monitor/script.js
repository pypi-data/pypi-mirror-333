...
async function fetchServices() {
  ...
  try {
    // Try first with /api/services path, then fall back to /services if that fails
    let endpoint = `${formattedApiUrl}/api/services`;
    ...
    try {
      ...
      // If the first endpoint returns a 404, try the alternative endpoint
      if (response.status === 404) {
        endpoint = `${formattedApiUrl}/services`;
        console.log(`First endpoint not found, trying: ${endpoint}`);
        response = await fetch(endpoint, {
          method: 'GET',
          headers: {
            'Accept': 'application/json'
      ...
      // If the first fetch fails completely, try the alternative endpoint
      console.log(`First endpoint fetch failed, trying: ${formattedApiUrl}/services`);
      endpoint = `${formattedApiUrl}/services`;
      response = await fetch(endpoint, {
        method: 'GET',
        headers: {
