	...
	,initBaseTrigger: function(){
		...
		this.baseTriggers[1].handler = function(){
            if (!this.readOnly && !this.disabled) {
				...
				/**
				* Добавлен ивент для разделения логики работы кнопки выбора текущей даты,
				* т.к. выбор из календаря вызывает select -> change, ввод с клавиатуры вызывает change,
				* а установка текущей даты select. Из чего нельзя найти такой способ, в котором были бы
				* обработаны все 3 ситуации без двойного срабатывания при выборе из календаря.
				*/
				this.fireEvent('setToday', this, today, oldValue);
            }
		};
