...
from merchantapi.request import ProductListLoadQuery, Module
from merchantapi.response import ProductListLoadQuery as ProductListLoadQueryResponse, Module as ModuleResponse
from merchantapi.version import Version
from . import helper
from . credentials import MerchantApiTestCredentials

helper.configure_logging()
helper.provision_store('Client.xml')
...
def test_client_ssh_authentication_sha256_pcks8_pem():
    ...
    client = SSHClient(MerchantApiTestCredentials.MERCHANT_API_ENDPOINT, 'Administrator', MerchantApiTestCredentials.TEST_DATA_PATH + '/ssh/private_key_no_password.pkcs8.pem', None, SSHPrivateKeyAuthenticator.DIGEST_SSH_RSA_SHA256, {
        'ssl_verify': False,
        'default_store_code': MerchantApiTestCredentials.MERCHANT_API_STORE_CODE
    })
...
def test_client_ssh_authentication_sha512_pcks8_pem():
    ...
    client = SSHClient(MerchantApiTestCredentials.MERCHANT_API_ENDPOINT, 'Administrator', MerchantApiTestCredentials.TEST_DATA_PATH + '/ssh/private_key_no_password.pkcs8.pem', None, SSHPrivateKeyAuthenticator.DIGEST_SSH_RSA_SHA512, {
        'ssl_verify': False,
        'default_store_code': MerchantApiTestCredentials.MERCHANT_API_STORE_CODE
    })
...
def test_client_global_headers():
    ...
    client.set_global_header('Foo', 'Bar')
    client.set_global_header('Bin', 'Baz')
    client.set_global_header('Bar', 'Foo')

    assert client.has_global_header('Foo')
    assert client.has_global_header('Bin')
    assert client.has_global_header('Bar')

    assert client.get_global_header('Foo') == 'Bar'
    assert client.get_global_header('Bin') == 'Baz'
    assert client.get_global_header('Bar') == 'Foo'

    client.remove_global_header('Bar')

    assert client.has_global_header('Bar') is False
    ...
    assert 'Foo' in response.get_http_response().request.headers
    assert 'Bin' in response.get_http_response().request.headers
    assert response.get_http_response().request.headers['Foo'] == 'Bar'
    assert response.get_http_response().request.headers['Bin'] == 'Baz'


def test_user_agent():
    client = helper.init_client()

    client.set_global_header('uSeR-aGeNt', '')

    request = Module(client)

    request.set_module_code('api_test_helper')
    request.set_module_function('System_Vars')

    response = request.send()

    helper.validate_response_success(response, ModuleResponse)
    
    assert response.get_data()['data']['HTTP_USER_AGENT'] == f'MerchantAPI/{Version.STRING} python/{platform.python_version()} {platform.system()}'


def test_user_agent_override():
    client = helper.init_client()

    client.set_global_header('uSeR-aGeNt', 'MyUserAgent')
    ...
    assert response.get_data()['data']['HTTP_USER_AGENT'] == 'MyUserAgent'