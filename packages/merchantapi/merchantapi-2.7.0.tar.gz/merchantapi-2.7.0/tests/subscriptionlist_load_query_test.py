...
def subscription_list_load_query_test_list_load():
	...
	subscription1 = helper.create_subscription(customer, products[0], 'Daily', int(time.mktime(datetime.date.today().timetuple())), addresses[0], card, methods[0].get_module().get_id(), 'SLLQ', 1, attributes)
	subscription2 = helper.create_subscription(customer, products[1], 'Daily', int(time.mktime(datetime.date.today().timetuple())), addresses[0], card, methods[0].get_module().get_id(), 'SLLQ', 1, attributes)
	...
	for subscription in response.get_subscriptions():
		...
		assert subscription.get_method() == "SLLQ"
		assert subscription.get_ship_data() == "SLLQ"
