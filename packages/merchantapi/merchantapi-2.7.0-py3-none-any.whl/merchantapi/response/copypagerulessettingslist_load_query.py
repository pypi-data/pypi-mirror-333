...
class CopyPageRulesSettingsListLoadQuery(ListQueryResponse):
	def __init__(self, request: ListQueryRequest, http_response: HttpResponse, data: dict):
		...
		if 'data' in self.data and 'data' in self.data['data'] and isinstance(self.data['data']['data'], list):
			for i, e in enumerate(self.data['data']['data'], 0):
				self.data['data']['data'][i] = merchantapi.model.CopyPageRulesSettings(e)

	def get_copy_page_rules_settings(self):
		"""
		Get copy_page_rules_settings.

		:returns: list of CopyPageRulesSettings
		"""
