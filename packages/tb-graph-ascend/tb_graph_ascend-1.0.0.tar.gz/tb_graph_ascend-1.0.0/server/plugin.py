...
class GraphsPlugin(base_plugin.TBPlugin):
    ...
    def get_all_screenNodes(self, request):
        ...
        if screen == 'precision_index':
            ...
            if '无匹配节点' in precision_set_str:
                ...
                precision_none = 1
            grouped_screen_set = [
                list(map(float, precision_set_str[i : i + 2])) for i in range(0, len(precision_set_str), 2)
            ]
        else:
    ...
    def get_all_data(self, request):
        ...
        file_path = self._get_first_visFilePath()
        json_data = self._read_json_file(file_path)
    ...
    def check_jsondata(self, request):
        ...
        if graph_data is None:
            ...
            self._current_file_data = graph_data
            ...
            for file in files:
                if file.endswith('.vis'):  # check for .vis extension
                    ...
                    if file_size > constants.MAX_FILE_SIZE:
                        logger.error(
                            f'Error: the vis file "{file_path}" exceeds the maximum limit size of 1GB and will be skipped.'
                        )
                        continue
    ...
    def _get_first_visFilePath(self):
        for _, _, files in os.walk(self.logdir):
            for file in files:
                if file.endswith('.vis'):
