...
from strawberry.federation.schema_directives import Shareable, Key
...
def set_schema(
        ...
        auto_foreign_keys: bool,
) -> None:
    """Set the Strawberry schema for a Graphemy class."""

    # 1) Build up foreign key constraints or do any
    #    other logic you need (like hooking up custom resolvers).
    for attr in [attr for attr in cls.__dict__.values() if hasattr(attr, "dl")]:
        returned_class: "Graphemy" = Setup.classes[attr.dl]
        # If you have foreign key auto-creation logic:
        if (
                attr.foreign_key
                or (
                attr.foreign_key is None
            ...
            # Validate that this isn't a special underscore or int
            if (
                    len(source) > 0
                    and len(target) > 0
                    and not any(
        ...
        # Fill out your "functions" dict if needed
    ...
    # 2) If user has already decorated the class with Strawberry,
    #    skip building a new schema. Just store it and return.
    if hasattr(cls, "__strawberry_definition__"):
        # Possibly add custom resolvers, etc., if you want them on the class
        if cls.__custom_resolvers__:
            for custom_resolver in cls.__custom_resolvers__:
                setattr(
                    cls,
                    custom_resolver.__name__,
                    strawberry.field(resolver=custom_resolver),
                )

        cls.__strawberry_schema__ = cls
        print(cls.__strawberry_schema__.parse_raw())
        return

    # 3) Otherwise, we do the usual dynamic generation (like your existing code).
    class Schema:
        pass

    # For each field in your Graphemy class that has "dl":
    for attr in [attr for attr in cls.__dict__.values() if hasattr(attr, "dl")]:
        returned_class: "Graphemy" = Setup.classes[attr.dl]
        setattr(
            Schema,
    ...
    # If you have resolvers or extra definitions
    if cls.__custom_resolvers__:
        for custom_resolver in cls.__custom_resolvers__:
            setattr(
                Schema,
    ...
    # Build with Strawberry + Pydantic or standard approach:
    # e.g. strawberry_schema = ...

    strawberry_schema = strawberry.experimental.pydantic.type(
        cls, all_fields=True, name=cls.__name__, directives=[Key(fields="id", resolvable=True)]
    )(Schema)

    # Merge with any "extra_schema" if you have that logic:
    # e.g. if extra_schema.__annotations__:
    #     strawberry_schema = merge_types(cls.__name__, (strawberry_schema, extra_schema))
