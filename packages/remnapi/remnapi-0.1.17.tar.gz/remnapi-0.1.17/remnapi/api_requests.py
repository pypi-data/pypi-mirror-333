...
class UserManager:
    ...
    async def update_user(self, uuid, days_to_add=0, **kwargs):
        """
        Обновляет данные пользователя по UUID с добавлением указанного количества дней к сроку действия.

        Параметры:
        uuid (str): UUID пользователя для обновления.
        days_to_add (int): Количество дней для добавления к сроку действия.
        **kwargs: Дополнительные параметры для обновления пользователя.

        Возможные параметры:
        - status (str): Статус пользователя (например, "ACTIVE").
        - trafficLimitBytes (int): Лимит трафика в байтах.
        - trafficLimitStrategy (str): Стратегия лимита трафика (например, "NO_RESET").
        - activeUserInbounds (list): Активные входящие подключения.
        - expireAt (str): Дата истечения срока действия (ISO 8601).
        - description (str): Описание пользователя.
        - telegramId (int): Telegram ID пользователя.
        ...
        user_data = await self.get_user(uuid, all=True)
        current_expire_at = user_data.get("expireAt", datetime.now(timezone.utc).isoformat())

        # Добавляем указанное количество дней к текущей дате истечения
        new_expire_at = (datetime.fromisoformat(current_expire_at.rstrip('Z')) + timedelta(days=days_to_add)).strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3] + "Z"
        ...
        user = await self.users.update_user(uuid, **default_params)
