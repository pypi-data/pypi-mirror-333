...
types_collection = _tc # 0.3.41
util = _util # 0.3.41

_Any = types_collection.Any
...
def _get_all_params(f: _T_func, /):
    ...
    else: # replaced with inspect.get_annotations (>= 3.10) in 0.3.41
        return [k for k in f.__code__.co_varnames if k not in ("self", "return")]
...
class Tense( # 0.3.24
    ...
    def isFinalProperty(self, v, /, *_, mode = _MODE_AND):
        ...
        # For now, there is no formal way checking, how property is final.
    ...
    def any(self, i: _tc.Iterable[_T], /, condition: _cal[[_T], bool] = ...):
        ...
        # 0.3.40
        _cond = condition if not self.isEllipsis(condition) else bool.__new__
