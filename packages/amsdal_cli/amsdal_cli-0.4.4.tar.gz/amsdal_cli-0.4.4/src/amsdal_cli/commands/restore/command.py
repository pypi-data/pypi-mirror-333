...
def _fetch_model_pks_and_fks(
    ...
    for schema in schemas:
        pk_data[schema[PRIMARY_PARTITION_KEY]] = _build_pks(schema, all_schemas=schemas)
        fk_data[schema[PRIMARY_PARTITION_KEY]] = _build_fks(schema, all_schemas=schemas)

    return pk_data, fk_data


def _build_pks(
    schema: dict[str, Any], all_schemas: list[dict[str, Any]]
) -> dict[str, type | glue.Schema | glue.SchemaReference]:
    from amsdal_data.connections.constants import PRIMARY_PARTITION_KEY

    pks: dict[str, type | glue.Schema | glue.SchemaReference] = {}
    for pk, fields in schema['primary_key'].items():
        if fields == [PRIMARY_PARTITION_KEY]:
            pks[pk] = str
            continue

        prop = schema['properties'][pk]
        ...
        if is_model_type(_type, all_schemas):
            for _type_schema in all_schemas:
                if _type_schema[PRIMARY_PARTITION_KEY] == _type:
                    _nested_pks = _build_pks(_type_schema, all_schemas)

                    for _nested_pk in _nested_pks:
                        pks[f'{pk}_{_nested_pk}'] = _nested_pks[_nested_pk]
            ...
            for field in fields:
                ...
                pks[f'{pk}_{field}'] = object_schema_type_to_glue_type(prop['type'])
...
def _build_fks(
    ...
    for prop_name, prop in schema['properties'].items():
        ...
        if is_model_type(_type, all_schemas):
            fks[prop_name] = (
                _build_fields(prop_name, _type, all_schemas),
                _get_db_table(_type, all_schemas),
                _get_pk_fields(_type, all_schemas),
            )
    return fks


def is_model_type(_type: str, schemas: list[dict[str, Any]]) -> bool:
    from amsdal_data.connections.constants import PRIMARY_PARTITION_KEY
...
def _build_fields(prop_name: str, model_name: str, all_schemas: list[dict[str, Any]]) -> dict[str, str]:
    from amsdal_data.connections.constants import PRIMARY_PARTITION_KEY

    for schema in all_schemas:
        if schema[PRIMARY_PARTITION_KEY] == model_name:
            db_fields = {}

            for pk, fields in schema['primary_key'].items():
                if fields == [PRIMARY_PARTITION_KEY]:
                    db_fields[f'{prop_name}_{pk}'] = CoreTypes.STRING.value
                    continue
                ...
                if is_model_type(_type, all_schemas):
                    _nested_fields = _build_fields(pk, _type, all_schemas)

                    for _nested_field in _nested_fields:
                        db_fields[f'{prop_name}_{_nested_field}'] = _nested_fields[_nested_field]
                else:
                    for field in fields:
                        db_fields[f'{prop_name}_{field}'] = prop['type']
...
def _get_db_table(model_name: str, all_schemas: list[dict[str, Any]]) -> str:
    from amsdal_data.connections.constants import PRIMARY_PARTITION_KEY
    ...
    msg = f'No schema found for model {model_name}'
    raise ValueError(msg)


def _get_pk_fields(model_name: str, all_schemas: list[dict[str, Any]]) -> list[str]:
    from amsdal_data.connections.constants import PRIMARY_PARTITION_KEY

    for schema in all_schemas:
        if schema[PRIMARY_PARTITION_KEY] == model_name:
            return schema['primary_key'].keys()
        ...
        schemas.append(
            (
                glue.SchemaReference(
                    ...
                    metadata={
                        META_PRIMARY_KEY: list(schema_data.get('primary_key', {}).keys()),
                        META_PRIMARY_KEY_FIELDS: model_pks.get(schema_data['title'], {}),
                        META_CLASS_NAME: schema_data.get('table_name'),
                    },
                ),
