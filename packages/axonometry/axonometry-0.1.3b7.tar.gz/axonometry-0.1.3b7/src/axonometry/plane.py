...
from .utils import pair_projections_lines, pair_projections_points
...
class ReferencePlane(Plane):
    ...
    def load_svg_file(self, file: str, scale: float | None = None):
        ...
        for line in svg_lines.lines:
            if line not in pen_paths:
                start, end = {
                    ...
                    "yz": (
                        Point(z=line.start.x, y=line.start.y),
                        Point(z=line.end.x, y=line.end.y),
                    ),
                    "zx": (
