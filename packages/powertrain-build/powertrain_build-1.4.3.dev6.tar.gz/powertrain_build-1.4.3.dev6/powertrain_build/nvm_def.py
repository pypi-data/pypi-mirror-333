...
class ZCNVMDef(NVMDef):
    ...
    def _append_nvm_rte_function_calls(self):
        ...
        error_status = ""
        rte_write_instructions = ""
        nvsw_init_instructions = ""
        manual_signals = []
        ...
        for nvm_name, nvm_data in self.valid_nvm_definitions.items():
            if nvm_data["method"] == "DIRECT-CALL":
                ...
                nvsw_init_instructions = "  // NVSWCOMPONENTS init calls are scheduled by the arxml.\n"
                if nvm_data["type"] == "type1":
                    main_function_lines.append(f"  const {nvm_name.upper()} {nvm_name.lower()}_const = {nvm_name.lower()};\n")  
                    main_function_lines.append(f"  (void)Rte_Write_{nvm_name.upper()}_{nvm_name.upper()}({nvm_name.lower()}_const);\n")
                elif nvm_data["type"] == "type2":
                    rte_write_instructions = "  // Nvm write calls are not generated for type2, they need to be added manually to src.\n  // Nvm signals:\n"
                    manual_signals.append(f"  // Nvm {nvm_name.lower()} is type 2 and needs a manually written write call in source code.\n")
        init_function_lines.append(nvsw_init_instructions)
        main_function_lines.append(error_status)
        main_function_lines.extend(manual_signals)

        if len(manual_signals) > 0:
            self.warning('NVSWCOMPONENT write calls are not generated automatically for type 2, they have to be written manually and added to source code.')
