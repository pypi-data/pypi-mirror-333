...
def test_features() -> None:
    ...
    """
    assert defs.FEATURES["docker-scry"] == defs.VERSION
    for value in (value for name, value in defs.FEATURES.items() if name != "docker-scry"):
...
def test_features_run() -> None:
    ...
    test_program: Final = pathlib.Path(sys.executable).parent / "docker-scry"
...
def test_docker_plugin_run() -> None:
    """Run the command-line tool with the Docker plugin metadata option, see what happens."""
    test_program: Final = pathlib.Path(sys.executable).parent / "docker-scry"
    mdata: Final = json.loads(
        subprocess.check_output(  # noqa: S603
            [test_program, "docker-cli-plugin-metadata"],
            encoding="UTF-8",
        ),
    )
    assert mdata["Version"] == defs.VERSION
