...
class RLmodel_bids(RLModelBase):
    ...
    def predict_action(self):
        ...
        # If state not found, use KDTree to find the nearest neighbor
        if state_index == -1:
            # Pad states to the same length
            max_length = max(len(state) for state, _ in loaded_state_to_index)
            state_tuples = [tuple(list(state) + [0] * (max_length - len(state))) for state, _ in loaded_state_to_index]

            # Create a KDTree from the padded states
            kdtree = KDTree(state_tuples)

            # Pad the current state to the same length
            padded_state = tuple(list(state_tuple) + [0] * (max_length - len(state_tuple)))

            # Find the nearest neighbor to the current state
            distance, index = kdtree.query(padded_state)
            nearest_state_tuple = state_tuples[index]

            # Round the nearest neighbor to the nearest integer and convert to tuple
            nearest_state_tuple = tuple(np.round(nearest_state_tuple).astype(int))

            # Get the index for the nearest state
            state_index = state_to_index_dict.get(nearest_state_tuple, -1)
        ...
        try:
            ...
            print(f"Action '{action}' not found in transition matrix. Using default transition.")
            trans_proba = {}
