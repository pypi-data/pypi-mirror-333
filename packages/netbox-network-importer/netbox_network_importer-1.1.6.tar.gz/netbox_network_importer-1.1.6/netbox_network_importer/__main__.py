...
@cli.command()
@click.option('--devices', '-d',  type=str, multiple=True, help="Run on specificied devices", required=True)
@logger.catch
# CLI callable command
def synchronize_device(devices):
    ...
    result_data = run_synchronizer(devices, [], False)
    return result_data
...
def run_synchronizer(devices, platforms, no_progress_bar=True):
    ...
    try:
        ...
        logger.error(
            f"Unable to create PyATS testbed structure through Netbox: {e}")
        logger.error("Exiting...")
        sys.exit(1)
