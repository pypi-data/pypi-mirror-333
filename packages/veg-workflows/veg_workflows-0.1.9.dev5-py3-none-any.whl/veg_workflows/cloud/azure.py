...
class AzureBlobReader:
    ...
    def container_client(self):
        return self.blob_service_client.get_container_client(self._container_name)
        ...
        for path in src_folder_path.rglob("*"):
            if path.is_file():
                ...
                dst_file_name = dst_folder_name.joinpath(
                    path.relative_to(src_folder_path)
                ).as_posix()
    ...
    def download_file(self, key, dst_file_path, overwrite=False):
        ...
        if not blob_client.exists():
            raise FileNotFoundError(f"The blob '{key}' does not exist")
