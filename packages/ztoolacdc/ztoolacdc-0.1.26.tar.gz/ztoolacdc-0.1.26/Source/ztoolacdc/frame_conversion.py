...
def dq2Jac(Y_old_frame=None, frequencies=None, results_folder=None, file_name=None, v0_dq=None, i0_dq=None, p0=None, q0=None, v0=None, q_lagging=True):
    ...
    W = np.array([[0, 1], [-1, 0]])
    if v0_dq is None and i0_dq is None:
        s0 = np.array([[-q0, p0], [p0, q0]])
        E = v0*np.eye(2)
        F = v0*W
        Y_new_frame = 3/2*np.matmul(np.matmul(E,Y_old_frame), F) + s0
    else:
