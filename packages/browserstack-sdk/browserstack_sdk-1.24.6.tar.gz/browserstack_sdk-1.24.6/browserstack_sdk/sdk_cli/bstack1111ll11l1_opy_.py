# coding: UTF-8
import sys
bstack11ll1l1_opy_ = sys.version_info [0] == 2
bstack1l11_opy_ = 2048
bstack111l_opy_ = 7
def bstack11_opy_ (bstack111_opy_):
    global bstack111ll1l_opy_
    bstack1llll1l_opy_ = ord (bstack111_opy_ [-1])
    bstack11ll1ll_opy_ = bstack111_opy_ [:-1]
    bstack11l1l1_opy_ = bstack1llll1l_opy_ % len (bstack11ll1ll_opy_)
    bstack111111_opy_ = bstack11ll1ll_opy_ [:bstack11l1l1_opy_] + bstack11ll1ll_opy_ [bstack11l1l1_opy_:]
    if bstack11ll1l1_opy_:
        bstack1ll111_opy_ = unicode () .join ([unichr (ord (char) - bstack1l11_opy_ - (bstack11l1lll_opy_ + bstack1llll1l_opy_) % bstack111l_opy_) for bstack11l1lll_opy_, char in enumerate (bstack111111_opy_)])
    else:
        bstack1ll111_opy_ = str () .join ([chr (ord (char) - bstack1l11_opy_ - (bstack11l1lll_opy_ + bstack1llll1l_opy_) % bstack111l_opy_) for bstack11l1lll_opy_, char in enumerate (bstack111111_opy_)])
    return eval (bstack1ll111_opy_)
import os
import threading
import os
from typing import Dict, Any
from dataclasses import dataclass
from collections import defaultdict
from datetime import timedelta
@dataclass
class bstack1111l1lll1_opy_:
    id: str
    hash: str
    thread_id: int
    process_id: int
    type: str
class bstack1111ll1l11_opy_:
    bstack1l11l1l11l1_opy_ = bstack11_opy_ (u"ࠣࡤࡨࡲࡨ࡮࡭ࡢࡴ࡮ࠦᒃ")
    context: bstack1111l1lll1_opy_
    data: Dict[str, Any]
    platform_index: int
    def __init__(self, context: bstack1111l1lll1_opy_):
        self.context = context
        self.data = dict({bstack1111ll1l11_opy_.bstack1l11l1l11l1_opy_: defaultdict(lambda: timedelta(microseconds=0))})
        self.platform_index = int(os.environ.get(bstack11_opy_ (u"ࠩࡅࡖࡔ࡝ࡓࡆࡔࡖࡘࡆࡉࡋࡠࡒࡏࡅ࡙ࡌࡏࡓࡏࡢࡍࡓࡊࡅ࡙ࠩᒄ"), bstack11_opy_ (u"ࠪ࠴ࠬᒅ")))
    def ref(self) -> str:
        return str(self.context.id)
    def bstack111l111l1l_opy_(self, target: object):
        return bstack1111ll1l11_opy_.create_context(target) == self.context
    def bstack1ll1l111ll1_opy_(self, context: bstack1111l1lll1_opy_):
        return context and context.thread_id == self.context.thread_id and context.process_id == self.context.process_id
    def bstack1l1ll1ll11_opy_(self, key: str, value: timedelta):
        self.data[bstack1111ll1l11_opy_.bstack1l11l1l11l1_opy_][key] += value
    def bstack1lll1lll11l_opy_(self) -> dict:
        return self.data[bstack1111ll1l11_opy_.bstack1l11l1l11l1_opy_]
    @staticmethod
    def create_context(
        target: object,
        thread_id=threading.get_ident(),
        process_id=os.getpid(),
    ):
        return bstack1111l1lll1_opy_(
            id=hash(target),
            hash=hash(target),
            thread_id=thread_id,
            process_id=process_id,
            type=target,
        )