...
class CommonService():
  ...
  @staticmethod
  async def save_file_to_azure(storage_info: dict, file: UploadFile, file_id: Optional[UUID] = None) -> str:
    try:
      ...
      unique_blob_name = f"{uuid4() if file_id is None else file_id}-{file.filename}"
