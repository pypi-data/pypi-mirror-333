...
class Xbid:
    ...
    def get_book(self, unit_id, date, hour):
        ...
        # Get the last Sunday of March and October for the given year
        year = date.year
        last_sunday_of_march = max(
            [datetime(year, 3, day) for day in range(25, 32) if datetime(year, 3, day).weekday() == 6]
        )
        last_sunday_of_october = max(
            [datetime(year, 10, day) for day in range(25, 32) if datetime(year, 10, day).weekday() == 6]
        )

        # Determine the correct flow_date format based on the date
        if last_sunday_of_march < date < last_sunday_of_october:
            flow_date_format = '%Y-%m-%dT22:00:00Z'
        else:
            flow_date_format = '%Y-%m-%dT23:00:00Z'

        # Format the flow_date accordingly
        payload = {
            "flow_date": date.strftime(flow_date_format),
            "hour": hour
        }
