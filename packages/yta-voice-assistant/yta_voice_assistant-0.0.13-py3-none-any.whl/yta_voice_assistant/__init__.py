...
class VoiceAssistant:
    ...
    def get_definitive_transcription(
        self
    ) -> str:
        """
        Get the text that has been transcripted as
        a definitive transcription.
        """
        return self.audio_transcriptor.get_transcription()
    
    def empty_definitive_transcription(
        self
    ) -> str:
        """
        Set the definitive transcription paragraph
        text as an empty string.
        """
        return self.audio_transcriptor.empty_transcription()
    ...
    def run(
        ...
        while (do_continue):
            command = self.get_definitive_transcription()
            ...
            if command == DEACTIVATION_COMMAND:
                ...
                self.empty_definitive_transcription()
            else:
                import time
                time.sleep(0.1)
