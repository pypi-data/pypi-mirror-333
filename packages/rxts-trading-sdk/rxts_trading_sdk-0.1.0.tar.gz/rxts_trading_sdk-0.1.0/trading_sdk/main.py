...
class SDKMain(ClientInterface):
    def __init__(self, logger, config: Config, callback: BaseCallback):
        ...
        data_path = os.path.join(script_dir, "../../apphome/data")
        data_dir = os.path.join(data_path, self.config.market.data_dir)
    ...
    def initialize(self):
        self.logger.info("初始化 SDK")
        #jsy self.client_bus.initialize()
        #jsy self.subscribe(self.config.bus.subscribe_topics)
        self.logger.info("SDK 初始化完成")

    def start(self):
        self.logger.info("启动 SDK")
        try:
            self.running = True
            self.client_bus.start()
            self.logger.info("SDK Trade 已启动并开始接收消息")
        except Exception as e:
            self.logger.error(f"SDK Trade 启动错误: {e}")
        
        try:
            self.monitor.start()
            self.logger.info("SDK Market 已启动并开始接收消息")
        except Exception as e:
            self.logger.error(f"SDK Market 启动错误: {e}")

    def stop(self):
        self.logger.info("停止 SDK")
        self.running = False
        self.client_bus.stop()
        self.monitor.stop()
        self.logger.info("SDK 已停止")
    ...
    def publish(self, envelope):
        ...
        # self.logger.info(f"发送 Envelope: {envelope}")
    ...
    def send_order(self, symbol: str, ord_price: float, ord_qty: int, ord_side: OrdSideEnum, ord_type: OrdTypeEnum):
        ...
        try:
            ...
            self.logger.debug(f"证券代码: {symbol}")
            self.logger.debug(f"委托价格: {ord_price}")
            self.logger.debug(f"委托数量: {ord_qty}")
            self.logger.debug(f"买卖方向: {ord_side}")
            self.logger.debug(f"订单价格类型: {ord_type}")
            ...
            self.logger.debug(f"生成业务层委托序号: {order.biz_index}")
            ...
            self.client_bus.publish(envelope)
            self.logger.info(f"发送委托请求: envelope={envelope}")
        except ValueError as e:
            self.logger.error(f"[send order failed] Invalid argument value: {e}")
        except Exception as e:
            self.logger.error(f"[send order failed] Failed to place order: {e}")
        finally:
            return biz_index
    ...
    def cancel_order(self, biz_index: int):
        ...
        """
        try:
            self.logger.info(f"Cancelling order: {biz_index}")
            ...
            self.client_bus.publish(envelope)
            self.logger.info(f"发送撤单请求: envelope={envelope}")
