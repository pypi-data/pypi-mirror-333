                      ...
                      int ncrit, float grav)
{
  ...
  if (ok1 && ok2 && ok3) {
    assert(n1==n2);
    assert(n1==n3);
    float * acc = new float[n1*3];
    float * pot = new float[n1];
    cfalcon::addGravity(n1,pos,mass,acc,pot,eps,grav,theta,kernel,ncrit);
    ...
    unsout->snapshot->setData(comp,"acc",n1,acc,false);
    unsout->snapshot->setData(comp,"pot",n1,pot,false);
    delete [] acc;
    delete [] pot;
  }  
}
