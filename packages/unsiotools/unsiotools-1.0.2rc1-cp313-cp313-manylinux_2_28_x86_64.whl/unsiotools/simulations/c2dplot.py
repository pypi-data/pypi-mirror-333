...
class C2dplot:
    ...
    def __grid_density_gaussian_filter(self,x0, y0, x1, y1, w, h, x,y,r,rho):
        ...
        if x.size>0 : # proceed only if there are particles
            ...
            if self.__vdebug:
                print("timing grid_density_gaussian_filter = ", time.time() - to,file=sys.stderr)
    ...
    def fullProcess(self,uns,out="",component=None,xrange=20.,sigma=6.0,mergers="",cmap="jet",
        ...
        if self.__init_plot==False:
            self.__init_plot=True
            # specify figure dimensions
            self.__figure=plt.figure(figsize=(w,h),dpi=mydpi)
            fig=self.__figure
            #fig=plt.figure(figsize=(w,h))
            if self.__vdebug:
                print ("FIG DPI =",fig.dpi)
            # create grid
            self.__gs = gridspec.GridSpec(nrows, ncols,wspace=0,hspace=0)#height_ratios=h,width_ratios=w)
            gs = self.__gs
        else:
            fig=self.__figure
            gs = self.__gs
        ...
        if self.__vdebug:
            ...
            print("Display time [%.3f] sec"%(time.time()-t1),file=sys.stderr)
        t3=time.time()
        if (out==''):
            #plt.ion() 
            if not nbd:
        ...
        #plt.close(fig)
        if self.__vdebug:
            print("Plot time [%.3f] sec"%(time.time()-t3),file=sys.stderr)
...
#               pass
        ...
        if self.__vdebug:
            print("Gaussian filtering : [%.3f] sec"%(time.time()-t1),file=sys.stderr)
        # plot time text
        ...
        if contour:
            ...
            if self.__vdebug:
                print("Contour : [%.3f] sec"%(time.time()-tc),file=sys.stderr)
