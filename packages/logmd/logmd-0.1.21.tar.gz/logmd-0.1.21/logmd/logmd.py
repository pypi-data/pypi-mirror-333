...
class LogMD:
    ...
    def mdanalysis(self, u):
        ...
        """
        if is_dev():
            url = 'https://alexander-mathiasen--logmd-upload-single-file-dev.modal.run'
        else:
            url = 'https://alexander-mathiasen--logmd-upload-single-file.modal.run'

        atoms = ase.io.read(u.filename)
        output_buffer = io.StringIO()

        for frame_idx in tqdm(range(len(u.trajectory))):
            u.trajectory[frame_idx]
            positions = u.atoms.positions
            atoms.positions = positions
            ase.io.write(output_buffer, atoms, format='proteindatabank')
            
        trajectory_data = output_buffer.getvalue()
        
        zip_buffer = io.BytesIO()
        with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED, compresslevel=9) as zip_file:
            zip_file.writestr('trajectory_analysis.pdb', trajectory_data.encode('utf-8'))
        zip_data = zip_buffer.getvalue()
        
        base64_encoded = base64.b64encode(zip_data).decode('ascii')

        try:
            response = httpx.post(
                url,
                json={"file_contents": base64_encoded}
            )
            if response.status_code == 200:
                result = response.json()
                rich.print(f"{LOGMD_PREFIX}Upload successful: {result.get('run_id', 'No run_id returned')}")
                url = f"{get_fe_base_url()}/logmd/{result.get('run_id', '')}"
                rich.print(f"{LOGMD_PREFIX}URL: {url}")
                return url
            else:
                rich.print(f"{LOGMD_PREFIX}[red]Upload failed with status {response.status_code}: {response.text}[/]")
