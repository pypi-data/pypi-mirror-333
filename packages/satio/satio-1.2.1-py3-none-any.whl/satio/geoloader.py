...
L2A_HRES_BANDS = ['B02', 'B03', 'B04', 'B05',
                  'B06', 'B07', 'B08', 'B8A',
                  'B11', 'B12', 'SCL']
...
class LandsatLoader(ParallelLoader):
    ...
    def load(self, collection, bands, resolution, resample=False):
        ...
        shifts = np.array([-(old_bounds[0] % 30),
                           -(old_bounds[1] % 30),
                           30 - (old_bounds[2] % 30),
                           30 - (old_bounds[3] % 30)])
    ...
    def _resample(self, arrs, bands, resolution, order=1):
        ...
        if resolution == 30:
            ...
            raise NotImplementedError("Can only resample 10 m or 30 m bands"
                                      "resolution.")
...
class S3ParallelLoader(ParallelLoader):
    ...
    def __init__(self,
        ...
        self._session = AWSSession(boto3_session,
    ...
    def _load_array_bounds(self, fname, bounds):
        ...
        with rasterio.Env(session=self._session,
                          **self._rio_gdal_options):
            with rasterio.open(fname) as src:
                window = rasterio.windows.from_bounds(*bounds, src.transform)
                arr = src.read(window=window, boundless=True,
                               fill_value=self._fill_value)
...
class AWSCOGSLoader(AWSParallelLoader):
    ...
    def __init__(self,
        ...
        super().__init__(aws_access_key_id=aws_access_key_id,
                         ...
                         max_workers=max_workers, fill_value=fill_value)
    ...
    try:
        ...
        if len(root[0][0][-1]) == 1:
            for i, resolution in enumerate([60, 20, 10]):
                filenames[resolution] = {child.text.split('_')[-2]: os.path.join(filename, child.text + '.jp2')  # NOQA
                                         for child in root[0][0][-1][0][0] if child.text.endswith("{}m".format(resolution))}  # NOQA
        else:
            for i, resolution in enumerate([60, 20, 10]):
                filenames[resolution] = {child.text.split('_')[-2]: os.path.join(filename, child.text + '.jp2')  # NOQA
        ...
        raise CorruptedL2AProduct(f"Error with product: {filename}.\n"
...
def get_l2a_aws_filenames(product_path):
    ...
    r10_files = [product_path / 'R10m' / f'B{n:02d}.jp2'
                 for n in [2, 3, 4, 8]]
    r20_files = ([product_path / 'R20m' / f'B{n:02d}.jp2'
                  for n in [5, 6, 7, 11, 12]]
                 + [product_path / 'R20m' / 'SCL.jp2'])
    r60_files = [product_path / 'R60m' / 'SCL.jp2']

    filenames = {10: {str(f.name)[:3]: f for f in r10_files},
...
def get_jp2_filenames(filenames, band, resolution):
    ...
    if resolution <= min_avl:
        ...
        target_res = resolution if 0 in diff else (
            np.max(diff[diff < 0]) + resolution)
...
def _load_array_bounds_latlon(fname, bounds,
                              rio_gdal_options=None,
                              boundless=True,
                              fill_value=np.nan,
                              nodata_value=None):
    ...
    with rasterio.Env(**rio_gdal_options):
        with rasterio.open(fname) as src:
            ...
            if nodata_value is None and boundless is True:
                logger.warning('Raster has no data value, defaulting boundless'
                               ' to False. Specify a nodata_value to read '
                               'boundless.')
                boundless = False

            arr = src.read(window=window, boundless=boundless,
                           fill_value=nodata_value)
            arr = arr.astype(np.float32)  # needed reprojecting with bilinear resampling  # noqa:e501
...
def load_reproject(filename, bounds,
                   epsg, resolution=10,
                   border_buff=0, fill_value=0,
                   nodata_value=None,
                   rio_gdal_options=None,
    ...
    utm_bounds = bbox.buffer(
        border_buff * resolution).bounds.values[0].tolist()

    width = max(1, int((utm_bounds[2] - utm_bounds[0]) / resolution))
    height = max(1, int((utm_bounds[3] - utm_bounds[1]) / resolution))

    gim = _load_array_bounds_latlon(filename,
                                    bounds,
                                    rio_gdal_options=rio_gdal_options,
                                    fill_value=fill_value,
    ...
    dst_transform = rasterio.transform.from_bounds(*utm_bounds,
                                                   width, height)

    dst = np.zeros((height, width), dtype=np.float32)

    reproject(gim.astype(np.float32),
              dst,
...
def load_reproject2(filename, bounds,
                    epsg, resolution=10,
                    border_buff=0,
                    nodata_value=None,
                    rio_gdal_options=None,
    ...
    bounds = bbox.buffer(
        border_buff * resolution).to_crs(src_crs).bounds.values[0]
    utm_bounds = bbox.buffer(
        border_buff * resolution).bounds.values[0].tolist()
...
def load_reproject3(filename, bounds,
                    epsg, resolution=10,
                    nodata_value=None,
                    rio_gdal_options=None,
                    resampling=Resampling.nearest):
    ...
    bb_ll = fiona_transformer(f'EPSG:{epsg}',
                              f'EPSG:{src_crs.to_epsg()}',
                              bbox)
...
def _load_raster_bounds(fname, bounds,
                        rio_gdal_options=None,
                        boundless=True,
                        fill_value=np.nan,
                        nodata_value=None):
...
def load_reproject4(filename, bounds,
                    epsg, resolution=10,
                    nodata_value=None,
                    rio_gdal_options=None,
                    resampling=Resampling.nearest):
    ...
    for bb in block_boxes_ll:
        ...
        width = max(1, int(round((utm_bounds[2] - utm_bounds[0]) / resolution)))
        height = max(1, int(round((utm_bounds[3] - utm_bounds[1]) / resolution)))

        gim = _load_raster_bounds(filename,
                                  src_bounds,
                                  rio_gdal_options=rio_gdal_options,
                                  fill_value=fill_value,
                                  nodata_value=nodata_value)
...
class LatLonReprojectLoader(ParallelLoader):
    ...
    def load_reproject(self, filename, bounds,
                       epsg, resolution=10, border_buff=0):
        """
        Read from latlon layer and reproject to UTM
        """
        if epsg == 4326:
    ...
    def load_arrays(self, filenames, bounds, epsg,
                    resolution=10, border_buff=0):

        def f_handle(filename):
            return self.load_reproject(filename, bounds,
                                       epsg, resolution, border_buff)
...
class WarpLoader:
    ...
    def load(self, collection, bands, resolution,
             ...
             resampling=Resampling.nearest) -> EOTimeseries:
        ...
        src_bounds = {src_epsg: self._get_src_bounds(dst_bounds,
                                                     dst_epsg,
                                                     src_epsg,
                                                     self._buffer_bounds)
        ...
        loaded_data = self._get_metadata(collection,
                                         bands,
                                         resolution)
    ...
    def _get_src_bounds(self, dst_bounds, dst_epsg,
                        src_epsg, buffer_dst_bounds):
        ...
        from satio.grid import fiona_transformer, buffer_bounds
        ...
        src_bbox = fiona_transformer(f'EPSG:{dst_epsg}',
                                     f'EPSG:{src_epsg}',
                                     dst_bbox)
        ...
        if src_bbox.geom_type == 'MultiPolygon':
            raise NotImplementedError("Requested data intersects anti-meridian"
        ...
        with rasterio.Env(session=self._session, **self._rio_gdal_options):
            with rasterio.open(fname) as src:
                ...
                if (vals % 1 > 0).any():
                    ...
                    new_bounds = rasterio.windows.bounds(window,
                                                         src.transform)
    ...
    def _parallel_load(self, loaded_data, bands, src_bounds):
        ...
        download_tuples = [(k, b) for b in bands
        ...
        def _download(download_tuple):
            ...
            try:
                ...
                new_bounds = self._round_bounds(new_bounds,
                                                self._tolerance)
    ...
    @staticmethod
    def _build_eotimeseries(loaded_data,
                            epsg,
                            bounds,
                            nodata_value=None) -> 'EOTimeseries':
        """
    ...
    def _round_bounds(bounds, tolerance=0.00001):
        ...
        new_bounds = tuple(map(lambda x: _round(x, tol=tolerance, ndigits=5),
                               bounds))
        return new_bounds
...
def round_rasterio_window(w):
    ...
    w_3 = Window(w_2.col_off,
                 w_2.row_off,
                 w.col_off + w.width - w_2.col_off,
                 w.row_off + w.height - w_2.row_off).round_lengths(op='ceil')
