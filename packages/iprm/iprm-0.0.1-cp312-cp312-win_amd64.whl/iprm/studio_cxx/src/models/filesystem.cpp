...
auto node_foreground_role(const FileNode& node) {
  ...
  return std::visit(overloaded{[](const auto&) { return QVariant(); },
                               [](const CMakeFile& n) {
...
static bool less_than(const FileNode& node_a, const FileNode& node_b) {
  ...
  bool is_dir_a = std::filesystem::is_directory(path_a);
  bool is_dir_b = std::filesystem::is_directory(path_b);
