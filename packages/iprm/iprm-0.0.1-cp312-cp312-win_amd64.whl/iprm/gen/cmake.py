...
class CMakeGenerator(Generator):
    ...
    def _write_configure(cls, build_dir, build_system, build_type):
        return f'cmake -G {build_system} -B {build_dir} -S . -DCMAKE_BUILD_TYPE={build_type}'

    @classmethod
    def _write_build(cls, build_dir, build_type):
        return f'cmake --build {build_dir} --config {build_type} --parallel'

    @classmethod
    def _write_test(cls, build_dir):
        return f'cd build{" || exit" if not Env.platform.windows else ''}\nctest'
