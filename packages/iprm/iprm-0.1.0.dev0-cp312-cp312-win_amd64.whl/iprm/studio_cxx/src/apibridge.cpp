...
void APIBridge::load_project() {
  ...
  try {
    ...
    pybind11::dict objects = sess_.value().attr("get_objects")();
...
void APIBridge::cmake_generate() {
  ...
  auto iprm_gen_cmake = pybind11::module::import("iprm.gen.cmake");
  if (!iprm_gen_cmake) {
  ...
  std::string dir = root_dir_.absolutePath().toLatin1().data();
  auto cmake_generator = iprm_gen_cmake.attr("CMakeGenerator")(dir);
  if (!cmake_generator) {
    Q_EMIT error(APIError("Failed to create CMakeGenerator instance"));
    return;
  }

  try {
    cmake_generator.attr("generate")();
    Q_EMIT cmake_generate_success();
  } catch (const pybind11::error_already_set& e) {
    Q_EMIT error(make_error("Error during project CMake generation", e));
  }
}
