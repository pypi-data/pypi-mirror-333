...
class Pesten:
    ...
    def draw(self):
        ...
        if not self.draw_stack and len(self.play_stack) > 1:
            ...
            mirror_top_card = None
            if top_card > 52 and top_card != BLACK_JOKER and top_card != RED_JOKER:
                mirror_top_card = top_card
                top_card = self.play_stack.pop()
            ...
            for _ in range(100):
                self.shuffle()
            if mirror_top_card != None:
                self.play_stack.append(top_card)
                self.play_stack.append(mirror_top_card)
            else:
                self.play_stack.append(top_card)
