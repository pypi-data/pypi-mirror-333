...
class PluginsUtils:
    ...
    def __parse_plugin(self, plugins_location, plugin_folder):
        ...
        with open(join(plugin_folder, 'META-INF', 'MANIFEST.MF'), "rb") as f:
            properties.load(f, "iso-8859-1")

        plugin = Plugin()
        plugin.id = properties.get('Bundle-SymbolicName').data.split(';')[0].strip()
        plugin.version = properties.get('Bundle-Version').data.strip()
        plugin.location = plugin_folder
    ...
    def __run_scancode_for_plugins(self, scancode_exe, plugin):
        json_file = join(Path(__file__).parents[2], "bin", plugin.id + '.json')
        ...
        subprocess.call(
            ...
            '--include COPYING'.format(scancode_exe, json_file, plugin.location), stdout=subprocess.DEVNULL,
    ...
    def __run_scancode_for_features(self, scancode_exe, feature):
        json_file = join(Path(__file__).parents[2], "bin", feature.id + '.json')

        subprocess.call(
            '{} -il --json-pp {} {} '
            '--license-text --is-license-text --classify --analyze-license-results --processes 3  --max-depth 2 '
            '--include feature.* --include *.html'.format(
                scancode_exe, json_file, feature.location), stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
    ...
    def scan_plugin(self, scancode_exe: str, plugin_name: str, plugins_location: str):
        if plugin_name not in os.listdir(plugins_location):
            return

        plugin_location = join(plugins_location, plugin_name)
        plugin = self.__parse_plugin(plugins_location, plugin_location)
    ...
    def parse_features_and_plugins(self, plugins_features_root):
        ...
        for resource_name in os.listdir(plugins_location):
            ...
            if plugin_location.endswith('.jar'):
                ...
                zip_ref.close()
                plugin_location = new_plugins_location
...
def extend_sbom(scancode_exe: str, sbom_path: str, netx_studio_path: str):
    ...
    for comp in sbom_contents['components']:
        ...
        if "type=eclipse-feature" in comp['bom-ref']:
            print("Scanning feature:" + comp['name'])
            for feature in features_list:
        ...
        if "type=eclipse-plugin" in comp['bom-ref']:
            print("Scanning plugin:" + comp['name'])
            for plugin in plugins_list:
