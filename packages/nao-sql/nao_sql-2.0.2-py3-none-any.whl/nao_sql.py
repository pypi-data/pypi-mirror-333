    ...
    def query(self, query:str, return_mode=ReturnMode.DICT):
        with self.connection.cursor() as cursor:
            ...
            try:
                ...
                if len(rows[0]) > 1 and return_mode == ReturnMode.SINGLE_VALUE:
                    raise IndexError('ReturnMode.SINGLE_VALUE selected, but more than 1 row present')

                match return_mode:
                    case ReturnMode.SINGLE_VALUE:
                        LOGGER.success(f'{query}')
                        return rows[0][0]
                    
                    case ReturnMode.TUPLE:
                        LOGGER.success(f'{query}')
...
if __name__=="__main__":
    ...
    print(db.query('SELECT MAX(receipt_id) AS r FROM receipt', return_mode=ReturnMode.SINGLE_VALUE))
    # print(db.select(table='receipt', return_mode=ReturnMode.SINGLE_VALUE))