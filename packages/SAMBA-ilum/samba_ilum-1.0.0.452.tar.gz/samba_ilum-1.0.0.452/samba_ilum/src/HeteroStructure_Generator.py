   ...
   for s in range(len(dir_matchings)):
       ...
       if (n_structures != 0):
          ...
          for i in range(n_structures):
              ...
              #-----------------------------------
              angle_V12 = np.arctan2(np.cross(V1, V2), np.dot(V1, V2))  # ângulo em radianos
              ...
              #=================================================================
              if (n_Lattice == 3):
                 angle_V13 = np.arctan2(np.cross(V1, V3), np.dot(V1, V3))  # ângulo em radianos
              ...
              if (mismatch_type == 0):
                 ...
                 angle_1 = np.arctan2(np.cross(A1, B1), np.dot(A1, B1))  # ângulo em radianos
                 angle_2 = np.arctan2(np.cross(A2, B2), np.dot(A2, B2))  # ângulo em radianos
                 if (n_Lattice == 3):
                    mod_A3 = np.linalg.norm(A3)
                    mod_B3 = np.linalg.norm(B3)
                    Area3 = np.linalg.norm(np.cross(A3, B3))
                    angle_3 = np.arctan2(np.cross(A3, B3), np.dot(A3, B3))  # ângulo em radianos

                 #----------------------------------------------------------------------------------------------------------------
                 # Alinhando a célula das redes a fim de criar a célula intermediária --------------------------------------------
                 #----------------------------------------------------------------------------------------------------------------
                 ...
                 theta = angle_2;  rotation_matrix = np.array([[np.cos(theta), -np.sin(theta)], [np.sin(theta), np.cos(theta)]])
                 B2_rotated = np.dot(rotation_matrix, A2_rotated)
                 #-----------------------------------------------
                 if (n_Lattice == 3):
                    ...
                    theta = angle_3;  rotation_matrix = np.array([[np.cos(theta), -np.sin(theta)], [np.sin(theta), np.cos(theta)]])
                    B3_rotated = np.dot(rotation_matrix, A3_rotated)
                 #-------------------------------------------------------
