...
@Array.implements(numpy.trapezoid)
@from_numpy
def trapezoid(x: Measured[real.ValueType], *args, **kwargs):
    """Integrate `x` via the composite trapezoidal rule."""
    if isinstance(x, Scalar):
        raise TypeError(
            "Cannot apply numpy.trapezoid to a scalar object"
        ) from None
    data = numpy.trapezoid(x.data, *args, **kwargs)
