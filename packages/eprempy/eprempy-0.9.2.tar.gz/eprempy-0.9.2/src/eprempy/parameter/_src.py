...
class VariableDeclaration:
    """Pattern parser for variable declarations."""
    ...
    def pattern(self):
        return re.compile(r"""
            ...
            # optional whitespace
            \s*
            # variable name with dereference operator
            \*(?P<name>\w+)
            # C statement terminator
            \;
            # optional whitespace, followed by the end of the string
...
class ConfigurationC(SourceFile):
    ...
    def _get_array_defaults(self, file: paths.TextFile):
        ...
        patterns = (VariableDeclaration(), VariableDefinition())
        defaults = {}
        for pattern in patterns:
            current = file.extract(pattern.match, pattern.parse)
