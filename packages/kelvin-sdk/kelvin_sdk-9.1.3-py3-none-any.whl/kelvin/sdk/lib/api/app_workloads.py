    ...
    try:
        ...
        if app_config.is_legacy():
            ...
            payload = LegacyWorkloadDeploy(
                ...
                cluster_name=deployment.node_name,
            ...
            payload.name = deployment.workload_name
            payload.title = deployment.workload_title
            payload.cluster_name = deployment.node_name
            payload.app_name = app_config.name
            payload.app_version = app_config.version

            client.app_workloads.create_workload(data=payload)

        success_message = ""
        if not deployment.quiet:
            success_message = f"""\n
                Workload "{deployment.workload_name}" successfully deployed.
            """
