...
class PlaneWaveKwargs(TypedDict, total=False):
    x: float
    y: float
    z: float
    x_span: float
...
class Settings(ModuleCollection):
    general: general.PlaneWave
    geometry: SourceGeometry
    freq_and_wavelength: frequency_wavelength.FrequencyWavelength
    beam_options: beam_options.PlaneWave

    def __init__(self, parent_object: Source):
        ...
        self.beam_options = beam_options.PlaneWave(parent_object)
...
class PlaneWave(Source):
    ...
    def __init__(self, name: str, sim, **kwargs: Unpack[PlaneWaveKwargs]) -> None:
