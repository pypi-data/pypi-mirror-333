...
# Std imports
from typing import TypedDict, Unpack, Union, Sequence, Literal, Iterable, Tuple
...
class Polygon(Structure):
    ...
    def _get_trimesh(self, absolute: bool = False) -> trimesh.Trimesh:
        ...
        polygon_2d = trimesh.path.polygons.Polygon(self.vertices)

        # Extrude the polygon along the z-axis to create a 3D mesh
        polygon_mesh = trimesh.creation.extrude_polygon(polygon_2d, height=self.z_span)

        # Translate the mesh to the correct position. Adjust z-coordinate to acount for only positive extrusion.
        polygon_mesh.apply_translation(position - np.array([0, 0, self.z_span / 2]))
    ...
    def _copy(self, name, **kwargs) -> Polygon:
        return Polygon(name, self._sim, **kwargs)
