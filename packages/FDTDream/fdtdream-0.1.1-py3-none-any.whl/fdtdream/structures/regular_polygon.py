...
from typing import TypedDict, Unpack, Union, Sequence, Literal, Tuple, Callable, Optional
...
class RegularPolygon(Structure):
    ...
    def _get_trimesh(self, absolute: bool = False) -> trimesh.Trimesh:
        ...
        polygon_2d = trimesh.path.polygons.Polygon(
            convert_length(self._get("vertices", np.ndarray), "m", self._units))
        ...
        position = convert_length(self._get_position(absolute), "m", self._units) - np.array([0, 0, self.z_span / 2])
