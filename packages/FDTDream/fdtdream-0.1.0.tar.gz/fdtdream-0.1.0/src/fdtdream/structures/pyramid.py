...
class Pyramid(Structure):
    ...
    def _get_trimesh(self, absolute: bool = False) -> trimesh.Trimesh:
        ...
        box_mesh = trimesh.creation.box(extents=[self.x_span_bottom, self.y_span_bottom, self.z_span])

        # Get the vertices of the box
        vertices = box_mesh.vertices.copy()

        # The z position of the top and bottom faces
        z_bottom = -self.z_span / 2
        z_top = self.z_span / 2
        ...
        for i, vertex in enumerate(vertices):
            if np.isclose(vertex[2], z_top):  # If vertex belongs to the top face
                ...
                vertices[i, 0] *= self.x_span_top / self.x_span_bottom
                vertices[i, 1] *= self.y_span_top / self.y_span_bottom

        # Replace the box's vertices with the modified ones
        box_mesh.vertices = vertices

        # Translate the pyramid to its desired position
