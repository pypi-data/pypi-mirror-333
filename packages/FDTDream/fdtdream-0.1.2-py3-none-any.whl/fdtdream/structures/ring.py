...
import math
from typing import TypedDict, Unpack, Union, Sequence
...
class Ring(Structure):
    ...
    def _get_trimesh(self, absolute: bool = False) -> trimesh.Trimesh:
        ...
        outer_cylinder = trimesh.creation.cylinder(radius=self.outer_radius, height=self.z_span, sections=128)
        inner_cylinder = trimesh.creation.cylinder(radius=self.inner_radius, height=self.z_span, sections=128)

        # Apply scaling for the second set of radii in the y-direction
        scale_matrix_outer = np.diag([1.0, self.outer_radius_2 / self.outer_radius, 1.0, 1.0])
        scale_matrix_inner = np.diag([1.0, self.inner_radius_2 / self.inner_radius, 1.0, 1.0])
        ...
        theta_start = np.deg2rad(self.theta_start)  # Convert degrees to radians
        theta_stop = np.deg2rad(self.theta_stop)  # Convert degrees to radians
        if theta_start != theta_stop:
            num_vertices = 64  # Number of vertices to approximate the curved edge

            angles = np.linspace(theta_start, theta_stop, num_vertices)
            outer_vertices = np.column_stack((np.cos(angles) * max(self.outer_radius, self.outer_radius_2) * 1.5,
                                              np.sin(angles) * max(self.outer_radius, self.outer_radius_2) * 1.5))
            mask_polygon = Polygon(np.vstack(([0, 0], outer_vertices)))
            # Extrude the 2D polygon into a 3D mask volume
            mask_volume = trimesh.creation.extrude_polygon(mask_polygon, height=self.z_span * 2)

            # Translate the mask to align with the ring
            mask_volume.apply_translation([0, 0, -self.z_span])

            # Subtract the mask from the ring
            ring = ring.intersection(mask_volume)

        # Translate the ring to its final position
        position = convert_length(self._get_position(absolute), "m", self._units)
    ...
    def _get_scripted(self, position: Sequence[float, float, float]) -> str:
        ...
        if self.settings.rotation.__getattribute__("_is_rotated"):
            axes, rotations = self.settings.rotation.__getattribute__("_get_rotation")()
