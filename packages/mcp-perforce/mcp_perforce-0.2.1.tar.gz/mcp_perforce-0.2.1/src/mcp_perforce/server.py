...
async def handle_list_tools() -> list[types.Tool]:
    ...
    return [
        types.Tool(
            ...
            inputSchema={
                "type": "object",
                "properties": {"changelist_id": {"type": "int"}},    
        ...
        types.Tool(
            ...
            inputSchema={
                "type": "object",
                "properties": {"action": {"type": "string"}, "file_path": {"type": "string"}, "revision": {"type": "int"}, "changelist_id": {"type": "int"}},
...
async def get_changelist_files_catalog(changelist_id: int) -> str:
    ...
    try:
        ...
        if not SWARM_USERNAME or not SWARM_PASSWORD:
            return "错误: 未配置Swarm用户名或密码，请在p4config.json中添加swarm_username和swarm_password"
        
        if not SWARM_BASE_URL or not SWARM_API_URL:
            return "错误: 未配置Swarm URL，请在p4config.json中添加swarm_base_url和swarm_api_url"
...
"""
        ...
        for file_info in files:
            ...
            if file_type == 'binary' or file_path.endswith('.pb.go') or file_path.endswith('.cs'):
                continue
    ...
    if os.path.exists(config_file_path):
        ...
        if file_config:
            ...
            if not SWARM_BASE_URL:
                print("警告: 未设置Swarm基础URL，请在p4config.json中添加swarm_base_url")
            if not SWARM_API_URL:
                print("警告: 未设置Swarm API URL，请在p4config.json中添加swarm_api_url")
    else:
        print(f"警告: 配置文件 {config_file_path} 不存在")
        print("请创建配置文件p4config.json，并包含以下内容:")
        print("""
{
...
def browser_login(username=None, password=None, base_url=None):
    ...
    if not base_url:
        print("错误: 未提供Swarm基础URL，请在p4config.json中添加swarm_base_url")
        return None
...
def create_api_client_with_cookies(session, api_url=None):
    ...
    if not api_url:
        print("错误: 未提供Swarm API URL，请在p4config.json中添加swarm_api_url")
        return None
...
async def main():
    ...
    if config_info:
        ...
        print("警告: 未设置任何Perforce配置参数")
        print("请创建配置文件p4config.json")
