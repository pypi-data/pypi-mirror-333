...
def get_iupac_index_range(start_index,length,max_len):
    if start_index+length>max_len:
        length = max_len-start_index
    
    csv_file = f'./data/pubchem324k/pubchem_data_{start_index}_{start_index+length}.csv'
    ...
    # 保存未完成数据的索引到一个文件
    unfinished_file = f'./data/pubchem324k/unfinished_indexes_{start_index}_{start_index+length}.txt'
...
if __name__ == "__main__":
    ...
    for i in range(start_index,max_len,length):
        get_iupac_index_range(i,length,max_len)
        time.sleep(120)