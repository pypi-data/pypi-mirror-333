...
from nomenclators_archetype.loggers import default_logger

from nomenclators_archetype.domain.usecase.commons import use_case_validator
...
class DefaultUseCaseNomenclatorCreatorIsolatedSession(ValidatorUseCaseIsolatedSession, Generic[D, S]):
    ...
    @use_case_validator(before_invoke)
...
class DefaultUseCaseNomenclatorUpdaterIsolatedSession(ValidatorUseCaseIsolatedSession, Generic[D, S]):
    ...
    def before_invoke(self, *params, **kparams) -> list[str]:
        """Method to validate the use case before invoke"""
        return (["The item identifier does not match the URL identifier"]
