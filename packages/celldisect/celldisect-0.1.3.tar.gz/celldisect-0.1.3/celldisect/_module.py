...
class CellDISECTModule(BaseModuleClass):
    ...
    gene_likelihood : Tunable[Literal["zinb", "nb", "poisson", "normal"]], optional
        One of 'nb' (Negative binomial distribution), 'zinb' (Zero-inflated negative binomial distribution), 'poisson' (Poisson distribution), or 'normal' (Normal/Gaussian distribution), by default "zinb".
    ...
    def inference(self,
        ...
        if self.gene_likelihood != "normal":
            ...
            library = torch.ones_like(x[:, 0]).unsqueeze(1)  # No library size normalization for normal distribution
    ...
    def generative(
        ...
        for dec_count in range(self.zs_num + 1):
            ...
            if self.gene_likelihood == "zinb":
                ...
                # For normal distribution, px_rate is the mean and px_scale is the standard deviation
                px = Normal(loc=px_rate, scale=px_scale)
    ...
    def sub_forward(self, idx,
        ...
        if self.gene_likelihood != "normal":
            ...
            library = torch.ones_like(x_[:, 0]).unsqueeze(1)  # No library size normalization for normal distribution
