...
class Well:
    ...
    def get_well_coords_by_trajectory(self, well_info, well_id, well_name, top_dict, bot_dict, calculation_type, depth_type):

        if calculation_type == 'top':
            if top_dict['name'] in list(well_info[well_name]['strat_levels'].keys()):
                depth = well_info[well_name]['strat_levels'][top_dict['name']]['level_depth']
                depth = depth + top_dict['shift'] * top_dict['direction']
            else:
                return np.array([np.inf, np.inf])
        elif calculation_type == 'bot':
            if bot_dict['name'] in list(well_info[well_name]['strat_levels'].keys()):
                depth = well_info[well_name]['strat_levels'][bot_dict['name']]['level_depth']
                depth = depth + bot_dict['shift'] * bot_dict['direction']
            else:
                return np.array([-np.inf, -np.inf])
        elif calculation_type == 'middle':
            if bot_dict['name'] in list(well_info[well_name]['strat_levels'].keys()):
                if top_dict['name'] in list(well_info[well_name]['strat_levels'].keys()):
                    depth_top = well_info[well_name]['strat_levels'][top_dict['name']]['level_depth']
                    depth_top = depth_top + top_dict['shift'] * top_dict['direction']

                    depth_bot = well_info[well_name]['strat_levels'][bot_dict['name']]['level_depth']
                    depth_bot = depth_bot + bot_dict['shift'] * bot_dict['direction']
        ...
        def get_well_coords_by_depth(trajectory_coords, depth):
            x = get_coord(depth,
