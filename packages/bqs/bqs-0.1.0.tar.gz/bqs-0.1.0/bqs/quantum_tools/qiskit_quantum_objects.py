...
class QiskitQuantumObjects(AbstractQuantumObjects):
    ...
    def __init__(self, backend_name="aer_simulator", **kwargs):
        ...
        elif backend_name == 'quantum_iqm':
            # IQM quantum computer
            provider = IQMProvider(url=url)
            self.backend = provider.get_backend()

        elif backend_name == 'quantum_iqm_mock':
            # IQM mock (CPU) simulator
            provider = IQMProvider(url=url)
            self.backend = provider.get_backend()
    ...
    def sample(
        ...
        if self.backend_name in ['quantum_iqm', 'quantum_iqm_mock']:

            qc.measure_active()
            circuits_list = [qc]
            job = self.backend.run(circuits_list, shots=num_samples)
        ...
        if self.backend_name in ['quantum_iqm', 'quantum_iqm_mock']:
            ...
            bitstrings = [
                bitstring[::-1] for bitstring, counts in results.get_counts().items() for _ in range(counts)
            ]
