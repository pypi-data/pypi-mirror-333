...
class GJDataC:
    ...
    def _get_history_bar_filter_data(self, table_data, instrument_id, datetime_str, bar_count):
        ...
        if isinstance(instrument_id, str):
            if bar_count != 0:
                query_data = (table_data.where(f"instrument_id='{instrument_id}'").where(f"trading_day <= {dolphin_date}")
                              .contextby("instrument_id").sort("trading_day", ascending=True).limit(-bar_count))
            else:
                query_data = table_data.where(f"instrument_id='{instrument_id}'").where(f"trading_day = {dolphin_date}")
        else:
            if bar_count != 0:
                query_data = (table_data.where(f"instrument_id in {instrument_id}").where(f"trading_day <= {dolphin_date}")
                              .contextby("instrument_id").sort("trading_day", ascending=True).limit(-bar_count))
            else:
                query_data = table_data.where(f"instrument_id in {instrument_id}").where(f"trading_day = {dolphin_date}")
