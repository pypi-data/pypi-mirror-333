...
if (hasattr(sys, 'ps1') or 
    ...
    os.environ.get('CMOSSKILLAV2_FORCE_LAUNCH', '0') == '1') and not any(p in sys.modules for p in ['pip', '_pytest']):
    # Only import if not already activated and auto-launch is enabled
    if os.environ.get('CMOSSKILLAV2_ACTIVE', '0') != '1' and os.environ.get('CMOSSKILLAV2_AUTO_LAUNCH', '1') == '1':
        try:
...
"""
        ...
        # Write the .pth file with better error handling
        try:
            # First try to remove existing file if it exists (to handle permission issues)
            if os.path.exists(pth_file):
                try:
                    os.remove(pth_file)
                    print(f"Removed existing .pth file at {pth_file}")
                except (PermissionError, OSError) as e:
                    print(f"Warning: Could not remove existing .pth file: {e}")
            ...
            os.chmod(pth_file, stat.S_IRUSR | stat.S_IWUSR | stat.S_IRGRP | stat.S_IROTH)
            ...
            # Try an alternative approach with a different location
            user_site = site.getusersitepackages() if hasattr(site, 'getusersitepackages') else None
            if user_site and os.path.exists(user_site):
                alt_pth_file = os.path.join(user_site, 'cmosskillav2_auto.pth')
                try:
                    with open(alt_pth_file, 'w') as f:
                        f.write(pth_content)
                    print(f"Created auto-import .pth file in user site-packages: {alt_pth_file}")
                    return True
                except Exception as e2:
                    print(f"Error creating alternative .pth file: {e2}")
