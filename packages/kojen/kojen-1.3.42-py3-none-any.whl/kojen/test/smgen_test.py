...
class TestFeatures(unittest.TestCase):
    ...
    def test_WTF(self):
        input = []
        input.append("<<<PER_MSG_BEGIN>>>")
        input.append("<<<PyAttr=return_type=int>>> <<<CLASSNAME>>>::<<<MSGNAME>>>(<<<SIGNATURE>>>)")
        input.append("{")
        input.append("    <<<MSGNAME>>>Parameters parameters<<<AGGREGATEINITIALIZATION>>>;")
        input.append("    <<<IF InjectEmbos>>>")
        input.append("    privilege_elevator->RunPrivileged(<<<MSGNAME>>>DeviceDriver, reinterpret_cast<void*>(&parameters));")
        input.append("    <<<ELSEIF AbstractInterface>>>")
        input.append("    parameters.implementation = implementation_;")
        input.append("    OS_MPU_CallDeviceDriverEx(<<<MSGNAME>>>DeviceDriver, &parameters);")
        input.append("    <<<ELSE>>>")
        input.append("    OS_MPU_CallDeviceDriverEx(<<<MSGNAME>>>DeviceDriver, &parameters);")
        input.append("    <<<ENDIF>>>")
        input.append("    return parameters.return_value;")
        input.append("}")
        input.append("void <<<CLASSNAME>>>::<<<MSGNAME>>>DeviceDriver(void* parameters)")
        input.append("{")
        input.append("    <<<IF InjectEmbos>>>")
        input.append("    <<<ELSE>>>")
        input.append("    MutexLock lock{mutex_};")
        input.append("    <<<ENDIF>>>")
        input.append("    auto* p = reinterpret_cast<<<<MSGNAME>>>Parameters*>(parameters);")
        input.append("    <<<IF AbstractInterface>>>")
        input.append("    p->return_value = p->implementation-><<<MSGNAME>>>(<<<PARAMETERS=p-> >>>);")
        input.append("    <<<ELSE>>>")
        input.append("    // {{{USER_<<<MSGNAME>>>}}}")
        input.append("    // {{{USER_<<<MSGNAME>>>}}}")
        input.append("    <<<ENDIF>>>")
        input.append("    <<<IF InjectEmbos>>>")
        input.append("    <<<ELSE>>>")
        input.append("    <<<PyAttr=is_asynchronous>>>event_.GetBlocked(); // dont forget to inject this event to be set in custom-code.")
        input.append("    <<<ENDIF>>>")
        input.append("}")
        input.append("<<<PER_MSG_END>>>")

        SomeEnumeration = Enum("SomeEnumeration")
        SomeEnumeration.SetDocumentation('''This is the documentation for 'SomeEnumeration'.''')
        SomeEnumeration.Add("kOptionOne", 0)
        SomeEnumeration.Add("kOptionTwo", 1)
        SomeEnumeration.Add("kOptionThree", 2)
        SomeEnumeration.Add("kOptionFour", 3)

        CustomReturnType = Struct('CustomReturnType')
        CustomReturnType.SetDocumentation('''This is the documentation for 'CustomReturnType'.''')
        CustomReturnType.AddType('return_one', 'uint8_t')
        CustomReturnType.AddType('return_two', 'uint16_t')
        CustomReturnType.AddType('return_three', 'uint16_t')

        SynchronousOperation = Message('SynchronousOperation', 0x01)
        SynchronousOperation.SetDocumentation('''This is the documentation for 'SynchronousOperation'.''')
        SynchronousOperation.AddType('parameter_one', 'uint8_t')
        SynchronousOperation.AddType('parameter_two', 'uint16_t')
        SynchronousOperation.AddType('parameter_three', 'uint16_t')
        SynchronousOperation.AddType('parameter_four', 'SomeEnumeration', 'SomeEnumeration::kOptionOne')

        OtherSynchronousOperation = Message('OtherSynchronousOperation', 0x02)
        OtherSynchronousOperation.SetDocumentation('''This is the documentation for 'OtherSynchronousOperation'.''')
        OtherSynchronousOperation.AddType('parameter_one', 'bool')
        OtherSynchronousOperation.AddType('parameter_two', 'uint16_t')
        OtherSynchronousOperation.AddType('parameter_three', 'uint16_t')

        SynchronousOperationCustomReturnType = Message('SynchronousOperationCustomReturnType', 0x03)
        SynchronousOperationCustomReturnType.SetDocumentation('''This is the documentation for 'SynchronousOperationCustomReturnType'.\nIt demonstrates how a custom return type from a device driver can be used.''')
        SynchronousOperationCustomReturnType.return_type = 'CustomReturnType'

        AsynchronousOperationCustomReturnType = Message('AsynchronousOperationCustomReturnType', 0x04)
        AsynchronousOperationCustomReturnType.SetDocumentation('''This is the documentation for 'AsynchronousOperationCustomReturnType'.\nIt demonstrates how a custom return type from a device driver can be used.''')
        AsynchronousOperationCustomReturnType.return_type = 'CustomReturnType'
        AsynchronousOperationCustomReturnType.is_asynchronous = '' # Note : this needs to be an empty string.

        interface = Interface("")
        interface.AddEnum(SomeEnumeration)
        interface.AddStruct(CustomReturnType)
        interface.AddMessage(SynchronousOperation)
        interface.AddMessage(OtherSynchronousOperation)
        interface.AddMessage(SynchronousOperationCustomReturnType)
        interface.AddMessage(AsynchronousOperationCustomReturnType)
        interface.PurgeMessageHeaders()

        #interface.AddUserTag("InjectEmbos", 'PrivilegeElevatorInterface')
        interface.AddUserTag("AbstractInterface", '')
