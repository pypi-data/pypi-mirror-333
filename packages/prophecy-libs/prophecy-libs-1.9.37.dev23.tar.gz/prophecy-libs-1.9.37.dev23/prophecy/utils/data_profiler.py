...
class DataProfiler:
    ...
    def _is_aggregatable_type(col_type: DataType) -> bool:
        return isinstance(
            col_type, (NumericType, DateType, DayTimeIntervalType)
        ...
        for col_name, col_type in columns_info:
            ...
            if is_str_type:
                agg_cols.extend(
                    [
                        ...
                        F.min_by(sanitized_col, F.length(sanitized_col)).alias(
                            "shortest_value"
                        ),
                        F.max_by(sanitized_col, F.length(sanitized_col)).alias(
                            "longest_value"
                        ),
                    ]
                )
            ...
            if calculate_histograms and min_val is not None and max_val is not None:
                ...
                if isinstance(col_type, NumericType):
                    ...
                    if isinstance(col_type, (ByteType, ShortType, IntegerType, LongType)):                            
            ...
            profile = Profile(
                ...
                top_values=structured_top_values,
    ...
    def profile_columns_as_dataframe(
        ...
        return spark.createDataFrame([cls.to_dict(profile) for profile in profiles], schema)
