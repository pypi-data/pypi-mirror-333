...
class PatternDistribution(object):
    ...
    def get_mean(self, tokens=False, **kwargs):
        ...
        return (weights.value * weights.pair_proba).sum()
    ...
    def get_weights(self, data, tokens=False):
        ...
        *The probability of a pair of cells is the product of the probability of the predictors
        ...
        def compute_weight(x, freq):
            ...
            pred_freq = x.predicted.map(freq.loc[x.predicted].result)
        ...
        data.loc[:, 'pair_proba'] = 1 / data.shape[0]
        data.loc[:, 'pred_proba'] = 1 / data.predictor.nunique()
        data.loc[:, 'target_proba'] = 1 / data.predicted.nunique()
