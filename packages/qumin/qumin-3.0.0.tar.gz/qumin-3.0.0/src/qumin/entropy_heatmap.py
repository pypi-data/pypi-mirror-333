...
def entropy_heatmap(results, md, cmap_name=False, freq_margins=True,
    ...
    if freq_margins and (results.proba_source == "tokens").all():
        pred_freqs = results.reset_index('predicted').pred_proba.drop_duplicates()
        target_freqs = results.reset_index('predictor').target_proba.drop_duplicates()
