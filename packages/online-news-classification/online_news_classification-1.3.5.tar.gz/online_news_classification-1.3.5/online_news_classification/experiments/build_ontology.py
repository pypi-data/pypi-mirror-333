...
def filter_categories(data):
    ...
    for category, subcategories in data.items():
        ...
        filtered_data[category] = [
            ...
            if keyword in sub and sub != keyword and sub != f"{keyword}/{keyword}"
        ]
...
def main():
    ...
    ddf = pd.read_csv(
        "../datasets/the_guardian/filtered_enriched_the_guardian.csv",
        sep=";", index_col=0
    )

    # # Convert categories and tags into a dictionary
    # data = convert_categories(ddf)
    #
    # # Filter subcategories to only include those containing the category name (for uk-news, use 'uk')
    # filtered_data = filter_categories(data)
    #
    # # Save filtered data to a JSON file
    # write_to_json("ontology_strucuture_without_filtering.json", filtered_data)
    #
    # build_ontology(filtered_data, "Ontology Structure (without filter categories)", "ontology_strucuture_without_filtering.png")
    ...
    #ddf = ddf[~ddf['category'].isin(['commentisfree', 'eyewitness-subscriber', 'uk-news', 'us-news', 'news', 'theguardian', 'theobserver', 'crosswords', 'government-computing-network', 'healthcare-network', 'australia-news', 'media-network', 'media', 'public-leaders-network', 'extra', 'cardiff', 'leeds', 'global-development-professionals-network', 'childrens-books-site', 'edinburgh', 'info', 'membership', 'working-in-development', 'gnm-press-office', 'teacher-network', 'culture-professionals-network', 'culture-professionals-network', 'housing-network', 'community', 'small-business-network', 'voluntary-sector-network', 'social-care-network', 'local-government-network', 'guardian-masterclasses'])]
    ddf = ddf[~ddf['category'].isin(['Week in Review', 'Opinion', 'Obituaries', 'Times Insider', ''])]

    ddf['filtered_tags'] = ddf.apply(filter_tags, axis=1)

    ddf['filtered_tags'] = ddf['filtered_tags'].apply(replace_slash)

    ddf['filtered_tags'] = ddf.apply(
        lambda row: [row['category']] if row['filtered_tags'] == [] else row['filtered_tags'], axis=1)
    ...
    valid_tags = tag_counts[tag_counts >= 50]
    ...
    #manage_datasets.save_dataset(ddf, "/the_guardian/filtered_enriched_the_guardian.csv")

    write_to_json("ontology_strucuture_with_filtering.json", new_filtered_data)
    #
    build_ontology(new_filtered_data, "Ontology Structure (with filter categories)",
                   "ontology_strucuture_with_filtering.png")

    manage_datasets.save_dataset(df1, "/the_guardian/filtered_enriched_the_guardian_1.csv")

    manage_datasets.save_dataset(df2, "/the_guardian/filtered_enriched_the_guardian_2.csv")
