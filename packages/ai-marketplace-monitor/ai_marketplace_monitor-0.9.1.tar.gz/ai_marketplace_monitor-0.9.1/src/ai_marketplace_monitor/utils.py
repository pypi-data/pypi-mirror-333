...
class MonitorConfig(BaseConfig):
    ...
    def handle_subscription_id(self: "MonitorConfig") -> None:
        if self.subscription_id is None:
            return
        # must be a string
        if not isinstance(self.subscription_id, str):
            raise ValueError(f"Item {hilight(self.name)} subscription_id must be a string.")
    ...
    def handle_proxy_server(self: "MonitorConfig") -> None:
        ...
        if not self.subscription_id:
            raise ValueError("A license code is required to use a proxy server")
