...
class Config(Generic[TAIConfig, TItemConfig, TMarketplaceConfig]):
    ...
    def is_pro_user(self: "Config") -> bool:
        # Check 1: Check if the config.py is under site-packages or dist-packages (already done)
        if not ("site-packages" in __file__ or "dist-packages" in __file__):
            return False

        # Check 3: Check if config.monitor has keys subscription_id
        if self.monitor.subscription_id:
            return False

        # upon the establishment of a license server, the license code
        # will be verified again the license server
        return True
