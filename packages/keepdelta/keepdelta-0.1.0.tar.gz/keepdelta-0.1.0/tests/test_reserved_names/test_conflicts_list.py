...
class TestReservedNamesList(unittest.TestCase):
    ...
    def test_change_type_from_conflict_to_normal(self):
        """
        List's conflicting element has format changed.
        This operation is NOT allowed.
        """
        for key in keys:
            conflict = keys[key]
    ...
    def test_change_type_from_normal_to_conflict(self):
        """
        List's normal element has format changed to a conflicting value.
        This operation is NOT allowed.
        """
        for key in keys:
            conflict = keys[key]
