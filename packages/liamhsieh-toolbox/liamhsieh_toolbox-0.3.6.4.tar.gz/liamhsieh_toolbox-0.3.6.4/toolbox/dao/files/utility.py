...
def df_dq_check(df, spec):
    ...
    for row in spec.itertuples():
        ...
        if dtype == 'datetime64[ns]':
            ...
            if invalid_rows.any():
                df.loc[invalid_rows, 'Invalid_Row_Message'] += f"Invalid date format in column '{row.column_name}'; "
        elif dtype == 'str' and re_pattern:
            invalid_rows = ~df[row.column_name].str.match(re_pattern)
            if invalid_rows.any():
                df.loc[invalid_rows, 'Invalid_Row_Message'] += f"Values do not match the expected pattern in column '{row.column_name}'; "
        else:
            invalid_rows = pd.to_numeric(df[row.column_name], errors='coerce').isnull()
            if invalid_rows.any():
                df.loc[invalid_rows, 'Invalid_Row_Message'] += f"Column '{row.column_name}' could not be converted to {dtype}; "
    ...
    if df['Invalid_Row_Message'].isna().all():
        ...
        df['Invalid_Row_Message'] = df['Invalid_Row_Message'].str.rstrip('; ')
