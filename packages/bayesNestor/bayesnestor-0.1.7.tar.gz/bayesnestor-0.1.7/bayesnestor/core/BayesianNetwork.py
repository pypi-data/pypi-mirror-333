...
from bayesnestor.utils.Utils import get_graph_layout_pos, merge_unique_lists
...
class BayesianNetwork(nx.DiGraph):
    """This class is a wrapper for the internal representation of Bayesian Networks.
        ...
        Currently this is only done half assed since it is just a wrapper around pgmpy and networkx.
    Attributes:
        model_elements (dict<str, CPT>): Lookup dictionary keyed by CPT name, value is instance of CPT
    ...
    def __build_model(self, node_connections: List[Tuple[str, str]]) -> None:
        ...
        super().__init__(node_connections, **{"name": self.name})
    ...
    def __leafs_and_roots(self) -> None:
        ...
        self.__leafs = [
            node
            for node in self.model_elements
            if super(BayesianNetwork, self).out_degree(node) == 0
        ]

        self.__roots = [
            node
            for node in self.model_elements
            if super(BayesianNetwork, self).in_degree(node) == 0
        ]
    ...
    def get_dag(self):
        """Returns the Directed Acyclic Graph underlying the Bayesian Network as a NetworkX-DiGraph instance.

        Returns:
            DiGraph: New NetworkX-DiGraph object.
        """

        return nx.DiGraph(self.node_connections)
    ...
    def add_cpts(self, *cpds) -> None:
        """Helper method to add CPT's to the network.
        Raises:
            TypeError: Raised if given CPTs are not instances of pynestor.core.CPT

        Args:
            *cpds: Collection of CPTs to add.
        """

        if not all(isinstance(elem, CPT) for elem in cpds):
            raise TypeError(
                f"CPTs to add must be instantiations of {type(CPT)}. Given CPTs are of type {[type(elem) for elem in cpds]}"
            )
    ...
    def get_variable_summary(self) -> Dict[str, List[Union[str, int, None]]]:
        """Helper method to get a string summary of all variables in the model including their state names.
        Returns:
            Dict[str, List[Union[str, int, None]]]: Dictionary containing the names of all variables as key and their state names (if available) as values.
        """

        variable_summary = dict()
        for name, cpt in self.model_elements.items():

            variable_summary[name] = variable_summary.get(name, [])
            state_names = cpt.state_names.get(name, [])
            variable_summary[name] = merge_unique_lists(
                variable_summary[name], state_names
            )
    ...
    def plot_graph(
        ...
        layout = get_graph_layout_pos(self)
    ...
    @staticmethod
    def get_prefixed_copy(
        bn_inst: "BayesianNetwork", prefix_str: str
    ) -> "BayesianNetwork":
        ...
        if not isinstance(bn_inst, BayesianNetwork):
            raise TypeError(
                f"Passed BN instance must be an instantiation of the class: {type(BayesianNetwork)}."
            )

        if not isinstance(prefix_str, str):
            raise TypeError(
                f"Passed prefix string must be string object and not an instantiation of: {type(prefix_str)}."
            )

        independent_nodes = bn_inst.get_independent_nodes()

        new_node_connections = [
            (f"{prefix_str}{src}", f"{prefix_str}{dest}")
            for src, dest in bn_inst.node_connections
        ]
        ...
        for old_name, old_cpd in bn_inst.model_elements.items():
            ...
            new_cpd_evidence = (
                [f"{prefix_str}{evi}" for evi in old_cpd.evidence]
                if old_cpd.evidence is not None
                else None
            )

            new_cpd_state_names = (
                {
                    f"{prefix_str}{var}": states
                    for var, states in old_cpd.state_names.items()
                }
                if old_cpd.state_names is not None
                else None
            )
            ...
            new_bn.add_cpts(
                CPT(
                    ...
                    values=old_cpd.values,
                    evidence=new_cpd_evidence,
                    evidence_card=old_cpd.evidence_card,
                    state_names=new_cpd_state_names,
                )
            )
