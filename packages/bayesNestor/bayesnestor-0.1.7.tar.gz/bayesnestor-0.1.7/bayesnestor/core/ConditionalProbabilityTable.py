...
seperate the actual implementation from the usage througout the package.
...
class CPT:
    ...
    def __init__(
        ...
        name: str,
        variable_card: int,
        values: Union[List[ndarray], ndarray, List[List[Union[float, int]]]],
        evidence: Optional[List[str]] = None,
        evidence_card: Optional[Union[ndarray, List[int]]] = None,
        state_names: Optional[Dict[str, List[str]]] = None,
    ) -> None:
        """Ctor of the CPT class.
        ...
        self.evidence = [] if evidence is None else list(np.ravel(evidence))
        self.evidence_card = (
            [] if evidence_card is None else list(np.ravel(evidence_card))
        )
        self.state_names = state_names if state_names is not None else dict()

        self.__scope = (
            [self.name] + self.evidence if self.evidence is not None else [self.name]
        )
        self.__cardinalities = (
            [*[self.variable_card], *self.evidence_card]
            if self.evidence_card is not None
            else [self.variable_card, -1]
        )
    ...
    def copy(self) -> "CPT":
        ...
        Returns:
            CPT: Deep copy of this instance.
        """
    ...
    def __build_index_mapping(self) -> None:
        ...
        if self.state_names:
            ...
            name_to_no = {
                var: {i: i for i in range(int(self.__cardinalities[index]))}
                for index, var in enumerate(self.__scope)
            }
    ...
    def get_value(self, node_states: Dict[str, Union[str, int]]) -> float:
        ...
        if len(set(node_states.keys()) ^ set(self.state_names.keys())) != 0:
            raise ValueError(
                f"Your given variables: {node_states.keys()} do not match with the CPTs named vars {self.state_names.keys()}"
            )

        if not all(
            state in self.state_names[var] for var, state in node_states.items()
        ):
            raise ValueError(
                f"Invalid states {node_states.values()} are not part of the listed state names: {self.state_names.values()}."
            )
    ...
    def get_index_of_state(self, state_name: str) -> int:
        ...
        if not isinstance(state_name, str):
            raise TypeError(
                f"Given state name: {state_name} is not a string: {type(state_name)}."
            )

        if state_name not in self.state_names[self.name]:
            raise ValueError(
                f"Given state name: {state_name} is not specified for this variable ({self.name})"
            )

        idx = [
            i
            for i in range(self.variable_card)
            if self.state_names[self.name][i] == state_name
        ][0]
    ...
    def __eq__(self, other: "CPT") -> bool:
        """Check if two CPT instances are identical.

        Args:
            other (CPT): Another instance of CPT.

        Returns:
            bool: True if the instances are identical, False otherwise.
        """
        if not isinstance(other, CPT):
            return NotImplemented

        if not self.name == other.name:
            raise ValueError(f"Difference in name: {self.name} != {other.name}")

        if not self.variable_card == other.variable_card:
            raise ValueError(
                f"Difference in variable cardinality: {self.variable_card} != {other.variable_card}"
            )

        if all((self.evidence, other.evidence)) and set(self.evidence) != set(
            other.evidence
        ):
            raise ValueError(
                f"Difference in evidence: {self.evidence} != {other.evidence}"
            )

        if not np.array(self.values).shape == np.array(other.values).shape:
            raise ValueError(
                f"Difference in shape: {np.array(self.values).shape} != {np.array(other.values).shape}"
            )

        if not np.array_equal(np.array(self.values), np.array(other.values)):
            raise ValueError(f"Difference in values: {self.values} != {other.values}")

        if not self.state_names == other.state_names:
            print(
                f"Warning -- Difference in states: {self.state_names} != {other.state_names}"
            )
