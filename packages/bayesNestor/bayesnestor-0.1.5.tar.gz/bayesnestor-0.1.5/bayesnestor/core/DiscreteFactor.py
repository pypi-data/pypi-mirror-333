...
class DiscreteFactor:
    ...
    def __init__(
        ...
        name: str,
        scope: List[str],
        cardinalities: Union[ndarray, List[int]],
        values: Union[ndarray, List[Union[float, int]]],
    ...
    def __build_index_mapping(self) -> None:
        ...
        if self.state_names:
            ...
            name_to_no = {
                var: {i: i for i in range(int(self.cardinalities[index]))}
    ...
    def get_value(self, node_states: Dict[str, str]) -> float:
        ...
        if len(set(node_states.keys()) ^ set(self.state_names.keys())) != 0:
            raise ValueError(
                f"Your given variables: {node_states.keys()} do not match with the factors named vars {self.state_names.keys()}"
            )
