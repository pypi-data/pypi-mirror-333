...
@pytest.mark.parametrize(
    "name  ,  scope            , cardinalities, values         , state_names",
    [
...
def test_get_probabilities_returns_correct():
    ...
    assert np.allclose(
        factor.get_probabilities().reshape(cards), values.reshape(cards), atol=1e-32
    )
...
def test_get_value_returns_correct():
    scope = ["A", "B", "C"]
    cards = [2, 2, 2]
    state_names = {"A": ["s_a1", "s_a2"], "B": ["s_b1", "s_b2"], "C": ["s_c1", "s_c2"]}
    values = [0, 1, 2, 3, 4, 5, 6, 7]
    combinations = [
        ({"A": "s_a1", "B": "s_b1", "C": "s_c1"}, 0),
...
@pytest.mark.parametrize(
    "bad_combination",
    [{"A": "s_a1", "B": "s_b1"}, {"A": "s_a1", "B": "s_b2", "C": "s_c0", "D": "s_d1"}],
)
...
@pytest.mark.parametrize(
    "bad_combination",
    [{"A": "s_a0", "B": "s_b1", "C": "s_c2"}, {"A": "s_a1", "B": "s_b1", "C": 0}],
)
