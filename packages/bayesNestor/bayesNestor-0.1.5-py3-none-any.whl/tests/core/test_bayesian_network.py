...
@pytest.mark.parametrize(
    ...
    [
        ("fixture_bn_confounder_param", ["NODE_A"]),
        ("fixture_bn_collider_param", ["NODE_A", "NODE_B"]),
    ],
)
...
def test_copy_success(fixture_bn_confounder_param):
    ...
    assert (
        len(set(model.model_elements.keys()) ^ set(copied_model.model_elements.keys()))
        == 0
    )
...
@pytest.mark.parametrize(
    "title, edge_labels, options",
    [
        ("Test_1", None, None),
        ("Test_2", {("NODE_A", "NODE_B"): "Test_2"}, None),
        (
            "Test_3",
            {("NODE_A", "NODE_B"): "Test_3"},
            {"font_size": 10, "node_size": 2000},
        ),
    ],
)
def test_plot_graph_executes_successfully(
    mock_show, title, edge_labels, options, fixture_bn_confounder_param
):
...
@pytest.mark.parametrize(
    "fixture, prefix, expected",
    [
        ("fixture_bn_confounder_param", "", set(["NODE_A", "NODE_B", "NODE_C"])),
        (
            "fixture_bn_collider_param",
            "test_1_",
            set(["test_1_NODE_A", "test_1_NODE_B", "test_1_NODE_C"]),
        ),
        (
            "fixture_bn_independent_nodes_only_param",
            "test_2_",
            set(["test_2_NODE_A", "test_2_NODE_B", "test_2_NODE_C"]),
        ),
    ],
)
...
def test_get_variable_summary_success(fixture_bn_confounder_param):
    model_name = "test"
    model, _ = fixture_bn_confounder_param(model_name)
