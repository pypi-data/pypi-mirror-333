...
def generate_epub_file(from_dir_path: str, epub_file_path: str):
...
def _write_assets(
  ...
  if has_cover:
    file.writestr(
      ...
      data=template.render("cover.xhtml").encode("utf-8"),
    )
...
def _write_chapters(
  ...
  if has_head_chapter:
    ...
    file.writestr(
      ...
      data=generate_part(template, chapter_xml).encode("utf-8"),
    )
