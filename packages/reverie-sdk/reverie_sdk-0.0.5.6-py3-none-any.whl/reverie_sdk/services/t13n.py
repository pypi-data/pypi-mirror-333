...
{outString}
    ...
    def __init__(self, **data) -> None:
        ...
        self.responseList: typing.List[ReverieT13nResponse.Result] = (
            [ReverieT13nResponse.Result(**e) for e in data["responseList"]]
    ...
    def __str__(self) -> str:
        ...
        return (
            """ReverieT13nResponse(\n"""
            f'{f"""    status          : {self.status}\n""" if self.status else ""}'
            f'{f"""    message         : {self.message}\n""" if self.message else ""}'
            f'{f"""    error_cause     : {self.error_cause}\n""" if self.error_cause else ""}'
            f'{f"""    responseList    : {responseList}\n""" if self.responseList else ""}'
            """)"""
        ).strip()
