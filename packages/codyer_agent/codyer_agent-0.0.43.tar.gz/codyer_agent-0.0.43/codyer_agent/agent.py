...
def anthropic_format_llm_inference(messages, stream=False, api_key=None, base_url=None, model=None, input_price=None, output_price=None):
    ...
    def _with_stream():
        ...
        try:
            result = ''
            stream = client.messages.create(max_tokens=8192 * 8, messages=messages, model=model, stream=True)
            for event in stream:
    ...
    def _without_stream():
        try:
            response = client.messages.create(max_tokens=8192 * 8, messages=messages, model=model, stream=False)
            if input_price is not None and output_price is not None:
