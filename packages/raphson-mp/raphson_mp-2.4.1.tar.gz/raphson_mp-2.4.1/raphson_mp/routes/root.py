...
# TODO: find a way to cache this, but immediately invalidate cache when language is changed
@route("/vars.js")
async def vars(_request: web.Request, _conn: Connection, user: auth.User):
    template = JINJA_ENV.get().get_template("vars.js.j2")
    text = await template.render_async(user=user, offline_mode=settings.offline_mode)
