...
// Add GPU detection functionality to the extension
const extension = {
...
const updateLogoColors = (svgElement, themeManager) => {
    ...
    const textPaths = svgElement.querySelectorAll('g[transform*="matrix(1,0,0,1"] path[style*="fill:white"]');
    textPaths.forEach(element => {
        // For elements with style attribute containing fill:white
        const style = element.getAttribute('style');
        if (style) {
            const newStyle = style.replace(/fill:white/, `fill:${textColor}`);
            element.setAttribute('style', newStyle);
        }
    });
    // Update the hexagon stroke color
    const hexagonPath = svgElement.querySelector('g[transform*="matrix(0.1742"] path[style*="stroke:white"]');
    if (hexagonPath) {
        const style = hexagonPath.getAttribute('style');
        if (style) {
            const newStyle = style.replace(/stroke:white/, `stroke:${textColor}`);
...
//# sourceMappingURL=lib_index_js.75eece214d52bce8d7b9.js.map