...
def visualize(
    sys: System,
    selection: Iterable[Atom] | None=None,
    show_idxs=True,
    size_scale=7.5,
    ghost_alpha=0.4,
    draw_bonds=True,
    ...
    # Plot the given selection
    if selection:
        for a in selection:
            r = np.reshape(a.r, (3, 1))
            # Make the selection larger than the atom marker
            s = (a.vdw_radius * size_scale * 1.75) ** 2 * np.pi
            ax.scatter(*r, c="cyan", s=s, edgecolors="none", alpha=0.5, zorder=25)
