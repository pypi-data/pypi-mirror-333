...
def main(
    ...
    s_train_ = predpca._create_basis_functions(s_train)
    s_test_ = predpca._create_basis_functions(s_test)
    ...
    for h in range(NT):
        ...
        predpca_sub._fit(s_sub_, s_sub)
        se_sub = predpca_sub._transform(s_sub_)  # input expectations (Ns, T_sub)
        se_test = predpca_sub._transform(s_test_)  # input expectations (Ns, T_test)
        ...
        predpca_sl._fit(s_sub_, x_sub)
        qx_sl_test = predpca_sl._transform(s_test_)  # hidden state expectation (Nx, T_test)
