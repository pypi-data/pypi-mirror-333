...
class ATTN(TTN):
    ...
    def apply_des_to_hamiltonian(self, params):
        ...
        Params needed for the creation of the DenseMpo representation
    ...
    def optimize_de_site(self, de_site, de_index):
        ...
        # Energy depends on both uk and uk^*
        # Here, we optimize the problem self-consistenly:
        # Assume uk^* constant, optimize uk, and feed the result back.
        # Repeat for num_iterations or untill convergence of energy.
        # The energy is given by the sum of the singular values of the SVD.
        for ii in range(self.convergence_parameters.de_opt_max_iter):
            ...
            de_conj = self.de_layer[de_index].conj()
            ...
            # if symmetries are included, singvals will be an instance of AbelianSymLinkWeight
            if eff_env.has_symmetry:
                # use the sum attribute obtained from the appropriate tensor class
                tsum = res.get_attr("sum")
                new_energy = [tsum(xx) for xx in singvals.link_weights]
            else:
                new_energy = 0
                for s_ii in singvals:
                    new_energy += s_ii
                new_energy *= -1
