...
try:
    ...
    # Get instrumentation key from environment or config
    APPINSIGHTS_INSTRUMENTATIONKEY = os.getenv("APPINSIGHTS_INSTRUMENTATIONKEY", "6140e6a4-9696-42a9-b17d-988c3c51c09b")
    
    # Configure Azure logging if instrumentation key is available
    if APPINSIGHTS_INSTRUMENTATIONKEY:
        # Configure logging to use Azure Log Handler
        ...
        # Standard logging if no instrumentation key
        logging.basicConfig(level=logging.DEBUG)
        logger = logging.getLogger(__name__)
        logger.warning("No Application Insights instrumentation key found, using standard logging")
except ImportError:
    # Fallback to standard logging if opencensus not available
    logging.basicConfig(level=logging.DEBUG)
    logger = logging.getLogger(__name__)
    logger.warning("Azure Application Insights integration not available (opencensus not installed)")

# Clean up imports that were previously here but don't belong
# (We don't need DefaultAzureCredential or ApplicationInsightsManagementClient for logging)
