...
class LucidConverter:
    def parse_transition(self, text):
        ...
        action_guard = parts[1] if len(parts) > 1 else ""
        action = None
        guard = None
        
        if '[' in action_guard:
            action_part = action_guard.split('[')[0].strip()
            guard = action_guard.split('[')[1].rstrip(']').strip()
            action = action_part if action_part else None
        else:
            action = action_guard.strip() if action_guard.strip() else None
