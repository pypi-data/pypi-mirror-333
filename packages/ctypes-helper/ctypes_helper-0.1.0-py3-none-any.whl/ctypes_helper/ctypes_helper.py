...
class CTypeDescriptor:
    ...
    def _calc_min_max(self) -> tuple[int | float | None]:
        if self.category == "int":
            if self.sign == "signed":
...
class CTypeRegistry:
    ...
    def _init_builtin_types(self) -> None:
        ...
        self.types["uintptr_t"] = CTypeDescriptor("uintptr_t", "unsigned", "pointer", 32, c_ulong)  # 32-bit platform

        # self.types["char_ptr"] = CTypeDescriptor("char_ptr", "unsigned", "pointer", 32, c_char_p)
        # self.types["wchar_ptr"] = CTypeDescriptor("char_ptr", "unsigned", "pointer", 32, c_wchar_p)
        # self.types["void_ptr"] = CTypeDescriptor("char_ptr", "unsigned", "pointer", 32, c_void_p)
