from src.rook_helper import convert_to_type
from src.rook_helper.structure import StructureBase
from src.rook_helper.structure.physical_health import (ActivityEvents, HeartRateEvent,
...
class Summary(StructureBase):
    ...
    def distance_data(cls, _data: dict) -> dict:
        ...
        active_steps_granular_array = []

        active_steps_granular_data_old = _data.get('active_steps_granular_data_steps_per_hr', [])

        if active_steps_granular_data_old:
            active_steps_granular_array = [
                {
                    'steps_int': entry.get('steps'),
                    'datetime_string': entry.get('datetime'),
                    'interval_duration_seconds_float': entry.get('interval_duration_seconds')
                    }
                for entry in active_steps_granular_data_old
                ]
        ...
        distance.update(
            {
                ...
                'active_steps_granular_data_array': active_steps_granular_array
                }
            )
