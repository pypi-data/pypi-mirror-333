...
def combine_shifts(home_shift,away_shift,json,game_id):
    ...
    away_shifts = construct_skaters_matrix(data,away,pd.json_normalize(json)['awayTeam.abbrev'][0],False)
    home_shifts = construct_skaters_matrix(data,home,pd.json_normalize(json)['homeTeam.abbrev'][0],True)
    ...
    for i in range(7):
        if i+1 < 7:
            shifts[f"home_on_{i+1}"].ffill(inplace=True)
        else:
            shifts['home_goalie'].ffill(inplace=True)

    for i in range(7):
        if i+1 < 7:
            shifts[f"away_on_{i+1}"].ffill(inplace=True)
        else:
            shifts['away_goalie'].ffill(inplace=True)
    ...
    shifts['away_skaters'] = shifts[away_on].replace(r'^\s*$', np.nan, regex=True).notna().sum(axis=1)
    shifts['home_skaters'] = shifts[home_on].replace(r'^\s*$', np.nan, regex=True).notna().sum(axis=1)
...
def create_timeline(pbp):
    ...
    on_ice_col = ['season','season_type','game_id','game_date',
        'away_team_abbr','home_team_abbr','period','period_type',
        "seconds_elasped","away_skaters","home_skaters","strength_state",'away_on_1','away_on_2','away_on_3','away_on_4','away_on_5','away_on_6',"away_goalie",
         'home_on_1','home_on_2','home_on_3','home_on_4','home_on_5','home_on_6',"home_goalie"]

    timeline = pd.merge(timeline,pbp[on_ice_col].drop_duplicates(subset=['seconds_elasped']),how="outer",on=['seconds_elasped'])
    
    timeline = timeline[on_ice_col].ffill()
...
def combine_data(pbp_df,shifts_df):
    ...
    df['event_type_before'] = df['event_type'].shift(-1)
    ...
    df['shift_type'] = np.where(np.logical_or(np.logical_or(df['event_type_last'].isin(lag_events),df['event_type_before'].isin(lead_events)),df['seconds_elasped'].isin(period_end_secs)),"Line Change","On The Fly")
