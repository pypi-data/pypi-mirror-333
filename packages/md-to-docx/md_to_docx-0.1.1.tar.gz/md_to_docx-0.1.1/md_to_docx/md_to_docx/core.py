...
def debug_code_blocks(md_content):
    ...
    for i, (lang, code) in enumerate(precise_blocks):
        ...
        print(f"  行数: {len(code.split('\\n'))}")
        print(f"  首行: {code.split('\\n')[0][:50]}")
...
def debug_code_block_processing(md_content, output_file="debug_code_blocks.docx"):
    """专门用于调试代码块处理的函数"""
    print("\n===== 开始调试代码块处理 =====")
...
def md_to_docx(md_content, output_file=None, debug_mode=False):
