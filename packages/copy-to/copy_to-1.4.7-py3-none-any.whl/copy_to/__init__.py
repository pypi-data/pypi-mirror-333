    ...
    for name, value in conf.envs.items():
        if not name == 'group':
            ...
            for e in value['src']:
                ...
                i+=1
                j=1
            source_parser.add_argument("path", type=lambda x: pathlib.Path(x).is_dir() ,help="Source path", metavar="Source path")
...
def main():
    ...
    add.add_argument("dest" , type=lambda x: pathlib.Path(x).is_dir(), metavar="Destination directory")
    add.add_argument("src" , nargs='*', type=lambda x: pathlib.Path(x).absolute().exists(), metavar="Source files and directories", help="Source files and directories")
    ...
    add_source.add_argument("src" , nargs='+', type=lambda x: pathlib.Path(x).absolute().exists(), metavar="Source files and directories", help="Source files and directories")
    ...
    reset_destination.add_argument("dest" , type=lambda x: pathlib.Path(x).absolute().is_dir(), metavar="Destination directory", help="Destination directory")
    ...
    reset_source.add_argument("src" , nargs='*', type=lambda x: pathlib.Path(x).absolute().exists(), metavar="Source files and directories", help="Source files and directories")
    output_stream = None
