...
class AsroNLP:
    ...
    def preprocess_and_optionally_stem_and_analyze(self, input_path, output_path="output.xlsx"):
        try:
        ...
        # Prompt user for further processing
    ...
    def sentiment_analysis_lexicon_indonesia(self, tokens):
        ...
        for word in tokens:
            word = word.lower()
            score += self.lexicon_positive_dict.get(word, 0)  # Add score from positive lexicon or 0 if not found
            score -= self.lexicon_negative_dict.get(word, 0)  # Subtract score from negative lexicon or 0 if not found
