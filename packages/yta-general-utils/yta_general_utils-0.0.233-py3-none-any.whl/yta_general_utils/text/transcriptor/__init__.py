...
class WebRealTimeAudioTranscriptor:
    ...
    def check_if_text_has_been_said(
        self,
        text: str
    ) -> bool:
        """
        ...
        """
        final_transcription = self._get_transcription()
        temp_transcription = self._get_temp_transcription()

        return (
            len(TextFinder.find_in_text(text, final_transcription)) > 0 or
