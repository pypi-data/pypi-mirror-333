...
impl HawkBacktester {
    ...
    fn run<'py>(
        ...
        // Return the results DataFrame and metrics dictionary together in a dictionary
        let results_dict = PyDict::new(py);
        results_dict.set_item("backtest_results", PyDataFrame(results_df))?;
        results_dict.set_item("backtest_metrics", metrics_dict)?;
