"""
HTTP tools for OpenAPI-MCP.

This module provides functionality for creating MCP tools from OpenAPI specifications.
"""
...
logger = logging.getLogger("openapi_mcp")
...
def create_mcp_tools_from_openapi(
    app_or_openapi_url: Union[FastAPI, str],
    mcp_server: FastMCP,
    ...
    """
    Create MCP tools from an OpenAPI schema, either from a FastAPI app or an OpenAPI spec URL.

    Args:
        app_or_openapi_url: Either a FastAPI application or a URL to an OpenAPI spec
        mcp_server: The MCP server to add tools to
        base_url: Base URL for API requests (defaults to http://localhost:$PORT for FastAPI or the server URL from the OpenAPI spec)
    ...
    """
    # Determine if we have a FastAPI app or an OpenAPI URL
    if isinstance(app_or_openapi_url, str):
        # We have an OpenAPI URL - fetch the schema
        ...
        try:
            ...
            logger.error(f"Failed to fetch OpenAPI schema from {openapi_url}: {e}")
            raise ValueError(f"Failed to fetch OpenAPI schema from {openapi_url}: {e}")
            
        # If base_url is not provided, try to extract it from the OpenAPI schema
        if not base_url and "servers" in openapi_schema and openapi_schema["servers"]:
            server_url = openapi_schema["servers"][0].get("url", "")
            if server_url:
                base_url = server_url
    else:
        # We have a FastAPI app
    ...
    # Default base_url if still not set
    if not base_url:
    ...
    for path, path_item in openapi_schema.get("paths", {}).items():
        for method, operation in path_item.items():
            ...
            if not operation_id:
                # If no operationId is provided, generate one
