...
async def create_vector_store(
    ...
    openai_key = get_openai_access_token(tool_config)
    client = AsyncOpenAI(api_key=openai_key)
    vector_store = await client.vector_stores.create(name=normalized_name)
    return vector_store
...
async def create_assistant(
    ...
    if not vector_store_id:
        vector_store = await client.vector_stores.create(name=assistant_name)
...
async def attach_file_to_vector_store(
    ...
    # Add the file to the specified vector store
    response = await client.vector_stores.files.create(
        vector_store_id=vector_store_id,
        file_id=file_id
    )
    return response
