# if you try edit this please give my credits too i had lot of hard work in reverse engineering this thanks you 😁
# Main Author : Rakhilukky
...
class Bypasse:
    def get(url, headers= {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/110.0.0.0 Safari/537.36"}, **kwargs):    
        ...
        cookies = {
            "__test": decrypted_value
        }
        
        r2 = requests.get(redirect_url, headers=headers, cookies=cookies, **kwargs)
        return r2
        
    def post(url, headers= {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/110.0.0.0 Safari/537.36"}, **kwargs):    
        r1 = requests.get(url, headers=headers, **kwargs)
        html_content = r1.text    
        a_match = re.search(r'var a=toNumbers\("([a-fA-F0-9]+)"\)', html_content)
        b_match = re.search(r'b=toNumbers\("([a-fA-F0-9]+)"\)', html_content)
        ...
        r2 = requests.post(redirect_url, headers=headers, cookies=cookies, **kwargs)
        return r2
