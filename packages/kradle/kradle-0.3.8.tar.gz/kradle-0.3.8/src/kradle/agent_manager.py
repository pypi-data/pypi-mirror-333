...
class AgentManager:
    ...
    def _is_port_available(self, host:str = 'localhost', port:int = 8080):
        try:
            print(f"trying to run app on {host}:{port}")
    ...
    def _setup_server(self, host: str = 'localhost', port: int = 8080) -> None:
        ...
        if not self._is_port_available(host,port):
            self._logger.log_error(f"Host {host} and port {port} is not available")
            raise ValueError(f"Host {host} and port {port} is not available")
        ...
        if not self._server_ready.wait(timeout=5.0):
            self._logger.log_error("Server failed to start within timeout")
    ...
    def _get_or_create_agent(self, participant_id: str, run_id:str, username:str = None) -> MinecraftAgent:
        ...
        agent = agent_class(api_client=self._api_client, participant_id=participant_id, run_id=run_id)
