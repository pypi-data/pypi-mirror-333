...
class RedisMemory(AbstractMemoryStore):
    def __init__(
        self, host="localhost", port=6379, db=0, participant_id: Optional[str] = None
    ):
    ...
    def save_memory(
        self, key: str, data: Any, participant_id: Optional[str] = None
    ) -> None:
    ...
    def load_memory(
        self, key: str, participant_id: Optional[str] = None
    ) -> Optional[Any]:
        full_key = self._get_key(key, participant_id)
    ...
    def load_all_memory(self, participant_id: Optional[str] = None) -> Memory:
        memory = Memory()
        prefix = (
            f"{participant_id or self._participant_id}:"
            if participant_id or self._participant_id
            else ""
        )
