...
document.addEventListener("DOMContentLoaded", initializeAllStreamContainers);

// Set up MutationObserver to detect and initialize new containers
const observer = new MutationObserver((mutations) => {
        ...
        if (mutation.addedNodes.length) {
            mutation.addedNodes.forEach(node => {
                ...
                if (node.nodeType === Node.ELEMENT_NODE) {
                    // Check if the node itself is a streaming container
                    if (node.hasAttribute && node.hasAttribute('data-stream-url')) {
                        shouldInitialize = true;
                    }
                    
                    // Check if the node contains streaming containers
                    if (node.querySelectorAll) {
                        const containers = node.querySelectorAll('[data-stream-url]');
    ...
    // Initialize all new containers if needed
    if (shouldInitialize) {
...
// Initial scan for any containers
initializeAllStreamContainers();
`;
                """)
            ]
