...
void svs_invoke(
    ...
    std::span<float> processed_query;
    ...
    if constexpr (std::is_same_v<std::remove_cv_t<QueryType>, float>) {
        ...
        processed_query = dataset.preprocess_queries(distance, query_data).get_datum(0);
    } else {
        ...
        processed_query =
            dataset.preprocess_queries(distance, query_data.cview()).get_datum(0);
    }
