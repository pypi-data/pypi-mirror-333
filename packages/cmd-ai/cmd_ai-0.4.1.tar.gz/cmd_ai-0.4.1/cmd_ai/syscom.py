...
def process_syscom(cmd):
    ...
    # *************************************** google functions  DONT WORK!!!!anymore
    elif cmd.strip() == ".g":
        if len(config.TOOLLIST)==0:
            print(f"i...  {bg.green}{fg.white} google search functions + meeting record ON {bg.default}{fg.default}")
            config.TOOLLIST = [  texts.tool_searchGoogle, texts.tool_getWebContent, texts.tool_setMeetingRecord]
    ...
    # *************************************** utility/test functions
        ...
        if len(config.TOOLLIST)==0:
            ...
            config.TOOLLIST = [ texts.tool_getCzechWeather, texts.tool_setMeetingRecord]
            for i in config.TOOLLIST:
                print(" ... ",i['function']['name'])
        else:
