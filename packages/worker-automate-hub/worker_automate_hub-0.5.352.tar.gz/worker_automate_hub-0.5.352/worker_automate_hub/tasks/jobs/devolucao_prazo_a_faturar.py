                    ...
                    if preenchimento_header_pre_venda.sucesso:
                        try:
                            ...
                            if 'gasolina' in descricao.lower() or 'diesel' in descricao.lower() or 'gnv' in descricao.lower() or 'etanol' in descricao.lower():
                                max_attempts = 60
                                i = 0
                                correct_nop_selected = False
                                pyautogui.write(5)
                                while i <= max_attempts:
                                    check_selecionar_nota_entrada = await is_window_open("Confirm")
                                    if check_selecionar_nota_entrada["IsOpened"] == True:
                                        console.print("Verificando a existência de Pop UP - Confirm (Deseja selecionar a nota de Entrada)... \n")
                                        ...
                                        natureza_oper_select = panel_TGroup_Box.child_window(class_name="TDBIComboBox", found_index=0)
                                        nop_selected = natureza_oper_select.window_text()
                                        console.print(f"Nop selecionada {nop_selected}...\n")
                                        if '5667' in nop_selected and (('c/' in nop_selected.lower() or 'c /' in nop_selected.lower()) and ('s/' in nop_selected.lower() or 's /' in nop_selected.lower())):
                                        #if nop_selected == "5667 - VENDA DE COMB OU LUBRI - S/ESTOQ E C/FINANC" or nop_selected == "5667 - VENDA DE COMB OU LUBRI - S/ ESTOQ E C/ FINANC":
                                            correct_nop_selected = True
                                            console.print("Nop encontrada, saindo... \n")
                                            break
                                        else:
                                            pyautogui.press('up')
                                            i = i+1
