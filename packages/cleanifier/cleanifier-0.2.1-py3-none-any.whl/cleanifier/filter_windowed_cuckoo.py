...
def build_filter(
    ...
    if subfilter_hashfunc_str is None:
        subfilter_hashfunc_str = populate_hashfunc_tuple(("random",), mod_value=nsubfilters)
    debugprint2(f"- using {subfilter_hashfunc_str=}")
    get_subfilter = compile_get_bucket(subfilter_hashfunc_str[0], universe, nsubfilters)

    # Get fingerprint hash function
    if fingerprint_hashfunc_str is None:
        fingerprint_hashfunc_str = populate_hashfunc_tuple(("random",), mod_value=nfingerprints)
    debugprint2(f"- using {fingerprint_hashfunc_str=}")

    # Use fingerprint size - 1 so 0 is an empty slot
    fingerprint_hash_off = compile_get_bucket(fingerprint_hashfunc_str[0], universe, nfingerprints)
    compute_fingerprint = njit('uint64(uint64)', nogil=True)(lambda x: fingerprint_hash_off(x) + 1)

    # Get the hash function "hash"
    if window_hashfunc_str is None:
        window_hashfunc_str = populate_hashfunc_tuple(("random", ), mod_value=nwindows_per_subfilter)
    debugprint2(f"- using {window_hashfunc_str=}")
    get_window = compile_get_bucket(window_hashfunc_str[0], universe, nwindows_per_subfilter)

    if offset_hashfunc_str is None:
        offset_hashfunc_str = populate_hashfunc_tuple(("random", ), mod_value=nwindows_per_subfilter - windowsize + 1, min_value=nwindows_per_subfilter)
    debugprint2(f"- using {offset_hashfunc_str=}")
    get_offset = compile_get_bucket(offset_hashfunc_str[0], universe, nwindows_per_subfilter - windowsize + 1)
