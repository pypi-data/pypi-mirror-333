...
def test_union_2():
    @dataclass(frozen=True)
    class NodeWithUnion2:
        node: Union[SimpleNode, SimpleNode2, SimpleNode3]
    
    data = {  "node": {"name": "n1", "value": "v1"} }
    node = from_dict(NodeWithUnion2, data, fd_check_types=True)
    assert isinstance(node.node, SimpleNode)
    assert node.node.name == "n1"
    assert node.node.value == "v1"
    ...
    data = {  "node": {"name": "n1", "field": "value"} }
    node = from_dict(NodeWithUnion2, data, fd_check_types=True)
    assert isinstance(node.node, SimpleNode3)
    assert node.node.name == "n1"
    assert node.node.field == "value"

    data = {  "node": {"name": "n1"} }
    node = from_dict(NodeWithUnion2, data, fd_check_types=True)
    assert isinstance(node.node, SimpleNode3)
    assert node.node.name == "n1"
    assert node.node.field == "Default-value"

    data = {  "node": {"name": "n1", "no_match": "node-X"} }
    node = from_dict(NodeWithUnion2, data)
    assert isinstance(node.node, dict)
    assert node.node["name"] == "n1"
    assert node.node["no_match"] == "node-X"
    ...
    with pytest.raises(TypeError) as e:
        node = from_dict(NodeWithUnion2, data, fd_check_types=True)
    assert str(e.value) == ('For "node", expected typing.Union[test_generics_with_dataclasses.SimpleNode, '
                            'test_generics_with_dataclasses.SimpleNode2, '
                            'test_generics_with_dataclasses.SimpleNode3] but found <class \'dict\'>')
