...
class KuberayConfig(ConfigBase):
    ...
    @attrs.define
    class _HeadNodeNodeSelector(ConfigBase):
        key: str = attrs.field()
        value: str = attrs.field()

        @classmethod
        def name(cls) -> str:
            return "headnode_node_selector"

        def to_dict(self) -> dict:
            return {self.key: self.value}

    headnode_node_selector: _HeadNodeNodeSelector | None = attrs.field(default=None)

    @attrs.define
    class _WorkerNodeSelector(ConfigBase):
        key: str = attrs.field()
        value: str = attrs.field()

        @classmethod
        def name(cls) -> str:
            return "worker_node_selector"

        def to_dict(self) -> dict:
            return {self.key: self.value}

    worker_node_selector: _WorkerNodeSelector | None = attrs.field(default=None)
...
def launch_kuberay_job(
    ...
    if kuberay_config.headnode_node_selector:
        job_definition["spec"]["rayClusterSpec"]["headGroupSpec"]["nodeSelector"] = (
            kuberay_config.headnode_node_selector.to_dict()
        )

    if kuberay_config.worker_node_selector:
        for worker_group_spec in job_definition["spec"]["rayClusterSpec"][
            "workerGroupSpecs"
        ]:
            worker_group_spec["nodeSelector"] = (
                kuberay_config.worker_node_selector.to_dict()
            )
