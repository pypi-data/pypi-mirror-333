...
class Table(LanceTable):
    ...
    def run(self, transforms: dict[str, UDF], columns: list[str], **kwargs) -> None:
        from geneva.runners.ray import run_ray_add_column

        uri = self._ltbl._dataset_uri
        run_ray_add_column(
            uri, columns, transforms, read_version=self._ltbl.version, **kwargs
        )
