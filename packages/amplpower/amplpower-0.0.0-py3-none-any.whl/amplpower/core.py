...
class PowerSystem:
    def __init__(self, case_file: str):
        ...
        self.case_file = 'src/amplpower/data/' + case_file + '.m'
        self.baseMVA = None
    ...
    def _load_data(self):
        ...
        try:
            ...
            for l in range(self.nlin):
                if self.branches.loc[l, 'RATE_A'] == 0:
                    self.branches.loc[l, 'RATE_A'] = self.default_branch_limit
    ...
    def compute_admittance_matrices(self):
        ...
        for l in range(self.nlin):
            branch = self.branches.iloc[l]  # Access branch data
            r = branch['BR_R']
            x = branch['BR_X']
            b = branch['BR_B']
            ...
            self.yff[l] = (ys + 1j * 0.5 * b) / (tau ** 2)
            self.yft[l] = -ys / (tau * np.exp(-1j * theta))
            self.ytf[l] = -ys / (tau * np.exp(1j * theta))
            self.ytt[l] = ys + 1j * 0.5 * b
