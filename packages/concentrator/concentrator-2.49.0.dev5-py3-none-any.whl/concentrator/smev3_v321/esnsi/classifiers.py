...
class ChildrenOrganizationClassifier(ClassifierBase):
    """Перечень детских школьных учреждений (для компенсации)"""

    model = Unit
    fields_info = {
        'CODE': FieldInfo(type=DT.STRING, length=255, required=True),
        'FULLNAME': FieldInfo(type=DT.STRING, length=255, required=True),
        'SHORTNAME': FieldInfo(type=DT.STRING, length=255, required=True),
        'OKATO': FieldInfo(type=DT.STRING, length=11, required=True),
        'NAMEREGION': FieldInfo(type=DT.STRING, length=255, required=True),
    }
    ...
    def queryset_filter(self):
        ...
        - НЕ проставлен чек бокс "Не показывать на портале"
        """
        return Q(kind=UnitKind.DOU) & ~Q(is_not_show_on_poral=False)

    @classmethod
    def get_data(cls, obj):
        """Данные для отправки."""

        # Парсим регион из полного адреса
        address = obj.address_full.split(',') if obj.address_full else ''
        if address:
            if address[0].isdigit() and len(address) > 1:
