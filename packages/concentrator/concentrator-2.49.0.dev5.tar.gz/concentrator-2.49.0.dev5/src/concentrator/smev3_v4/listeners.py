...
class DelegateEditWindowListener(BaseEditWinListener):
    """Добавляет в окно создания/редактирования карточки родителя чек-бокс Документ о праве нахождения в РФ."""
...
class DelegateSaveActionListener(BaseSaveListener):
    """Выполняет сохранение карточки родителя с данными чек-бокса Документ о праве нахождения в РФ."""
    ...
    def _get_instance(self, parent_model_instance, context):
        """Модель не расширяется какой-либо другой моделью, поэтому возвращаем модель Delegate."""
