...
from .constants import (
    ...
    REQUEST_REQUIRED_UNIT_STATUSES
)
...
class ChildrenOrganizationObserver(ChangesObserver):
    """Класс для отправки запроса при изменении статуса и удалении организации (ДОУ).

    При наличии изменении статуса на "Закрыто", "Ликвидировано", "Присоединена к другой организации",
    проставлен чек бокс "Не показывать на портале:" или при удалении организации (ДОУ)
    отправляется запрос CnsiRequest с вложением ClassifierDataDeleteRequest.
    """

    model = Unit
    tracked_fields = [
        'status',
        'is_not_show_on_poral',
    ]
    ...
    def validate(self, instance):
        return (instance.kind_id == self.unit_kind
                and (instance.status in REQUEST_REQUIRED_UNIT_STATUSES or instance.is_not_show_on_poral))

    def post_save(self, instance, created, **kwargs):
        if self.has_changes(instance) and self.validate(instance) :
            self.post_delete(instance=instance, context=self._get_context(instance))


    def post_delete(self, instance, context, **kwargs):
            ESNSIUnitTaskFactory(
                get_classifier_instance(ChildrenOrganizationClassifier),
...
children_organization_observer = None
if apps.is_installed('concentrator.smev3_v4'):
    ...
    children_organization_observer = ChildrenOrganizationObserver()
