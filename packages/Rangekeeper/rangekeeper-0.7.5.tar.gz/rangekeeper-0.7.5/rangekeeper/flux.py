...
def _format_series(series: pd.Series, units: pint.Unit, decimals: int = 2):
...
class Flow:
    ...
    def __init__(
        self, movements: pd.Series, units: Optional[pint.Unit] = None, name: str = None
    ):
        """
    ...
    def duplicate(self, name: str = None) -> Flow:
    ...
    def display(self, tablefmt: str = "github", decimals: int = 2):
        name = "Name: " + self.name
    ...
    def from_sequence(
        ...
        series = pd.Series(
            data=data, index=pd.Series(data=dates, name="date"), name=name, dtype=float
        )
        return cls(movements=series, units=units, name=name)

    @classmethod
    def from_dict(
        cls, movements: Dict[pd.Timestamp, float], units: pint.Unit, name: str = None
    ) -> Flow:
        """
    ...
    def pv(self, frequency: rk.duration.Type, rate: float, name: str = None) -> Flow:
        """
    ...
    def resample(self, frequency: rk.duration.Type) -> Flow:
        """
    ...
    def to_periods(self, frequency: rk.duration.Type) -> pd.Series:
        """
    ...
    def trim_to_span(self, span: rk.span.Span, name: str = None) -> Flow:
        """
        Returns a Flow with movements trimmed to the specified span
        """
        return self.__class__(
            movements=self.movements.copy(deep=True).truncate(
                before=span.start_date, after=span.end_date
            ),
            units=self.units,
            name=self.name if name is None else name,
        )

    def to_stream(self, frequency: rk.duration.Type, name: str = None) -> Stream:
        """
...
class Stream:
    ...
    def _format_flows(self, decimals: int = 2) -> pd.DataFrame:
    ...
    def __add__(self, other):
        ...
        return self.__class__(name=self.name, flows=flows, frequency=self.frequency)
    ...
    @classmethod
    def from_DataFrame(cls, name: str, data: pd.DataFrame, units: pint.Unit) -> Stream:
        flows = []
    ...
    def sum(self, name: str = None) -> Flow:
        """
    ...
    def collapse(self) -> Stream:
        ...
        """
        flows = [
            self.extract(flow_name=flow_name) for flow_name in list(self.frame.columns)
        ]
        return self.__class__(
            name=self.name,
    ...
    def resample(self, frequency: rk.duration.Type) -> Stream:
    ...
    @classmethod
    def merge(cls, streams, name: str, frequency: rk.duration.Type) -> Stream:
        # Check Units:
        ...
        return cls(name=name, flows=flows, frequency=frequency)
