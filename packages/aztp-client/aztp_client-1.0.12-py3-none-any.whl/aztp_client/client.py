...
class Aztp:
    ...
    async def verify_identity(self, agent: SecuredAgent, selfValidating: bool = True, requestAztpId: Optional[str] = None) -> bool:
        ...
        response = self.session.post(
            ...
            json={"aztpId": agent.identity.aztp_id, "selfValidating": selfValidating, "requestAztpId": requestAztpId},
            timeout=self.config.timeout,
            verify=False,  # Disable SSL verification
        )
        response.raise_for_status()
    ...
    async def verify_identity_using_agent_name(
        ...
        method: str = "node",
        selfValidating: bool = True,
        requestAztpId: Optional[str] = None
    ) -> bool:
        ...
        response = self.session.post(
            self._get_url("verify-identity"),
            json={"aztpId": aztp_id, "selfValidating": selfValidating, "requestAztpId": requestAztpId},
            timeout=self.config.timeout,
            verify=False,  # Disable SSL verification
        )
        response.raise_for_status()
