...
def _check_virtual_box_7_1() -> bool:  # noqa: WPS114
    response = execute_command(['VBoxManage', '--version'], catch_error=False)
    return response.startswith('7.1')
...
def _start_vm_by_uuid(vm_guid, start_type='gui'):
    ...
    try:
        ...
        raise exc
    print_done()
    if _check_virtual_box_7_1():
        nvram_file = _find_vm_nvram_file(vm_guid)
        if not nvram_file or not Path(nvram_file).is_file():
            print_message('VBoxManage 7.1.x version is detected. Reboot VM is required for nvram initialization!')
            reboot_vm_command = [
                'VBoxManage',
                'controlvm',
                vm_guid,
