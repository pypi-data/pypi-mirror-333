...
def run_pytest(*tests, **kwargs):
    ...
    try:
        ...
        raise RuntimeError('Pytest not installed. Install it first (i.e. `pip install pytest`)')
    return pytest.main(list(tests))
# [end] run_pytest

def run_maya(*tests, **kwargs):
    ...
    if cli:
        ...
        cmdStr += f'from utwrite import executor; executor.run_unittest(*{tests}, **{kwargs})'
        cmd.append(cmdStr)
        # print(f'cmd={cmd}')
        try:
...
def main(*tests, **kwargs):
    ...
    Examples::
        ...
        # From CLI this is called when running the executor.py file
        # i.e.
        # $ python utwrite/executor.py -t tests/
    ...
    args.update(kwargs)
    if app not in apps:
        raise RuntimeError('"%s": Given app not supported. Acceptable: %s'%(app, apps))

    if app =='maya':
        cmd += [
            'from utwrite import executor; executor.run_%s(*%s, cli=True, **%s)'%(app, tests, args)
        ]
    else:
        cmd += [
            'from utwrite import executor; executor.run_%s(*%s, **%s)'%(app, tests, args)
        ]
