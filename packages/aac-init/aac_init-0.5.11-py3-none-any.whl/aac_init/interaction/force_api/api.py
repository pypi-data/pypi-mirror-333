...
BASE_DIR = os.getenv('API_BASE_DIR')
if not BASE_DIR:
    raise ValueError("API_BASE_DIR environment variable is not set.")

FABRIC_NAME = os.getenv('FABRIC_NAME')
if not FABRIC_NAME:
    raise ValueError("FABRIC_NAME environment variable is not set.")

DATA_PATH = os.path.join(BASE_DIR, "fabric_data")
DB_PATH = os.path.join(BASE_DIR, FABRIC_NAME, "api", "task_records.db")
OUTPUT_DIR = os.path.join(BASE_DIR, FABRIC_NAME, "logs")
APP_LOG_DIR = os.path.join(BASE_DIR, FABRIC_NAME, "api")
...
async def execute_tasks(task_groups, output_dir, user_id):
    ...
    context_cache.set("is_running", False)
    return results
...
async def run_tasks(request: RunTaskRequest, background_tasks: BackgroundTasks):
    if context_cache.get("is_running"):
        return {"message": "Task execution is already in progress."}
    ...
    for task in tasks:
        ...
        converted_operations_list = []
        for operation in operations:
            if operation.isdigit():
                op_order = int(operation)
                operation = controller.get_operation_by_order(project, product, op_order)
                if not operation:
                    context_cache.set("is_running", False)
                    return {"message": f"Invalid operation order for {project}/{product}: {op_order}"}

            converted_operations_list.append(operation)

        ordered_operations = controller.resolve_execution_order(converted_operations_list, project, product)
        task_groups.append({
            "project": project,
            "product": product,
    ...
    output_dir_name = base_dir_template.format(fabric_name=fabric_name, user=user_id)
...
async def get_progress(user_id: str = Query(..., description="The user ID to query")):
    ...
    if not record:
        return {"message": "No progress found for the user."}
...
async def get_user_history(user_id: str = Query(..., description="User ID to query")):
    ...
    if not records:
        return {"message": "No history found for the user."}
