...
class EnvironmentFile:
    event_list: EventList = field(default_factory=EventList)
    objects: list[Decor] = field(default_factory=list)
    ...
    def read(self, read: PbdfReader, sector_count: int) -> None:
        self.event_list = read.any(EventList)
        self.objects = [read.any(Decor, True) for _ in range(read.i32())]
    ...
    def write(self, write: PbdfWriter, sector_count: int) -> None:
        ...
        [write.any(x) for x in self.objects]
...
class AnimFile:
    event_list: EventList = field(default_factory=EventList)
    anim_infos: list[AnimInfo] = field(default_factory=list)
    ...
    def read(self, read: PbdfReader, sector_count: int) -> None:
        self.event_list = read.any(EventList)
        self.anim_infos = [read.any(AnimInfo) for _ in range(read.i32())]
    ...
    def write(self, write: PbdfWriter, sector_count: int) -> None:
        write.any(self.event_list)
        write.i32(len(self.anim_infos))
        [write.any(x) for x in self.anim_infos]
