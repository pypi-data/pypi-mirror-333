            ...
            for j, mask in enumerate(r.masks.xy):
                ...
                if class_id == 0:
                    ...
                    if person_image.size > 0:
                        ...
                        # 计算标签的大小
                        (label_width, label_height), _ = cv2.getTextSize(emotion_label, cv2.FONT_HERSHEY_SIMPLEX, 0.9, 2)

                        # 绘制标签背景
                        cv2.rectangle(frame, (x1, y1), (x1 + label_width, y1 + label_height), color, -1)

                        # 绘制标签文本
                        cv2.putText(frame, emotion_label, (x1, y1 + label_height), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 255, 255), 2)

        # 如果没有检测到人的情况，直接返回原始图像
        if not has_person:
            return frame
    ...
    def process_video(self, cap):
        ...
        try:
            while cap.isOpened():
                ...
                resized_frame = self.resize_image(processed_frame)
