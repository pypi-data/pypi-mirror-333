...
class Data(ssc.DataDict):
    ...
    PHX_min_dT: Final[float] = OUTPUT(label='PHX min temperature difference', units='C', type='NUMBER', required='*', meta='PHX Design Solution')
    ...
    od_max_htf_m_dot: Matrix = INPUT(label='Columns: 0) T_htf_C, 1) T_amb_C,2) f_N_rc (=1 use design, <0, frac_des = abs(input),3) f_N_mc (=1 use design, <0, frac_des = abs(input),4) PHX_f_dP (=1 use design, <0 = abs(input), Rows: cases', type='MATRIX')
    ...
    od_generate_udpc: Array = INPUT(label='Columns 0) True/False,1) f_N_rc (=1 use design, =0 optimize, <0, frac_des = abs(input),2) f_N_mc (=1 use design, =0 optimize, <0, frac_des = abs(input),3) PHX_f_dP (=1 use design, <0 = abs(input)', type='ARRAY')
