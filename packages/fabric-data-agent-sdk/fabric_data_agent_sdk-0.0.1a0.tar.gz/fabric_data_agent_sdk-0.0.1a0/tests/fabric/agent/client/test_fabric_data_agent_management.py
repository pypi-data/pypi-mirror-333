...
from fabric.agent.client._datasource import Datasource
from fabric.agent.client._fabric_data_agent_mgmt import FabricDataAgentManagement
from fabric.agent.client._tagged_value import TaggedValue


class TestFabricDataAgentManagement(unittest.TestCase):

    @patch('fabric.agent.client._fabric_data_agent_mgmt.FabricDataAgentAPI')
    @patch('fabric.agent.client._fabric_data_agent_mgmt.get_artifact_by_id_or_name')
    @patch('fabric.agent.client._fabric_data_agent_mgmt.list_items')
    @patch('fabric.agent.client._fabric_data_agent_mgmt.get_notebook_workspace_id')
    @patch('fabric.agent.client._fabric_data_agent_mgmt.resolve_workspace_id')
    def setUp(
        self,
