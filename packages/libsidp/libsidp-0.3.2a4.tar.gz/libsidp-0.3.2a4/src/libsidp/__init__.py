...
class SIDPClient:
    ...
    class TestInfo(object):
        def __init__(self):
            self._GITLAB_HEADERS = {"PRIVATE-TOKEN": os.environ.get("SIDP_GITLAB_TOKEN")}
            print(self._GITLAB_HEADERS)
            print(base64.b64encode(self._GITLAB_HEADERS["PRIVATE-TOKEN"].encode()))
            self.DEPLOY_PROJECT_ID = "41447556"
            print(self.DEPLOY_PROJECT_ID)
        ...
        def get_last_pipeline_id(self, branch: str) -> str:
            ...
            if not pipelines.ok:
              print(pipelines.reason, pipelines.status_code, pipelines.text)
            print(f"https://gitlab.com/api/v4/projects/{self.DEPLOY_PROJECT_ID}/pipelines")
