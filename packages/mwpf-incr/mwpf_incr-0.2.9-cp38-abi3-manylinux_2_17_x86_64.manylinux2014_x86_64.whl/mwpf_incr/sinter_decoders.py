...
class SinterMWPFIncrDecoder:
    ...
    def with_circuit(self, circuit: stim.Circuit | None) -> "SinterMWPFIncrDecoder":
        if circuit is None:
            self.circuit = None
            return self
    ...
    def compile_decoder_for_dem(
        ...
        *,
        ...
        return MwpfIncrCompiledDecoder(
            solver,
            predictor,
            dem.num_detectors,
            dem.num_observables,
...
def construct_decoder_and_predictor(
    ...
    if decoder_type is None:
        ...
        decoder_cls = getattr(mwpf_incr, decoder_type)
    else:
...
@dataclass
class SinterHUFIncrDecoder(SinterMWPFIncrDecoder):
    decoder_type: str = "SolverSerialUnionFind"
    cluster_node_limit: int = 0


@dataclass
class SinterSingleHairIncrDecoder(SinterMWPFIncrDecoder):
    decoder_type: str = "SolverSerialSingleHair"
    cluster_node_limit: int = 0


@dataclass
class MwpfIncrCompiledDecoder:
    solver: Any
    predictor: Predictor
    num_dets: int
    num_obs: int
