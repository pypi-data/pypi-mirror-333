import numpy as np
from DSTS.calibration1 import *
from DSTS.mixup import *
from DSTS.y1_generation import *
from DSTS.utils import NNsorting

class dsts:
    def __init__(self, sort, method, no_cal, scale_method, test_method):
        """
    ...
    def generate(self, tot_iter=5, aug=5, n_comp=2) -> np.ndarray:
        ...
        print("Start mixup")
        rmixup, sf_mixup = r_mixup(self.data, k, self.scale_method, self.test_method)    
        ...
        final_data, weights = calib_data

        return synth[:int(aug*len(self.data))], final_data
