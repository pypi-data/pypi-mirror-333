...
def aissemble_encrypt_simple_aes(plain_text):
    ...
    if (plain_text is not None):
        if not os.environ.get('KRAUSENING_BASE'):
            NativeInboundWithCustomDataTypeBase.logger.warn('KRAUSENING_BASE environment variable was not set.  Using default path -> ./config')
...
class NativeInboundWithCustomDataTypeBase(AbstractPipelineStep):
    ...
    def check_and_apply_encryption_policy(self, inbound: PersonWithOneToOneRelation) -> None:
        """
        ...
        NativeInboundWithCustomDataTypeBase.logger.info('Checking encryption policies')
            ...
            for key, encrypt_policy in retrieved_policies.items():
                ...
                if encrypt_policy.encryptPhase:
                    if self.step_phase.lower() == encrypt_policy.encryptPhase.lower():
                        ...
                        NativeInboundWithCustomDataTypeBase.logger.info('Encryption policy does not apply to this phase: ' + self.step_phase)
    ...
    def apply_encryption_to_dataset(self, inbound: PersonWithOneToOneRelation, fields_to_update: List[str], algorithm: str) -> PersonWithOneToOneRelation:
        '''
            This method applies encryption to the given fields
        '''
        NativeInboundWithCustomDataTypeBase.logger.info('applying encryption')
    ...
    def get_fields_list(self, inbound: PersonWithOneToOneRelation) -> List[str]:
        '''
            This method gets the field names from the given data type
        '''
        return [p for p in dir(PersonWithOneToOneRelation) if isinstance(getattr(PersonWithOneToOneRelation,p),property)]
