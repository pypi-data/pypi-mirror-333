...
def aissemble_encrypt_simple_aes(plain_text):
    ...
    if (plain_text is not None):
        if not os.environ.get('KRAUSENING_BASE'):
            NativeInboundWithCustomDataTypeAsyncBase.logger.warn('KRAUSENING_BASE environment variable was not set.  Using default path -> ./krausening/base')
...
def aissemble_encrypt_with_vault_key(key, plain_text):
    ...
    if (plain_text is not None):
        if not os.environ.get('KRAUSENING_BASE'):
            NativeInboundWithCustomDataTypeAsyncBase.logger.warn('KRAUSENING_BASE environment variable was not set.  Using default path -> ./config')
            os.environ['KRAUSENING_BASE'] = 'krausening/base/'
...
class NativeInboundWithCustomDataTypeAsyncBase(AbstractPipelineStep):
    ...
    def check_and_apply_encryption_policy(self, inbound: PersonWithMToOneRelation) -> None:
        """
        ...
        NativeInboundWithCustomDataTypeAsyncBase.logger.info('Checking encryption policies')
            ...
            for key, encrypt_policy in retrieved_policies.items():
                ...
                if encrypt_policy.encryptPhase:
                    if self.step_phase.lower() == encrypt_policy.encryptPhase.lower():
                        ...
                        NativeInboundWithCustomDataTypeAsyncBase.logger.info('Encryption policy does not apply to this phase: ' + self.step_phase)
    ...
    def apply_encryption_to_dataset(self, inbound: PersonWithMToOneRelation, fields_to_update: List[str], algorithm: str) -> PersonWithMToOneRelation:
        '''
            This method applies encryption to the given fields
        '''
        NativeInboundWithCustomDataTypeAsyncBase.logger.info('applying encryption')
    ...
    def get_fields_list(self, inbound: PersonWithMToOneRelation) -> List[str]:
        '''
            This method gets the field names from the given data type
        '''
        return [p for p in dir(PersonWithMToOneRelation) if isinstance(getattr(PersonWithMToOneRelation,p),property)]
