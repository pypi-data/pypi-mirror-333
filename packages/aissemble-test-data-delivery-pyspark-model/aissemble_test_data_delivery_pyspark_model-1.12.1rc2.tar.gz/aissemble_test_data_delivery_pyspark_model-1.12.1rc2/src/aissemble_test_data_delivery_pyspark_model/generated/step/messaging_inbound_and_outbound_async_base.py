...
def aissemble_encrypt_simple_aes(plain_text):
    ...
    if (plain_text is not None):
        if not os.environ.get('KRAUSENING_BASE'):
            MessagingInboundAndOutboundAsyncBase.logger.warn('KRAUSENING_BASE environment variable was not set.  Using default path -> ./krausening/base')
...
def aissemble_encrypt_with_vault_key(key, plain_text):
    ...
    if (plain_text is not None):
        if not os.environ.get('KRAUSENING_BASE'):
            MessagingInboundAndOutboundAsyncBase.logger.warn('KRAUSENING_BASE environment variable was not set.  Using default path -> ./config')
            os.environ['KRAUSENING_BASE'] = 'krausening/base/'
