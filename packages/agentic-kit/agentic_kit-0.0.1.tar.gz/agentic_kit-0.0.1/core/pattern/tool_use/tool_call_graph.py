...
from ..component.utils.tools import tool_call_invoke
...
# 当前的工具tool调用任务是: {task}，
可供调用的工具tool和详细参数结构、描述，分别是：{tool},
# 生成规则：
...
class ToolCallSingleTaskGraph(ToolCallGraphBase):
    ...
    def invoke(self, state: ToolCallState):
        ...
        is_success, tool_call_response = tool_call_invoke(ai_message=response, tools=self.tools)
...
class ToolCallMultiTasksGraph(ToolCallSingleTaskGraph):
    ...
    def invoke(self, state: ToolCallState):
        ...
        for task in tasks:
            ...
            is_success, tool_call_response = tool_call_invoke(ai_message=ai_response, tools=self.tools)
            if is_success is False:
                # note: exception for retry
                raise Exception('retry')
