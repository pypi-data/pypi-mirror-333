...
from .schema import RePlannerState

default_cot_replanner_prompt = '''
# 你是一个任务规划专家
...
class CotRePlanner(PlannerBase):
    ...
    def invoke(self, state: RePlannerState):
        task = state["task"]
    ...
    def create(cls, llm: BaseChatModel, tools: List[BaseTool], **kwargs):
        ...
        if prompt is None:
            prompt = default_cot_replanner_prompt
        if plan_parser is None:
            plan_parser = PlanParserYaml()

        # note: prompt中必须包含的字段
        assert check_prompt_required_filed(prompt=prompt, required_field=['{task}', '{tool}', '{init_steps}', '{past_step_results}']) is True
