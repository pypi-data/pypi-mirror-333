...
def get_steps_v2(scopes: list[str], limit: int = 100, reverse: bool = False):
    result = []
...
def make_promise(scopes: list[str], reverse: bool = False):
    ...
    try:
        ...
        send(s, SPLIT_TOKEN.join([b'get', orjson.dumps({'scopes': scopes, 'reverse': reverse})]))
        data = receive(s)
...
async def work():
    ...
    with make_promise(settings.worker.scopes.split(',')) as (steps, args, commit):
        if not steps:
