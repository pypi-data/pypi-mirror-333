...
class ArrayUtils:
    ...
    def assign_array_3d(array3: np.ndarray, i1: int, i2: int, j1: int, j2: int, k1: int, k2: int, value: Union[float, int]):
        ...
        if i1 < 0 or j1 < 0 or k1 < 0 or i2 >= rows or j2 >= cols or k2 >= layers or i1 > i2 or j1 > j2 or k1 > k2:
    ...
    def copy_array_3d(source_array: np.ndarray, target_array: np.ndarray, i1: int, i2: int, j1: int, j2: int, k1: int, k2: int):
        ...
        # 检查索引是否超出源数组的范围
        if i1 < 0 or j1 < 0 or k1 < 0 or i2 >= src_rows or j2 >= src_cols or k2 >= src_layers:
            raise IndexError(
                f"Source array index out of range: i1={i1}, i2={i2}, j1={j1}, j2={j2}, k1={k1}, k2={k2}, source_shape={source_array.shape}"
            )

        # 检查索引是否超出目标数组的范围
        if i1 < 0 or j1 < 0 or k1 < 0 or i2 >= tgt_rows or j2 >= tgt_cols or k2 >= tgt_layers:
            raise IndexError(
    ...
    def array_1d_to_text(array: np.ndarray) -> str:
        """
        将一维数组转换为文本,相邻的重复数据采用 m*A的方法
        """
        if not isinstance(array, np.ndarray) or array.ndim != 1:
            raise ValueError("输入必须是一维 numpy 数组")

        text, count, prev = [], 1, array[0]
        for current in array[1:]:
            if current == prev:
                count += 1
            else:
                text.append(f"{count}*{prev}" if count > 1 else str(prev))
                count, prev = 1, current
        text.append(f"{count}*{prev}" if count > 1 else str(prev))  # 最后一个元素
        return " ".join(text)
