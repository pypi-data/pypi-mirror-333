...
from ._generated import ConvertToManagedIcebergTableRequest, RefreshIcebergTableRequest
...
class IcebergTableCollection(SchemaObjectCollectionParent["IcebergTableResource"]):
    ...
    def create(
        ...
        *,
        ...
        if as_select:
            self._api.create_snowflake_managed_iceberg_table_as_select(
            ...
            self._api.create_snowflake_managed_iceberg_table_like(
                database=self.database.name,
                var_schema=self.schema.name,
                name=like,
        ...
        elif from_aws_glue_catalog:
            self._api.create_unmanaged_iceberg_table_from_aws_glue_catalog(
                database=self.database.name,
                var_schema=self.schema.name,
            ...
            self._api.create_unmanaged_iceberg_table_from_delta(
                database=self.database.name,
                var_schema=self.schema.name,
                iceberg_table_from_delta=IcebergTableFromDelta(**iceberg_table.to_dict()),
                create_mode=StrictStr(real_mode),
            )
        elif from_iceberg_files:
            self._api.create_unmanaged_iceberg_table_from_iceberg_files(
                database=self.database.name,
                var_schema=self.schema.name,
                iceberg_table_from_iceberg_files=IcebergTableFromIcebergFiles(**iceberg_table.to_dict()),
                create_mode=StrictStr(real_mode),
            )
        elif from_iceberg_rest:
            self._api.create_unmanaged_iceberg_table_from_iceberg_rest(
                database=self.database.name,
                var_schema=self.schema.name,
                iceberg_table_from_iceberg_rest=IcebergTableFromIcebergRest(**iceberg_table.to_dict()),
                create_mode=StrictStr(real_mode),
            )
        elif clone_iceberg_table:
            ...
            self._api.clone_snowflake_managed_iceberg_table(
                source_database,
                source_schema,
            ...
            self._api.create_snowflake_managed_iceberg_table(
                self.database.name,
                self.schema.name,
                iceberg_table,
                create_mode=StrictStr(real_mode),
