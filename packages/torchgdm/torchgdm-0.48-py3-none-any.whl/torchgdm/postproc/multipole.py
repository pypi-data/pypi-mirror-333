...
def scs(
    ...
    **kwargs,
    ...
    if normalization_E0:
        e_inc = get_e0(sim, wavelength, r0.unsqueeze(0), illumination_index)
    ...
    return dict(scs_ed=scs_ed.real, scs_md=scs_md.real, scs_eq=scs_eq.real, scs_mq=scs_mq.real)
...
def ecs(
    ...
    *Caution:*
    ...
    e_inc = get_e0(sim, wavelength, r0.unsqueeze(0), illumination_index)[:, 0, :]
    e_inc_cj = torch.conj(e_inc)
    h_inc = get_h0(sim, wavelength, r0.unsqueeze(0), illumination_index)[:, 0, :]
    h_inc_cj = torch.conj(h_inc)
