...
def mount_outlet(
    ...
    return _current_mount_impl(auth).mount_outlet(key, auth, authority, interface)


def get_outlet(key: str, auth: av.AvAuthorization):
    return _current_mount_impl(auth).get_outlet(key, auth)
...
class _Mount:
    ...
    @staticmethod
    def detect_current_implementation(auth: av.AvAuthorization) -> "_Mount":
        """
    ...
    @abstractmethod
    def get_outlet(self, key: str, auth: av.AvAuthorization) -> av.AvEntity:
        pass
...
@cache
def _get_mount_outlet() -> av.AvEntity:
    return env.get_or_raise(env.MOUNT_OUTLET, av.AvEntity.from_str)


def _current_mount_impl(auth: av.AvAuthorization) -> _Mount:
    if not hasattr(_current_mount_impl, "res"):
        res = _Mount.detect_current_implementation(auth)
        _current_mount_impl.res = res  # pyright: ignore noqa
    return _current_mount_impl.res  # pyright: ignore noqa
