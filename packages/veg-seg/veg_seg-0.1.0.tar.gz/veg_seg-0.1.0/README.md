...
If using a Windows environment, you need to manually install the following packages:
```
conda install graphviz=2.42.3 cudnn=8.1 cudatoolkit=11.2
```

Graphviz needs to be installed separately from the conda-forge channel for plotting to work properly. The cudnn and cudatoolkit packages are required for the GPU version of Tensorflow to work properly.
