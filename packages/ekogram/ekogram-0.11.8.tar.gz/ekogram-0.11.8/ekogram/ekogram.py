...
def create_answer(prompt: str = None, messages: str = None, model: str = "openai", system: str = "", json_mode: bool = False, private: bool = True):
    """
    Доступные модели:
    ...
    try:
        if prompt is not None:
            payload = {
                ...
                "system": system,
                "private": private}
            response = requests.get(f"https://text.pollinations.ai/{model}", headers={"Content-Type": "application/json"}, data=json.dumps(payload), timeout=60.0)
        elif messages is not None:
            payload = {"model": model, "messages": messages, "jsonMode": json_mode}
            response = requests.post(f"https://text.pollinations.ai/{model}", headers={"Content-Type": "application/json"}, data=json.dumps(payload), timeout=60.0)
        else:
            raise ValueError("Забыли указать prompt или messages")
...
def generate_image(prompt: str, width: int = 1024, height: int = 1024, seed: int = None, enhance: bool = True, private: bool = True, nologo: bool = False, model: str = 'flux', save: bool = False):
    ...
    if save:
        ...
        response.raise_for_status()
        with open('gen_img.jpg', 'wb') as file:
