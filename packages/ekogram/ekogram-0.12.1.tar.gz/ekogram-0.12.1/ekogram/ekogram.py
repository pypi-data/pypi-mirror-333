    ...
    try:
        if prompt is not None:
            payload = {
                ...
                "system": system,
                "private": private}
            response = requests.get(f"https://text.pollinations.ai/", headers={"Content-Type": "application/json"}, data=json.dumps(payload), timeout=60.0)
        elif messages is not None:
            payload = {"messages": messages, "model": model, "jsonMode": json_mode}
