...
def create_prompt_answer(prompt: str, messages: str, model: str = "openai", system: str = "", json_mode: bool = False, private: bool = True):
    """
    Доступные модели:
    ...
    try:
        if prompt:
            payload = {"model": model, "prompt": prompt, "json": json_mode, "system": system, "private": private}
            response = requests.get(f"https://text.pollinations.ai/openai", headers={"Content-Type": "application/json"}, data=json.dumps(payload), timeout=60.0)
        elif messages:
            payload = {"model": model, "messages": messages, "jsonMode": json_mode}
            ...
            raise Exception("Забыли указать prompt или messages")
...
def generate_vision(messages: dict, model: str = "openai"):
    """
    Доступные модели:
    "openai" - OpenAI GPT-4o-mini |
    "openai-large" - OpenAI GPT-4o |
    ...
    try:
        if not messages:
            return "Забыли указать messages"
...
def generate_audio(prompt: str, messages: dict, voice: str = "alloy"):
    """
    Голоса:
    "alloy" | "echo" | "fable" | "onyx" | "nova" | "shimmer" | "coral" | "verse" | "ballad" |
    "ash" | "sage" | "amuch" | "aster" | "brook" | "clover" | "dan" | "elan" | "marilyn" | "meadow" |
    """
    try:
        if prompt:
            response = requests.get(f"https://text.pollinations.ai/{prompt}?model=openai-audio&voice={voice}", timeout=60.0)
        elif messages:
            payload = {"model": "openai-audio", "messages": messages, "voice": voice}
            response = requests.post(f"https://text.pollinations.ai/", headers={"Content-Type": "audio/mpeg"}, data=json.dumps(payload), timeout=60.0)
        else:
            raise Exception("Забыли указать prompt или message")
        response.raise_for_status()
...
def generate_image(prompt: str, width: int = 1024, height: int = 1024, seed: int = None, enhance: bool = True, private: bool = True, nologo: bool = False, model: str = 'flux', save: bool = False):
    """
    Доступные модели:
    "turbo" - Stable Diffusion 1.5 |
    "flux" - Stable Diffusion 2.1 |
    """
    seed = random.randint(20, 80)
    if not prompt:
        return "Забыли указать prompt"
    if save:
