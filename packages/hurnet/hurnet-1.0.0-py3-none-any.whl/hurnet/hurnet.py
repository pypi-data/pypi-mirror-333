...
class HurNet:
	def __init__(self, architecture='multi_layer', fx=True):
		try:
			...
			if architecture == 'multi_layer' and fx: from _hurnet import MultiLayerHurNetFX as HUR_NET
			elif architecture == 'single_layer' and fx: from _hurnet import SingleLayerHurNetFX as HUR_NET
			elif architecture == 'multi_layer' and not fx: from _hurnet import MultiLayerHurNet as HUR_NET
			elif architecture == 'single_layer' and not fx: from _hurnet import SingleLayerHurNet as HUR_NET
			else: from _hurnet import MultiLayerHurNetFX as HUR_NET
	...
	def addHiddenLayer(self, num_neurons=1):
		try:
			HUR_NET = None
			if not self.__architecture.startswith('multi') and self.__fx: from _hurnet import MultiLayerHurNetFX as HUR_NET
			elif not self.__architecture.startswith('multi') and not self.__fx: from _hurnet import MultiLayerHurNet as HUR_NET
...
def measure_execution_time(function=print, display_message=True, *args, **kwargs):
	from _hurnet import measure_execution_time
	return measure_execution_time(function, display_message, args, kwargs)
def tensor_similarity_percentage(obtained_output=[], expected_output=[]):
	from _hurnet import tensor_similarity_percentage
