...
class Image:
    ...
    def edit(self, image_url: str, prompt: str, output_path: str = None, upload_first: bool = False):
        if upload_first:
            image_url = self.computer.files.upload(image_url)
        url = self.computer.ai.cloud("edit_image", {"image": image_url, "prompt": prompt})
        if output_path:
            print("Saving image to", output_path)
        else:
            return url
