...
def get_artifact_names_from_file(source_uri: str) -> Optional[str]:
    """
    Get artifact names from artifact.txt file.

    Args:
        source_uri: Source URI where the file should be located

    Returns:
        Content of the file or None if file doesn't exist
    """
    artifact_name_path = os.path.join(source_uri, "artifact.txt")
...
def run() -> None:
    ...
    if job_success:
        ...
        if args.artifact_names == "" and job_kind == EXPORT_SKILL_JOB_KIND:
            report_job_success(
                job_client, tracker, parsed_job_name, job_kind, args.dest_uri, job_tags
            )
            return
