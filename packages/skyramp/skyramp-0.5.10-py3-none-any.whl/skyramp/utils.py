...
def log_to_robot(test_case_data, error=False):
    ...
    if error:
        level = "ERROR"
    tc_name = (
        f"Skyramp Test Case for {test_case_data['method']} {test_case_data['path']}"
    )
    if "description" in test_case_data:
        tc_name = test_case_data["description"]
    ...
    if BuiltIn().get_variable_value("${listenerType}") == "RobotListenerV1":
        ...
        BuiltIn().log(
            json.dumps(test_case_data, indent=2, sort_keys=True, ensure_ascii=False),
            level=level,
        )
