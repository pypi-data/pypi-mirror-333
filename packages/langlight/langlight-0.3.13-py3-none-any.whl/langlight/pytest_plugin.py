...
def pytest_addoption(parser):
    ...
    Skip if --langlight-output is already defined.
    """
    try:
        # Try to add the option, will raise if it already exists
        group = parser.getgroup("langlight", "LangSmith")
        group.addoption(
            "--langlight-output",
            action="store_true",
            default=False,
...
def _handle_output_args(args):
    ...
    if any(opt in args for opt in ["--langlight-output"]):
...
def pytest_runtest_call(item):
    """Apply LangSmith tracking to tests marked with @pytest.mark.langlight."""
    marker = item.get_closest_marker("langlight")
    if marker:
...
def pytest_report_teststatus(report, config):
    ...
    if config.getoption("--langlight-output"):
        return "", "", ""
...
def pytest_configure(config):
    """Register the 'langlight' marker."""
    config.addinivalue_line(
        "markers", "langlight: mark test to be tracked in LangSmith"
    )
    if config.getoption("--langlight-output"):
        if not importlib.util.find_spec("rich"):
            msg = (
                "Must have 'rich' installed to use --langlight-output. "
                "Please install with: `pip install -U 'langlight[pytest]'`"
            )
            raise ValueError(msg)
        if os.environ.get("PYTEST_XDIST_TESTRUNUID"):
            msg = (
                "--langlight-output not supported with pytest-xdist. "
                "Please remove the '--langlight-output' option or '-n' option."
            )
            raise ValueError(msg)
        if ls_utils.test_tracking_is_disabled():
            msg = (
                "--langlight-output not supported when env var"
                "LANGSMITH_TEST_TRACKING='false'. Please remove the"
                "'--langlight-output' option "
