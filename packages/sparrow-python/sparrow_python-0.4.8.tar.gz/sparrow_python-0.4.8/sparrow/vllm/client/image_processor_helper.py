...
class ImageProcessor:
    ...
    async def get_pil_image(self, image_source: str, max_width: Optional[int] = None, max_height: Optional[int] = None, max_pixels: Optional[int] = None, use_cache: bool = True, cache_dir: str = DEFAULT_CACHE_DIR, force_refresh: bool = False, return_cache_path: bool = False) -> Image.Image:
        """获取图像，支持多种输入源和大小限制
        ...
        if return_cache_path:
            image, cache_path = await get_pil_image(image_source, session, use_cache=use_cache, cache_dir=cache_dir, force_refresh=force_refresh, return_cache_path=return_cache_path)
        else:
            image = await get_pil_image(image_source, session, use_cache=use_cache, cache_dir=cache_dir, force_refresh=force_refresh, return_cache_path=return_cache_path)
