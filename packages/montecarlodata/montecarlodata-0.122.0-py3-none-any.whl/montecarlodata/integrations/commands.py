...
def add_teradata(ctx, name, sslmode, logmech, ssl_ca_directory, **kwargs):
    """
    ...
    if ssl_ca_directory and kwargs.get("ssl_ca"):
        complain_and_abort("You can only use ca_directory OR ca_data.")
    elif ssl_ca_directory:
        dir_path = Path(ssl_ca_directory)
        pem_files = list(dir_path.glob("*.pem"))

        contents = [pem.read_text() for pem in pem_files]
        ca_data = "\n".join(contents)

        kwargs.setdefault("connection_settings", {}).setdefault("ssl_options", {})["ca_data"] = (
            ca_data
        )
