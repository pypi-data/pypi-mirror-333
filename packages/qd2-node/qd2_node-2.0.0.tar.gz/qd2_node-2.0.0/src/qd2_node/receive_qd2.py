            ...
            async for message in input_queue:  # Asynchronously consume messages from the input queue
                async with message.process():
                    ...
                    if any("key_ID" in key for key in message_body):
                        ...
                        call_id = message_body["call_id"]
                        if n in container:
                            # Obtener las listas de key_IDs y keys
                            print(container)
                            print(key_ID)
                            key_IDs = container[n]['key_IDs']
                            keys = container[n]['keys']

                            # Verificar si el key_ID está en la lista
                            if key_ID in key_IDs:
                                # Obtener la posición del key_ID
                                index = key_IDs.index(key_ID)
                                
                                # Extraer la key correspondiente
                                key = keys[index]
                                message = {"key":str(key)}
                            else:
                            ...
                            print("No key with specified node")
                            message={"key":"No key with specified node"}
                            await asyncio.sleep(1)
                            await exchange.publish(
                                aio_pika.Message(json.dumps(message).encode()),
                                routing_key=call_id,
