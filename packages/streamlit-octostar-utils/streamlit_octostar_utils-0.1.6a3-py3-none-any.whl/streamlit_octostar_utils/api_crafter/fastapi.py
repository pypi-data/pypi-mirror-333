...
class DefaultErrorRoute:
    ...
    def add_default_exceptions_handler(fs_app, debug=False, excs_to_status_codes=DEFAULT_STATUS_CODE_MAPPINGS):
        async def async_handle_error(request: Request, exc: Exception, excs_to_status_codes: dict):
        ...
        fs_app.add_exception_handler(
            RequestValidationError, lambda r, e: async_handle_error(r, e, excs_to_status_codes)
        )
        fs_app.add_exception_handler(
            StarletteHTTPException, lambda r, e: async_handle_error(r, e, excs_to_status_codes)
        )
        fs_app.add_exception_handler(BaseException, lambda r, e: async_handle_error(r, e, excs_to_status_codes))
