...
from .Weighted import find_diff, find_stan_optimal_weights
...
def wave_prediction_with_models(delay_definition_flag, delayed_start_matrix, predictions, prediction_length, delay, WAVE, waveID,
    ...
    fit_pred_weeks_ml = pd.concat([fit_weeks_ml.tail(training_period), pred_weeks_ml], axis=0).reset_index(drop=True)
    fit_pred_y_ML = pd.concat([train_y[-training_period:], test_y], axis=0).reset_index(drop=True)
    print('fit_pred_y_STAT', fit_pred_y_STAT.dtypes)  # Should be numeric
    print('fit_pred_y_ML', fit_pred_y_ML.dtypes)  # Should be numeric
    ...
    for wave in WAVES_future:
        ...
        for index, row in wave_df.iterrows():
            ...
            if len(lagged_data) < lagged_amount+1:#what I can do is, start from history wave to get the ML data
                ...
                WAVE_history.add_one_week(row_df, df)
                continue
            else:
