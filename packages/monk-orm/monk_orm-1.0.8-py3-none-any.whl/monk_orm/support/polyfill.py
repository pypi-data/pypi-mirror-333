...
from monk_orm.support.util import refresh_dirty, refresh_table
...
def patch_autoincrement_timestamp():
    ...
    This is used by MonkDB's MLflow adapter.

    TODO: Maybe enable through a dialect parameter `monkdb_polyfill_autoincrement` or such.
    """
...
def check_uniqueness_factory(sa_entity, *attribute_names):
    ...
    MonkDB does not support the UNIQUE constraint on columns. This attempts to emulate it.

    This is used by MonkDB's MLflow adapter.

    TODO: Maybe enable through a dialect parameter `monkdb_polyfill_unique` or such.
    """  # noqa: E501
    ...
    def check_uniqueness(mapper, connection, target):
        ...
        if isinstance(target, sa_entity):
            ...
            for attribute_name in attribute_names:
                stmt = stmt.filter(
                    getattr(sa_entity, attribute_name) == getattr(
...
def refresh_after_dml_session(session: sa.orm.Session):
    ...
    MonkDB is eventually consistent, i.e. write operations are not flushed to
    ...
    This is used by MonkDB's LangChain adapter.

    TODO: Maybe enable through a dialect parameter `monkdb_dml_refresh` or such.
    """  # noqa: E501
...
def refresh_after_dml_engine(engine: sa.engine.Engine):
    ...
    This is used by MonkDB's Singer/Meltano and `rdflib-sqlalchemy` adapters.
    """
