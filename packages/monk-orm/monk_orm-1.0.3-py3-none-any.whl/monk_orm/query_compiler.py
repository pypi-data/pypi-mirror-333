...
from .sql_alchemy_version import SQL_ALCHEMY_1_4, SQL_ALCHEMY_VERSION
from .supported_types.geospatial import Geopoint, Geoshape
from .supported_types.objects import MutableDict, ObjectTypeImpl
...
def rewrite_update(clauseelement, multiparams, params):
    ...
    clause._monkdb_specific = True
    return clause, _multiparams, params


@sa.event.listens_for(sa.engine.Engine, "before_execute", retval=True)
def monkdb_before_execute(conn, clauseelement, multiparams, params, *args, **kwargs):
    is_monkdb = type(conn.dialect).__name__ == "MonkDBDialect"
    if is_monkdb and isinstance(clauseelement, sa.sql.expression.Update):
        ...
        clauseelement, multiparams, params = rewrite_update(
...
class MonkDBDDLCompiler(compiler.DDLCompiler):
    ...
    def get_column_specification(self, column, **kwargs):
        ...
        if column.dialect_options["monkdb"].get("index") is False:
        ...
        if column.dialect_options["monkdb"].get("columnstore") is False:
    ...
    def visit_foreign_key_constraint(self, constraint, **kw):
        """
        MonkDB does not support foreign key constraints.
        """
        warnings.warn(
            "MonkDB does not support foreign key constraints, "
    ...
    def visit_unique_constraint(self, constraint, **kw):
        """
        MonkDB does not support unique key constraints.
        """
        warnings.warn(
            "MonkDB does not support unique constraints, "
            "they will be omitted when generating DDL statements.",
            stacklevel=2,
        )
        return
...
class MonkDBTypeCompiler(compiler.GenericTypeCompiler):
    ...
    def visit_ARRAY(self, type_, **kw):
        if type_.dimensions is not None and type_.dimensions > 1:
            raise NotImplementedError(
                "MonkDB doesn't support multidimensional arrays")
    ...
    def visit_FLOAT_VECTOR(self, type_, **kw):
        ...
        if dimensions is None:
            raise ValueError(
                "FloatVector must be initialized with dimension size")
...
class MonkDBCompiler(compiler.SQLCompiler):
    ...
    def visit_ilike_op_binary(self, binary, operator, **kw):
        ...
        if binary.modifiers.get("escape", None) is not None:
            raise NotImplementedError(
                "Unsupported feature: ESCAPE is not supported")
        if self.dialect.has_ilike_operator():
            return "%s ILIKE %s" % (
    ...
    def for_update_clause(self, select, **kw):
        warnings.warn(
            "MonkDB does not support the 'INSERT ... FOR UPDATE' clause, "
...
class MonkDBIdentifierPreparer(sa.sql.compiler.IdentifierPreparer):
    """
    Define MonkDB's reserved words to be quoted properly.
    """

    reserved_words = set(
        list(POSTGRESQL_RESERVED_WORDS) + MONKDB_RESERVED_WORDS)

    def _unquote_identifier(self, value):
        if value[0] == self.initial_quote:
            value = value[1:-
                          1].replace(self.escape_to_quote, self.escape_quote)
        return value
