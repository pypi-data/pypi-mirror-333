...
from monk_orm.compiler import MonkDBCompiler


class MonkDBCompilerSA10(MonkDBCompiler):
    ...
    def visit_update(self, update_stmt, **kw):
        ...
        # MonkDB patch.
        if not update_stmt.parameters and not hasattr(update_stmt, "_monkdb_specific"):
        ...
        table_text = self.update_tables_clause(
            update_stmt, update_stmt.table, extra_froms, **kw)
        ...
        for k, v in crud_params:
            clause = k._compiler_dispatch(
                self, include_table=include_table) + " = " + v
        ...
        if self.returning or update_stmt._returning:
            ...
            if self.returning_precedes_values:
                text += " " + \
                    self.returning_clause(update_stmt, self.returning)
...
def _get_crud_params(compiler, stmt, **kw):
    ...
    # deactivated in order to satisfy the rewriting logic of the MonkDB
