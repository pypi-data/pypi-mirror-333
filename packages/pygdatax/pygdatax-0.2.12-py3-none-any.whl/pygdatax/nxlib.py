...
def loadfile(file, mode='r+'):
    ...
    if extension == '.edf':
        ...
        pygdatax.cansas.paxy2nxsas(file)
        extension = '.nxs'
...
def get_detector_number(root):
    """
    Count the number of detectors present withn the NXintrument field of the nexus file

    :param root: NXroot of the data file
    :type root: NXroot
    :return: number of detector nDet
    :rtype: int
    """
    nDet = 0
    entry_key = None
    for key in root.keys():
        if isinstance(root[key], nx.NXentry):
            entry_key = key
    if entry_key is None:
        raise KeyError('No NXentry found within the file')
    entry = root[entry_key]
    inst_key = None
    for key in entry.keys():
        if isinstance(entry[key], nx.NXinstrument):
            inst_key = key
    if inst_key is None:
        raise KeyError('No NXinstrument found within file')

    path = entry_key + '/' + inst_key
    instrument = root[path]
    for key in instrument.keys():
        if isinstance(instrument[key], nx.NXdetector):
            nDet += 1
    if nDet == 0:
        raise KeyError('No NXdetector found within file')
    return nDet


def get_nxdata_key(root, entry_key):
    """
    Return the NXdata fields names present within a given entry
    :param root:
    :type root: NXroot
    :param entry_key: name of the entry where to search for NXdata
    :type entry_key: str
    :return: A list of the nxdata field names
    :rtype list
    """
    data_key = []
    if entry_key in root:
        entry = root[entry_key]
        for key in entry:
            if isinstance(entry[key], nx.NXdata) and not isinstance(entry[key], nx.NXmonitor):
                data_key.append(key)
    return data_key


def concatnxdata(nxdataList):
    """
        Concatenate 1D NXdata
        :param nxdataList: list of NXdata nxdata to be concatenated
        :type nxdataList: list
        :return: Concantenated NXdata
        :rtype: NXdata
    """
    if len(nxdataList) <= 1:
        return
    field_keys = list(nxdataList[0].keys())
    field_keys.remove('title')
    attrsList = [nxdataList[0][i].attrs for i in field_keys]
    signal_key = nxdataList[0].signal
    axes_key = nxdataList[0].axes
    data_conc = nx.NXdata()
    for key, attr in zip(field_keys, attrsList):
        m = np.concatenate([data[key].nxdata for data in nxdataList])
        data_conc.insert(nx.NXfield(m, name=key, attrs=attr))
    data_conc.attrs['signal'] = signal_key
    data_conc.attrs['axes'] = axes_key
    return data_conc
