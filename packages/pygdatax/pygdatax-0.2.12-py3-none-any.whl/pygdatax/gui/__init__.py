    ...
    if os.path.isfile(nxs_filepath):
        if nxs_filepath.endswith('.nxs'):
            try:
                ...
                des.append(datetime.fromtimestamp(root.nxfile.mtime).strftime('%Y-%m-%d %H:%M:%S'))
                root.close()
...
class FileBrowserWidget(qt.QWidget):
    """
    QWidget to select a file by typing its path or by using a file browser button.
    By typing the path must be validated by pressing enter/return key. If the text is red, this means that the file does not exist
    """
    fileChanged = qt.pyqtSignal(str)

    def __init__(self, label='file:', directory="", extensions="All files (*.*)", parent=None):
        """
        Contruct the class
        :param label: label of the file
        :type label: str
        :param directory: base directory for file browser button
        :type directory: str
        :param extensions: list of extension to be taken into account
        :type extensions: str
        ...
        self.fileLineEdit.setToolTip("press enter to validate file path")
        self.fileLineEdit.setStyleSheet("color: red")
        self.browseButton = qt.QPushButton()
        self.browseButton.setIcon(getQIcon('directory.ico'))

        layout = qt.QHBoxLayout()
        layout.addWidget(qt.QLabel(self.label))
        ...
        self.fileLineEdit.editingFinished.connect(self.on_textChanged)

    def browse_file(self):
        fname, ext = qt.QFileDialog.getOpenFileName(self, self.label, self.directory, self.extensions,
                                                    options=qt.QFileDialog.DontUseNativeDialog)
        if fname:
            self.fileLineEdit.setStyleSheet("color: black")
    ...
    def on_textChanged(self):
        ...
        if not os.path.isfile(text):
            ...
            self.fileLineEdit.setStyleSheet("color: black")
            self.directory = os.path.dirname(text)
...
class DirectoryBrowserWidget(qt.QWidget):
    directoryChanged = qt.pyqtSignal(str)

    def __init__(self, label='directory:', directory="", parent=None):
        super(DirectoryBrowserWidget, self).__init__(parent=parent)
        if os.path.isdir(directory):
            self.directory = directory
        else:
            self.directory = os.path.expanduser("~")
        self.label = label
        self.directoryLineEdit = qt.QLineEdit(parent=self)
        self.directoryLineEdit.setToolTip("press enter to validate directory path")
        self.directoryLineEdit.setStyleSheet("color: red")
        self.browseButton = qt.QPushButton()
        self.browseButton.setIcon(getQIcon('directory.ico'))
        ...
        # connect signal
        self.browseButton.clicked.connect(self.browse_file)
        self.directoryLineEdit.editingFinished.connect(self.on_textChanged)

    def browse_file(self):
        fname = qt.QFileDialog.getExistingDirectory(self, self.label, self.directory,
                                                    options=qt.QFileDialog.DontUseNativeDialog)
        if fname:
            self.directoryLineEdit.setStyleSheet("color: black")
            self.directoryLineEdit.setText(fname)
    ...
    def get_directory(self):
        return self.directoryLineEdit.text()

    def set_directory(self, dirname):
        self.directoryLineEdit.setText(dirname)
        self.directoryLineEdit.editingFinished.emit()


class SaveFileBrowserWidget(qt.QWidget):
    fileChanged = qt.pyqtSignal(str)

    def __init__(self, label='save file:', directory="", extensions="All files (*.*)", parent=None):
        super(SaveFileBrowserWidget, self).__init__(parent=parent)
        if os.path.isdir(directory):
            self.directory = directory
        else:
            self.directory = os.path.expanduser("~")
        ...
        self.fileLineEdit = qt.QLineEdit(parent=self)
        self.fileLineEdit.setToolTip("press enter to validate directory path")
        self.fileLineEdit.setStyleSheet("color: red")
        self.browseButton = qt.QPushButton()
        self.browseButton.setIcon(getQIcon('directory.ico'))
    ...
    def browse_file(self):
        fname, ext = qt.QFileDialog.getSaveFileName(self, self.label, self.directory, self.extensions,
                                                    options=qt.QFileDialog.DontUseNativeDialog)
        if fname:
            self.fileLineEdit.setStyleSheet("color: black")
            self.fileLineEdit.setText(fname)
...
if __name__ == "__main__":
    ...
    des  = get_nxs_description("/home/achennev/Bureau/PIL pour tiago/PIL NP/2021-07-21_TOC/2021-07-21_TOC_0_89050.nxs")
    print(des)
    des2  = get_nxs_description("/home/achennev/Bureau/PIL pour tiago/PIL NP/2021-07-21_TOC/2021-07-21_TOC_0_89060.nxs")
    print(des2)
