...
def azimutal_integration(root: nx.NXroot, mask: str = None, x0: float = None, y0: float = None, bins: int = 900,
    ...
    entry.data.nxaxes = nx.NXfield(r, name='r', attrs={'units': 'mm'})
...
def resu(root: nx.NXroot, dark_file: str = None, ec_file: str = None, eb_file: str = None,
    ...
    data.nxerrors = fs.nxerrors
    data.nxaxes = fs.nxaxes
    data.r_errors = fs.r_errors
    del entry['data']
    entry['data'] = data
    q_scale(root.nxfilename, distance=distance)#, new_entry=False)
    return
...
def cut(root: nx.NXroot, xmin: float = None, xmax: float = None) -> None:
    ...
    if np.ndim(entry.data.nxsignal.nxdata) == 1:
        ...
        data = nx.NXdata()
        data.nxsignal = entry.data.nxsignal[index]
        data.nxaxes = entry.data.nxaxes[0][index]
        if entry.data.signal + '_errors' in entry.data:
            i_errors = entry.data[entry.data.signal + '_errors'].nxdata[index]
            data.nxerrors = i_errors

        if entry.data.axes[0] + '_errors' in entry.data:
           x_errors_key = entry.data.axes[0] + '_errors'
