...
class Commands():
    ...
    def migrate(self, batch_size: int = None, dump: bool = False) -> None:
        ...
        """
        try:
            self.logger.logger.log(logging.INFO, "MIGRATION %s HAS STARTED", self.config["title"])
                        ...
                        for item in prunings:
                            ...
                            if relationship in ("one-to-one", "many-to-one"):
                                ...
                                if pruning_type == "link":
                                    ...
                                    raise ValueError(f"Incorrect 'pruning_type' was received: {pruning_type}. Allowed values: 'link', 'object'.")
                                ...
                                if pruning_type != "link":
                                    raise ValueError("Many-to-Many supports only 'link' pruning type")
                                ...
                                raise ValueError(f"Incorrect relationship type was received: {relationship}. Allowed values: '<one/many>-to-<one/many>'.")
        except Exception as err:
