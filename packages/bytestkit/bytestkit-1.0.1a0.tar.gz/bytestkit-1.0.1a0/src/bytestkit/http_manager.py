...
__all__ = "generate_signature", "_HttpManager"
...
@dataclass
class _HttpManager(ABC):
    """
    Bybit OpenAPI Http Manager

    *[Usage]
    1. Public Market API
    class MarketReq(_HttpManager):
    ...
    use_rsa_authentication: bool = field(default=False)
    max_retries: bool = field(default=3)
    retry_delay: bool = field(default=1)
    ...
    @abstractmethod
    def get_request_domain(self) -> str:
        return ""
    ...
    def send_request(self, method: str = None, path: str = None, payload: dict = None, auth: bool = None) -> Response:
        ...
        """
        method = method.upper()
        endpoint = self.get_request_domain() + path if path.startswith("/") else f"/{path}"
    ...
    def get_server_timestamp(self) -> int:
        try:
            url = self.get_request_domain() + "/v5/market/time"
