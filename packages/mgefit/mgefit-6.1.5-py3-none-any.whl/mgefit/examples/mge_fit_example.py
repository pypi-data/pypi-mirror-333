...
def fit_ngc5831_twist():
    ...
    mask = img > 0   # mask empty pixels before sky subtraction

    # Mask a nearby galaxy. Not necessary in this case, 
    # but just to illustrate how it can be done
    mask &= dist_circle(1408.1, 357.7, img.shape) > 200

    # This image does not have enough sky pixels to properly use the
    # mge.sky_level() procedure, but I do it anyway to show how it works.
    sky, skysig = mge.sky_level(img[mask], plot=True)  # estimate sky level
    plt.pause(1)  # Allow plot to appear on the screen
