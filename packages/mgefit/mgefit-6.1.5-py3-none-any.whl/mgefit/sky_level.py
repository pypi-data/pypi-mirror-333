...
def sky_level(img, plot=False, itmax=5, quiet=False):
    """
    ...
    statistics (via biweight_location and biweight_sigma). A histogram of the
    image counts is then constructed over a range defined by these initial
    values. In each iteration, an asymmetric range is selected around the
    current mean, and a Gaussian model is fitted to the histogram data in this
    range. The iterative process terminates when the change in the mean is less
    than the estimated error from the fit.
    """
    # Initial estimates
    ...
    for j in range(itmax):  # Do at most itmax iterations
        ...
        scale = np.sqrt(sol.chi2/len(sol.x))  # Assume chi2/DOF=1

        if abs(sol.x[1] - mean0) < sol.x_err[1]*scale:
            break
    ...
    if not quiet:
        print(f"Sky level: {sol.x[1]:.4g} +- {sol.x_err[1]*scale:.2g}")
        print(f"Sky sigma: {sol.x[2]:.4g} +- {sol.x_err[2]*scale:.2g}")
