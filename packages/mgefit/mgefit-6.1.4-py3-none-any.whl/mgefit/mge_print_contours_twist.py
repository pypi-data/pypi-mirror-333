...
- V2.3.2 : Used N_ELEMENTS instead of KEYWORD_SET to test non-logical keywords, Leiden, 9 May 2003, MC
- V2.4.0 : Convolved image with a Gaussian kernel instead of using the SMOOTH function before binning. Always showed contours in steps of 0.5 mag/arcsec². Replaced LOGRANGE and NLEVELS keywords with MAGRANGE, Leiden, 30 April 2005, MC
- V2.4.1 : Added /CONVOL keyword, MC, Oxford, 23 September 2008
- V2.4.2 : Used Coyote Library to select red contour levels for MGE model, MC, Oxford, 8 August 2011
- V3.0.0 : Translated from IDL into Python, MC, Aspen Airport, 8 February 2014
- V3.0.1 : Used input scale to label axes if given, avoided use of log, used data rather than model as reference for contour levels, allowed scalar ``sigmaPSF``, MC, Oxford, 18 September 2014
- V3.0.2 : Fixed extent in contour plot, MC, Oxford, 18 June 2015
- V3.0.3 : Fixed bug introduced by contour change in Matplotlib 1.5, MC, Oxford, 19 January 2016
- V3.0.4 : Removed warning about non-integer indices in Numpy 1.11, MC, Oxford, 20 January 2017
- V3.0.5 : Included ``mask`` keyword to identify bad pixels on the contour. Updated documentation, MC, Oxford, 20 March 2017
- V4.0.0 : Converted mge_print_contours into mge_print_contours_twist, MC, Oxford, 27 July 2017
- V4.0.1 : Fixed MatplotlibDeprecationWarning in Matplotlib V2.2, MC, Oxford, 13 April 2018
- V4.0.2 : Fixed DeprecationWarning in Numpy 1.9, MC, Oxford, 11 August 2020
- V4.1.0 : Computed analytic integral for the central pixel, MC, Oxford, 6 December 2021
- V4.2.0 : Added ``minlevel`` keyword, MC, Oxford, 30 March 2023
    ...
    sol : array-like, shape (3, Ngauss)
        The best-fitting MGE solution produced by ``mge_fit_sectors``:
            - ``sol[0]``: TotalCounts of each Gaussian component.
              The relation ``TotalCounts = Height*(2*np.pi*qObs*Sigma**2)``
              can be used to compute the Gaussian central surface brightness (Height).
