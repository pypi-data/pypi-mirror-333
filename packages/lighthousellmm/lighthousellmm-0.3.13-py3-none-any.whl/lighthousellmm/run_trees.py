"""Schemas for the lighthousellmm API."""
...
from lighthousellmm import utils
from lighthousellmm.client import ID_TYPE, RUN_TYPE_T, Client, _dumps_json, _ensure_uuid

logger = logging.getLogger(__name__)

lighthousellmm_PREFIX = "lighthousellmm-"
lighthousellmm_DOTTED_ORDER = sys.intern(f"{lighthousellmm_PREFIX}trace")
lighthousellmm_DOTTED_ORDER_BYTES = lighthousellmm_DOTTED_ORDER.encode("utf-8")
lighthousellmm_METADATA = sys.intern(f"{lighthousellmm_PREFIX}metadata")
lighthousellmm_TAGS = sys.intern(f"{lighthousellmm_PREFIX}tags")
lighthousellmm_PROJECT = sys.intern(f"{lighthousellmm_PREFIX}project")
...
class RunTree(ls_schemas.RunBase):
    ...
    def from_dotted_order(
        ...
        **kwargs: Any,
        ...
        """
        headers = {
            lighthousellmm_DOTTED_ORDER: dotted_order,
        }
    ...
    def from_headers(
        ...
        lighthousellmm_trace = cast(Optional[str], headers.get(lighthousellmm_DOTTED_ORDER))
        if not lighthousellmm_trace:
            lighthousellmm_trace_bytes = cast(
                Optional[bytes], headers.get(lighthousellmm_DOTTED_ORDER_BYTES)
            )
            if not lighthousellmm_trace_bytes:
                return  # type: ignore[return-value]
            lighthousellmm_trace = lighthousellmm_trace_bytes.decode("utf-8")

        parent_dotted_order = lighthousellmm_trace.strip()
    ...
    def to_headers(self) -> Dict[str, str]:
        ...
        headers = {}
        if self.trace_id:
            headers[f"{lighthousellmm_DOTTED_ORDER}"] = self.dotted_order
        baggage = _Baggage(
...
class _Baggage:
    ...
    def from_header(cls, header_value: Optional[str]) -> _Baggage:
        ...
        try:
            for item in header_value.split(","):
                ...
                if key == lighthousellmm_METADATA:
                    metadata = json.loads(urllib.parse.unquote(value))
                elif key == lighthousellmm_TAGS:
                    tags = urllib.parse.unquote(value).split(",")
                elif key == lighthousellmm_PROJECT:
    ...
    def to_header(self) -> str:
        ...
        if self.metadata:
            ...
            items.append(
                f"{lighthousellmm_PREFIX}metadata={urllib.parse.quote(serialized_metadata)}"
            )
        if self.tags:
            serialized_tags = ",".join(self.tags)
            items.append(
                f"{lighthousellmm_PREFIX}tags={urllib.parse.quote(serialized_tags)}"
            )
        if self.project_name:
            items.append(
                f"{lighthousellmm_PREFIX}project={urllib.parse.quote(self.project_name)}"
            )
        return ",".join(items)
