...
from lighthousellmm import client as ls_client
from lighthousellmm import run_trees, schemas, utils
from lighthousellmm._internal import _aiter as aitertools
from lighthousellmm.env import _runtime_env
    ...
    **kwargs: Any,
    ...
    Args:
        ...
        parent: The parent run to use for the context. Can be a Run/RunTree object,
            ...
            Defaults to None.
        client: The client to use for logging the run to lighthousellmm. Defaults to None,
...
def is_traceable_function(func: Any) -> TypeGuard[SupportslighthousellmmExtra[P, R]]:
...
) -> SupportslighthousellmmExtra[P, R]:
...
class lighthousellmmExtra(TypedDict, total=False):
    ...
    """Optional lighthousellmm client."""
...
@runtime_checkable
class SupportslighthousellmmExtra(Protocol, Generic[P, R]):
    """Implementations of this Protoc accept an optional lighthousellmm_extra parameter.

    Args:
        *args: Variable length arguments.
        lighthousellmm_extra (Optional[lighthousellmmExtra): Optional dictionary of
            additional parameters for lighthousellmm.
    ...
    def __call__(  # type: ignore[valid-type]
        self,
        *args: P.args,
        lighthousellmm_extra: Optional[lighthousellmmExtra] = None,
        **kwargs: P.kwargs,
    ) -> R:
        ...
        Args:
            *args: Variable length argument list.
            lighthousellmm_extra: Optional dictionary containing additional
                parameters specific to lighthousellmm.
...
) -> Callable[[Callable[P, R]], SupportslighthousellmmExtra[P, R]]: ...
...
def traceable(
    ...
    **kwargs: Any,
    ...
    """Trace a function with lighthousellmm.
    ...
    Args:
        ...
        metadata: The metadata to add to the run. Defaults to None.
        tags: The tags to add to the run. Defaults to None.
        client: The client to use for logging the run to lighthousellmm. Defaults to
        ...
        dangerously_allow_filesystem: Whether to allow filesystem access for attachments.
            ...
            Traces that reference local filepaths will be uploaded to lighthousellmm.
    ...
    Note:
            - Requires that lighthousellmm_TRACING_V2 be set to 'true' in the environment.
    ...
    Examples:
        ...
        Manually passing lighthousellmm_extra:
        ...
        .. code-block:: python
            ...
            manual_extra_function(5, lighthousellmm_extra={"metadata": {"version": "1.0"}})
    """
    run_type = cast(
    ...
    def decorator(func: Callable):
        ...
        async def async_wrapper(
            ...
            **kwargs: Any,
            ...
            run_container = await aitertools.aio_to_thread(
                ...
                lighthousellmm_extra=lighthousellmm_extra,
                args=args,
                kwargs=kwargs,
            )
        ...
        async def async_generator_wrapper(
            *args: Any, lighthousellmm_extra: Optional[lighthousellmmExtra] = None, **kwargs: Any
        ) -> AsyncGenerator:
            if not func_accepts_config:
                kwargs.pop("config", None)
            run_container = await aitertools.aio_to_thread(
        ...
        setattr(selected_wrapper, "__lighthousellmm_traceable__", True)
...
class trace:
    """Manage a lighthousellmm run in context.
    ...
    Args:
        ...
        client (Optional[ls_client.Client], optional): lighthousellmm client for custom settings. Defaults to None.
        ...
        extra (Optional[Dict], optional): Extra data to send to lighthousellmm. Use 'metadata' instead. Defaults to None.
...
def as_runnable(traceable_fn: Callable) -> Runnable:
    """Convert a function wrapped by the lighthousellmm @traceable decorator to a Runnable.

    Args:
        traceable_fn (Callable): The function wrapped by the @traceable decorator.

    Returns:
        Runnable: A Runnable object that maintains a consistent lighthousellmm
            tracing context.
    ...
    if not is_traceable_function(traceable_fn):
        ...
        raise ValueError(
            f"as_runnable expects a function wrapped by the lighthousellmm"
    ...
    class RunnableTraceable(RunnableLambda):
        ...
        This helps maintain a consistent lighthousellmm tracing context.
        """
        ...
        def _wrap_sync(
            ...
            def wrap_traceable(inputs: dict, config: RunnableConfig) -> Any:
                ...
                return func(**inputs, lighthousellmm_extra={"run_tree": run_tree})
        ...
        def _wrap_async(
            ...
            if not is_traceable_function(afunc):
                raise ValueError(
                    "RunnableTraceable expects a function wrapped by the lighthousellmm"
            ...
            async def awrap_traceable(inputs: dict, config: RunnableConfig) -> Any:
                run_tree = run_trees.RunTree.from_runnable_config(cast(dict, config))
                return await afunc_(**inputs, lighthousellmm_extra={"run_tree": run_tree})
...
def _collect_extra(extra_outer: dict, lighthousellmm_extra: lighthousellmmExtra) -> dict:
    run_extra = lighthousellmm_extra.get("run_extra", None)
    if run_extra:
...
def _get_parent_run(
    lighthousellmm_extra: lighthousellmmExtra,
    config: Optional[dict] = None,
) -> Optional[run_trees.RunTree]:
    parent = lighthousellmm_extra.get("parent")
    if parent == "ignore":
        return None
    ...
    if isinstance(parent, dict):
        return run_trees.RunTree.from_headers(
            parent,
            client=lighthousellmm_extra.get("client"),
            # Precedence: headers -> cvar -> explicit -> env var
            project_name=_get_project_name(lighthousellmm_extra.get("project_name")),
        )
    if isinstance(parent, str):
        ...
        return dort
    run_tree = lighthousellmm_extra.get("run_tree")
    if run_tree:
        return run_tree
    ...
    if _runtime_env.get_langchain_core_version() is not None:
        if rt := run_trees.RunTree.from_runnable_config(
            config, client=lighthousellmm_extra.get("client")
        ):
            ...
            if (
                ...
                # If the LangChain dotted order is more nested than the lighthousellmm
...
def _setup_run(
    ...
    lighthousellmm_extra = lighthousellmm_extra or lighthousellmmExtra()
    name = lighthousellmm_extra.get("name") or container_input.get("name")
    client_ = lighthousellmm_extra.get("client", client) or _CLIENT.get()
    parent_run_ = _get_parent_run(
        {**lighthousellmm_extra, "client": client_}, kwargs.get("config")
    )
    ...
    selected_project = (
        ...
        or lighthousellmm_extra.get("project_name")  # at invocation time
        or container_input["project_name"]  # at decorator time
        or utils.get_tracer_project()  # default
    )
    reference_example_id = lighthousellmm_extra.get("reference_example_id")
    id_ = lighthousellmm_extra.get("run_id")
    if not parent_run_ and not utils.tracing_is_enabled():
        utils.log_once(
            logging.DEBUG,
            "lighthousellmm tracing is not enabled, returning original function.",
        )
        return _TraceableContainer(
            ...
            outer_metadata=None,
            outer_tags=None,
            on_end=lighthousellmm_extra.get("on_end"),
    ...
    extra_inner = _collect_extra(extra_outer, lighthousellmm_extra)
    outer_metadata = _METADATA.get()
    outer_tags = _TAGS.get()
    context = copy_context()
    metadata_ = {
        **(lighthousellmm_extra.get("metadata") or {}),
        **(outer_metadata or {}),
    }
    ...
    tags_ = (lighthousellmm_extra.get("tags") or []) + (outer_tags or [])
...
def _is_traceable_function(func: Any) -> bool:
    return getattr(func, "__lighthousellmm_traceable__", False)
