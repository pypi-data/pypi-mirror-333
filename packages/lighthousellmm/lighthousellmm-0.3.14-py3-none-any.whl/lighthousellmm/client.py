"""Client for interacting with the lighthousellmm API.

Use the client to customize API keys / workspace connections, SSL certs,
etc. for tracing.

Also used to create, read, update, and delete lighthousellmm resources
...
import lighthousellmm
from lighthousellmm import env as ls_env
from lighthousellmm import schemas as ls_schemas
from lighthousellmm import utils as ls_utils
from lighthousellmm._internal import _orjson
from lighthousellmm._internal._background_thread import (
    TracingQueueItem,
)
from lighthousellmm._internal._background_thread import (
...
from lighthousellmm._internal._constants import (
    _AUTO_SCALE_UP_NTHREADS_LIMIT,
    _BLOCKSIZE_BYTES,
    _BOUNDARY,
    _SIZE_LIMIT_BYTES,
)
from lighthousellmm._internal._multipart import (
    MultipartPart,
    MultipartPartsAndContext,
    join_multipart_parts_and_context,
)
from lighthousellmm._internal._operations import (
...
from lighthousellmm.schemas import AttachmentInfo
...
if TYPE_CHECKING:
    ...
    from lighthousellmm import schemas
    from lighthousellmm.evaluation import evaluator as ls_evaluator
...
def _parse_token_or_url(
    ...
    if len(path_parts) >= num_parts:
        ...
        raise ls_utils.lighthousellmmUserError(f"Invalid public {kind} URL: {url_or_token}")
...
def _default_retry_config() -> Retry:
    ...
    return ls_utils.lighthousellmmRetry(**retry_params)  # type: ignore
...
def _validate_api_key_if_hosted(api_url: str, api_key: Optional[str]) -> None:
    ...
    Raises:
        lighthousellmmUserError: If the API key is not provided when using the hosted service.
    """
    # If the domain is langchain.com, raise error if no api_key
    if not api_key:
        if _is_langchain_hosted(api_url):
            warnings.warn(
                "API key must be provided when using hosted lighthousellmm API",
                ls_utils.lighthousellmmMissingAPIKeyWarning,
            )
...
def _get_tracing_sampling_rate() -> float | None:
    ...
    if sampling_rate < 0 or sampling_rate > 1:
        raise ls_utils.lighthousellmmUserError(
            "lighthousellmm_TRACING_SAMPLING_RATE must be between 0 and 1 if set."
            f" Got: {sampling_rate}"
        )
...
def _get_write_api_urls(_write_api_urls: Optional[Dict[str, str]]) -> Dict[str, str]:
    _write_api_urls = _write_api_urls or json.loads(
        os.getenv("lighthousellmm_RUNS_ENDPOINTS", "{}")
    )
    ...
    for url, api_key in _write_api_urls.items():
        ...
        if not processed_url:
            raise ls_utils.lighthousellmmUserError(
                "lighthousellmm runs API URL within lighthousellmm_RUNS_ENDPOINTS cannot be empty"
            )
...
class _lighthousellmmHttpAdapter(requests_adapters.HTTPAdapter):
    __attrs__ = [
        "max_retries",
        "config",
        "_pool_connections",
...
class Client:
    ...
    def __init__(
        ...
        *,
        ...
        info: Optional[Union[dict, ls_schemas.lighthousellmmInfo]] = None,
        api_urls: Optional[Dict[str, str]] = None,
    ) -> None:
        """Initialize a Client instance.

        Args:
            api_url (Optional[str]): URL for the lighthousellmm API. Defaults to the LANGCHAIN_ENDPOINT
                environment variable or https://api.smith.langchain.com if not set.
            api_key (Optional[str]): API key for the lighthousellmm API. Defaults to the LANGCHAIN_API_KEY
            ...
            web_url (Optional[str]): URL for the lighthousellmm web app. Default is auto-inferred from
                the ENDPOINT.
            ...
            info (Optional[ls_schemas.lighthousellmmInfo]): The information about the lighthousellmm API.
        ...
        if (
            os.getenv("lighthousellmm_ENDPOINT") or os.getenv("LANGCHAIN_ENDPOINT")
        ) and os.getenv("lighthousellmm_RUNS_ENDPOINTS"):
            raise ls_utils.lighthousellmmUserError(
                "You cannot provide both lighthousellmm_ENDPOINT / LANGCHAIN_ENDPOINT "
                "and lighthousellmm_RUNS_ENDPOINTS."
            )
        ...
        self._info = (
            info
            if info is None or isinstance(info, ls_schemas.lighthousellmmInfo)
            else ls_schemas.lighthousellmmInfo(**info)
        )
        ...
        adapter = _lighthousellmmHttpAdapter(
        ...
        # To trigger this code, set the `lighthousellmm_USE_PYO3_CLIENT` env var to any value.
        self._pyo3_client = None
        if ls_utils.get_env_var("USE_PYO3_CLIENT") is not None:
            lighthousellmm_pyo3 = None
            try:
                import lighthousellmm_pyo3  # type: ignore[import-not-found, no-redef]
            except ImportError as e:
                logger.warning(
                    "Failed to import `lighthousellmm_pyo3` when PyO3 client was requested, "
                    "falling back to Python impl: %s",
                    repr(e),
                )

            if lighthousellmm_pyo3:
                ...
                try:
                    self._pyo3_client = lighthousellmm_pyo3.BlockingTracingClient(
                        self.api_url,
                        self.api_key,
                        queue_capacity,
                        batch_size,
                    ...
                    logger.warning(
                        "Failed to instantiate `lighthousellmm_pyo3.BlockingTracingClient` "
                        "when PyO3 client was requested, falling back to Python impl: %s",
                        repr(e),
                    )

        self._settings: Union[ls_schemas.lighthousellmmSettings, None] = None
    ...
    def _repr_html_(self) -> str:
        ...
        """
        link = self._host_url
        return f'<a href="{link}", target="_blank" rel="noopener">lighthousellmm Client</a>'
    ...
    def _headers(self) -> Dict[str, str]:
        ...
        """
        headers = {
            "User-Agent": f"lighthousellmm-py/{lighthousellmm.__version__}",
    ...
    @property
    def info(self) -> ls_schemas.lighthousellmmInfo:
        """Get the information about the lighthousellmm API.

        Returns:
            ls_schemas.lighthousellmmInfo: The information about the lighthousellmm API, or None if the API is
                not available.
        """
        if self._info is None:
            try:
                ...
                self._info = ls_schemas.lighthousellmmInfo(**response.json())
            except BaseException as e:
                logger.warning(
                    f"Failed to get info from {self.api_url}: {repr(e)}",
                )
                self._info = ls_schemas.lighthousellmmInfo()
        return self._info

    def _get_settings(self) -> ls_schemas.lighthousellmmSettings:
        ...
        if self._settings is None:
            ...
            ls_utils.raise_for_status_with_text(response)
            self._settings = ls_schemas.lighthousellmmSettings(**response.json())
    ...
    def _content_above_size(self, content_length: Optional[int]) -> Optional[str]:
        if content_length is None or self._info is None:
            return None
        info = cast(ls_schemas.lighthousellmmInfo, self._info)
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            retry_on (Optional[Sequence[Type[BaseException]]]): The exceptions to retry on. In addition to:
                [lighthousellmmConnectionError, lighthousellmmAPIError].
        ...
        Raises:
            lighthousellmmAPIError: If a server error occurs.
            lighthousellmmUserError: If the request fails.
            lighthousellmmConnectionError: If a connection error occurs.
            lighthousellmmError: If the request fails.
        """
        ...
        logging_filters = [
            ls_utils.FilterlighthousellmmRetry(),
            ls_utils.FilterPoolFullWarning(host=str(self._host)),
        ]
        retry_on_: Tuple[Type[BaseException], ...] = (
            *(retry_on or ()),
            *(
                ls_utils.lighthousellmmConnectionError,
                ls_utils.lighthousellmmRequestTimeout,  # 408
                ls_utils.lighthousellmmAPIError,  # 500
            ),
        )
        ...
        for idx in range(stop_after_attempt):
            try:
                try:
                    ...
                    if response is not None:
                        ...
                        if response.status_code == 500:
                            raise ls_utils.lighthousellmmAPIError(
                                f"Server error caused failure to {method}"
                                f" {pathname} in"
                                f" lighthousellmm API. {repr(e)}"
                                f"{_context}"
                            )
                        elif response.status_code == 408:
                            raise ls_utils.lighthousellmmRequestTimeout(
                                f"Client took too long to send request to {method}"
                                f"{pathname} {_context}"
                            )
                        elif response.status_code == 429:
                            raise ls_utils.lighthousellmmRateLimitError(
                                f"Rate limit exceeded for {pathname}. {repr(e)}"
                                f"{_context}"
                            )
                        elif response.status_code == 401:
                            raise ls_utils.lighthousellmmAuthError(
                                f"Authentication failed for {pathname}. {repr(e)}"
                                f"{_context}"
                            )
                        elif response.status_code == 404:
                            raise ls_utils.lighthousellmmNotFoundError(
                                f"Resource not found for {pathname}. {repr(e)}"
                                f"{_context}"
                            )
                        elif response.status_code == 409:
                            raise ls_utils.lighthousellmmConflictError(
                                f"Conflict for {pathname}. {repr(e)}" f"{_context}"
                            )
                        else:
                            raise ls_utils.lighthousellmmError(
                                f"Failed to {method} {pathname} in lighthousellmm"
                                f" API. {repr(e)}"
                            )

                    else:
                        raise ls_utils.lighthousellmmUserError(
                            f"Failed to {method} {pathname} in lighthousellmm API."
                            f" {repr(e)}"
                        )
                    ...
                    raise ls_utils.lighthousellmmConnectionError(
                        f"Connection error caused failure to {method} {pathname}"
                        f" in lighthousellmm API. {recommendation}"
                        f" {repr(e)}"
                    ...
                    if args:
                        ...
                        emsg = msg
                    raise ls_utils.lighthousellmmError(
                        f"Failed to {method} {pathname} in lighthousellmm API. {emsg}"
                        f"{_context}"
                    ) from e
                ...
                if response is not None:
                    logger.debug("Passing on exception %s", e)
                    return response
            except ls_utils.lighthousellmmRateLimitError:
                if idx + 1 == stop_after_attempt:
                    raise
                if response is not None:
                    try:
    ...
    def upload_dataframe(
        ...
        *,
        ...
        """Upload a dataframe as individual examples to the lighthousellmm API.
        ...
        Examples:
            ...
            .. code-block:: python
                ...
                from lighthousellmm import Client
                import os
                import pandas as pd
    ...
    def upload_csv(
        ...
        *,
        ...
        """Upload a CSV file to the lighthousellmm API.
    ...
    def create_run(
        ...
        **kwargs: Any,
        ...
        """Persist a run to the lighthousellmm API.
    ...
    def _create_run(self, run_create: dict):
        for api_url, api_key in self._write_api_urls.items():
            ...
            self.request_with_retries(
                ...
                to_ignore=(ls_utils.lighthousellmmConflictError,),
            )
    ...
    def batch_ingest_runs(
        ...
        *,
        ...
        """Batch ingest/upsert multiple runs in the lighthousellmm system.
        ...
        Raises:
            lighthousellmmAPIError: If there is an error in the API request.
        ...
        for api_url, api_key in self._write_api_urls.items():
            for idx in range(1, attempts + 1):
                try:
                    ...
                    break
                except ls_utils.lighthousellmmConflictError:
                    break
                except (
                    ls_utils.lighthousellmmConnectionError,
                    ls_utils.lighthousellmmRequestTimeout,
                    ls_utils.lighthousellmmAPIError,
                ) as exc:
                    if idx == attempts:
    ...
    def update_run(
        ...
        **kwargs: Any,
        ...
        """Update a run in the lighthousellmm API.
    ...
    def _load_child_runs(self, run: ls_schemas.Run) -> ls_schemas.Run:
        ...
        Raises:
            lighthousellmmError: If a child run has no parent.
        """
        ...
        for child_run in sorted(
            ...
            if child_run.parent_run_id is None:
                raise ls_utils.lighthousellmmError(f"Child run {child_run.id} has no parent")
    ...
    def read_run(
        ...
        """Read a run from the lighthousellmm API.
        ...
        Returns:
            Run: The run read from the lighthousellmm API.
    ...
    def list_runs(
        ...
        **kwargs: Any,
        ...
        """List runs from the lighthousellmm API.
    ...
    def create_project(
        ...
        *,
        ...
        """Create a project on the lighthousellmm API.
    ...
    def update_project(
        ...
        *,
        ...
        """Update a lighthousellmm project.
    ...
    def _get_optional_tenant_id(self) -> Optional[uuid.UUID]:
        ...
        try:
            ...
            logger.debug(
                "Failed to get tenant ID from lighthousellmm: %s", repr(e), exc_info=True
            )
        return None
    ...
    def _get_tenant_id(self) -> uuid.UUID:
        ...
        if tenant_id is None:
            raise ls_utils.lighthousellmmError("No tenant ID found")
        return tenant_id
    ...
    def read_project(
        ...
        *,
        ...
        """Read a project from the lighthousellmm API.
    ...
    def has_project(
        ...
        except ls_utils.lighthousellmmNotFoundError:
            return False
        return True
    ...
    def list_projects(
        ...
        """List projects from the lighthousellmm API.
    ...
    def delete_project(
        ...
        """Delete a project from lighthousellmm.
    ...
    def create_dataset(
        ...
        *,
        ...
        """Create a dataset in the lighthousellmm API.
    ...
    def read_dataset(
        ...
        *,
        ...
        """Read a dataset from the lighthousellmm API.
    ...
    def list_datasets(
        ...
        *,
        ...
        """List the datasets on the lighthousellmm API.
    ...
    def delete_dataset(
        ...
        *,
        ...
        """Delete a dataset from the lighthousellmm API.
    ...
    def clone_public_dataset(
        ...
        *,
        ...
        """Clone a public dataset to your own lighthousellmm tenant.
        ...
        Args:
            ...
            source_api_url (Optional[str]): The URL of the lighthousellmm server where the data is hosted.
    ...
    def _update_examples_multipart(
        ...
        *,
        ...
        if not (self.info.instance_flags or {}).get(
            ...
            raise ValueError(
                "Your lighthousellmm version does not allow using the latest examples "
                "endpoints, please update to the latest version or downgrade your SDK "
                "to lighthousellmm<0.3.9."
            )
        if updates is None:
            updates = []
    ...
    def _upload_examples_multipart(
        ...
        *,
        ...
        if not (self.info.instance_flags or {}).get(
            ...
            raise ValueError(
                "Your lighthousellmm version does not allow using the multipart examples endpoint, please update to the latest version."
            )
        if uploads is None:
            uploads = []
        encoder, data, opened_files_dict = self._prepare_multipart_data(
    ...
    def create_examples(
        ...
        **kwargs: Any,
        ...
        Returns:
            The lighthousellmm JSON response. Includes 'count' and 'example_ids'.
    ...
    def create_example(
        ...
        """Create a dataset example in the lighthousellmm API.
    ...
    def read_example(
        ...
        """Read an example from the lighthousellmm API.
        ...
        *,
        ...
        if attachments_operations is not None:
            if not (self.info.instance_flags or {}).get(
                ...
                raise ValueError(
                    "Your lighthousellmm version does not allow using the attachment operations, please update to the latest version."
                )
        example_dict = dict(
            inputs=inputs,
            outputs=outputs,
    ...
    def update_examples(
        ...
        **kwargs: Any,
        ...
        Returns:
             The lighthousellmm JSON response. Includes 'message', 'count', and 'example_ids'.
        ...
        # For backwards compatibility
            ...
            if (
                ...
                raise ValueError(
                    "Your lighthousellmm version does not allow using the attachment "
    ...
    def _select_eval_results(
        ...
        *,
        ...
        from lighthousellmm.evaluation import evaluator as ls_evaluator  # noqa: F811
    ...
    def create_feedback(
        ...
        **kwargs: Any,
        ...
        """Create a feedback in the lighthousellmm API.
        ...
        try:
            ...
            if (
                ...
                self.request_with_retries(
                    ...
                    retry_on=(ls_utils.lighthousellmmNotFoundError,),
                )
    ...
    def update_feedback(
        ...
        *,
        ...
        """Update a feedback in the lighthousellmm API.
    ...
    def read_feedback(self, feedback_id: ID_TYPE) -> ls_schemas.Feedback:
        """Read a feedback from the lighthousellmm API.
    ...
    def list_feedback(
        ...
        **kwargs: Any,
        ...
        """List the feedback objects on the lighthousellmm API.
    ...
    def create_presigned_feedback_token(
        ...
        *,
        ...
        feedback data directly to lighthousellmm without accessing the
        API key.
    ...
    def list_annotation_queues(
        ...
        *,
        ...
        """List the annotation queues on the lighthousellmm API.
    ...
    def create_annotation_queue(
        ...
        *,
        ...
        """Create an annotation queue on the lighthousellmm API.
    ...
    def get_run_from_annotation_queue(
        ...
        Raises:
            lighthousellmmNotFoundError: If the run is not found at the given index.
            lighthousellmmError: For other API-related errors.
        """
    ...
    def create_comparative_experiment(
        ...
        *,
        ...
        """Create a comparative experiment on the lighthousellmm API.
    ...
    async def arun_on_dataset(
        ...
        **kwargs: Any,
        ...
        .. deprecated:: 0.1.0
           ...
           This method is deprecated. Use :func:`lighthousellmm.aevaluate` instead.
        """  # noqa: E501
        warnings.warn(
    ...
    ) -> ls_utils.lighthousellmmUserError:
        return ls_utils.lighthousellmmUserError(
    ...
    def pull_prompt_commit(
        ...
        *,
        ...
        """Pull a prompt object from the lighthousellmm API.
    ...
    def push_prompt(
        ...
        *,
        ...
        """Push a prompt to the lighthousellmm API.
    ...
    def evaluate(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            upload_results (bool, default=True): Whether to upload the results to lighthousellmm.
                Defaults to True.
        ...
        Examples:
            ...
            .. code-block:: python
                ...
                from lighthousellmm.evaluation import LangChainStringEvaluator
        ...
        .. versionadded:: 0.2.0
        """  # noqa: E501
        from lighthousellmm.evaluation._runner import evaluate as evaluate_
    ...
    async def aevaluate(
        ...
        **kwargs: Any,
        ...
        Environment:
            - lighthousellmm_TEST_CACHE: If set, API calls will be cached to disk to save time and
                cost during testing. Recommended to commit the cache files to your repository
                for faster CI/CD runs.
                Requires the 'lighthousellmm[vcr]' package to be installed.
        ...
        """  # noqa: E501
        from lighthousellmm.evaluation._arunner import aevaluate as aevaluate_
...
def convert_prompt_to_openai_format(
    ...
    Raises:
        ...
        ls_utils.lighthousellmmError: If there is an error during the conversion process.
    """
    try:
    ...
    try:
        ...
        raise ls_utils.lighthousellmmError(f"Error converting to OpenAI format: {e}")
...
def convert_prompt_to_anthropic_format(
    ...
    try:
        ...
        raise ls_utils.lighthousellmmError(f"Error converting to Anthropic format: {e}")
