...
from lighthousellmm._internal._beta_decorator import _warn_once
...
from lighthousellmm.evaluation.integrations import LangChainStringEvaluator
...
def evaluate(
    ...
    client: Optional[lighthousellmm.Client] = None,
    blocking: bool = True,
    ...
    **kwargs: Any,
    ...
    Args:
        ...
        max_concurrency (int | None): The maximum number of concurrent
            ...
            Defaults to 0.
        client (lighthousellmm.Client | None): The lighthousellmm client to use.
            Defaults to None.
    ...
    Examples:
        ...
        >>> from lighthousellmm.schemas import Example, Run
        >>> client = Client()
    ...
    if isinstance(target, (str, uuid.UUID, schemas.TracerSession)):
        ...
        msg = (
            ...
            "from lighthousellmm import aevaluate\n\n"
            "await aevaluate(\n"
...
def evaluate_existing(
    ...
    Args:
        ...
        max_concurrency (int | None): The maximum number of concurrent
            evaluations to run. If None then no limit is set. If 0 then no concurrency.
            Defaults to 0.
        client (Optional[lighthousellmm.Client]): Optional lighthousellmm client to use for evaluation.
    ...
    Examples:
        >>> from lighthousellmm.evaluation import evaluate, evaluate_existing
        >>> dataset_name = "Evaluate Examples"
        >>> def predict(inputs: dict) -> dict:
        ...     # This can be any function or just an API call to your app.
        ...     return {"output": "Yes"}
...
def evaluate_comparative(
    ...
    Args:
        ...
        max_concurrency (int): The maximum number of concurrent evaluations to run.
            Defaults to 5.
        client (Optional[lighthousellmm.Client]): The lighthousellmm client to use.
            Defaults to None.
    ...
    Examples:
        ...
        >>> import openai
        >>> from lighthousellmm.evaluation import evaluate
        >>> from lighthousellmm.wrappers import wrap_openai
        ...
        >>> import json
        >>> from lighthousellmm.evaluation import evaluate_comparative
        >>> from lighthousellmm import schemas
...
def _load_experiment(
    project: EXPERIMENT_T, client: lighthousellmm.Client
...
def _load_examples_map(
    client: lighthousellmm.Client, project: schemas.TracerSession
...
class _ExperimentManager(_ExperimentManagerMixin):
    ...
    Args:
        ...
        client (Optional[lighthousellmm.Client]): The lighthousellmm client used for
             the experiment.
...
def _forward(
    fn: rh.SupportslighthousellmmExtra,
    example: schemas.Example,
    experiment_name: str,
    metadata: dict,
    client: lighthousellmm.Client,
    ...
    with rh.tracing_context(enabled="local" if not upload_results else True):
        ...
        lighthousellmm_extra = rh.lighthousellmmExtra(
...
def _ensure_traceable(
    target: TARGET_T | rh.SupportslighthousellmmExtra[[dict], dict] | Runnable,
) -> rh.SupportslighthousellmmExtra[[dict], dict]:
    ...
    if rh.is_traceable_function(target):
        fn: rh.SupportslighthousellmmExtra[[dict], dict] = target
    else:
...
def _get_random_name() -> str:
    from lighthousellmm.evaluation._name_generation import random_name  # noqa: F401
