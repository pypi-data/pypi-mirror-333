...
class lighthousellmmError(Exception):
    """An error occurred while communicating with the lighthousellmm API."""


class lighthousellmmAPIError(lighthousellmmError):
    """Internal server error while communicating with lighthousellmm."""


class lighthousellmmRequestTimeout(lighthousellmmError):
    """Client took too long to send request body."""


class lighthousellmmUserError(lighthousellmmError):
    """User error caused an exception when communicating with lighthousellmm."""


class lighthousellmmRateLimitError(lighthousellmmError):
    """You have exceeded the rate limit for the lighthousellmm API."""


class lighthousellmmAuthError(lighthousellmmError):
    """Couldn't authenticate with the lighthousellmm API."""


class lighthousellmmNotFoundError(lighthousellmmError):
    """Couldn't find the requested resource."""


class lighthousellmmConflictError(lighthousellmmError):
    """The resource already exists."""


class lighthousellmmConnectionError(lighthousellmmError):
    """Couldn't connect to the lighthousellmm API."""


## Warning classes


class lighthousellmmWarning(UserWarning):
    """Base class for warnings."""


class lighthousellmmMissingAPIKeyWarning(lighthousellmmWarning):
    """Warning for missing API key."""


def tracing_is_enabled(ctx: Optional[dict] = None) -> Union[bool, Literal["local"]]:
    """Return True if tracing is enabled."""
    from lighthousellmm.run_helpers import get_current_run_tree, get_tracing_context
...
def get_env_var(
    ...
    *,
    namespaces: Tuple = ("lighthousellmm", "LANGCHAIN"),
) -> Optional[str]:
    ...
    Args:
        ...
        namespaces (Tuple, optional): A tuple of namespaces to search for the
            environment variable. Defaults to ("lighthousellmm", "LANGCHAINs").
...
def get_tracer_project(return_default_value=True) -> Optional[str]:
    """Get the project name for a lighthousellmm tracer."""
...
class FilterlighthousellmmRetry(logging.Filter):
    ...
    def filter(self, record) -> bool:
        ...
        msg = record.getMessage()
        return "lighthousellmmRetry" not in msg


class lighthousellmmRetry(Retry):
...
def get_cache_dir(cache: Optional[str]) -> Optional[str]:
    ...
    Returns:
        ...
        from the lighthousellmm_TEST_CACHE environment variable.
    """
...
def with_cache(
    ...
    from lighthousellmm._internal import _patch as patch_urllib3
...
def _format_exc() -> str:
    ...
    filtered_lines = [line for line in tb_lines if "lighthousellmm/" not in line]
...
def get_api_url(api_url: Optional[str]) -> str:
    """Get the lighthousellmm API URL from the environment or the given value."""
    ...
    if not _api_url.strip():
        raise lighthousellmmUserError("lighthousellmm API URL cannot be empty")
