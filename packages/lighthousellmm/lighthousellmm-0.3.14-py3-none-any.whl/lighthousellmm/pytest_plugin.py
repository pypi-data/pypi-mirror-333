...
from lighthousellmm.testing._internal import test as ls_test

logger = logging.getLogger(__name__)


def pytest_addoption(parser):
    """Set a boolean flag for lighthousellmm output.

    Skip if --lighthousellmm-output is already defined.
    """
    try:
        # Try to add the option, will raise if it already exists
        group = parser.getgroup("lighthousellmm", "lighthousellmm")
        group.addoption(
            "--lighthousellmm-output",
            action="store_true",
            default=False,
            help="Use lighthousellmm output (requires 'rich').",
        )
    except ValueError:
        # Option already exists
        logger.warning(
            "lighthousellmm output flag cannot be added because it's already defined."
        )


def _handle_output_args(args):
    """Handle output arguments."""
    if any(opt in args for opt in ["--lighthousellmm-output"]):
...
def pytest_runtest_call(item):
    """Apply lighthousellmm tracking to tests marked with @pytest.mark.lighthousellmm."""
    marker = item.get_closest_marker("lighthousellmm")
    if marker:
...
def pytest_report_teststatus(report, config):
    ...
    if config.getoption("--lighthousellmm-output"):
        return "", "", ""


class lighthousellmmPlugin:
    """Plugin for rendering lighthousellmm results."""
...
lighthousellmm URL: [bright_cyan]{self.test_suite_urls[suite_name]}[/bright_cyan]"""  # noqa: E501
...
def pytest_configure(config):
    """Register the 'lighthousellmm' marker."""
    config.addinivalue_line(
        "markers", "lighthousellmm: mark test to be tracked in lighthousellmm"
    )
    if config.getoption("--lighthousellmm-output"):
        if not importlib.util.find_spec("rich"):
            msg = (
                "Must have 'rich' installed to use --lighthousellmm-output. "
                "Please install with: `pip install -U 'lighthousellmm[pytest]'`"
            )
            raise ValueError(msg)
        if os.environ.get("PYTEST_XDIST_TESTRUNUID"):
            msg = (
                "--lighthousellmm-output not supported with pytest-xdist. "
                "Please remove the '--lighthousellmm-output' option or '-n' option."
            )
            raise ValueError(msg)
        if ls_utils.test_tracking_is_disabled():
            msg = (
                "--lighthousellmm-output not supported when env var"
                "lighthousellmm_TEST_TRACKING='false'. Please remove the"
                "'--lighthousellmm-output' option "
                "or enable test tracking."
            )
            raise ValueError(msg)
        config.pluginmanager.register(lighthousellmmPlugin(), "lighthousellmm_output_plugin")
