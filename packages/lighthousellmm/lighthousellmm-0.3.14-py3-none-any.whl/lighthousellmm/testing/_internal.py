...
from lighthousellmm import run_helpers as rh
from lighthousellmm import run_trees
from lighthousellmm import run_trees as rt
...
from lighthousellmm._internal._serde import dumps_json
from lighthousellmm.client import ID_TYPE
...
def test(*args: Any, **kwargs: Any) -> Callable:
    """Trace a pytest test case in lighthousellmm.

    This decorator is used to trace a pytest test to lighthousellmm. It ensures
    ...
    recorded and reported by lighthousellmm.
    ...
    Args:
        ...
        - client (Optional[ls_client.Client]): An instance of the lighthousellmm client
            to be used for communication with the lighthousellmm service. If not provided,
    ...
    Environment:
        - lighthousellmm_TEST_CACHE: If set, API calls will be cached to disk to
        ...
        - lighthousellmm_TEST_TRACKING: Set this variable to the path of a directory
            to enable caching of test results. This is useful for re-running tests
             without re-executing the code. Requires the 'lighthousellmm[vcr]' package.

    Example:
        For basic usage, simply decorate a test function with `@pytest.mark.lighthousellmm`.
        ...
        .. code-block:: python
            ...
            # from lighthousellmm import test
            # @test
            @pytest.mark.lighthousellmm
            def test_addition():
                assert 3 + 4 == 7
        ...
        .. code-block:: python
            ...
            import pytest
            from lighthousellmm import traceable
        ...
        `lighthousellmm_TEST_CACHE=path/to/cache`. Check in these files to speed up
        CI/CD pipelines, so your results only change when your prompt or requested
        model changes.

        Note that this will require that you install lighthousellmm with the `vcr` extra:

        `pip install -U "lighthousellmm[vcr]"`
        ...
        .. code-block:: python
            ...
            # os.environ["lighthousellmm_TEST_CACHE"] = "tests/cassettes"
            import openai
            import pytest
            from lighthousellmm import wrappers
        ...
        LLMs are stochastic. Naive assertions are flakey. You can use lighthousellmm's
        `expect` to score and make approximate assertions on your results.

        .. code-block:: python

            import pytest
            from lighthousellmm import expect
            ...
            def test_output_semantically_close():
                ...
                # The embedding_distance call logs the embedding distance to lighthousellmm
                expect.embedding_distance(
                    prediction=response.choices[0].message.content,
                    reference="Hello!",
                    # The following optional assertion logs a
                    # pass/fail score to lighthousellmm
                ...
                expect.edit_distance(
                    prediction=response.choices[0].message.content,
                    reference="Hello!",
                    # And then log a pass/fail score to lighthousellmm
                ).to_be_less_than(1.0)

        The `@test` decorator works natively with pytest fixtures.
        The values will populate the "inputs" of the corresponding example in lighthousellmm.
        ...
        .. code-block:: python
            ...
            @pytest.mark.lighthousellmm(output_keys=["expected"])
        ...
        .. code-block:: python
            ...
            @pytest.mark.lighthousellmm(id=str(example_id))
        ...
        .. code-block:: python
            ...
            @pytest.mark.lighthousellmm(output_keys=["expected_output"])
    ...
    if disable_tracking:
        logger.info(
            "lighthousellmm_TEST_TRACKING is set to 'false'."
            " Skipping lighthousellmm test tracking."
        )
...
def _get_experiment_name(test_suite_name: str) -> str:
    ...
    if os.environ.get("lighthousellmm_EXPERIMENT"):
        prefix = os.environ["lighthousellmm_EXPERIMENT"]
    else:
...
def _get_test_suite_name(func: Callable) -> str:
    ...
    raise ValueError("Please set the lighthousellmm_TEST_SUITE environment variable.")
...
def _end_tests(test_suite: _lighthousellmmTestSuite):
...
class _lighthousellmmTestSuite:
...
class _TestCase:
    def __init__(
        self,
        test_suite: _lighthousellmmTestSuite,
        example_id: uuid.UUID,
        run_id: uuid.UUID,
        pytest_plugin: Any = None,
        pytest_nodeid: Any = None,
...
def _create_test_case(
    ...
    **kwargs: Any,
    ...
    test_suite = _lighthousellmmTestSuite.from_test(
    ...
    pytest_plugin = (
        pytest_request.config.pluginmanager.get_plugin("lighthousellmm_output_plugin")
        if pytest_request
        else None
    )
...
def log_inputs(inputs: dict, /) -> None:
    ...
    Should only be used in pytest tests decorated with @pytest.mark.lighthousellmm.
    ...
    Example:
        .. code-block:: python
            ...
            from lighthousellmm import testing as t
    ...
    if ls_utils.test_tracking_is_disabled():
        logger.info(
            "lighthousellmm_TEST_TRACKING is set to 'false'." " Skipping log_inputs."
        )
        return
    ...
    if not run_tree or not test_case:
        msg = (
            ...
            "@pytest.mark.lighthousellmm, and with tracing enabled (by setting the "
            "lighthousellmm_TRACING environment variable to 'true')."
        )
        raise ValueError(msg)
...
def log_outputs(outputs: dict, /) -> None:
    ...
    if ls_utils.test_tracking_is_disabled():
        logger.info(
            "lighthousellmm_TEST_TRACKING is set to 'false'." " Skipping log_outputs."
        )
        return
    run_tree = rh.get_current_run_tree()
    test_case = _TEST_CASE.get()
...
def log_reference_outputs(reference_outputs: dict, /) -> None:
    ...
    Example:
        .. code-block:: python
            ...
            from lighthousellmm import testing
    ...
    if not test_case:
        msg = (
            ...
            "decorated with @pytest.mark.lighthousellmm."
        )
        raise ValueError(msg)
...
def log_feedback(
    ...
    **kwargs: Any,
    ...
    if ls_utils.test_tracking_is_disabled():
        logger.info(
            "lighthousellmm_TEST_TRACKING is set to 'false'." " Skipping log_feedback."
        )
        return
