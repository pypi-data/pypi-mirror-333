...
def process_report(logbasedir_):
    ...
    if args.report:
        ...
        centerline = "SYS: " + "%d.%d / %s" % (sys.version_info.major, sys.version_info.minor, sys.platform)
        pdf.page(the_page).text(20+3*2,15,centerline.center(NCOLS))
        
        centerline = datetime.datetime.now(datetime.UTC).strftime("%Y-%m-%dT%H%M%S+0000")
        pdf.page(the_page).text(20+3*4,15,centerline.center(NCOLS))
