...
def process_report(logbasedir_):
    ...
    if args.report:
        ...
        for report_i in range(0, len(play)):
            ...
            pdf.page(the_page).text(12,10, "URL BEFORE %s" % url_pre)
            pdf.page(the_page).text(13,10, "URL AFTER %s" % url_after)
        ...
        pdf.page(the_page).text(1, 10, "[APPENDIX] - [COOKIES]")
        ...
        if global_cookie_latest_json_dump_filename != None:
            ...
            for exit_cookie in exit_cookies:
                ...
                import uuid
                cookie_table_sorter[str(exit_cookie["name"]).upper() + "-" + str(uuid.uuid4())] = [exit_cookie["name"] + " (%d)" % cookie_counter, "%s - %s - %d" % ('HTTP' if exit_cookie["httpOnly"] == True else '____', 'SEC' if exit_cookie["secure"] == True else '___',len(exit_cookie["value"])),exit_cookie["domain"],exit_cookie["path"]]
            ...
            pdf.page(the_page).text(3, 10, "Accumulated Length of values: %d" % cookie_value_accu)
            pdf.page(the_page).text(5, 10, cookie_table_txt)
