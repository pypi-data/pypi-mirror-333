...
def find_chat_title(html_doc: Tag) -> str:
    title_div = html_doc.select_one("div.page_header > .content > div.text")
    if not title_div:
...
def handle_file_attachment(
    ...
    *,
    ...
    title_div = media_file_anchor.select_one("a.media_file > div.body > div.title")
    if not title_div:
        raise MessageParsingException(
            "unable to find file attachment title", media_file_anchor
        )
    title = text_from_tag(title_div)
