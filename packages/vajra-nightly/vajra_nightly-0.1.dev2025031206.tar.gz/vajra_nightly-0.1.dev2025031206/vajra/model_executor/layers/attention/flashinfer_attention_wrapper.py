...
class FlashinferAttentionWrapper:
    def __init__(
        ...
        if self.use_native_execution_backend:
            ...
            # TODO(rayyan): Add support for native execution backend for BatchPrefillWithPagedKVCacheWrapper
    ...
    def begin_forward(
        ...
        if self.use_native_execution_backend:
            ...
            # Get KVP sequences metadata from native handle
            self.kvp_seqs_offset = self.native_handle.kvp_seqs_offset
            self.kvp_seqs_qo_indptr = self.native_handle.kvp_seqs_qo_indptr
            self.kvp_seqs_group_ids = self.native_handle.kvp_seqs_group_ids

            # Call plan with the tensors from native handle
            self.wrapper.plan(
                self.native_handle.qo_indptr_tensor,
                self.native_handle.kv_page_indptr_tensor,
                self.native_handle.kv_page_indices_tensor,
                self.native_handle.kv_last_page_len_tensor,
                self.num_q_heads,
                self.num_kv_heads,
                self.head_dim,
                self.block_size,
