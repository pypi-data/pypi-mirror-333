...
FlashinferAttentionWrapper::FlashinferAttentionWrapper(
    ...
    : num_q_heads_(num_q_heads),
      ...
      num_q_tokens_(0),
      kvp_seqs_offset_(0) {
  kvp_seqs_qo_indptr_.push_back(0);
...
void FlashinferAttentionWrapper::BeginForward(
  ...
  if (!all_save_kv_cache && !none_save_kv_cache) {
  ...
  for (const auto& seq_metadata : seq_metadata_list) {
    ...
    if (seq_metadata.is_kvp_request) {
      ...
      if (started_kvp_seqs) {
        throw std::runtime_error("Non-KVP sequences should come first.");
      }
      if (!seq_metadata.save_kv_cache) {
        throw std::runtime_error("Non-KVP sequences should save KV cache.");
      }
    }
  ...
  // Set metadata initialization flag to true after successful initialization
