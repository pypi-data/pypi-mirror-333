...
#            print 'loop data i is %s, data is %s' %(i, dic)
    ...
    def execute(self, log_dir=None):
        ...
        mo_stats = dict()
        stats_p = re.compile('^\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}.\d{3}\s+\d+\s+\d+\s+([A-Z]{1})\s+(\w+)\s*:')
        sleep(10)
        for log_file in os.listdir(log_dir):
            if log_file.startswith('logcat_'):
                ...
                if hasattr(self, 'args') and self.args.startup_keyword:
                    ...
                    nwstart_p = re.compile('^\d{2}-\d{2}\s+(\d{2}:\d{2}:\d{2}.\d{3})\s+\d+\s+\d+\s+[A-Z]{1}\s+\w+\s*:\sStageManagerService.startStage line=\d+ verbose enter uri=\S*')
                    appshow_p = re.compile('^\d{2}-\d{2}\s+(\d{2}:\d{2}:\d{2}.\d{3})\s+\d+\s+\d+\s+[A-Z]{1}\s+\w+\s*:\s%s'%(keywords[0]))
                    blitzshow_p = re.compile('^\d{2}-\d{2}\s+(\d{2}:\d{2}:\d{2}.\d{3})\s+\d+\s+\d+\s+[A-Z]{1}\s+\w+\s*:\sfirst render end')
                    comsumetime = dict()
                    nwtime = dict()
...
class DurableCommand(Command):
    def execute(self):
        if self.command and self.is_done():
            ...
            if 'monkey' in self.command and self.command['monkey']:
