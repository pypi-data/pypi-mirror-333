...
def resend_mo_data(report_id):
    ...
    for attempt in range(schedule_settings.resend_count):
        ...
        for mo_id, exception in send_data_tasks_results.get():
            mo = Unit.objects.get(id=mo_id)
...
class ReportService(GisdoJsonDumpsMixin, ReportCacheMixin):
    ...
    def _collect_mo_data(self):
        ...
        with allow_join_result():
            for current_mo_num, (mo_id, task_result, exception) in enumerate(collect_data_tasks_results.get(), 1):
    ...
    def _send_mo_data(self, mo_for_send):
        ...
        send_error_count = 0
        with allow_join_result():
            for current_mo_num, (mo_id, exception) in enumerate(send_data_tasks_results.get(), 1):
