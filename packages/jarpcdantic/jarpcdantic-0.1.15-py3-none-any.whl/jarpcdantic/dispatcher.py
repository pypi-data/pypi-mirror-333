...
class JarpcDispatcher:
    ...
    def __getitem__(self, method_name: str) -> Callable:
        try:
            ...
            raise JarpcMethodNotFound(method=method_name, declared_methods=list(self.method_map.keys()))
    ...
    def declare_method(self, method_name: str | None = None):
        ...
        def decorated(method_function: _T) -> _T:
            self.method_map[(method_name.__str__() if method_name else None) or method_function.__name__] = method_function
            return method_function
