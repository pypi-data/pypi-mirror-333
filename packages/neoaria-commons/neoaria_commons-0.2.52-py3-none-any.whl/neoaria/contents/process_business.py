...
class ProcessBusiness(CommonBusiness):
    ...
    def create_meta_event(self, operation: EventOperation, cid: str):
        """
        메타 이벤트 객체를 생성합니다.
        하위 클래스에서 구현해야 합니다.
        """
    ...
    def create(self, request: Request, data: BusinessParameter):
        try:
            ...
            # DB 저장
            self.contents_collection.insert_one(data.body['template']) # TODO DB 변경 필요
            
            # 승인 단계 생성
            status = { # TODO 스키마 확인 필요
                "id": str(uuid.uuid4()),
            ...
            # 이벤트 발행
            self.produce_to_meta(EventOperation.CREATE, cid_v2, createTime, request.state.account.name, data.body['meta'])
    ...
    def change(self, request: Request, data: BusinessParameter):
        try:
            ...
            if approvalStatus == ElementStatus.APPROVED.value:
                ...
                self.produce_to_meta(EventOperation.CHANGE, cid, changeTime, request.state.account.name, meta)
    ...
    def update(self, request: Request, data: BusinessParameter):
        try:
            ...
            self.produce_to_meta(EventOperation.UPDATE, cid_v2, updateTime, request.state.account.name, data.body['meta'])
    ...
    def deployments(self, request: Request, data: BusinessParameter):
        try:
            ...
            self.produce_to_meta(EventOperation.DEPLOY, cid, deployTime, request.state.account.name, meta)
    ...
    def delete(self, request: Request, data: BusinessParameter):
        try:
            ...
            self.produce_to_meta(EventOperation.DELETE, cid, deleteTime, "SYSTEM", meta)
    ...
    def update_phase(self, data: BusinessParameter):
        try:
                ...
                if approval_phase and "services" in approval_phase and all(value is True for value in approval_phase["services"].values()):
                    ...
                    if approval_phase['phase'] == ElementStatus.APPROVALREQUESTPROCESSING.value:
                        message = {
                            ...
                            'operation': 'UPDATE',
                            'application': self.service_name,
                            'core': self.get_core_name(core_lang),  # 언어코드 어떻게 앎?
                            'targetDocument': {
                                'cid_v1': '',
                                'cid_v2': target,
                                'cid_v3': ''
                    ...
                    # 승인 처리 중인 경우
                        ...
                        self.produce_to_meta(EventOperation.CHANGE, target, int(time.time() * 1000), "SYSTEM", meta)
            ...
            self.produce_to_meta(EventOperation.CREATE, target, updateTime, "SYSTEM", meta)
    ...
    def produce_to_meta(self, operation: EventOperation, cid, Time, Changer, meta = None):
        ...
        metaEvent = self.create_meta_event(operation.value, cid)
        metaEvent.payload = payload

        self.producer_send_message(self.meta_topic_index, metaEvent.model_dump_json(), self.producer_meesage_callback)
