...
class SinterMWPFFastDecoder:
    ...
    def with_circuit(self, circuit: stim.Circuit | None) -> "SinterMWPFFastDecoder":
        if circuit is None:
            self.circuit = None
            return self
    ...
    def compile_decoder_for_dem(
        ...
        *,
        ...
        return MwpfFastCompiledDecoder(
            solver,
            predictor,
            dem.num_detectors,
            dem.num_observables,
...
def construct_decoder_and_predictor(
    ...
    if decoder_type is None:
        ...
        decoder_cls = getattr(mwpf_fast, decoder_type)
    else:
...
@dataclass
class SinterHUFFastDecoder(SinterMWPFFastDecoder):
    decoder_type: str = "SolverSerialUnionFind"
    cluster_node_limit: int = 0


@dataclass
class SinterSingleHairFastDecoder(SinterMWPFFastDecoder):
    decoder_type: str = "SolverSerialSingleHair"
    cluster_node_limit: int = 0


@dataclass
class MwpfFastCompiledDecoder:
    solver: Any
    predictor: Predictor
    num_dets: int
    num_obs: int
