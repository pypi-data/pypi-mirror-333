...
def SP_detection(raw, stage, target_stage=('N2', 'SWS'), method: str = "Hahn2020",
                 l_freq=10, h_freq=16, dur_lower=0.5, dur_upper=math.inf, baseline: bool = True, verbose: bool = True):
    """
    ...
    all_stage_dfs = []
    n_channels = eeg_centered.shape[0]
    ...
    for s_val in target_stage:
        ...
        for idx, (val, duration_s, _) in enumerate(stage):
            ...
            for ch_idx in range(n_channels):
                signal_seg = eeg_centered[ch_idx, seg_start_idx:seg_end_idx]

                # Detect spindles on this single-epoch segment
                sp_list = detect_spindles_for_epoch(method, signal_seg, dur_lower, dur_upper, sfreq)
...
def swa(raw: Raw, l_freq=0.5, h_freq=4):
    """
    Compute slow wave activity (SWA) from an mne.Raw object.

    This function filters the raw EEG data in the slow wave band (0.5-4 Hz),
    computes the Hilbert transform to extract the amplitude envelope,
    and then averages this envelope over time for each channel.
    ...
    --------
    pd.DataFrame
        A DataFrame containing the slow wave activity for each channel.
    """
    # Sampling frequency
    fs = raw.info['sfreq']
    # Channel names
    ...
    swa_values = np.mean(amplitude_envelope, axis=1)
...
def detect_spindles_for_epoch(method, signal_1d, dur_lower, dur_upper, sfreq):
    ...
    if method == "Hahn2020":
        spindle_candidates = hahn(sfreq, signal_1d)

    elif method == "Martin2013":
        spindle_candidates = martin(sfreq, signal_1d)

    elif method == "Wamsley2012":
        spindle_candidates = wamsley(sfreq, signal_1d)

    elif method == "Wendt2012":
        spindle_candidates = wendt(sfreq, signal_1d)

    elif method == "Ferrarelli2007":
        spindle_candidates = ferrarelli(dur_lower, sfreq, signal_1d)

    else:
        # Fallback to Hahn2020 approach if method is unknown
...
def ferrarelli(dur_lower, sfreq, signal_1d):
    # (Ferrarelli 2007)
...
def wendt(sfreq, signal_1d, spindle_candidates):
    spindle_candidates = []
    rectified = np.abs(signal_1d)
...
def wamsley(sfreq, signal_1d, spindle_candidates):
    # (Wamsley 2012)
    ...
    # freq range
    freqs = np.arange(10, 17, 1)  # 10 to 16 Hz inclusive
...
def martin(sfreq, signal_1d, spindle_candidates):
...
def hahn(sfreq, signal_1d, spindle_candidates):
    # (Hahn et al. 2020)
