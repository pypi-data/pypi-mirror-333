...
class GroundedNessEvaluator(BaseEvaluator):
    ...
    def run(
        ...
        faithfulness_metrics = FaithfulnessMetric(
        ...
        try:
            ...
            faithfulness_metrics.measure(test_case=de_test_case, _show_indicator=False)
            duration = (time.time() - start_time) * 1000
            res = convertResult(
                metric=faithfulness_metrics,
                duration=duration,
            ...
            msg = str(e)
            # TODO: Verbose logs print(f"Error: Groundedness evaluation failed: {msg}")
            self.error = f"Groundedness evaluation failed: {msg}"
