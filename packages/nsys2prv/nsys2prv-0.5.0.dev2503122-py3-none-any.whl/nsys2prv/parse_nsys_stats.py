...
def main():
    ...
    print("nsys2prv v0.5.0-dev2503122")
    ...
    with open(trace_name+".prv", "w") as prv_file:
        ...
        if t_nccl:
            def ser_nccl_kernels(r):
                ...
                return create_combined_events_record(r.iloc[0], r.iloc[1], int(r["thread"]), int(r["task"]), types, [r["event_value"]] + [int(r['GrdX']), int(r['GrdY']), int(r['GrdZ']), int(r['BlkX']), int(r['BlkY']), int(r['BlkZ']), int(r['Reg/Trd']), r["CorrID"]] + [int(r[rowname]) for rowname in valid_payload["Name"].tolist()])
