...
class PDBFile(ABC):
    ...
    def zip(self) -> None:
        ...
        with ZipFile(Path.cwd() / fname, 'w') as zf:
            self.compile()
            files = [self.tex_file]
            files += [config.PDB_DB_DIR/figure for _, _, figure in self.figures]
            for f in set(files):
                zf.write(
                    f.with_suffix(".tex"),
...
class CheckForTikzMixin:
    ...
    def _list_figures(self, tex_content: str) -> list[tuple[int, str, str]]:
        return [
            (match.span()[0], match.group(1), match.group(2))
...
class Exercise(PDBFile, CheckForTikzMixin):
    ...
    def __init__(self, pdb_file: Path, pdb_data: dict):
        ...
        if not pdb_data:
            ...
            self.title: str = pdb_data['title']
            self.tags = Tags(pdb_data["tags"])
            self.figures: list[tuple[int, str, str]] = pdb_data["figures"]
    ...
    def _parse_texfile(self) -> None:
        with self.tex_file.open() as f:
            ...
            self.figures = self._list_figures(tex_content)
...
class QCM(PDBFile, CheckForTikzMixin):
    ...
    def _parse_texfile(self) -> None:
        with self.tex_file.open() as f:
            ...
            self.figures = self._list_figures(f.read())
