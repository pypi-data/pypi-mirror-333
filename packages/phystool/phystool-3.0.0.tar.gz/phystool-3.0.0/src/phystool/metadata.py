...
class Metadata:
    ...
    def _remove_extra_files(self) -> None:
    ...
    def _consolidate_figures(self) -> None:
        pdb_files_using_updated_figures = set()
        for tex_file in config.PDB_DB_DIR.glob('*.tex'):
            pdb_file = PDBFile.open(tex_file.stem)
            if pdb_file.PDB_TYPE == Figure.PDB_TYPE:
                if pdb_file.compile():
                    pdb_files_using_updated_figures |= set(pdb_file.used_by)

        # compile all Exercises using previously compiled Figures
        for uuid in pdb_files_using_updated_figures:
            pdb_file = PDBFile.open(uuid)
            pdb_file.compile()

    def _consolidate_pdb(self) -> None:
        """Parse tex_file of all PDB files and update Figures.used_by"""
        figure_used_by: dict[str, set[str]] = dict()
        for tex_file in config.PDB_DB_DIR.glob('*.tex'):
            pdb_file = PDBFile.open(tex_file.stem)
            ...
            if pdb_file.PDB_TYPE != Figure.PDB_TYPE:
                for _, _, uuid in pdb_file.figures:
                    try:
        ...
        for uuid, used_by in figure_used_by.items():
            pdb_file = PDBFile.open(uuid)
            if used_by != set(pdb_file.used_by):
                pdb_file.used_by = sorted(used_by)
                pdb_file.save()
                self.update(pdb_file)

    def consolidate(self) -> None:
        self._consolidate_figures()
        self._consolidate_pdb()
