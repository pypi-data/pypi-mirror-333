...
class GWSheet(object):
    def __init__(self, wbook, wsheet):
        ...
        self._wsheet=wsheet
        self._cells=[]
        self._nbCols=self.nbCols
        self._nbRows=self.nbRows
    ...
    def resize(self, nbrows, nbcols):
        if nbrows and nbcols:
            self.wsheet.resize(rows=nbrows, cols=nbcols)
        elif nbrows:
            self.wsheet.resize(rows=nbrows)
        elif nbcols:
            self.wsheet.resize(cols=nbcols)
    ...
    def update(self):
        try:
            if self._cells:
                self.resize(self._nbRows, self._nbCols)
    ...
    def write(self, value, a):
        try:
            # (r, c)=self.wsheet.get_int_addr(a)
            (r, c)=gspread.utils.a1_to_rowcol(a)
            if self._nbRows<r:
                self._nbRows=r
            if self._nbCols<c:
                self._nbCols=c

            # a=self.wsheet.get_addr_int(r, c)
            a=gspread.utils.rowcol_to_a1(r, c)
            cell=self.wsheet.acell(a)
            cell.value=value
