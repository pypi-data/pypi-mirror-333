...
class BasePyAVReader(PyAVInterface):
    def __init__(self, path, buffer_size=32768, options={}):
...
class PyAVReader(BasePyAVReader):
    def __init__(self, path, start=0, end=(2 << 62) - 1, buffer_size=32768, options={}):
        super().__init__(path, buffer_size=buffer_size, options=options)
        self.start = start
        self.end = end
...
class PyAVWriter(PyAVInterface):
    def __init__(
        ...
        *,
        ...
        self.container = av.open(path, "w", buffer_size=buffer_size, format=format, options=options)
        self.streams = []
