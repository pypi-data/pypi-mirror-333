...
def get_prefill_info_for_skjema(
    ...
    try:
        ...
        content = client.get(path=f"/system{constants.PREFILL_API_STAT_PATH}/skjema?{query}", context=context)
...
def get_prefill_for_enhet(
    ...
    try:
        ...
        content = client.get(path=f"/public{constants.PREFILL_API_STAT_PATH}/skjema/enhet?{query}", context=context)
        content_json = json.loads(content)
...
def delete_prefill_for_skjema(
    ...
    try:
        ...
        if periode_nr:
            query += f"&periode_nummer={periode_nr}"
        content = client.delete(path=f"/public{constants.PREFILL_API_STAT_PATH}/skjema?{query}", context=context)
        content_json = json.loads(content)
...
def delete_prefill_for_enhet(
    ...
    try:
        ...
        if periode_nr:
            query += f"&periode_nummer={periode_nr}"
        query += f"&enhetsident={enhetsident}&enhetstype={enhetstype}"
        content = client.delete(path=f"/public{constants.PREFILL_API_STAT_PATH}/skjema/enhet?{query}", context=context)
        content_json = json.loads(content)
...
def save_prefill_for_enhet(
    ...
    try:
        ...
        result = client.post(path=f"/public{constants.PREFILL_API_STAT_PATH}/skjema/enhet", body_json=json.dumps(data), context=context)
...
def validate_skjemadata(
    ...
    try:
        x=get_prefill_meta_paged(self)
        # Extract dictionary with required mapping
        sti_navn_that_has_dublett_controlllist = [
            item['sti_navn'] for item in x.get('results', []) if item['dublett_sjekk']
        ]

        # **Apply Filtering**
        filtered_skjemadata = filter_skjemadata(skjemadata, sti_navn_that_has_dublett_controlllist)

        # Convert to JSON string
        skjemadata_str = json.dumps(filtered_skjemadata)
        ...
        url = (
            f"/public{constants.PREFILL_API_STAT_PATH}/validate/skjemadata/"
...
def filter_skjemadata(data, allowed_keys, parent_key=""):
    """
    Recursively filters the skjemadata dictionary to only include keys present in allowed_keys.
    """
    if isinstance(data, dict):
        filtered_data = {}
        for key, value in data.items():
            key_path = f"{parent_key}.{key}" if parent_key else key
            sub_dicts = [path for path in allowed_keys if path.startswith(key_path)]
            if sub_dicts:
                if isinstance(value, (dict, list)):
                    filtered_value = filter_skjemadata(value, sub_dicts, key_path)
                    if filtered_value:
        ...
        if data and isinstance(data[0], dict):
            # Filter each dictionary inside the list
            filtered_list = [filter_skjemadata(item, allowed_keys, parent_key) for item in data]
            # Remove empty dictionaries from the list
            return [item for item in filtered_list if item]
        return data  # If list contains primitives, return as is
