...
class Symbol(BaseSchema):
    name: str = Field(..., description="Stock ticker symbol used for trading identification")

class Exchange(BaseSchema):
    name: str = Field(..., description="Stock exchange code or identifier where securities are traded")
...
class Country(BaseSchema):
    name: str = Field(..., description="Country name where the company is headquartered or registered")
