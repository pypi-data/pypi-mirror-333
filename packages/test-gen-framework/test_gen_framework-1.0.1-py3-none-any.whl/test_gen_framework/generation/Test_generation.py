...
def generate_tests(pr_number):
    ...
    if code_diff_array:
        ...
        for file_data in code_diff_array:
            if file_data["localfilepath"].endswith(".py"):
                ...
                if file_content:
                    ...
                    if full_function_code:
                        test_script = generate_test_cases(
                            file_data["localfilepath"], full_function_code
                        )
                        if test_script != "":
                            function_name = extract_function_name(full_function_code)
                            if function_name:
...
def extract_full_function(file_content, changed_lines) -> str:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.FunctionDef):
            if node.lineno <= min(changed_lines) <= node.end_lineno:
