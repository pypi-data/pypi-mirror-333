...
def run():
    ...
    try:
        ...
        # 提取模型名称
        # 获取 ObjectDetectOperator 和 CvLargeModelOperator 类型的节点
        model_artifact_names = [
            prop.value for kind in ("ObjectDetectOperator", "CvLargeModelOperator")
            for node in (GraphContent(nodes=skill.graph["nodes"]).get_nodes(kind) or [])
            if (prop := node.get_property("modelName"))
        ]
        ...
        logger.info("Found ObjectDetectOperator and CvLargeModelOperator nodes with %d models. model_artifact_names = %s",
