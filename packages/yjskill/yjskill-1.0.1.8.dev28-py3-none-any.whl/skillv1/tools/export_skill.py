...
def run():
    ...
    try:
        ...
        if data_type == DATA_TYPE_SKILL_TEMPLATE:
            ...
            # 上传文件
            output_uri = f"s3://{filesystem['endpoint']}/{job.name}/{skill.localName}_{artifact_name.version}.json"
            success_message['outputUri'] = output_uri
            success_message['outputFileFormat'] = "json"
            success_message['outputFileName'] = f"{skill.localName}_{str(artifact_name.version)}.json"
