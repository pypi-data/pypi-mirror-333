...
class Drevo:
    ...
    def cvet_uzlov(self, mas):  # получает на вход список и выдаёт два списка, один с узлами, а второй с их цветом
        ...
        for i in range(len(mas)):#Цикл по длине списка входных данных, счётчик i показывает номер ветки которую обрабатывает функция
            x = mas[i] # Запись ветки списка под номером i в вспомогательную переменную
            ...
            for j in range(len(x) - 1): # Цикл, проходящий по длинне списка x - 1
                isxodn_uzl.append(x[j] + ' ' + x[j + 1]) # Добавление в список двух соседних элементов, которые будут образовывать ребро
                cvet_uzlov.append(i + 1) # Добавление номера ветки, к которой относиться ребро записанное в список  isxodn_uzl
        return isxodn_uzl, cvet_uzlov # вывод списка вершин образующих ребра и списка принадлежности каждой вершины к определённой ветке дерева
    ...
    def otdelno_uzl(self, mas):  # Функция нужна для нахождения всех рёбер в дереве без повторений
        ...
        for i in range(len(mas)): # Цикл переберающий все ветки из входных данных
            ...
            for j in range(len(x) - 1): # Цикл, запущенный до длинны  списка x -1 (переберает все вершины из ветки лежащей в x)
                con_mass.append(x[j] + ' ' + x[j + 1]) #добавляет в вспомогательный список ребро, которое образовано двумя соседними вершинами в ветке x
        return list(set(con_mass)) #вывод списка ребер, с помощью list(set()), из списка удаляются все повторяющиеся элементы и остаются только уникальные ребра
    ...
    def conus(self, mas, iskom):
        ...
        for j in mas: # Цикл, бегущий по веткам дерева из списка mas
            if j.count(iskom) != 0: # Если в ветке есть элемент iskom, то
                massiv_conusov.append(j[j.index(iskom):]) # В вспомогательный список добавляется ветка дерева, с элемента iskom и до конца
    ...
    def konecha(self, mas_isxod_uzl, cvet_uzlov, conus_mas):  # сравнивает значения с исходным цветом узлов с конусом
        ...
        for i in conus_mas: # Цикл бежащий по элементам conus_mas
            conus_cvet.append(cvet_uzlov[mas_isxod_uzl.index(i)]) # Добавление в список элемента из списка cveta_uzlov c индексом таким же как у ребра в mas_isxod_uzl
    ...
    def postroenie(self, mas):
        ...
        v, n = self.cvet_uzlov(mas)  # С помощью функции cvet_uzlov в вспомогательные переменные "v" и "n
        # заносяться список ребер всего дерева и список принадлежности каждой вершины к определеной ветке.
        s.append(self.konecha(v, n, self.otdelno_uzl(self.conus(mas, mas[0][0])))) # Добавление в вспомогательный список цветов ребер.
        s.append(self.otdelno_uzl(self.conus(mas, mas[0][0]))) # Добавление в вспомогательный список ребер дерева.
    ...
    def uroven(self, mas, c):
        ...
        uzels = [] # Вспомогательный список, в котором будут храниться вершины дерева, которые находяться на искомом уровне.
        for i in range(len(mas)): # Запускается цикл по длинне переменной mas.
            x = mas[i].split()  # В вспомогательную переменную "x" заносится ветка под номером i, с помошью функции строка.split(), содержащая ветку дерева.
            if len(x) >= c: # Если количество вершин в x>= переменной "c".
                uzels.append(x[c - 1]) # В вспомогательный список uzels добавляется элемент из обрабатываемой ветки под номером "c".
        uzels = list(set(uzels))  # С помощью функций list(set()) в списке uzels, остаются только уникальные элементы.
        stroka_vod = str(c) + ':' # Создвется вспомогательная переменная, в которую записывается str(переменная "c") и ":", в ней будет храниться то, что будет возвращать функция.
        for i in uzels: # Цикл по элементам списка uzels.
            stroka_vod += ' ' + str(i) # Добавление в строку ответа  " " и элемента из списка uzels
    ...
    def codirovka(self, Stroca, Set_elementov):
        ...
        conecn = [] # Вспомогательный список, в котором будет храниться то, что возвращает данная функция.
        Stroca1 = Stroca.split() # С помощью функции строка.split(), переменная Stroca становиться списком.
        for i in range(len(Stroca1)): # Цикл по длинне списка Stroca.
            conecn.append(Set_elementov.index(Stroca1[i]) + 1) # В список ответа добавляется номер, на котором стоит вершина из списка Stroca в списке Set_elementov
    ...
    def p_sme(self, mas):  # список всех вершин без повторений
        ...
        nabor_elementov = ''  # строка, которая впосредствии будет представленна в виде отсортированого набора уникальных элементов
        ...
        nabor_elementov1 = list(set(nabor_elementov.split()))  # превращаем строку в список и убираем при помощи set() все повторения и снова превращаем set() в список
        return sorted(nabor_elementov1)  # сортируем набор вершин для удобства в будущем
    ...
    def tabl_smegmm(self, mas):  # вход масив входных данных выводит таблицу смежности
        ...
        set_elementov = self.p_sme(mas)  # На основе функции p_sme() в вспомогательную переменную записываются все уникальные вершины в дереве.
        conechn_mas = [] # Вспомогательный список, который будет хранить уникальный код для каждой вершины.

        for i in range(len(mas)):#цикл по длинне списка веток
            conechn_mas.append(self.codirovka(mas[i], set_elementov)) # Создание и запись уникальных кодов для каждой вершины, на основе функции codirovka
        ...
        for i in range(len(set_elementov)):
            ...
            tablica_smeg[i + 1][0] = set_elementov[i]  # завполныем вершины в столбце
    ...
    def postroenie_Excel(self, mas):  # Функция для построения таблицы Exel
        ...
        massiv = self.tabl_smegmm(mas)  # В вспомогательную переменную записывается таблица смежности всего дерева.
        ...
        for i in range(len(massiv)): # Цикл по количеству строк в таблице смежности
            ...
            for j in range(len(massiv[i])): # Цикл по количеству элементов в строке
                c = sheet.cell(row=i + 1, column=j + 1) # Указывается ячейка в которую можно вносить изменение.
                c.value = massiv[i][j] # В ячейку заноситься элемент из таблицы смежноости
...
def ctenie(mas):  # функция для чтения входных данных
    ...
    konecnkl_massiv = []  # Вспомогательный список, в котором будут храниться обработанные ветки дерева.
    for i in mas: # Цикл по ветка дерева, лежащим в переменой mas.
        stroka = i[i.index(' ') + 1:]  # Удаление номера ветки.
        konechnai_stroka = '' # Вспомогательная строка в которой будет храниться ветка дерева, с которой в данный момент работает цикл.
        for i in stroka: # цикл по ветке дерева у которой удалили номер.
            if i != ',': # Проверка символа, если он является ",", то он не добавляется в строку konechnai_stroka
                konechnai_stroka += i
        konecnkl_massiv.append(konechnai_stroka)  # добавление отформатирован ветки дерева в вспомогательный список.
