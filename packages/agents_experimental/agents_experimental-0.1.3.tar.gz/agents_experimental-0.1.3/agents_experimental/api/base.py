from typing import Any, Dict, Optional, TypeVar, Generic, List, Union, cast
import json
...
class APIBase:
    ...
    _RESOURCE_PATH = ""
    _HEADERS = {"cdf-version": "20240101-alpha"}
    ...
    def _post(self, url_path: str, json: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: The response JSON. If there was an error parsing the JSON response,
                           the dictionary will contain a 'raw_response' key with the raw response text
                           and an 'error' key with the error message.
        """
        url = f"{self._get_base_url()}{url_path}"
        # Set Accept header to application/json to ensure JSON response
        headers = {**self._HEADERS, "Accept": "application/json"}
        response = self._cognite_client.post(url, json=json, headers=headers)
    ...
    def _get(self, url_path: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        # Set Accept header to application/json to ensure JSON response
        headers = {**self._HEADERS, "Accept": "application/json"}
        response = self._cognite_client.get(url, params=params, headers=headers)
