...
class LLMToolExecutor[RuntimeContextType: BaseModel]:
    ...
    async def _handle_tool_calls(
        ...
        try:
            ...
            if is_human_confirmation:
                msg_with_calls = validate_not_none(messages[-3])
                messages = messages[:-2]
    ...
    async def run_tools(
        ...
        non_ignored_messages = [msg for msg in messages if not msg.role.is_provider_ignored]
        last_message = validate_not_none(non_ignored_messages[-1])
        msg_with_calls = last_message
        is_human_confirmation = last_message.role == LLMRole.HUMAN_CONFIRMATION

        if is_human_confirmation:
            msg_with_calls = validate_not_none(non_ignored_messages[-2])
        ...
        if human_in_the_loop_calls:
            ...
            for call, tool in human_in_the_loop_calls:
                ...
                artifact_result = await human_in_the_loop.confirmation_artifact_factory(context)
