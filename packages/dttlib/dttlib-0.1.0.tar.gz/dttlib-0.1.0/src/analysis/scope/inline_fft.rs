...
use crate::gds_sigp::fft::{create_fft_param, destroy_fft_param, fft, FFTTypeInfo};
...

{
        ...
        T: PipeData,
    {
        ...
        let seg_count = real_input_length as f64 / (length as f64 * (1.0 - self.overlap));
        let real_seg_count = seg_count.round() as usize;

        let real_overlap = 1.0 - (real_input_length as f64 / (length as f64 * real_seg_count as f64));
    ...
    fn generate<'a, T>(rc: Box<dyn UserMsgProvider>, config: &'a InlineFFT, _state: &'a mut (), input: Arc<TimeDomainArray<T>> )
        ...
        T: PipeDataPrimitive + FFTTypeInfo + Default,
    {
            ...
            // generate fft plan
            let fft_plan = match create_fft_param::<T>(segs[0].len(), config.window.clone())
            {
                Ok(p) => p,
                Err(e) => {
            ...
            let bucket_width_hz = 1.0 / (input.rate_hz / (segs.len() as f64));
            ...
            for (i, seg) in segs.into_iter().enumerate() {
                ...
                freq_array.set_mut_sequence_values(num_segs, i);
                ffts.push(Arc::new(freq_array));
            }

            // clean up fft plan
            destroy_fft_param(fft_plan);
            rc.user_message_handle().clear_message("InlineFFT");
            ffts.into()
    }

    pub (crate) async fn create<T>(rc: Box<dyn UserMsgProvider>, name: impl Into<String>, config_rx: tokio::sync::watch::Receiver<InlineFFT>,
                             input: &PipelineSubscriber<TimeDomainArray<T>>)
        -> PipelineSubscriber<FreqDomainArray<c128>>
    where
        T: PipeDataPrimitive + FFTTypeInfo + Default,
    {
        pipelines::unsynced_pipe::UnsyncPipe1::create(rc, name.into(), InlineFFT::generate, Some(config_rx), (), None, None, input).await
    }
