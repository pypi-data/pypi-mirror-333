...
class CycleStats(dict):
    ...
    fields = ('mode', 'toa', 'toa_mode', 
              ...
              'version_number','sequence_number','data_rate','num_data_frames',
              'num_bad_data_frames')
    ...
    def from_nmea_msg(cls, msg, log_datetime=None, drop_packet_timeout=True):
        ...
        if version_number >= 6:
            ...
            # Add a try/except case here to catch firmware bugs where an invalid timestamp is returned
            try:
                ...
                # Set the TOA to Jan 1 1970 if there is a parsing error
                values['toa'] = datetime.datetime.utcfromtimestamp(0)
        ...
        if version_number >= 8:
            values['sequence_number'] = int(msg['params'][30])
            values['data_rate'] = int(msg['params'][31])
            values['num_data_frames'] = int(msg['params'][32])
            values['num_bad_data_frames'] = int(msg['params'][33])
        else:
            values['sequence_number'] = -1
            values['data_rate'] = -1
            values['num_data_frames'] = -1
            values['num_bad_data_frames'] = -1
    ...
    def from_nmea_msg(cls, msg, log_datetime=None, drop_data_timeout=True):
        ...
        if version_number in (6, 7, 8):
            ...
            if version_number >= 8:
                ...
                # Don't add a field right now, since it breaks backward compatibility
                pass
