...
class OptionDataset(Dataset):
    def __init__(self, dataframe, is_train=False, target_scaler=None):
        ...
        self.data["log_gamma"] = torch.log(
            torch.tensor(gamma_values, dtype=torch.float32) + self.epsilon
        )
        ...
        self.data["sqrt_omega"] = torch.sqrt(
            torch.tensor(omega_values, dtype=torch.float32) + self.epsilon
        )

        T_values = self.data["T"].values
        self.data["inv_T"] = 1 / (
            torch.tensor(T_values, dtype=torch.float32) + self.epsilon
        )

        alpha_values = self.data["alpha"].values
        beta_values = self.data["beta"].values
        self.data["alpha_beta"] = torch.tensor(
            alpha_values, dtype=torch.float32
        ) * torch.tensor(beta_values, dtype=torch.float32)
        ...
        self.data["time_decay"] = torch.exp(
            -0.05 * torch.tensor(T_values, dtype=torch.float32)
        )
...
def train_test_split(data, test_size=0.3, random_state=42):
    ...
    val_dataset = OptionDataset(
        val_data, is_train=False, target_scaler=train_dataset.target_scaler
    )
...
def cleandataset(data):
    """
    Clean the dataset by filtering out rows where V <= 0.5
    This removes potentially invalid or outlier option prices
    Args:
        data (pd.DataFrame): Input DataFrame to clean
    Returns:
        pd.DataFrame: Cleaned DataFrame with V > 0.5 and reset index
    """
