...
class TableInput(ProcedureInputInfo):
    def __init__(self, required=True, readonly=False, label: str | None = None, tooltip: str | None = None, can_add_rows=True, can_delete_rows=True):
        super().__init__()
        self.type = "table"
        self.required = required
        self.readonly = readonly
...
class SplitLayout(WidgetsLayout):
    def __init__(self, *, widgets: tuple[WidgetName, WidgetName] | None, initial_orientation: Literal["horizontal", "vertical"], resize: Literal["start", "end", "auto"] | None = None, initial_size: str | None = None):
        self.widget_start = None if widgets is None else widgets[0]
        self.widget_end = None if widgets is None else widgets[1]
