...
class Mediator(metaclass=SingletonMeta):
    ...
    def send(
        ...
        if not handler:
            raise ValueError(
                f"Request {type(request).__name__} cannot be handled; no handler has been registered for {type(request)}."
            )
    ...
    def register_handler(
        ...
        if request_type in self._request_handlers:
            ...
            if type(handler) is not type(old_handler):
                raise ValueError(
                    f"Handler {type(handler).__name__} cannot be registered; another handler for request type {request_type} has already been registered. Check {type(self._request_handlers[request_type])}."
                )
    ...
    def __check_request_validity(self, request_type: type) -> None:
        ...
        if not hasattr(request_type, "_rmediator_decorated_request"):
            raise ValueError(
                f"Handler {request_type.__name__} cannot be registered; the request {request_type} class has not been decorated using @request."
            )
    ...
    def __check_request_handler_validity(self, handler_type: type) -> None:
        ...
        if not hasattr(handler_type, "_rmediator_decorated_request_handler"):
            raise ValueError(
                f"Handler {handler_type.__name__} cannot be registered; the request handler {type(handler_type)} class has not been decorated using @request_handler."
            )
