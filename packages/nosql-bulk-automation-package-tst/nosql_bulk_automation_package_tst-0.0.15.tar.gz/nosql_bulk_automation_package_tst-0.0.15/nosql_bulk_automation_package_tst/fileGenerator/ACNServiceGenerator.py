...
class ACNServiceGenerator:
    ...
    def acnServiceGenerator(self,datastore,app,paas_name,sec_zone,phase,cid,reg,LZ):
        ...
        if datastore=='Elasticsearch':
            ...
            #########################################
            # UNDO THIS CHANGE FOR PUSHIGN THE  ACN SERVICES #####            
            ####################################
            # paths=f"../inventory/argocd-nosqlpaas-{truePhase}/{truePhase}/az/{paas_name}/acn-services"
            paths=f"dummyInventory/argocd-nosqlpaas-{truePhase}/{truePhase}/az/{paas_name}/acn-services"
            ...
            try:
                with open(f"{paths}/datastore-{datastore.lower()}-acn-{app}-{phase}-{app}.yaml",'r+') as f:
                    ...
                    # yaml.dump(file,f,sort_keys=False)
                        with open(f"{paths}/datastore-{datastore.lower()}-acn-{app}-{phase}-{app}.yaml","w+") as f1:
                            yaml.dump(file,f1,sort_keys=False)
            except IOError:
                print("ACNS not found need to create new ancs file")
                with open(f"{paths}/datastore-{datastore.lower()}-acn-{app}-{phase}-{app}.yaml","w+") as f:
