...
from gfatpy.utils.utils import moving_average_xarray
...
def average_dc_signal(dc):
    ...
    logger.info("Start DC Averaging")
    try:
        # force to 2D
        ...
        weights = np.array([1, 2, 4, 6, 8, 6, 4, 2, 1], dtype=float)
        weights /= np.sum(weights)
        dc_pp = sp.ndimage.convolve(dc_avg_t, weights, mode="constant")
        dc_pp = np.concatenate([dc_pp[:-4], dc_pp[-8:-4]])
    except Exception as e:
        dc_pp = np.zeros(dc.size)
        logger.error("preprocessing dc. DC set to 0")
        raise RuntimeError(e)

    logger.info("End DC Averaging")
    return dc_pp
...
def ff_2D_overlap_from_channels(
    ...
    overlap_raw = ff.where(nf > 0., other=1.) / nf.where(nf > 0., other=1.)

    #Lower limit    
    overlap = overlap_raw.where(overlap_raw.range > min_range, other=0.)    

    #Find when full overlap is reached
    filtered_overlap = moving_average_xarray(overlap, dim='range', window_size=window_size)
    full_overlap_reached_range = overlap.range[(filtered_overlap > 1.).argmax(dim="range")]

    # Set to 1.0 when full overlap is reached
    overlap = overlap.where(overlap.range <= full_overlap_reached_range, other=1.0)

    #Smooth overlap
    overlap = moving_average_xarray(overlap, dim=('range', 'time'), window_size=(window_size,window_size))
