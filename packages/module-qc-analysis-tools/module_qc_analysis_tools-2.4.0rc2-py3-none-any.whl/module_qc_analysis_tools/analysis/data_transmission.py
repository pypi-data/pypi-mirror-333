...
def analyze(
    ...
    for chip in input_json:
        chipresult = func(chip)
        if chipresult:
            results.append(chipresult)
            log.debug(f" chip result {chipresult}")
    log.debug(f" RESULTS FROM ANALYZE {results}")
    return zip(*results)
...
def analyze_chip(
    ...
    for inputDF in inputDFs:
        ...
        if inputDF._subtestType == "DT_MERGE":
            ...
            _passes_qc_merge, summary_merge, _rounded_results = perform_qc_analysis(
