...
async def main():
    ...
    [
        await Asset.update_or_create(
            {"free": amount}, coin=(await Coin.get_or_create(ticker=coin))[0], agent=maker, type_=AssetType.found
        )
    ...
    [
        await Asset.update_or_create(
            {"free": amount}, coin=await Coin.get(ticker=coin), agent=taker, type_=AssetType.found
        )
