...
class AnalysisTreeNode(AnyNode):
    ...
    Attributes:
        id (int, optional): The unique identifier of the node, if not supplied one chosen by the root node.
    ...
    def _execute_algorithm(self, algorithm_name, args):
        ...
        try:
            ...
            # If "active_node" is in the algorithm's argument schema, add it to args
            if "active_node" in schema_properties:
                args["active_node"] = self
...
class Concordance:
    ...
    def load(
        ...
        root.line_count = len(self.metadata)
    ...
    def subset_at_node(self, node: AnalysisTreeNode) -> ConcordanceSubset:
        ...
        # If the node has selected_lines attribute (a list of line IDs to keep)
        if hasattr(node, 'selected_lines'):
            selected_lines = node.selected_lines

            # Use the selected_lines to filter the relevant data
            ...
            # No selected lines, use the entire dataset
