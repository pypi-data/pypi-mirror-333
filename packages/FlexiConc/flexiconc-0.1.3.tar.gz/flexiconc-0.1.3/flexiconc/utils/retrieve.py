...
from flexiconc.utils.logging import get_function_info, add_to_tree
...
def retrieve_from_cwb(
    ...
    self.root = AnalysisTreeNode(id=0, node_type="subset", parent=None, concordance=self, line_count=len(self.metadata), label="root")

    # add default sorting by cpos
    self.root.add_arrangement_node([{"algorithm_name": "Sort by Corpus Position", "args":{}}])


# File: utils/retrieve.py
...
def retrieve_from_clic(
    ...
    subset: str = "all",
    contextsize: int = 20,
    api_base_url: str = "https://dhss-clic.phil.uni-erlangen.de/api/concordance",
    ...
    Parameters:
    - self: The Concordance object.
    - query (str): The query string used to retrieve concordance lines.
    - corpora (str): The corpus or corpora to search within.
    - subset (str): The subset of the corpora to search ('all', 'quote', 'nonquote', 'shortsus', 'longsus'). Defaults to 'all'.
    - contextsize (int): The number of context words on each side. Defaults to 20.
    - api_base_url (str): The base URL of the CLiC API. Defaults to "https://clic.bham.ac.uk/api/concordance".
    - metadata_attrs (Optional[List[str]]): List of metadata attributes to include. Defaults to ['text_id', 'chapter', 'paragraph', 'sentence'].
    - tokens_attrs (Optional[List[str]]): List of token-level attributes to include. Defaults to ['word'].

    Updates:
    - self.metadata: A DataFrame containing the structural metadata.
    - self.tokens: A DataFrame containing token-level information.
    - self.matches: A DataFrame containing match information.
    - self.info: Dictionary containing additional info, including the query used.
    - self.root: Updates the analysis tree with the root node containing concordance line information.
    """
    ...
    self.info["query"] = query
    self.root = AnalysisTreeNode(id=0, node_type="subset", parent=None, concordance=self, line_count=len(self.metadata))

    '''
    from flexiconc.utils.logging import add_to_tree, get_function_info
    self.tree = []
    add_to_tree(
        self,
        type="query",
        function_name="retrieve_from_clic",
        args_dict={'query': query, 'corpora': corpora, 'contextsize': contextsize},
        values={
    ...
    # Add a default ordering node
    self.add_node(
        algorithms={'order': [{'algorithm_name': 'Sort by Corpus Position', 'args': {}}]},
        active_node=0
    )
    '''
