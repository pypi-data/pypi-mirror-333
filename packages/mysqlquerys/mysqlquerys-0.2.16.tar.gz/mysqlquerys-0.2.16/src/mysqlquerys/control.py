...
def main():
    ...
    # chelt_db = r"D:\Python\MySQL\bla.ini"
    # ini_file = r"D:\Python\MySQL\cheltuieli_online\src\rappmysql\static\wdb.ini"
    # test_db = r"D:\Python\MySQL\test_db.ini"
    ...
    active_table = mysql_rm.Table(conf.credentials, 'hyundai_ioniq')
    print(active_table.columnsNames, type(active_table.columnsNames))
    matches = [('type', 'electric')]
    ids = active_table.returnCellsWhere('id', matches)
    # print(ids)
    for i in ids:
        match = [('id', i)]
        comment = active_table.returnCellsWhere('comment', match)[0]
        print(i, comment)
        active_table.changeCellContent('eProvider', comment, 'id', i)
        active_table.changeCellContent('comment', None, 'id', i)
