...
def selecto(options, exit_messages=True, select_multiple=False, multi_index_jump=5):
    if not options:
        print("I don't know chief, that list be looking kinda empty." if exit_messages else "Options list is empty.")
        return None
    ...
    def menu():
        ...
        while True:
            ...
            if key in (b'\xe0', b'\x0e'):  
                ...
                if select_multiple:
                    return [options[i] for i in sorted(selected_items)]  
                else:
