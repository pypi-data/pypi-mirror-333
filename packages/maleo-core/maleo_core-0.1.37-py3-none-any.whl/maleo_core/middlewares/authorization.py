...
class AuthorizationMiddleware(BaseHTTPMiddleware):
    ...
    async def _authenticate_and_authorize(self, request:Request, allowed_roles:BaseTransfers.Parameter.AllowedRoles, call_next:RequestResponseEndpoint) -> Response:
        ...
        if authorization_header and authorization_header.startswith("Bearer "):
            ...
            if access_token_payload and access_token_payload.exp > datetime.now(timezone.utc):
                if allowed_roles == "*" or (isinstance(allowed_roles, list) and access_token_payload.role_id in allowed_roles):
