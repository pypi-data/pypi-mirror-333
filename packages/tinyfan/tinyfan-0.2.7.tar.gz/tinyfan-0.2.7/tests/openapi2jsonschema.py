...
def allow_null_optional_fields(data, parent=None, grand_parent=None, _=None):
    new = {}
    try:
        for k, v in iter(data.items()):
            new_v = v
...
def write_schema_file(schema, f):
    schemaJSON = ""
...
def openapi2jsonschema(crdFile: str) -> Result | None:
    ...
    with f:
        ...
        for y in defs:
            if "spec" in y:
                res.kind = y["spec"]["names"]["kind"]
                res.group = y["spec"]["group"].split(".")[0]
                res.fullgroup = y["spec"]["group"]
            if "spec" in y and "versions" in y["spec"] and y["spec"]["versions"]:
                for version in y["spec"]["versions"]:
                    if "schema" in version and "openAPIV3Schema" in version["schema"]:
                        res.version = version["name"]
                        res.schema = version["schema"]["openAPIV3Schema"]
                    elif "validation" in y["spec"] and "openAPIV3Schema" in y["spec"]["validation"]:
                        res.version = version["name"]
                        res.schema = y["spec"]["validation"]["openAPIV3Schema"]
            elif "spec" in y and "validation" in y["spec"] and "openAPIV3Schema" in y["spec"]["validation"]:
                res.version = y["spec"]["version"]
                res.schema = y["spec"]["validation"]["openAPIV3Schema"]
        if res.schema:
            res.schema = additional_properties(
                res.schema,
                skip=not os.getenv("DENY_ROOT_ADDITIONAL_PROPERTIES"),
            )
            res.schema = replace_int_or_string(res.schema)
            # schemaJSON = json.dumps(schema, indent=2)
            return res
        return None
