...
class Sweeper:
    ...
    def convert_si_value(value, unit):
        """
        ...
        unit: Unit string, e.g., "V", "mV", "kV", etc. (assuming the prefix is a single character)
        
        Returns:
        (new_value, new_unit) e.g., (100.0, "mV")
        """
        ...
        return f"{new_value:>8.4f} [{new_unit}]"
        ...
        if status == 'start':
            ...
            with open(f"{self.log_filename}.txt", 'a') as file:
                ...
                for output_gate in self.outputs.gates:
                    file.write(
                        f"{' & '.join(line.label for line in output_gate.lines):<50} {self.convert_si_value(output_gate.voltage(), 'V')} \n")
                file.write("\n")
        if status == 'end':
