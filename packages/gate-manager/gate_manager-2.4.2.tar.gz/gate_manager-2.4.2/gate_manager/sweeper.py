...
class Sweeper:
    ...
    def _get_unique_filename(self, base_filename: str) -> str:
        ...
        # Build the full path with the base filename and extension
        base_filepath = os.path.join(directory, base_filename)
        
        counter = 1
        # Build the initial unique file path
        unique_filepath = f"{base_filepath}_run{counter}.txt"
        
        # Increment the counter until a non-existent file is found
        while os.path.isfile(unique_filepath):
            counter += 1
            unique_filepath = f"{base_filepath}_run{counter}.txt"
