...
class yarr(hardware_control_base):
    ...
    def run_scan(self, scan, output=None, skip_reset=False):
        if output:
            cmd = f'{self.scanConsole_exe} -r {self.controller} -c {self.connectivity} -s {scan} -o {output} {"--skip-reset" if skip_reset else ""} {self.exe_postOpts}'
        else:
            cmd = f'{self.scanConsole_exe} -r {self.controller} -c {self.connectivity} -s {scan} {"--skip-reset" if skip_reset else ""} {self.exe_postOpts}'
