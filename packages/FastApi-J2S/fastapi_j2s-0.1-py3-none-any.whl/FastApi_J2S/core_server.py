...
class Set_Server:
	def __init__(kimin, config, modul, base_dir):
		kimin.modul = modul
		kimin.config = kimin.modul['ext'](modul=kimin.modul).ReadFile(path=config, tipe='json', mode='r')['data']
			...
			if route not in ['modul_name', 'class_name']:
				if kimin.config['routes'][route] not in kimin.use_routes:
					...
					if kimin.config['system'].get('import_config', False):
						modul = getattr(modul, kimin.config['routes']['class_name'])(modul=kimin.modul, cfg=kimin.config, server=server)
					else:
						modul = getattr(modul, kimin.config['routes']['class_name'])(modul=kimin.modul)
					function = kimin.config['routes'][route]['function']
	...
	def Server(kimin):
		...
		server = kimin.modul['FI']()
		server.mount("/static", kimin.modul['static'](directory=f"{kimin.base_dir}/{kimin.config['server']['static_path']}"), name="static")
		kimin.modul['template'](directory=f"{kimin.base_dir}/{kimin.config['server']['template_path']}")
		return server