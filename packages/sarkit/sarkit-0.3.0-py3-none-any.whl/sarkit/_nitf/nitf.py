...
def _construct_block_bounds(
    image_header: ImageSegmentHeader,
    ...
    def get_raw_dtype() -> numpy.dtype:
        if pvtype == "INT":
            ...
            if bpp not in [4, 8, 16]:
                raise ValueError(
                    "Got PVTYPE = C and NBPP = {} (not 32, 64 or 128), which is unsupported.".format(
                        nbpp
                    )
                )
...
def _get_format_function(
    ...
    if complex_order is not None:
        return ComplexFormatFunction(
            raw_dtype, complex_order, band_dimension=band_dimension
        )
...
def _verify_image_segment_compatibility(
    img0: ImageSegmentHeader, img1: ImageSegmentHeader
) -> bool:
    """
    ...
    if (comp_order0 is None and comp_order1 is not None) or (
        ...
        comp_order0 is not None
        and comp_order1 is not None
        and (comp_order0 != comp_order1)
    ):
        return False
...
class NITFDetails(object):
    """Parsing of the header information in a NITF file."""
    ...
    __slots__ = (
        ...
        "_nitf_header",
        "_img_headers",
        "img_subheader_offsets",
        "img_subheader_sizes",
        "img_segment_offsets",
        "img_segment_sizes",
        "graphics_subheader_offsets",
        "graphics_subheader_sizes",  # only 2.1
        "graphics_segment_offsets",
        "graphics_segment_sizes",
        "text_subheader_offsets",
        "text_subheader_sizes",
        "text_segment_offsets",
        "text_segment_sizes",
        "des_subheader_offsets",
        "des_subheader_sizes",
        "des_segment_offsets",
        "des_segment_sizes",
        "res_subheader_offsets",
        "res_subheader_sizes",  # only 2.1
        "res_segment_offsets",
        "res_segment_sizes",
    )
    ...
    def __init__(self, file_object: Union[str, BinaryIO]):
        ...
        if isinstance(file_object, str):
            ...
            raise TypeError(
                "file_object is required to be a file like object, or string path to a file."
            )

        fhdr = self._file_object.read(4).decode("ascii")
        vers_string = self._file_object.read(5).decode("ascii")
        if fhdr != "NITF" or vers_string != "02.10":
            raise ValueError(f"{file_object} is not a NITF02.10 file")
        ...
        (
            ...
            self.img_subheader_sizes,
            self.img_segment_offsets,
            self.img_segment_sizes,
        ) = self._element_offsets(cur_loc, self._nitf_header.ImageSegments)

        # populate graphics segment offset information - only version 2.1
        (
            cur_loc,
            self.graphics_subheader_offsets,
            self.graphics_subheader_sizes,
            self.graphics_segment_offsets,
            self.graphics_segment_sizes,
    ...
    def _element_offsets(
        cur_loc: int, item_array_details: Union[_ItemArrayHeaders, None]
    ) -> Tuple[
        int,
        ...
        subhead_offsets[1:] += numpy.cumsum(subhead_sizes[:-1]) + numpy.cumsum(
            item_sizes[:-1]
        )
    ...
    @property
    def nitf_header(self) -> NITFHeader:
        """
    ...
    @property
    def img_headers(self) -> list[ImageSegmentHeader]:
        ...
        self._img_headers = [
            self.parse_image_subheader(i)
            for i in range(self.img_subheader_offsets.size)
        ]
        return self._img_headers
    ...
    def _fetch_item(
        self, name: str, index: int, offsets: numpy.ndarray, sizes: numpy.ndarray
    ) -> bytes:
    ...
    def parse_image_subheader(self, index: int) -> ImageSegmentHeader:
        """
    ...
    def parse_text_subheader(self, index: int) -> TextSegmentHeader:
        """
    ...
    def parse_des_subheader(self, index: int) -> DataExtensionHeader:
        """
    ...
    def parse_res_subheader(self, index: int) -> ReservedExtensionHeader:
        """
    ...
    def get_headers_json(self) -> dict:
        ...
        out = OrderedDict(
            [
                ("header", self._nitf_header.to_json()),
            ]
        )
        if self.img_subheader_offsets is not None:
            out["Image_Subheaders"] = [
                self.parse_image_subheader(i).to_json()
                for i in range(self.img_subheader_offsets.size)
            ]
        if self.graphics_subheader_offsets is not None:
            out["Graphics_Subheaders"] = [
                self.parse_graphics_subheader(i).to_json()
                for i in range(self.graphics_subheader_offsets.size)
            ]
        if self.text_subheader_offsets is not None:
            out["Text_Subheaders"] = [
                self.parse_text_subheader(i).to_json()
                for i in range(self.text_subheader_offsets.size)
            ]
        if self.des_subheader_offsets is not None:
            out["DES_Subheaders"] = [
                self.parse_des_subheader(i).to_json()
                for i in range(self.des_subheader_offsets.size)
            ]
        if self.res_subheader_offsets is not None:
            out["RES_Subheaders"] = [
                self.parse_res_subheader(i).to_json()
                for i in range(self.res_subheader_offsets.size)
            ]
        return out
...
class NITFReader(BaseReader):
    ...
    __slots__ = (
        "_nitf_details",
        "_unsupported_segments",
        "_image_segment_collections",
        "_reverse_axes",
        "_transpose_axes",
    ...
    def __init__(
        ...
        image_segment_collections: list-like of list-like of int, optional
            Collections of image segment indices that associate segment(s) to images.
            If None, each supported segment (see `_check_image_segment_for_compliance`) is considered independent.
        ...
        try:
            ...
            if not isinstance(nitf_details, NITFDetails):
                raise TypeError(
                    "The input argument for NITFReader must be a NITFDetails object."
                )
        ...
        if reverse_axes is not None:
            ...
            for entry in reverse_axes:
                if not 0 <= entry < 2:
                    raise ValueError(
                        "reverse_axes values must be restricted to `{0, 1}`."
                    )
        ...
        if len(self._unsupported_segments) == len(self.nitf_details.img_headers):
            raise SarpyIOError(
                f"There are no supported image segments in NITF file {self.file_name}"
            )

        # our supported images are assembled into collections for joint presentation
        if image_segment_collections is None:
            self._image_segment_collections = self.find_image_segment_collections()
        else:
            self._image_segment_collections = tuple(
                tuple(x) for x in image_segment_collections
            )
        self.verify_collection_compliance()

        data_segments = self.get_data_segments()
        BaseReader.__init__(
            self, data_segments, reader_type=reader_type, close_segments=True
        )
    ...
    def _check_image_segment_for_compliance(
        self, index: int, img_header: ImageSegmentHeader
    ) -> bool:
        """
    ...
    def _get_mask_details(
        self, image_segment_index: int
    ) -> Tuple[Optional[numpy.ndarray], int, int]:
        """
    ...
    def verify_collection_compliance(self) -> None:
        ...
        for collection_index, the_indices in enumerate(self.image_segment_collections):
            ...
            for the_index in the_indices[1:]:
                t_compat = self._verify_image_segment_compatibility(
                    the_indices[0], the_index
                )
                if not t_compat:
                    logger.error(
    ...
    def _get_collection_element_coordinate_limits(
        ...
        image_headers = [
            self.nitf_details.img_headers[image_ind]
            for image_ind in self.image_segment_collections[collection_index]
        ]
        # noinspection PyTypeChecker
        return _get_collection_element_coordinate_limits(
            image_headers, return_clevel=False
        )

    def _handle_no_compression(
        self, image_segment_index: int, apply_format: bool
    ) -> DataSegment:
        # NB: Natural order inside the block is (bands, rows, columns)
        image_header = self.get_image_header(image_segment_index)
        if image_header.IMODE not in ["B", "R", "P"] or image_header.IC not in [
            "NC",
            "NM",
        ]:
            raise ValueError(
        ...
        (
            ...
            complex_order,
            lut,
        ) = self._get_dtypes(image_segment_index)
        ...
        raw_shape = _get_shape(
            image_header.NROWS,
            image_header.NCOLS,
            raw_bands,
            band_dimension=raw_band_dimension,
        )

        # get mask definition details
        mask_offsets, exclude_value, additional_offset = self._get_mask_details(
            image_segment_index
        )
        ...
        block_offsets = (
            ...
            else numpy.arange(len(block_bounds), dtype="int64") * block_size
        )
        ...
        if len(block_bounds) != len(block_offsets):
            raise ValueError(
                "Got mismatch between block definition and block offsets definition"
            )

        final_block_ending = (
            numpy.max(block_offsets[block_offsets != exclude_value])
            + block_size
            + additional_offset
        )
        ...
        if apply_format:
            ...
            formatted_dtype = raw_dtype
            formatted_shape = _get_shape(
                image_header.NROWS, image_header.NCOLS, raw_bands, band_dimension=2
            )
        ...
        if raw_bands == 1:
            ...
            reverse_axes = (
                ...
                else tuple(entry + 1 for entry in use_reverse)
            )
        elif image_header.IMODE == "R":
            # order inside the block is (rows, bands, columns)
            ...
            reverse_axes = (
                None
                if use_reverse is None
                else tuple(reverse_mapping[entry] for entry in use_reverse)
            )
        elif image_header.IMODE == "P":
        ...
        for block_index, (block_definition, block_offset) in enumerate(
            zip(block_bounds, block_offsets)
        ):
            ...
            b_raw_shape = _get_shape(
                b_rows, b_cols, raw_bands, band_dimension=raw_band_dimension
            )
            ...
            row_start, row_end = (
                block_definition[0],
                min(block_definition[1], image_header.NROWS),
            )
            col_start, col_end = (
                block_definition[2],
                min(block_definition[3], image_header.NCOLS),
            )
    ...
    def _handle_imode_s_no_compression(
        self, image_segment_index: int, apply_format: bool
    ) -> DataSegment:
        image_header = self.get_image_header(image_segment_index)
        ...
        final_block_ending = (
            numpy.max(block_offsets_flat[block_offsets_flat != exclude_value])
            + block_size
            + additional_offset
        )
        populated_ending = self.nitf_details.img_segment_sizes[image_segment_index]
        ...
        for band_number in range(raw_bands):
            band_raw_shape = _get_shape(
                image_header.NROWS, image_header.NCOLS, 1, band_dimension=2
            )
            data_segments = []
            child_arrangement = []
            for block_index, (block_definition, block_offset) in enumerate(
        ...
        return BandAggregateSegment(
            band_segments,
            2,
            formatted_dtype=formatted_dtype,
    ...
    def _create_data_segment_from_imode_b(
        self, image_segment_index: int, apply_format: bool
    ) -> DataSegment:
        image_header = self.get_image_header(image_segment_index)
        if image_header.IMODE != "B":
    ...
    def _create_data_segment_from_imode_p(
        self, image_segment_index: int, apply_format: bool
    ) -> DataSegment:
        image_header = self.get_image_header(image_segment_index)
    ...
    def _create_data_segment_from_imode_r(
        self, image_segment_index: int, apply_format: bool
    ) -> DataSegment:
        image_header = self.get_image_header(image_segment_index)
    ...
    def _create_data_segment_from_imode_s(
        self, image_segment_index: int, apply_format: bool
    ) -> DataSegment:
        image_header = self.get_image_header(image_segment_index)
        ...
        if image_header.IC in ["NC", "NM"]:
            return self._handle_imode_s_no_compression(
                image_segment_index, apply_format
            )
        else:
            raise ValueError("Got unhandled IC `{}`".format(image_header.IC))
    ...
    def create_data_segment_for_image_segment(
        ...
        if image_header.IMODE == "B":
            out = self._create_data_segment_from_imode_b(
                image_segment_index, apply_format
            )
        elif image_header.IMODE == "P":
            out = self._create_data_segment_from_imode_p(
                image_segment_index, apply_format
            )
        elif image_header.IMODE == "S":
            out = self._create_data_segment_from_imode_s(
                image_segment_index, apply_format
            )
        elif image_header.IMODE == "R":
            out = self._create_data_segment_from_imode_r(
                image_segment_index, apply_format
            )
        else:
            raise ValueError(
    ...
    def create_data_segment_for_collection_element(
        self, collection_index: int
    ) -> DataSegment:
        """
        ...
        block_definition = self._get_collection_element_coordinate_limits(
            collection_index
        )
        ...
        for img_index, block_def in zip(block, block_definition):
            ...
            if raw_bands is None:
                raw_bands = (
                    1
                    if child_segment.formatted_ndim == 2
                    else child_segment.formatted_shape[2]
                )
        ...
        raw_shape = (
            ...
            if raw_bands == 1
            else (total_rows, total_columns, raw_bands)
        )

        formatted_shape = (
            raw_shape[:2] if transpose is None else (raw_shape[1], raw_shape[0])
        )
...
def _flatten_bytes(value: Union[bytes, Sequence]) -> bytes:
    if value is None:
        ...
        raise TypeError(
            "input must be a bytes object, or a sequence with bytes objects as leaves"
        )
...
class SubheaderManager(object):
    ...
    def __init__(self, subheader, item_bytes: Optional[bytes] = None):
        ...
        if item_bytes is None:
            if self.item_bytes_required:
                raise ValueError(
                    "item_bytes is required by class {}.".format(self.__class__)
                )
        else:
    ...
    def subheader_offset(self, value) -> None:
        if self._subheader_offset is not None:
            raise ValueError(
                "subheader_offset is read only after being initially defined."
            )
    ...
    def write_subheader(self, file_object: BinaryIO) -> None:
        ...
        if len(the_bytes) != self._subheader_size:
            raise ValueError(
                ...
                "and the anticipated size of the subheader {}".format(
                    len(the_bytes), self._subheader_size
                )
            )
...
class ImageSubheaderManager(SubheaderManager):
    ...
    def item_size(self, value):
        ...
        if self.subheader.mask_subheader is None:
            ...
            self._item_size = (
                int(value) + self.subheader.mask_subheader.get_bytes_length()
            )
    ...
    def write_item(self, file_object: BinaryIO) -> None:
        ...
        if self.subheader.mask_subheader is None:
            ...
            file_object.seek(
                self.item_offset + self.subheader.mask_subheader.get_bytes_length(),
                os.SEEK_SET,
            )
        file_object.write(self.item_bytes)
        self.item_written = True
...
class NITFWritingDetails(object):
    ...
    def __init__(
        ...
        image_segment_coordinates: Optional[
    ...
    def image_managers(self, value):
        ...
        for entry in value:
            if not isinstance(entry, ImageSubheaderManager):
                raise TypeError(
                    "image_managers entries must be of type {}".format(
                        ImageSubheaderManager
                    )
                )
    ...
    def image_segment_collections(self, value):
        ...
        for entry in value:
            ...
            if last_index == -1:
                if entry[0] != 0:
                    raise ValueError(
                        "The first entry of image segment collection must start at 0."
                    )

            for item in entry:
                if not isinstance(item, int) or item < 0:
                    raise TypeError(
                        "image segment collection must be a tuple of tuples of non-negative ints"
                    )
                if item != last_index + 1:
                    raise ValueError(
                        "image segment collection entries must be arranged in ascending order"
                    )
                last_index = item
        if last_index != len(self.image_managers) - 1:
            raise ValueError(
                "Mismatch between the number of image segments and the collection entries"
            )
    ...
    def image_segment_coordinates(self, value):
        ...
        for coll in self.image_segment_collections:
            image_headers = [
                self.image_managers[image_ind].subheader for image_ind in coll
            ]
            coordinate_scheme, clevel = _get_collection_element_coordinate_limits(
                image_headers, return_clevel=True
            )
            collections_clevel.append(clevel)
            # noinspection PyTypeChecker
            coordinate_scheme = tuple(
                tuple(entry) for entry in coordinate_scheme.tolist()
            )
    ...
    def graphics_managers(self, value):
        ...
        for entry in value:
            if not isinstance(entry, GraphicsSubheaderManager):
                raise TypeError(
                    "graphics_managers entries must be of type {}".format(
                        GraphicsSubheaderManager
                    )
                )
    ...
    def text_managers(self, value):
        ...
        for entry in value:
            if not isinstance(entry, TextSubheaderManager):
                raise TypeError(
                    "text_managers entries must be of type {}".format(
                        TextSubheaderManager
                    )
                )
    ...
    def des_managers(self, value):
        ...
        for entry in value:
            if not isinstance(entry, DESSubheaderManager):
                raise TypeError(
                    "des_managers entries must be of type {}".format(
                        DESSubheaderManager
                    )
                )
    ...
    def res_managers(self, value):
        ...
        for entry in value:
            if not isinstance(entry, RESSubheaderManager):
                raise TypeError(
                    "res_managers entries must be of type {}".format(
                        RESSubheaderManager
                    )
                )
    ...
    def _get_sizes(
        ...
        for i, entry in enumerate(managers):
            ...
            if item_size is None:
                if require:
                    raise ValueError(
                        "item_size for {} at index {} is unset".format(name, item_size)
                    )
                else:
                    item_size = 0
    ...
    def _write_items(
        self, managers: Optional[Sequence[SubheaderManager]], file_object: BinaryIO
    ) -> None:
        if managers is None:
            return
    ...
    def _verify_item_written(
        self, managers: Optional[Sequence[SubheaderManager]], name: str
    ) -> None:
        if managers is None:
            return
    ...
    def _get_image_sizes(self, require: bool = False) -> ImageSegmentsType:
        ...
        subhead_sizes, item_sizes = self._get_sizes(
            self.image_managers, "Image", require=require
        )
    ...
    def _get_graphics_sizes(self, require: bool = False) -> GraphicsSegmentsType:
        ...
        subhead_sizes, item_sizes = self._get_sizes(
            self.graphics_managers, "Graphics", require=require
        )
    ...
    def _get_text_sizes(self, require: bool = False) -> TextSegmentsType:
        ...
        subhead_sizes, item_sizes = self._get_sizes(
            self.text_managers, "Text", require=require
        )
    ...
    def _get_des_sizes(self, require: bool = False) -> DataExtensionsType:
        ...
        subhead_sizes, item_sizes = self._get_sizes(
            self.des_managers, "DES", require=require
        )
    ...
    def _get_res_sizes(self, require: bool = False) -> ReservedExtensionsType:
        ...
        subhead_sizes, item_sizes = self._get_sizes(
            self.res_managers, "RES", require=require
        )
        return ReservedExtensionsType(
            subhead_sizes=subhead_sizes, item_sizes=item_sizes
        )
        ...
        if self.image_managers is not None:
            for index, entry in enumerate(self.image_managers):
                ...
                if entry.item_size is None or entry.item_size == 0:
                    if require:
                        raise ValueError(
                            "image manager at index {} has item_size unpopulated or populated as 0".format(
                                index
                            )
                        )
                    else:
        ...
        if self.graphics_managers is not None:
            for index, entry in enumerate(self.graphics_managers):
                ...
                if entry.item_size is None or entry.item_size == 0:
                    if require:
                        raise ValueError(
                            "graphics manager at index {} has item_size unpopulated or populated as 0".format(
                                index
                            )
                        )
                    else:
        ...
        if self.text_managers is not None:
            for index, entry in enumerate(self.text_managers):
                ...
                if entry.item_size is None or entry.item_size == 0:
                    if require:
                        raise ValueError(
                            "text manager at index {} has item_size unpopulated or populated as 0".format(
                                index
                            )
                        )
                    else:
        ...
        if self.des_managers is not None:
            for index, entry in enumerate(self.des_managers):
                ...
                if entry.item_size is None or entry.item_size == 0:
                    if require:
                        raise ValueError(
                            "des manager at index {} has item_size unpopulated or populated as 0".format(
                                index
                            )
                        )
                    else:
        ...
        if self.res_managers is not None:
            for index, entry in enumerate(self.res_managers):
                ...
                if entry.item_size is None or entry.item_size == 0:
                    if require:
                        raise ValueError(
                            "res manager at index {} has item_size unpopulated or populated as 0".format(
                                index
                            )
                        )
                    else:
    ...
    def set_header_clevel(self) -> None:
        ...
        self.header.CLEVEL = (
            mem_clevel
            if self._collections_clevel is None
            else max(mem_clevel, max(self._collections_clevel))
        )
    ...
    def write_header(self, file_object: BinaryIO, overwrite: bool = False) -> None:
        ...
        if len(the_bytes) != self._header_size:
            raise ValueError(
                ...
                "does not match the actual header length {}".format(
                    self._header_size, len(the_bytes)
                )
            )
...
class NITFWriter(BaseWriter):
    ...
    def __init__(
        ...
        if isinstance(file_object, str):
            if check_existence and os.path.exists(file_object):
                raise SarpyIOError(
                    "Given file {} already exists, and a new NITF file cannot be created here.".format(
                        file_object
                    )
                )
        ...
        self.nitf_writing_details.set_all_sizes(
            require=True
        )  # NB: while no compression supported...
    ...
    def nitf_writing_details(self, value):
        ...
        if not isinstance(value, NITFWritingDetails):
            raise TypeError(
                "nitf_writing_details must be of type {}".format(NITFWritingDetails)
            )
    ...
    def _handle_no_compression(
        ...
        offset = (
            0
            if self._in_memory
            else self.image_managers[image_segment_index].item_offset
        )
        (
            raw_dtype,
            formatted_dtype,
        ...
        self.image_managers[image_segment_index].item_size = (
            final_block_ending - additional_offset
        )
        if not self._in_memory:
    ...
    def create_data_segment_for_image_segment(
        ...
        if image_manager.end_of_item is not None:
            if image_segment_index < len(self.image_managers) - 1:
                self.image_managers[
                    image_segment_index + 1
                ].subheader_offset = image_manager.end_of_item
        elif not self._in_memory:
            raise ValueError(
                "file processing, and item_size unpopulated for image segment at index {}".format(
                    image_segment_index
                )
            )
    ...
    def create_data_segment_for_collection_element(
        ...
        block_definition = numpy.array(
            self._get_collection_element_coordinate_limits(collection_index),
            dtype="int64",
        )
    ...
    def flush(self, force: bool = False) -> None:
        ...
        try:
            ...
            if check:
                self.nitf_writing_details.write_header(
                    self._file_object, overwrite=True
                )
            self.nitf_writing_details.write_all_populated_items(self._file_object)
        except AttributeError:
            return
