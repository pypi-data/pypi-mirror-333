...
class FormatFunction(object):
    ...
    def set_transpose_axes(self, value: Optional[Tuple[int, ...]]) -> None:
        ...
        self._reverse_transpose_axes = tuple(
            [value.index(i) for i in range(len(value))]
        )

    def _get_populated_transpose_axes(self) -> Tuple[int, ...]:
        trans_axes = (
            tuple(range(len(self.raw_shape)))
            if self.transpose_axes is None
            else self.transpose_axes
        )
        return trans_axes
    ...
    def _reverse_and_transpose(
        self, array: numpy.ndarray, inverse=False
    ) -> numpy.ndarray:
        """
    ...
    def __call__(
        self, array: numpy.ndarray, subscript: Tuple[slice, ...], squeeze=True
    ) -> numpy.ndarray:
        """
    ...
    def inverse(
        self, array: numpy.ndarray, subscript: Tuple[slice, ...]
    ) -> numpy.ndarray:
        """
    ...
    def transform_raw_slice(self, subscript: Tuple[slice, ...]) -> Tuple[slice, ...]:
        """
...
class IdentityFunction(FormatFunction):
    ...
    def validate_shapes(self) -> None:
        ...
        expected_formatted_shape = tuple(
            [self.raw_shape[index] for index in trans_axes]
        )
    ...
    def transform_formatted_slice(
        ...
        rev_transpose_axes = (
            tuple(range(len(self.raw_shape)))
            if self.transpose_axes is None
            else self._reverse_transpose_axes
        )
    ...
    def transform_raw_slice(self, subscript: Tuple[slice, ...]) -> Tuple[slice, ...]:
        ...
        reverse_axes = () if self.reverse_axes is None else self.reverse_axes
        transpose_axes = (
            tuple(range(len(self.formatted_shape)))
            if self.transpose_axes is None
            else self.transpose_axes
        )
...
class ComplexFormatFunction(FormatFunction):
    ...
    def validate_shapes(self) -> None:
        ...
        after_mapping_shape[self.band_dimension] = int(
            after_mapping_shape[self.band_dimension] / 2
        )
    ...
    def _forward_functional_step(
        self, data: numpy.ndarray, subscript: Tuple[slice, ...]
    ) -> numpy.ndarray:
        ...
        if self.formatted_ndim < self.raw_ndim:
            out_shape = (
                data.shape[: self.band_dimension]
        ...
        if self.order == "IQ":
            out.real = numpy.reshape(
                data.take(indices=range(0, band_dim_size, 2), axis=self.band_dimension),
                out.shape,
            )
            out.imag = numpy.reshape(
                data.take(indices=range(1, band_dim_size, 2), axis=self.band_dimension),
                out.shape,
            )
        elif self.order == "QI":
            out.imag = numpy.reshape(
            ...
            if self.order == "MP":
                mag = numpy.reshape(
                    data.take(
                        indices=range(0, band_dim_size, 2), axis=self.band_dimension
                    ),
                    out.shape,
                )
                theta = numpy.reshape(
                    data.take(
                        indices=range(1, band_dim_size, 2), axis=self.band_dimension
                    ),
                    out.shape,
                )
            else:
    ...
    def _reverse_functional_step(
        ...
        if data.ndim != self.formatted_ndim:
            raise ValueError(
                "Expected formatted data of dimension {}".format(self.formatted_ndim)
            )
...
class SingleLUTFormatFunction(FormatFunction):
    ...
    def __init__(
        ...
        if not isinstance(lookup_table, numpy.ndarray):
            raise ValueError(
                "requires a numpy.ndarray, got {}".format(type(lookup_table))
            )
        if lookup_table.dtype.name != "uint8":
            raise ValueError(
                "requires a numpy.ndarray of uint8 dtype, got {}".format(
                    lookup_table.dtype
                )
            )
    ...
    def _forward_functional_step(
        ...
        if array.dtype.name not in ["uint8", "uint16"]:
            raise ValueError(
                "requires a numpy.ndarray of uint8 or uint16 dtype, got {}".format(
                    array.dtype.name
                )
            )
        ...
        if array.ndim != 2:
            raise ValueError(
                "Requires a two-dimensional numpy.ndarray, got shape {}".format(
                    array.shape
                )
            )
