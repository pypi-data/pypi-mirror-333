...
def _parse_int(val, length, default, name, instance):
    ...
    raise ValueError(
        ...
        "attribute {} of class {}".format(
            val, length, name, instance.__class__.__name__
        )
    )
...
class _BasicDescriptor(object):
    ...
    def _format_docstring(self):
        ...
        if docstring is None:
            docstring = ""
        if (self._typ_string is not None) and (
            not docstring.startswith(self._typ_string)
        ):
    ...
    def __get__(self, instance, owner):
        ...
        if fetched is not None or not self.required:
            ...
            msg = "Required field {} of class {} is not populated.".format(
    ...
    def __set__(self, instance, value):
        ...
        if value is None:
            ...
            if default_value is not None:
                ...
                raise ValueError(
                    "Attribute {} of class {} cannot be assigned None.".format(
                        self.name, instance.__class__.__name__
                    )
                )
...
class _StringDescriptor(_BasicDescriptor):
    ...
    def __set__(self, instance, value):
        if super(_StringDescriptor, self).__set__(
            instance, value
        ):  # the None handler...kinda hacky
            return
        self.data[instance] = _parse_str(
            value, self.length, self._default_value, self.name, instance
        )
...
class _StringEnumDescriptor(_BasicDescriptor):
    ...
    **The valid entries are case-sensitive and should be stripped of white space on each end.**
    ...
    def __init__(
        self, name, required, length, values, default_value=None, docstring=None
    ):
        self.values = values
        self._default_value = default_value
        ...
        if (self._default_value is not None) and (
            self._default_value not in self.values
        ):
    ...
    def __set__(self, instance, value):
        ...
        if val in self.values:
            ...
            msg = (
                "Attribute {} of class {} received {}, but values ARE REQUIRED to be "
...
class _IntegerDescriptor(_BasicDescriptor):
    ...
    def __set__(self, instance, value):
        if super(_IntegerDescriptor, self).__set__(
            instance, value
        ):  # the None handler...kinda hacky
            return
...
class _FloatDescriptor(_BasicDescriptor):
    ...
    def __set__(self, instance, value):
        if super(_FloatDescriptor, self).__set__(
            instance, value
        ):  # the None handler...kinda hacky
            return
...
class _RawDescriptor(_BasicDescriptor):
    ...
    def __set__(self, instance, value):
        if super(_RawDescriptor, self).__set__(
            instance, value
        ):  # the None handler...kinda hacky
            return
...
class _NITFElementDescriptor(_BasicDescriptor):
    ...
    def __init__(self, name, required, the_type, default_args=None, docstring=None):
        ...
        super(_NITFElementDescriptor, self).__init__(
            name, required, None, docstring=docstring
        )
    ...
    def __set__(self, instance, value):
        if super(_NITFElementDescriptor, self).__set__(
            instance, value
        ):  # the None handler...kinda hacky
            return

        self.data[instance] = _parse_nitf_element(
            value, self.the_type, self._default_args, self.name, instance
        )
...
class NITFElement(BaseNITFElement):
    ...
    def __init__(self, **kwargs):
        for fld in self._ordering:
            ...
            try:
                ...
                logger.critical(
                    "Failed setting attribute {} for class {}".format(
                        fld, self.__class__
                    )
                )
                raise
    ...
    def _get_attribute_length(self, fld):
        ...
        if fld in self._lengths:
            ...
            if val is None:
                ...
                raise TypeError(
                    "Unhandled type {} for attribute {} of class {}".format(
                        type(val), fld, self.__class__.__name__
                    )
                )
    ...
    def _get_attribute_bytes(self, fld):
        ...
        if isinstance(val, BaseNITFElement):
            ...
            raise ValueError(
                "Unhandled attribute {} for class {}".format(
                    fld, self.__class__.__name__
                )
            )
    ...
    def _parse_attribute(cls, fields, attribute, value, start):
        ...
        if attribute in cls._binary_format:
            ...
            fields[attribute] = struct.unpack(
                cls._binary_format[attribute], value[start:end]
            )[0]
            return end
            ...
            raise ValueError(
                "Cannot parse attribute {} for class {}".format(attribute, cls)
            )
...
class NITFLoop(NITFElement):
    ...
    def __init__(self, values=None, **kwargs):
        if not issubclass(self._child_class, NITFElement):
            raise TypeError(
                "_child_class for {} must be a subclass of NITFElement".format(
    ...
    def get_bytes_length(self):
        return self._count_size + sum(
            entry.get_bytes_length() for entry in self._values
        )
    ...
    def to_bytes(self):
        return self._counts_bytes() + b"".join(
            entry.to_bytes() for entry in self._values
        )
...
class Unstructured(NITFElement):
    ...
    def __init__(self, data=None, **kwargs):
        ...
        if not (isinstance(self._size_len, int) and self._size_len > 0):
            raise TypeError(
                "class variable _size_len for {} must be a positive integer".format(
                    self.__class__.__name__
                )
            )
    ...
    def data(self, value):
        ...
        if not isinstance(value, (bytes, NITFElement)):
            raise TypeError(
                "data requires bytes or NITFElement type. Got type {}".format(
                    type(value)
                )
            )
    ...
    def _parse_attribute(cls, fields, attribute, value, start):
        if attribute == "data":
            ...
            return start + length
        return super(Unstructured, cls)._parse_attribute(
...
class _ItemArrayHeaders(BaseNITFElement):
    ...
    def from_bytes(cls, value, start):
        ...
        if len(value) < start + 3:
            raise ValueError(
                "value must have length at least {}. Got {}".format(
                    start + 3, len(value)
                )
            )
        loc = start
...
class TRE(BaseNITFElement):
    ...
    @property
    def TAG(self):  # noqa N802
        """
        str: The RETRE/CETAG/TRETAG tag.
        """

        raise NotImplementedError

    @property
    def DATA(self):  # noqa N802
        """
        The TRE REDATA/CEDATA/TREDATA.
        """

        raise NotImplementedError

    @property
    def EL(self):  # noqa N802
        """
        int: The TRE element length. (REL/CEL/TREL)
        """
...
class UnknownTRE(TRE):
    ...
    def __init__(self, TAG, data):  # noqa N802
        """
        ...
        if isinstance(TAG, bytes):
            TAG = TAG.decode("utf-8")  # noqa N802
...
class UserHeaderType(Unstructured):
    ...
    def __init__(self, OFL=None, data=None, **kwargs):  # noqa N802
        self._ofl = None
        self._data = None
        self.OFL = OFL  # noqa N802
        super(UserHeaderType, self).__init__(data=data, **kwargs)

    @property
    def OFL(self):  # noqa N802
        return self._ofl

    @OFL.setter
    def OFL(self, value):  # noqa N802
        if value is None:
            self._ofl = 0
            return
    ...
    def _get_attribute_bytes(self, attribute):
        if attribute == "data":
            ...
            if isinstance(data, bytes):
                return (
                    siz_frm.format(len(data) + self._ofl_len).encode("utf-8")
                    + ofl_frm.format(self._ofl).encode("utf-8")
                    + data
                )
            else:
    ...
    def _parse_attribute(cls, fields, attribute, value, start):
        if attribute == "data":
            ...
            if length > 0:
                ...
                fields["data"] = None
            return start + length
        return super(UserHeaderType, cls)._parse_attribute(
            fields, attribute, value, start
        )
