...
class ImageBand(NITFElement):
    ...
    IFC = _StringEnumDescriptor(
        ...
        default_value="N",
        docstring=" Image Filter Condition.",
    )  # type: str
    ...
    def LUTD(self, value):  # noqa N802
        ...
        if value.dtype.name != "uint8":
            raise ValueError(
                "LUTD must be a numpy array of dtype uint8, got {}".format(
                    value.dtype.name
                )
            )
        if value.ndim != 2:
    ...
    def _get_attribute_bytes(self, attribute):
        if attribute == "LUTD":
            if self.NLUTS == 0:
                ...
                out = "{0:d}{1:05d}".format(
                    self.NLUTS, self.NELUTS
                ).encode() + struct.pack(
                    "{}B".format(self.NLUTS * self.NELUTS), *self.LUTD.flatten()
                )
            return out
        else:
    ...
    def _parse_attribute(cls, fields, attribute, value, start):
        if attribute == "LUTD":
            ...
            if nluts == 0:
                ...
                siz = nluts * neluts
                lutd = numpy.array(
                    struct.unpack("{}B".format(siz), value[loc : loc + siz]),
                    dtype=numpy.uint8,
                ).reshape((nluts, neluts))
                fields["LUTD"] = lutd
                loc += siz
            return loc
...
class ImageComment(NITFElement):
    _ordering = ("COMMENT",)
    _lengths = {"COMMENT": 80}
    COMMENT = _StringDescriptor(
        "COMMENT", True, 80, default_value="", docstring="The image comment"
    )
...
class MaskSubheader(NITFElement):
    _ordering = ("IMDATOFF", "BMRLNTH", "TMRLNTH", "TPXCDLNTH", "TPXCD", "BMR", "TMR")
    _lengths = {"IMDATOFF": 4, "BMRLNTH": 2, "TMRLNTH": 2, "TPXCDLNTH": 2}
    _binary_format = {
        "IMDATOFF": ">I",
        "BMRLNTH": ">H",
        "TMRLNTH": ">H",
    ...
    BMRLNTH = _IntegerDescriptor(
        "BMRLNTH", True, 5, docstring="Block mask record length"
    )  # type: int
    TMRLNTH = _IntegerDescriptor(
        "TMRLNTH", True, 5, docstring="Transparent Pixel Mask Record Length"
    )  # type: int
    TPXCDLNTH = _IntegerDescriptor(
        "TPXCDLNTH", True, 5, docstring="Transparent Output Pixel Code Length in bits."
    )  # type: int
    ...
    @TPXCD.setter
    def TPXCD(self, value):  # noqa N802
        if self.TPXCDLNTH == 0:
            self._TPXCD = None
            return
    ...
    @BMR.setter
    def BMR(self, value):  # noqa N802
        if value is None:
            if self.BMRLNTH != 0:
                raise ValueError(
                    "BMR array is None, but BMRLNTH={}".format(self.BMRLNTH)
                )
            self._BMR = None
        else:
            if self.BMRLNTH != 4:
                raise ValueError(
                    "BMR array is provided, but BMRLNTH={}".format(self.BMRLNTH)
                )
            if not isinstance(value, numpy.ndarray):
                value = numpy.array(value, dtype="uint32")
            if value.shape != (self.band_depth, self.blocks):
                raise ValueError(
                    "BMR array is of shape {}, and must be of shape {}".format(
                        value.shape, (self.band_depth, self.blocks)
                    )
                )
            if value.dtype.name != "uint32":
                raise ValueError(
                    "BMR array has dtype {}, and must be of dtype uint32".format(
                        value.dtype.name
                    )
                )
            self._BMR = value
    ...
    @TMR.setter
    def TMR(self, value):  # noqa N802
        if value is None:
            if self.TMRLNTH != 0:
                raise ValueError(
                    "TMR array is None, but TMRLNTH={}".format(self.TMRLNTH)
                )
            self._TMR = None
        else:
            if self.TMRLNTH != 4:
                raise ValueError(
                    "TMR array is provided, but TMRLNTH={}".format(self.TMRLNTH)
                )
            if not isinstance(value, numpy.ndarray):
                value = numpy.array(value, dtype="uint32")
            if value.shape != (self.band_depth, self.blocks):
                raise ValueError(
                    "TMR array is of shape {}, and must be of shape {}".format(
                        value.shape, (self.band_depth, self.blocks)
                    )
                )
            if value.dtype.name != "uint32":
                raise ValueError(
                    "TMR array has dtype {}, and must be of dtype uint32".format(
                        value.dtype.name
                    )
                )
            self._TMR = value
    ...
    def _parse_attribute(cls, fields, attribute, value, start):
        if attribute == "BMR":
            if fields["BMRLNTH"] == 0:
                ...
                array = numpy.array(
                    struct.unpack(">{}I".format(count), value[start:end]),
                    dtype="uint32",
                )
                fields["BMR"] = numpy.resize(
                    array, (fields["band_depth"], fields["blocks"])
                )
                return end
        elif attribute == "TMR":
            if fields["TMRLNTH"] == 0:
            ...
            return super(MaskSubheader, cls)._parse_attribute(
    ...
    def _get_attribute_bytes(self, fld):
        if fld in ["BMR", "TMR"]:
            ...
            if value is None:
                ...
                return struct.pack(
                    ">{}I".format(value.size), *numpy.reshape(value, (-1,))
                )
        elif fld == "TPXCD":
    ...
    def from_bytes(cls, value, start, band_depth=1, blocks=1):
        fields = {"band_depth": band_depth, "blocks": blocks}
        loc = start
...
class ImageSegmentHeader(NITFElement):
    ...
    IDATIM = _StringDescriptor(
        ...
        "acquisition in the format :code:`YYYYMMDDhhmmss`.",
    )  # type: str
    TGTID = _StringDescriptor(
        ...
        "character country code as specified in FIPS PUB 10-4.",
    )  # type: str
    ...
    Security = _NITFElementDescriptor(
        ...
        NITFSecurityTags,
        default_args={},
        docstring="The image security tags.",
    )  # type: NITFSecurityTags
    ENCRYP = _StringEnumDescriptor(
        "ENCRYP", True, 1, {"0"}, default_value="0", docstring="Encryption."
    )  # type: str
    ISORCE = _StringDescriptor(
        ...
        "the classification, including codeword(s).",
    )  # type: str
    ...
    PVTYPE = _StringEnumDescriptor(
        ...
        "used for the value for each pixel for each band in the image. ",
    )  # type: str
    ...
    ICORDS = _StringEnumDescriptor(
        ...
        "Image Geographic Location field (`IGEOLO`).",
    )  # type: str
    Comments = _NITFElementDescriptor(
        ...
        ImageComments,
        default_args={},
        docstring="The image comments.",
    )  # type: ImageComments
    Bands = _NITFElementDescriptor(
        "Bands", True, ImageBands, default_args={}, docstring="The image bands."
    )  # type: ImageBands
    ISYNC = _IntegerDescriptor(
        ...
        1,
        default_value=0,
        docstring="Image Sync code. This field is reserved for future use. ",
    )  # type: int
    ...
    NBPR = _IntegerDescriptor(
        ...
        "single block, this field shall contain the value one.",
    )  # type: int
    ...
    NPPBV = _IntegerDescriptor(
        ...
        r"the value in `NROWS`.",
    )  # type: int
    NBPP = _IntegerDescriptor(
        "NBPP", True, 2, default_value=0, docstring="Number of Bits Per Pixel Per Band."
    )  # type: int
    ...
    ILOC = _StringDescriptor(
        ...
        "column values indicate offsets up and to the left.",
    )  # type: str
    IMAG = _StringDescriptor(
        ...
        "the original image has been reduced by a factor of 2.",
    )  # type: str
    ...
    ExtendedHeader = _NITFElementDescriptor(
        ...
        UserHeaderType,
        default_args={},
        docstring="Extended subheader - TRE list.",
    )  # type: UserHeaderType
    ...
    @COMRAT.setter
    def COMRAT(self, value):  # noqa N802
    ...
    @IGEOLO.setter
    def IGEOLO(self, value):  # noqa N802
    ...
    def _parse_attribute(cls, fields, attribute, value, start):
        if attribute == "IC":
            ...
            return super(ImageSegmentHeader, cls)._parse_attribute(
                fields, attribute, value, start
            )
