...
def _find_slice_overlap(
    slice_in: slice, ref_slice: slice
    ...
    if slice_in.step > 0:
        ...
        child_start = (
            0
            if start_ind <= slice_in.start
            else int(numpy.ceil((start_ind - slice_in.start) / slice_in.step))
        )
        ...
        if slice_in.start < start_ind or (
            slice_in.stop is not None and slice_in.stop >= stop_ind
        ):
            # there is no overlap
        ...
        child_start = (
            0
            if slice_in.start < stop_ind
            else int(numpy.ceil((stop_ind - 1 - slice_in.start) / slice_in.step))
        )
        parent_start = slice_in.start - start_ind + child_start * slice_in.step
    ...
    if ref_slice.step < 0:
        # noinspection PyTypeChecker
        return _reverse_slice(
            slice(parent_start, parent_stop, slice_in.step)
        ), _reverse_slice(slice(child_start, child_stop, 1))
    else:
        # noinspection PyTypeChecker
        return slice(parent_start, parent_stop, slice_in.step), slice(
            child_start, child_stop, 1
        )
...
def _infer_subscript_for_write(
    ...
    if start_indices is not None:
        ...
        if len(start_indices) < len(full_shape):
            start_indices = start_indices + tuple(
                0 for _ in range(len(start_indices), len(full_shape))
            )
        subscript = tuple(
            [
                slice(entry1, entry1 + entry2, 1)
                for entry1, entry2 in zip(start_indices, data.shape)
            ]
        )
...
def extract_string_from_subscript(
    subscript: Union[None, int, slice, Tuple],
) -> Tuple[Union[None, int, slice, Sequence], Tuple[str, ...]]:
    """
...
class DataSegment(object):
    ...
    def _set_raw_shape(self, value: Tuple[int, ...]) -> None:
        if not isinstance(value, tuple):
            raise TypeError(
                "raw_shape must be specified by a tuple of ints, got type `{}`".format(
                    type(value)
                )
            )
        for entry in value:
            if not isinstance(entry, int):
                raise TypeError(
                    "raw_shape must be specified by a tuple of ints, got `{}`".format(
                        value
                    )
                )
            if entry <= 0:
                raise ValueError(
                    "raw_shape must be specified by a tuple of positive ints, got `{}`".format(
                        value
                    )
                )
    ...
    def _set_formatted_shape(self, value: Tuple[int, ...]) -> None:
        if not isinstance(value, tuple):
            raise TypeError(
                "formatted_shape must be specified by a tuple of ints, got type `{}`".format(
                    type(value)
                )
            )
        for entry in value:
            if not isinstance(entry, int):
                raise TypeError(
                    "formatted_shape must be specified by a tuple of ints, got `{}`".format(
                        value
                    )
                )
            if entry <= 0:
                raise ValueError(
                    "formatted_shape must be specified by a tuple of positive ints, got `{}`".format(
                        value
                    )
                )
    ...
    def _set_transpose_axes(self, value: Union[None, Tuple[int, ...]]) -> None:
        ...
        if set(value) != set(range(self.raw_ndim)):
            raise ValueError(
                "transpose_axes must be a permutation of range(raw_ndim), got\n\t{}".format(
                    value
                )
            )
    ...
    def _set_format_function(self, value: Optional[FormatFunction]) -> None:
        ...
        if not isinstance(value, FormatFunction):
            raise ValueError(
                "Got unexpected format_function value of type `{}`".format(type(value))
            )
    ...
    def verify_raw_subscript(
        self,
        subscript: Union[
            None, int, slice, Sequence[Union[int, slice, Tuple[int, ...]]]
        ],
    ) -> Tuple[slice, ...]:
        """
    ...
    def __getitem__(
        self,
        subscript: Union[None, int, slice, str, Sequence[Union[None, int, slice, str]]],
    ) -> numpy.ndarray:
        """
        ...
        if data.dtype.itemsize != self.raw_dtype.itemsize:
            raise ValueError(
                "Expected data dtype itemsize `{}`, got `{}`".format(
                    self.raw_dtype.itemsize, data.dtype.itemsize
                )
            )
        if data.dtype != self.raw_dtype:
            logger.warning(
                "Expected data dtype `{}`, got `{}`.".format(self.raw_dtype, data.dtype)
            )
    ...
    def write(
        ...
        **Only one of `start_indices` and `subscript` should be specified.**
        ...
        if data.dtype != self.formatted_dtype:
            logger.warning(
                "Expected data dtype `{}`, got `{}`.".format(
...
class SubsetSegment(DataSegment):
    ...
    __slots__ = (
        "_parent",
        "_formatted_subset_definition",
        "_raw_subset_definition",
        "_original_formatted_indices",
        "_original_raw_indices",
        "_squeeze",
        "_close_parent",
        "_pixels_written",
        "_expected_pixels_written",
    )
    ...
    def __init__(
        ...
        self._original_formatted_indices = (
            None  # the original indices matched to the new, with entry -1 when flat
        )
    ...
    def _validate_subset_definition(
        self, subset_definition: Tuple[slice, ...], coordinate_basis: str
        ...
        if coordinate_basis == "raw":
            ...
            raise ValueError(
                "Got unexpected coordinate basis `{}`".format(coordinate_basis)
            )
        ...
        for index, entry in enumerate(form_def):
            ...
            if self._squeeze and this_size == 1:
                logger.info(
                    "Entry at index {} of subset definition yields a single entry".format(
                        index
                    )
                )
        ...
        for index, entry in enumerate(raw_def):
            ...
            if self._squeeze and this_size == 1:
                logger.info(
                    "Raw slice at index {} of subset definition yields a single entry".format(
                        index
                    )
                )
    ...
    def _get_parent_subscript(
        ...
        out = []
        for full_size, out_index, slice_def in zip(
            full_shape, use_indices, subset_definition
        ):
            if out_index == -1:
                ...
                if len(the_array) < 1:
                    raise KeyError(
                        "Got invalid slice definition {} for shape {}".format(
                            norm_subscript, this_shape
                        )
                    )
                start = the_array[0]
    ...
    def get_parent_raw_subscript(
        self, subscript: Union[None, int, slice, Sequence[Union[int, slice]]]
    ) -> Tuple[slice, ...]:
        """
        ...
        return self._get_parent_subscript(
            self.verify_raw_subscript(subscript),
            self.raw_shape,
            self.parent.raw_shape,
            self._original_raw_indices,
            self._raw_subset_definition,
        )
    ...
    def get_parent_formatted_subscript(
        ...
        out = self._get_parent_subscript(
            self.verify_formatted_subscript(subscript),
            self.formatted_shape,
            self.parent.formatted_shape,
            self._original_formatted_indices,
            self._formatted_subset_definition,
        )
        return out
...
class BandAggregateSegment(DataSegment):
    ...
    def __init__(
        ...
        raw_dtype, raw_shape, form_shape, the_mode = self._set_children(
            children, transpose_axes
        )
    ...
    def _set_band_dimension(
        ...
        if transpose_axes is not None:
            if value != transpose_axes[value]:
                raise ValueError(
                    "band_dimension is not permitted to be changed by transpose_axes."
                )
    ...
    def _set_children(
        self, children: Sequence[DataSegment], transpose_axes: Optional[Tuple[int, ...]]
    ) -> Tuple[numpy.dtype, Tuple[int, ...], Tuple[int, ...], str]:
        if len(children) < 2:
            raise ValueError(
                "Cannot define a BandAggregateSegment based on fewer than 2 segments."
            )
        ...
        for child in children:
            ...
            if child.mode == "w" and not child.can_write_regular:
                raise ValueError(
                    "write mode requires that all children can write regular data"
                )
    ...
    def read_raw(
        ...
        for out_index, index in enumerate(
            ...
            child_subscript = (
                norm_subscript[: self.band_dimension]
                + norm_subscript[self.band_dimension + 1 :]
            )
            band_subscript = (
                full_band_subscript[: self.band_dimension]
                + (out_index,)
                + full_band_subscript[self.band_dimension + 1 :]
            )
            out[band_subscript] = self.children[index].read(
                child_subscript, squeeze=False
            )
    ...
    def check_fully_written(self, warn: bool = False) -> bool:
        ...
        for i, child in enumerate(self.children):
            ...
            if warn and not done:
                logger.error(
                    "Band {} of BandAggregateSegment indicates incomplete writing".format(
                        i
                    )
                )
            out &= done
...
class BlockAggregateSegment(DataSegment):
    ...
    __slots__ = (
        "_children",
        "_formatted_child_arrangement",
        "_raw_child_arrangement",
        "_missing_data_value",
        "_close_children",
    )
    ...
    def _set_children(
        ...
        if len(children) != len(child_arrangement):
            raise ValueError(
                "We must have the same number of children as child_arrangement entries"
            )

        coordinate_basis = coordinate_basis.strip().lower()
        if coordinate_basis == "raw":
            raw_arrangement = [
                self.verify_raw_subscript(entry) for entry in child_arrangement
            ]
            formatted_arrangement = [
                self.format_function.transform_raw_slice(entry)
                for entry in raw_arrangement
            ]
        elif coordinate_basis == "formatted":
            formatted_arrangement = [
                self.verify_formatted_subscript(entry) for entry in child_arrangement
            ]
            raw_arrangement = [
                self.format_function.transform_formatted_slice(entry)
                for entry in formatted_arrangement
            ]
        else:
            raise ValueError(
                "Got unexpected coordinate basis `{}`".format(coordinate_basis)
        ...
        for i, (child, raw_def, form_def) in enumerate(
            zip(children, raw_arrangement, formatted_arrangement)
        ):
            ...
            for entry in raw_def:
                if entry.step not in [1, -1]:
                    raise ValueError(
                        "Each entry of child_arrangement must have step +/-1."
                    )
            ...
            if result_shape != child.formatted_shape:
                raise ValueError(
                    ...
                    "but it has formatted_shape {}".format(
                        i, result_shape, child.formatted_shape
                    )
                )
    ...
    def read_raw(
        ...
        subscript, formatted_shape = get_subscript_result_size(
            subscript, self.raw_shape
        )
        out = numpy.full(
            formatted_shape, fill_value=self._missing_data_value, dtype=self.raw_dtype
        )
        ...
        for entry, child in zip(self._raw_child_arrangement, self._children):
            ...
            if use_block:
                out[tuple(parent_subscript)] = child.read_raw(
                    tuple(child_subscript), squeeze=False
                )
    ...
    def check_fully_written(self, warn: bool = False) -> bool:
        ...
        for i, child in enumerate(self.children):
            ...
            if warn and not done:
                logger.error(
                    "Block {} of BlockAggregateSegment indicates incomplete writing".format(
                        i
                    )
                )
            out &= done
    ...
    def write_raw(
        ...
        **Only one of `start_indices` and `subscript` should be specified.**
        ...
        for entry, child in zip(self._raw_child_arrangement, self._children):
            ...
            data_subscript = []
            child_subscript = []
            for lim, data_slice, block_slice in zip(
                self.raw_shape, norm_subscript, entry
            ):
                child_entry, par_entry = _find_slice_overlap(data_slice, block_slice)
            ...
            if use_block:
                child.write(
                    data[tuple(data_subscript)],
                    subscript=tuple(child_subscript),
                    **kwargs,
                )
...
class NumpyArraySegment(DataSegment):
    ...
    def __init__(
        ...
        if formatted_shape is None:
            if format_function is None:
                if transpose_axes is None:
                    ...
                    formatted_shape = [
                        underlying_array.shape[index] for index in transpose_axes
                    ]
            else:
                raise ValueError(
        ...
        DataSegment.__init__(
            self,
            underlying_array.dtype,
            underlying_array.shape,
            formatted_dtype,
            formatted_shape,
            reverse_axes=reverse_axes,
            transpose_axes=transpose_axes,
...
class NumpyMemmapSegment(NumpyArraySegment):
    ...
    __slots__ = ("_file_object", "_memory_map", "_close_file")
...
class FileReadDataSegment(DataSegment):
    ...
    __slots__ = ("_file_object", "_data_offset", "_close_file")
    ...
    def read_raw(
        ...
        pixel_per_row = (
            1 if self.formatted_ndim == 1 else int(numpy.prod(self.raw_shape[1:]))
        )
        ...
        if len(data) != total_size:
            raise ValueError(
                ...
                "which attempts to read more data than the file contains".format(
                    total_size, len(data)
                )
            )
