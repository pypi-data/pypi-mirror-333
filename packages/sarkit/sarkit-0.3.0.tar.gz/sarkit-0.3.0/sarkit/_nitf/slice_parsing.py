...
def verify_slice(
    item: Union[None, int, slice, Tuple[int, ...]], max_element: int
) -> slice:
    """
    ...
    **New in version 1.3.0.**
    ...
    def check_bound(entry: Union[None, int]) -> Union[None, int]:
        if entry is None:
            ...
            raise ValueError(
                "Got out of bounds argument ({}) in slice limited by `{}`".format(
                    entry, max_element
                )
            )
    ...
    if item is None:
        ...
        raise ValueError(
            "Got unexpected argument of type {} in slice".format(type(item))
        )
...
def verify_subscript(
    ...
    **New in version 1.3.0**
    ...
    if subscript is None or subscript is Ellipsis:
        ...
        out = [
            verify_slice(slice(subscript, subscript + 1, 1), corresponding_shape[0]),
        ]
        out.extend([slice(0, corresponding_shape[i], 1) for i in range(1, ndim)])
        return tuple(out)
    elif isinstance(subscript, slice):
        out = [
            verify_slice(subscript, corresponding_shape[0]),
        ]
        out.extend([slice(0, corresponding_shape[i], 1) for i in range(1, ndim)])
        return tuple(out)
        ...
        for index, entry in enumerate(subscript):
            if entry is Ellipsis:
                if ellipsis_location is None:
                    ...
                    raise KeyError(
                        "slice definition cannot contain more than one ellipsis"
                    )

        if ellipsis_location is not None:
            if len(subscript) > ndim:
                raise ValueError(
                    "More subscript entries ({}) than shape dimensions ({}).".format(
                        len(subscript), ndim
                    )
                )
        ...
        out = [
            verify_slice(item_i, corresponding_shape[i])
            for i, item_i in enumerate(subscript)
        ]
        if len(out) < ndim:
            out.extend(
                [slice(0, corresponding_shape[i], 1) for i in range(len(out), ndim)]
            )
        return tuple(out)
    else:
...
def get_slice_result_size(slice_in: slice) -> int:
    ...
    **New in version 1.3.0.**
    ...
    if slice_in.step > 0:
        return int(
            numpy.floor((slice_in.stop - 1 - slice_in.start) / slice_in.step) + 1
        )
