from .lpStateVar import LPStateVar,LPStateVar_add,LPStateVar_Decision_add
class Equation:
    ...
    def __init__(self,var_lst:list,sense:str,b:float,description:str):
        ...
        for var in var_lst:
            if len(var)==3 and (isinstance(var[0],LPStateVar_add) or isinstance(var[0],LPStateVar_Decision_add)):
                raise ValueError(f'The variable {var[0].name} is not time dependent and should not be called with a time stemp')
