...
class import_agent(graph_walker, Walker):
    ...
    def import_from_daf(self, agents_node: Agents, daf_name: str, version: Optional[str]='') -> Agent:
        agent_node = None
        ...
        if not safe_to_import:
            if (package_data := RegistryAPI.download_package(daf_name, version)):
                try:
        ...
        if safe_to_import and (package_path := Utils.find_package_folder(package_root, daf_name)):
            ...
            if os.path.exists(descriptor_yaml_path):
                with open(descriptor_yaml_path, 'r') as file:
                    try:
                        descriptor_yaml = file.read()
                        agent_node = self.import_from_descriptor(agents_node, descriptor_yaml)
                    except Exception as e:
                        self.logger.error(f'an exception occurred, {traceback.format_exc()}')
                        return None
    ...
    def on_agents(self, here: Agents) -> None:
        if self.descriptor:
            ...
            if (agent_node := self.import_from_daf(here, self.daf_name, self.daf_version)):
                if self.reporting:
                    Jac.report(agent_node.export())