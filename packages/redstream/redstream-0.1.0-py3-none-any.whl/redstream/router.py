...
logger = logging.getLogger("redis_stream_router")
...
class RedisStreamRouter:
        ...
        while not self.shutdown_event.is_set():
            try:
                ...
                if stream in self.handlers:
                    processed_data = await self.handlers[stream](message_data, message_id, stream)
    ...
    async def send_request_with_timeout(self, target_stream, response_stream, message, timeout=5, max_retries=3):
        """
        ...
        async with self.request_semaphore:
            ...
            try:
                # Отдельное подключение для ответов с таймаутами
                ...
                response_group = f"{response_stream}_response_group"
                consumer_name = f"consumer_{correlation_id}"

                # Создание группы потребителей для ответов
                try:
                    await response_redis_conn.xgroup_create(
                        response_stream,
                        response_group,
                        id="0",
                        mkstream=True
                ...
                for attempt in range(max_retries):
                    try:
                        await self.redis_conn.xadd(target_stream, message)
                        logger.info(f"📤 Запрос {correlation_id} отправлен (попытка {attempt+1})")
                        break
                    except Exception as e:
                            ...
                            for stream, msg_list in messages:
                                for msg_id, data in msg_list:
                                    if data.get("correlation_id") == correlation_id:
                                        ...
                                        logger.debug("Получен чужой ответ")
                ...
                logger.warning(f"⚠️ Таймаут для {correlation_id}")
                return None

            finally:
                if response_redis_conn:
                    try:
                        # Удаление временного потребителя
                        await response_redis_conn.xgroup_delconsumer(
                            response_stream,
                            response_group,
                            consumer_name
                        )
