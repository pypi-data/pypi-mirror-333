...
def generate_contour(imagein, arraydata, voxelsize, folderout, type, sigma=1, connectivity=3,target_normalization_max=255,replace_in_name="",replace_to="",flat_normalization=False):
    ...
    if flat_normalization:
        smoothed[smoothed>0] = target_normalization_max
    del im_cyto_erod
    image_out = imagein
...
def compute_membranes_from_junctions(embryo_folder, junctionimput, reducvoxelsize=0.3, target_normalization_max=255,
    ...
    for image in res: # Process the contour creation for each image
        ...
        generate_contour(image, image_filled, voxelsize, folder_contour, "identity", sigma=2, connectivity=1,
                         target_normalization_max=target_normalization_max,replace_in_name="_junctions",replace_to="_fuse",flat_normalization=True)  # Generating the contour
