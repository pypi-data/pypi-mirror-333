...
from .utils import wrap_model, EnsembleModel, clamp, img_min, img_max  # Ensure these are defined in utils.py
...
class Attack(object):
    ...
    """
    def __init__(self, attack, model_name, epsilon, targeted, random_start, norm, loss, decay=0.9, device=None):
        """
        ...
        Arguments:
            ...
            decay (float): momentum decay factor (default=0.9).
            device (torch.device): the device for data. If None, the device is the same as model.
        """
    ...
    def get_loss(self, logits, label):
        """
        The loss calculation, which should be overridden when the attack changes the loss calculation (e.g., ATA, etc.)
        """
        # Calculate the loss
