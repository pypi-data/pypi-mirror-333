...
def determine_stroke_type(index: int, lane_info: LaneInformation) -> str:
    ...
    if stroke_type.lower() == STROKE_TYPES.PARA_MEDLEY.value.lower():
        if pool_type == "SCM":
            refined_index = index // 2
        else:
            refined_index = index
        try:
            return STROKE_TYPES_FOR_150IM[refined_index]
        except IndexError as e:
            print("pool type: " + str(pool_type))
            print("index: " + str(index) + "\trefined index: " + str(refined_index))
            raise Exception("There was an index out of bound error on STROKE_TYPES_FOR_150IM. The " + str(pool_type) + " cannot except index " + str(index) + " and " + str(refined_index))
