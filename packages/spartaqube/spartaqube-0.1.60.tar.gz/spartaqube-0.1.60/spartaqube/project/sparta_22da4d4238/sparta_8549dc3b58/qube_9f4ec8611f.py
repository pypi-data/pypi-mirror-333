...
from project.sparta_8f29384cd2.sparta_031b689a30.qube_8bdcb6e304 import sparta_ce8ba6bdcc
from project.sparta_8f29384cd2.sparta_031b689a30.qube_f08258e053 import sparta_a75fcac5d0
...
from project.sparta_8f29384cd2.sparta_47f0106b48.qube_2d0baaf631 import sparta_2bf050f749,sparta_fe33ff5f82,get_api_key_async
class NotebookWS(AsyncWebsocketConsumer):
	...
	async def notebook_permission_code_exec(A,json_data):from project.sparta_8f29384cd2.sparta_027dc65844 import qube_fdbbe48852 as B;return await coreNotebook.notebook_permission_code_exec(json_data)
		...
		if len(t)>0:
			...
			if E=='init-socket'or E=='reconnect-kernel'or E=='reconnect-kernel-run-all':
				...
				try:
					...
					AP=json.loads(A['execCodeInput']);x=N.join([A[Y]for A in AP]);U=W(x,A);U+=N+L;await B.sender_kernel_obj.send_zmq_request(sender_dict={D:P,J:U},b_send_websocket_msg=_B);V=sparta_ce8ba6bdcc(x);y=A['guiInputVarName'];AQ=A['guiOutputVarName'];AR=A[AH];n=[y,AQ,AR];X=[]
					for S in n:
				...
				try:
					z=M;d=M
					if m in A:e=A[m];A0=sparta_a75fcac5d0(e);z=A0['assign_state_variable'];d=A0['assign_code']
					AS=json.loads(A['execCodeOutput']);A1=N.join([A[Y]for A in AS]);U=d+N;U+=z+N;U+=A1;await B.sender_kernel_obj.send_zmq_request(sender_dict={D:P,J:U},b_send_websocket_msg=_B);V=sparta_ce8ba6bdcc(A1)
					try:V.append(A[m][l])
				...
				try:
					...
					f+=N+g;logger.debug('cmd to execute');logger.debug('cmd_to_exec');logger.debug(f);await B.sender_kernel_obj.send_zmq_request({D:P,J:f});V=sparta_ce8ba6bdcc(g)
				except Exception as H:C=json.dumps({F:-1,D:E,T:str(H)});await B.send(text_data=C);return
