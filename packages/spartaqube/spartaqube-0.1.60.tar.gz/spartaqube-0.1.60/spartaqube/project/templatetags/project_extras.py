...
def get_item(dictionary,key):
	if settings.IS_VITE:return f"http://localhost:3000/src/{key}"
	A=dictionary.get(key,'');B=A.lstrip('/');C=urljoin(settings.STATIC_URL,B);D=str(PurePosixPath(C));return D