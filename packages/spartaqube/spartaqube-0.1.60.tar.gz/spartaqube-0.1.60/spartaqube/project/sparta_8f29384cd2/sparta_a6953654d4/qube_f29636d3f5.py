...
def sparta_c48e113b65():return str(uuid.uuid4())
def sparta_bcc520fef9(app_id):
	B=coreApps.get_app_folder_default_path();A=os.path.join(B,app_id)
	if not os.path.exists(A):os.makedirs(A)
	return A
def sparta_f6e9b986e9(project_path):
	A=project_path
	try:
		if not os.path.exists(A):os.makedirs(A)
		return{_A:1}
	except Exception as B:return{_A:-1,_C:str(B)}
def sparta_a8c61e81ec(path):
	with open(path,'a'):os.utime(path,None)
def sparta_2d4c9acb99(json_data,userObj):
	A=json_data;logger.debug('CREATE RESOURCE');logger.debug(A);C=A[_B];D=sparta_f6e9b986e9(C)
	if D[_A]==-1:return D
	...
	try:
		...
		elif not os.path.exists(B):sparta_a8c61e81ec(B)
	...
	return{_A:1}
def sparta_6539e946d6(json_data,userObj):
	...
	return{_A:1}
def sparta_524586ce69(json_data,user_obj,file_obj_to_upload):
	D=file_obj_to_upload;B=json_data;A=B[_B];E=sparta_f6e9b986e9(A)
	if E[_A]==-1:return E
	...
	I={_A:1};return I
def sparta_c59e69fc1b(json_data,userObj):A=json_data[_B];B=qube_662a9cb8a2.get_folder_and_files_recursively_from_path(A);C={_A:1,'folderStructure':B};return C
def sparta_1048403f40(json_data,userObj):
	A=json_data
	try:
...
def sparta_e7d8de31d5(json_data,userObj):
	...
	return{_A:1}
def sparta_866f20a121(json_data,userObj):
	...
	return{_A:1}
def sparta_373767c36f(json_data,userObj):
...
def sparta_5c923ad42a(json_data,userObj):
	...
	return{_A:-1}
def sparta_f08af50e35(json_data,userObj):
