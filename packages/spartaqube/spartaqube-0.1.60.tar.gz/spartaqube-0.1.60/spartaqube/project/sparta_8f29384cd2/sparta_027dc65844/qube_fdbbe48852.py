...
def sparta_6faeb64289(user_obj):
	...
	return B
def sparta_8d24af527d(project_path,has_django_models=_A):
	C=has_django_models;B=project_path
	if not os.path.exists(B):os.makedirs(B)
	G=B;D=os.path.join(sparta_f7d2e15968()[_a],_K,_b)
	for E in os.listdir(D):
		...
		if os.path.isdir(A):
			...
			shutil.copy2(A,F)
	return{_L:B}
def sparta_696a214e29(json_data,user_obj):
	F=json_data;B=user_obj;A=F[_H];A=sparta_85e1a912d2(A);G=Notebook.objects.filter(project_path=A).all()
	if G.count()>0:
		C=G[0];H=sparta_6faeb64289(B)
	...
	try:
		...
		L=F['hasDjangoModels'];M=sparta_8d24af527d(A,L);A=M[_L];return{_B:1,_L:A}
	except Exception as E:return{_B:-1,_E:f"Failed to create folder: {str(E)}"}
def sparta_b3e88c91e7(json_data,user_obj):A=json_data;A['bAddGitignore']=_A;A['bAddReadme']=_A;return qube_85e33f7abf.sparta_43e24674ec(A,user_obj)
def sparta_49db98160c(json_data,user_obj):
	L='%Y-%m-%d';K='Recently used';E=user_obj;G=sparta_6faeb64289(E)
...
def sparta_9540d8f3f3(json_data,user_obj):
	...
	if not B.is_anonymous:
		...
		if G.count()==1:
			A=G[G.count()-1];E=A.notebook_id;I=sparta_6faeb64289(B)
		...
		J=H.get('modalPassword',_D);logger.debug(f"DEBUG DEVELOPER VIEW TEST >>> {J}");C=sparta_04024a5454(E,B,password_notebook=J);logger.debug('MODAL DEBUG DEBUG DEBUG notebook_access_dict');logger.debug(C)
...
def sparta_83ddd4ef0f(json_data,user_obj):
	I=user_obj;A=json_data;logger.debug('Save notebook');logger.debug(A);logger.debug(A.keys());N=A['isNew']
	if not N:return sparta_253fc8e84e(A,I)
	C=datetime.now().astimezone(UTC);J=str(uuid.uuid4());G=A[_Y];E=_D
	if G:E=A[_j];E=qube_684e4bab71.sparta_eb3a26c915(E)
	O=A[_Z];P=A[_I];Q=A[_Q];D=A[_H];D=sparta_85e1a912d2(D);R=A[_k];S=A[_l];G=A[_Y];T=A[_m];U=A.get('notebookVenv',_D);B=A[_P]
	if len(B)==0:B=A[_I]
	K=slugify(B);B=K;L=1
	...
	if F is not _D:
		try:
			F=F.split(',')[1];V=base64.b64decode(F);D=sparta_f7d2e15968()[_o];M=os.path.join(D,_p,_q,_K);os.makedirs(M,exist_ok=_A);H=str(uuid.uuid4());W=os.path.join(M,f"{H}.png")
			with open(W,'wb')as X:X.write(V)
		except:pass
	Y=Notebook.objects.create(notebook_id=J,name=P,slug=B,description=Q,is_expose_notebook=R,is_public_notebook=S,has_password=G,password_e=E,is_exec_code_display=T,lumino_layout=O,project_path=D,notebook_venv=U,thumbnail_path=H,date_created=C,last_update=C,last_date_used=C,spartaqube_version=sparta_309795cbf8());Z=ShareRights.objects.create(is_admin=_A,has_write_rights=_A,has_reshare_rights=_A,last_update=C);NotebookShared.objects.create(notebook=Y,user=I,share_rights=Z,is_owner=_A,date_created=C);return{_B:1,_M:J}
def sparta_253fc8e84e(json_data,user_obj):
	...
	if I.count()==1:
		A=I[I.count()-1];H=A.notebook_id;M=sparta_6faeb64289(G)
		...
		if N:
			...
			if E is not _D:
				...
				try:
					b=sparta_f7d2e15968()[_o];S=os.path.join(b,_p,_q,_K);os.makedirs(S,exist_ok=_A)
...
def sparta_5cd7eb52f1(json_data,user_obj):
	...
	return{_B:1}
def sparta_f078f54cf0(json_data,user_obj):
	...
	return{_B:1}
def sparta_04024a5454(notebook_id,user_obj,password_notebook=_D):
			...
			if A.is_public_notebook:
				...
				if not L:logger.debug('no password');return{_B:1,_J:A}
					...
					if F is _D:logger.debug('empty passord provided');return{_B:2,_E:'Require password',_J:A}
						...
						try:
							if qube_684e4bab71.sparta_3e7385f078(A.password_e)==F:return{_B:1,_J:A}
	...
	return{_B:-1,J:2}
def sparta_f5eaea0838(json_data,user_obj):A=sparta_85e1a912d2(json_data[_H]);return sparta_6dd866c6b7(A)
def sparta_d405da39d4(json_data,user_obj):A=sparta_85e1a912d2(json_data[_H]);return sparta_9386dcf5bb(A)
def sparta_660efa4370(json_data,user_obj):
	...
	if D.count()==1:
		A=D[D.count()-1];F=A.notebook_id;G=sparta_6faeb64289(C)
		...
		if E.count()>0:
			...
			if I.is_admin or I.has_write_rights:H=_A
		if H:K=sparta_85e1a912d2(B['fullPath']);A.main_ipynb_fullpath=K;A.save()
	return{_B:1}
async def notebook_permission_code_exec_DEPREC(json_data):
	...
	try:
		...
		if await D.acount()>0:
			...
			H=qube_4dbbaf8fe9.sparta_38595e9788(A);I=H['cells']
			for B in I:
...
async def notebook_permission_code_exec(json_data):
	...
	return''
def sparta_6f800c7141(json_data,user_obj):0
from django.core.management import call_command
from io import StringIO
def sparta_aef1762a5b(project_path,python_executable=_u):
...
def sparta_6e481d957a():
	...
	return B
def sparta_55d7d28259(json_data,user_obj):
	D=sparta_85e1a912d2(json_data[_H]);A=D;B=os.path.join(sparta_f7d2e15968()[_a],_K,_b);C=os.path.join(A,_O)
...
def sparta_51abb481f5(json_data,user_obj):
	A=sparta_85e1a912d2(json_data[_H]);A=os.path.join(A,_O);G,C,D=sparta_aef1762a5b(A);B=_A;E=1;F=''
	if D:
...
def sparta_09acc05efe(project_path,python_executable=_u):
...
def sparta_4a76b575dd(json_data,user_obj):
	A=sparta_85e1a912d2(json_data[_H]);A=os.path.join(A,_O);B,C=sparta_09acc05efe(A);D=1;E=''
	if not B:D=-1;E=C
	return{_B:D,'res_migration':B,_y:C,_E:E}
def sparta_21ff6d55c5(json_data,user_obj):return{_B:1}
def sparta_903a4c2b8b(json_data,user_obj):return{_B:1}