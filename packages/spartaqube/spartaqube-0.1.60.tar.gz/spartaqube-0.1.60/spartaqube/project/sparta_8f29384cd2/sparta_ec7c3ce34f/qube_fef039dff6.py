...
from project.sparta_8f29384cd2.sparta_496014b703 import qube_77faf0d520 as qube_77faf0d520
def sparta_779ef544eb(json_data,userObj):
...
def sparta_ea9ef8cd75(json_data,user_obj):
	...
	if M==0:
		...
		if N.count()==1:
			F=N[0];G=qube_5af9e48ff5.sparta_9c3e29af31(A);C=[A.userGroup for A in G]
		...
		if O.count()==1:
			K=O[0];G=qube_5af9e48ff5.sparta_9c3e29af31(A);C=[A.userGroup for A in G]
	...
	return{_B:-1}
def sparta_b19f27275d(json_data,user_obj):
	F=json_data;A=user_obj;G=[];K=[];L=int(F[_F])
	if L==0:
		S=F[_G];M=Portfolio.objects.filter(ptf_id=S,is_delete=_A).all()
		if M.count()==1:
			D=M[0];H=qube_5af9e48ff5.sparta_9c3e29af31(A);C=[A.userGroup for A in H]
		...
		if N.count()==1:
			E=N[0];H=qube_5af9e48ff5.sparta_9c3e29af31(A);C=[A.userGroup for A in H]
...
def sparta_6ef16b1428(json_data,user_obj):
	Y='Your are not allowed to share this object';X='errorMsg';R='member';K=json_data;B=user_obj;L=datetime.now().astimezone(UTC);P=qube_5af9e48ff5.sparta_9c3e29af31(B);M=[A.userGroup for A in P];U=K['member2ShareArr'];I=int(K[_F])
	...
	if I==0:
		...
		if V.count()==1:
			C=V[0];P=qube_5af9e48ff5.sparta_9c3e29af31(B);M=[A.userGroup for A in P]
		...
		if W.count()==1:
			D=W[0];P=qube_5af9e48ff5.sparta_9c3e29af31(B);M=[A.userGroup for A in P]
	...
	return{_B:1}
def sparta_6f99196225(json_data,user_obj):
