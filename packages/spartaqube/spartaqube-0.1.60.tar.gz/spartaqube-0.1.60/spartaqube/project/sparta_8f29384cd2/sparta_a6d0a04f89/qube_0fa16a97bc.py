...
from project.sparta_8f29384cd2.sparta_ccd7e0da68.qube_67447782f4 import convert_to_dataframe,convert_dataframe_to_json,sparta_85e1a912d2
...
def sparta_9520fee16b(user_obj):from project.sparta_8f29384cd2.sparta_47f0106b48.qube_2d0baaf631 import sparta_fe33ff5f82 as A;return A(user_obj)
def sparta_6faeb64289(user_obj):
	A=qube_5af9e48ff5.sparta_9c3e29af31(user_obj)
	if len(A)>0:B=[A.user_group for A in A]
	else:B=[]
	return B
def sparta_6216f2a23f(json_data,user_obj):
	D=user_obj;E=sparta_6faeb64289(D)
...
def sparta_d30ef30557():return{_A:1,'available_engines':qube_663812ae0c.sparta_d30ef30557()}
def sparta_cbf30f93a9(json_data,user_obj):
	...
	return{_A:1}
def sparta_85c02a44d4(json_data):
...
def sparta_062a6547b1(json_data,user_obj):
	...
	if D.count()>0:
		A=D[D.count()-1];F=sparta_6faeb64289(C)
...
def sparta_8b22180497(json_data,user_obj):
	...
	return{_A:1}
def sparta_d06efb8c5b(package_name):
...
def sparta_da08de56e8(json_data,user_obj):
	B=_D;C=[];D=json_data['pip_cmds']
	for E in D:
		A=sparta_d06efb8c5b(E)
...
def sparta_868bff07fa(obj):
	A=obj
	if isinstance(A,dict):
		B=DotDict()
		for(C,D)in A.items():B[C]=sparta_868bff07fa(D)
		return B
	elif isinstance(A,list):return[sparta_868bff07fa(A)for A in A]
	else:return A
def sparta_72869398c0(json_data,user_obj):
	B=json_data;D=B.get(_AK,_D)
	if D:A=sparta_868bff07fa(B)
	else:
		E=B[_H];A=sparta_8b47fe1f7a(E,user_obj)
...
def sparta_747b507226(json_data,user_obj):
	A=json_data;G=A[_H];E=A[_O];H=int(A.get(_p,'0'))==1;B=[];C=sparta_8b47fe1f7a(G,user_obj)
...
def sparta_c9d0fe0db3(json_data,db_connector_obj):
	B=db_connector_obj
...
def sparta_a685aca195(json_data,user_obj):
	A=json_data;print(_AL);print(A);H=A.get(_AK,_D);I=A[_H];C=A.get(_O,_B);F=int(A.get(_p,'0'))==1;G=int(A.get('previewTopMod','0'))==0
	if H:D=sparta_868bff07fa(A)
	else:
		D=sparta_8b47fe1f7a(I,user_obj)
		if D is _B:return{_A:-1,_E:_g}
	sparta_c9d0fe0db3(A,D);B=Connector(db_engine=D.db_engine);B.init_with_model(D)
	if F is not _B:
		if F:
			J=A[_A0]
...
def sparta_4326f26ba5(json_data,user_obj):
	A=json_data;F=A[_H];D=A.get(_O,'');G=int(A.get(_p,'0'))==1;K=A.get(_N,_B);B=sparta_8b47fe1f7a(F,user_obj)
	if B is _B:return{_A:-1,_E:_g}
	sparta_c9d0fe0db3(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B)
	if G:
		H=A[_A0]
...
def sparta_0a2c4bf773(json_data,user_obj):
	C=json_data
	...
	return{_A:1,'table':B}
def sparta_f4411eb724(json_data,user_obj):
	A=json_data;D=A[_H];F=A.get(_O,_B);G=int(A.get(_p,'0'))==1;B=sparta_8b47fe1f7a(D,user_obj)
	if B is _B:return{_A:-1,_E:_g}
	sparta_c9d0fe0db3(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B);E=C.get_db_connector().get_wss_structure();return{_A:1,_F:convert_dataframe_to_json(E)}
def sparta_6eeebb0fe0(json_data,user_obj):
	A=json_data;M=A[_M];D=_B
	if M:D=A[_s];D=qube_684e4bab71.sparta_eb3a26c915(D)
...
def sparta_eb24557041(json_data,user_obj):
	...
	if H.count()>0:
		...
		if I.count()>0:
			...
			if L.is_admin or L.has_write_rights:
				P=B[_M];C=_B
				if P:C=B[_s];C=qube_684e4bab71.sparta_eb3a26c915(C)
				D=_B;E=B.get(_AM,_B)
				if E is not _B:
	...
	return{_A:1}
def sparta_e208b83140(json_data,user_obj):0
def sparta_eacd683e50(json_data,user_obj):
	D=user_obj;F=sparta_6faeb64289(D)
...
def sparta_6ba60cdb53(json_data,user_obj):
	...
	if I.count()==1:
		...
		if E:
			...
			if Q.count()>0:
				J=Q[0];e=J.user;f=sparta_9520fee16b(e);R=J.user;K=[];A=J.plot_db_chart;g=A.is_static_data
				if g:0
				else:
					for B in A.data_source_list:
						...
						if L:K.append(B[b])
							...
							if _I in B:
								h=B[_H];S=sparta_8b47fe1f7a(h,R);T=[]
...
def sparta_45d582ee71(json_data,user_obj):
	...
	return{_A:1}
def sparta_e60737c707(user_obj,widget_id):
...
def sparta_3ac2716e52(json_data,user_obj):
	...
	try:
		...
		if L.count()>0:
			A=L[L.count()-1];T=sparta_6faeb64289(K)
...
def sparta_3712beee90(json_data,user_obj):
	...
	return{_A:1}
def sparta_09a235a7b1(token_permission):
	...
	if A.is_expose_widget:
		if A.is_public_widget:
			if not F:return _C
				...
				try:
					if qube_684e4bab71.sparta_3e7385f078(A.widget_password_e)==password_widget:return _C
					else:return _D
				except:return _D
		else:return _D
	return _D
def sparta_02e99f72ee(plot_chart_id,user_obj,password_widget=_B):
		...
		if A.is_expose_widget:
			if A.is_public_widget:
				if not J:return{_A:1,_K:A}
					...
					try:
						if qube_684e4bab71.sparta_3e7385f078(A.widget_password_e)==F:return{_A:1,_K:A}
	...
	return{_A:-1}
def sparta_8658beaa7a(plot_chart_id,user_obj):
	...
	if A.count()==1:D=_C
		...
		if A.count()==1:D=_C
	if D:
		B=A[A.count()-1];G=sparta_6faeb64289(C)
	...
	return{_A:1,_i:_D}
def sparta_e9ab51e5af(plot_db_chart_obj):
	...
	return C
def sparta_656c0e9a6c(json_data,user_obj):
	B=user_obj;D=sparta_6faeb64289(B)
	...
	if F.count()>0:
		...
		if G.count()>0:
			...
			if I.is_admin or I.has_write_rights:
				N=B[_M];C=_B
				if N:C=B[_s];C=qube_684e4bab71.sparta_eb3a26c915(C)
...
def sparta_20a491997e(json_data,user_obj):
	...
	if E.count()>0:
		...
		if F.count()>0:
			...
			if H.is_admin or H.has_write_rights:
				K=B[_M];C=_B
				if K:C=B[_s];C=qube_684e4bab71.sparta_eb3a26c915(C)
	...
	return{_A:1}
def sparta_79fb7e49bc(json_data,user_obj):
	...
	return{_A:1}
def sparta_a50b76245c(user_obj):
	...
	return E
def sparta_69ef32d776(json_data,user_obj):
	try:A=sparta_8658beaa7a(json_data[_AX],user_obj);B=A[_i];return{_A:1,_i:B}
	except:pass
	return{_A:-1}
def sparta_2133adcdc3(json_data,user_obj):
	B=user_obj;A=json_data;C=sparta_8658beaa7a(A[_AX],B);D=C[_i]
	if D:E=C[_K];A[_G]=E.plot_chart_id;F=sparta_6ba60cdb53(A,B);return{_A:1,_F:[A[_F]for A in F[_AA]]}
	return{_A:-1}
def sparta_598d883911(json_data,user_obj):
	F='Error quantstats';C=user_obj;A=json_data;from project.sparta_8f29384cd2.sparta_a6d0a04f89 import qube_b0b6f0ee6e as D;E=A['service']
	if E=='quantstats':
		try:return D.sparta_dc2196574b(A,C)
		except Exception as B:logger.debug(F);logger.debug(traceback.format_exc());logger.debug(F);return{_A:-1,_E:str(B)}
	elif E=='matplotlib':
		try:return D.sparta_9e772ba3b8(A,C)
		except Exception as B:logger.debug('Error matpltlib');logger.debug(traceback.format_exc());return{_A:-1,_E:str(B)}
	return{_A:1}
def sparta_35e8d12261(json_data,user_obj):
...
def sparta_1f3b52aab8(json_data,user_obj):
