...
from project.sparta_8f29384cd2.sparta_ccd7e0da68.qube_1abe2b0e42 import sparta_9de0f4d9c8,sparta_0fae30ed55
from project.logger_config import logger
def sparta_af06d9d69a():keygen_fernet='spartaqube-api-key';key=keygen_fernet.encode(_B);key=hashlib.md5(key).hexdigest();key=base64.b64encode(key.encode(_B));return key.decode(_B)
def sparta_fbc08fd1d2():keygen_fernet='spartaqube-internal-decoder-api-key';key=keygen_fernet.encode(_B);key=hashlib.md5(key).hexdigest();key=base64.b64encode(key.encode(_B));return key.decode(_B)
def sparta_0d72745593(f,str_to_encrypt):data_to_encrypt=str_to_encrypt.encode(_B);token=f.encrypt(data_to_encrypt).decode(_B);token=base64.b64encode(token.encode(_B)).decode(_B);return token
def sparta_b6f40f29f9(api_token_id):
	if api_token_id=='public':
		try:return User.objects.filter(email='public@spartaqube.com').all()[0]
		except:return
	try:
		f_private=Fernet(sparta_fbc08fd1d2().encode(_B));api_key=f_private.decrypt(base64.b64decode(api_token_id)).decode(_B).split('@')[1];user_profile_set=UserProfile.objects.filter(api_key=api_key,is_banned=_D).all()
...
def sparta_fe33ff5f82(user_obj):
...
def sparta_2bf050f749(user_obj,domain_name):api_key=sparta_fe33ff5f82(user_obj);random_nb=str(randint(0,1000));data_to_encrypt=f"apikey@{api_key}@{random_nb}";f_private=Fernet(sparta_fbc08fd1d2().encode(_B));private_encryption=sparta_0d72745593(f_private,data_to_encrypt);data_to_encrypt=f"apikey@{domain_name}@{private_encryption}";f_public=Fernet(sparta_af06d9d69a().encode(_B));public_encryption=sparta_0d72745593(f_public,data_to_encrypt);return public_encryption
def sparta_3a4fb7e403(json_data,user_obj):api_key=sparta_fe33ff5f82(user_obj);domain_name=json_data['domain'];public_encryption=sparta_2bf050f749(user_obj,domain_name);return{_A:1,'token':public_encryption}
def sparta_80df400f34(json_data,user_obj):userprofile_obj=UserProfile.objects.get(user=user_obj);api_key=str(uuid.uuid4());userprofile_obj.api_key=api_key;userprofile_obj.save();return{_A:1}
def sparta_222250ded1():plot_types=sparta_9de0f4d9c8();plot_types=sorted(plot_types,key=lambda x:x['Library'].lower(),reverse=_D);return{_A:1,'plot_types':plot_types}
def sparta_045bb1b6d8(json_data):logger.debug('DEBUG get_plot_options json_data');logger.debug(json_data);plot_type=json_data['plot_type'];plot_input_options_dict=sparta_0fae30ed55(plot_type);plot_input_options_dict[_A]=1;return plot_input_options_dict
def sparta_2b5432a7b2(code):
	tree=ast.parse(code)
...
def sparta_0923cc0737(json_data,user_obj):
	A='errorMsg';user_code_example=json_data['userCode'];resp=_E;error_msg=''
	try:
		logger.debug('EXECUTE API EXAMPLE DEBUG DEBUG DEBUG');api_key=sparta_fe33ff5f82(user_obj);core_api_path=sparta_f7d2e15968()['project/core/api'];ini_code='import os, sys\n';ini_code+=f'sys.path.insert(0, r"{str(core_api_path)}")\n';ini_code+='from spartaqube import Spartaqube as Spartaqube\n';ini_code+=f"Spartaqube('{api_key}')\n";user_code_example=ini_code+'\n'+user_code_example;exec(user_code_example,globals(),locals());last_expression_str=sparta_2b5432a7b2(user_code_example)
...
def sparta_23b5d36c97(json_data,user_obj):
	...
	return{_A:-1}
def sparta_e60737c707(json_data,user_obj):widget_id=json_data['widgetId'];return qube_0fa16a97bc.sparta_e60737c707(user_obj,widget_id)
def sparta_b3071b9c26(json_data,user_obj):
	api_service=json_data['api_service']
	if api_service=='get_status':output=sparta_a335c2297a()
	elif api_service=='get_status_ws':return sparta_692386781f()
	elif api_service=='get_connectors':return sparta_4ee19cb069(json_data,user_obj)
	elif api_service=='get_connector_tables':return sparta_007a28fce2(json_data,user_obj)
	elif api_service=='get_data_from_connector':return sparta_8d5f2b348b(json_data,user_obj)
	elif api_service=='get_widgets':output=sparta_6869c32e21(user_obj)
	elif api_service=='has_widget_id':return sparta_69ef32d776(json_data,user_obj)
	elif api_service=='get_widget_data':return sparta_7954e517a8(json_data,user_obj)
	elif api_service=='get_plot_types':return sparta_9de0f4d9c8()
	return{_A:1,_F:output}
def sparta_a335c2297a():return 1
def sparta_4ee19cb069(json_data,user_obj):
	A='db_connectors';keys_to_retain=['connector_id',_C,'db_engine'];res_dict=qube_0fa16a97bc.sparta_6216f2a23f(json_data,user_obj)
	if res_dict[_A]==1:res_dict[A]=[{k:d[k]for k in keys_to_retain if k in d}for d in res_dict[A]]
	return res_dict
def sparta_007a28fce2(json_data,user_obj):res_dict=qube_0fa16a97bc.sparta_72869398c0(json_data,user_obj);return res_dict
def sparta_8d5f2b348b(json_data,user_obj):res_dict=qube_0fa16a97bc.sparta_a685aca195(json_data,user_obj);return res_dict
def sparta_6869c32e21(user_obj):return qube_0fa16a97bc.sparta_a50b76245c(user_obj)
def sparta_69ef32d776(json_data,user_obj):return qube_0fa16a97bc.sparta_69ef32d776(json_data,user_obj)
def sparta_7954e517a8(json_data,user_obj):return qube_0fa16a97bc.sparta_2133adcdc3(json_data,user_obj)
def sparta_d8622f9caf(json_data,user_obj):date_now=datetime.now().astimezone(UTC);session_id=str(uuid.uuid4());pickled_data=json_data['data'];NewPlotApiVariables.objects.create(user=user_obj,session_id=session_id,pickled_variables=pickled_data,date_created=date_now,last_update=date_now);return{_A:1,'session_id':session_id}
def sparta_be81ecc302():return sparta_9de0f4d9c8()
def sparta_b9e7010e1c():cache.clear();return{_A:1}
def sparta_692386781f():
	global is_wss_valid;is_wss_valid=_D
	try:
		api_path=sparta_f7d2e15968()['api']
...
def sparta_8429480a6c(json_data,user_obj):
	...
	if not user_obj.is_anonymous:
		api_get_widgets=sparta_6869c32e21(user_obj)
...
def sparta_7e4cf39db2(notebook_id):
...
def sparta_126c1b60c4(dashboard_id):
