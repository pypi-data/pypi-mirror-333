...
def sparta_a193612b02():0
def sparta_ea6e64a5db():A='name';B='https://api.github.com/repos/SpartaQube/spartaqube-version/tags';C=requests.get(B,proxies=proxies_dict);D=json.loads(C.text);E=max(D,key=lambda t:parse(t[A]));return E[A]
def sparta_3f60c854ba():A='https://spartaqube-version.pages.dev/latest_version.txt';B=requests.get(A,proxies=proxies_dict);return B.text.split('\n')[0]
def sparta_91a072da66():
...
def sparta_309795cbf8():
	...
	return A
def sparta_d5c3db4502():
	G='res'
	try:
		C=sparta_309795cbf8();A=sparta_3f60c854ba();D=AppVersioning.objects.all();E=datetime.now().astimezone(UTC)
