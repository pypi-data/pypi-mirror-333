...
from project.sparta_8f29384cd2.sparta_a6d0a04f89.qube_0fa16a97bc import sparta_eacd683e50
from project.sparta_8f29384cd2.sparta_ccd7e0da68.qube_1abe2b0e42 import sparta_9de0f4d9c8
...
def sparta_6faeb64289(user_obj):
	...
	return B
def sparta_e592ceb8df(json_data,user_obj):
...
def sparta_0e378947da(json_data,user_obj):
...
def sparta_c2a1e0df07(json_data,user_obj):
	...
	if J.count()==1:
		A=J[J.count()-1];I=A.dashboard_id;N=sparta_6faeb64289(H)
		...
		if O:
			...
			if b:
				G=B[_S]
				if len(G)>0:G=qube_684e4bab71.sparta_eb3a26c915(G);A.password_e=G;A.has_password=_C
			else:A.has_password=_D
			A.save()
	return{_A:1,_H:I}
def sparta_dde3636a44(json_data,user_obj):
	E=json_data;B=user_obj;F=E[_G];C=Dashboard.objects.filter(dashboard_id__startswith=F,is_delete=_D).all()
	if C.count()==1:
		A=C[C.count()-1];F=A.dashboard_id;G=sparta_6faeb64289(B)
	...
	return{_A:1}
def sparta_d8f27e9d30(json_data,user_obj):
	...
	if D.count()==1:
		...
		if E.count()>0:
			...
			if I.is_admin or I.has_write_rights:H=_C
		if H:K=sparta_85e1a912d2(B[_f]);A.main_ipynb_fullpath=K;A.save()
	return{_A:1}
def sparta_a96fab4998(json_data,user_obj):
...
def has_dashboard_access(dashboard_id,user_obj,password_dashboard=_B):
		...
		if A.is_expose_dashboard:
			if A.is_public_dashboard:
				if not K:return{_A:1,_E:A}
					...
					try:
						if qube_684e4bab71.sparta_3e7385f078(A.password_e)==G:return{_A:1,_E:A}
	...
	return{_A:-1}
def sparta_e33fa92206(json_data,user_obj):A=sparta_eacd683e50(json_data,user_obj);return A
