...
from project.sparta_22da4d4238.sparta_0d12b1779d.qube_bd77c02cd4 import sparta_30eedabc8a
ADMIN_EMAIL=conf_settings.ADMIN_EMAIL_TICKET
MAX_TICKETS=conf_settings.MAX_TICKETS
def sparta_a20e005e5a(json_data,user_obj):
...
def sparta_8e607138c6(message,typeCase=0,companyName=_E):
	...
	if E.count()>0:
		...
		A.send()
def sparta_c959666dc6(json_data,user_obj):
	...
	if E.is_staff:
		...
		if B.count()>0:
			for D in B:A=sparta_30eedabc8a(model_to_dict(D));del A[G];A[_F]=naturalday(parser.parse(str(A[_F])));C.append(A)
		return{_A:1,H:C}
	else:
		B=ticket.objects.filter(user=E,is_delete=0,has_user_closed=F).order_by('-date_created');C=[]
		if B.count()>0:
			for D in B:A=sparta_30eedabc8a(model_to_dict(D));del A[G];C.append(A)
		return{_A:1,H:C}
def sparta_fe4f7138d5(json_data,user_obj):
	...
	if F.count()>0:
		...
		if B.count()>0:
			...
			for(K,L)in enumerate(B):
				H=L.user;A=sparta_30eedabc8a(model_to_dict(L));A[_F]=naturalday(parser.parse(str(A[_F])))
				if D==H:A['me']=1
				else:A['me']=0
				if H==J:
					C.append(A)
...
def sparta_3c64d98867(json_data,user_obj):
	...
	if C.count()>0:
		...
		if A.b_send_email and not B.is_staff:A.b_send_email=_B;A.save();sparta_8e607138c6(F,A.type_ticket,_E)
...
def sparta_48a587ed6a(json_data,user_obj):
	...
	return{_A:1}
def sparta_40899db6aa(json_data):
	...
	return{_A:1}
def sparta_355baa4dc6(user_obj):A=ticket.objects.filter(user=user_obj,b_show_user_notification=_C,has_user_closed=_B);return{_A:1,'nbNotifications':A.count()}