...
def sparta_95ca5d2e2e(path):
	try:A=git.Repo(path).git_dir;return _B
	except git.exc.InvalidGitRepositoryError:return _E
def sparta_03f96f382a(json_data,user_obj):
	A=json_data['notebookProjectId'];B,C=qube_f29636d3f5.get_notebookProjectObj(A,user_obj)
	if B is not _F:return _B
	return _E
def sparta_26d826d4af(remoteBranchToTrack):A=Popen(f"git branch -u {remoteBranchToTrack}",stdout=PIPE,stderr=subprocess.STDOUT,bufsize=1,universal_newlines=_B,shell=_B);B=A.stdout.readline();logger.debug('realtime_output 1');logger.debug(B);A=Popen(f"git config push.default upstream",stdout=PIPE,stderr=subprocess.STDOUT,bufsize=1,universal_newlines=_B,shell=_B);logger.debug('realtime_output 2');logger.debug(B)
def sparta_492b8a998f(func):
	def A(json_data,user_obj):
		B=user_obj;A=json_data
		if not sparta_03f96f382a(A,B):return{_A:-1,_D:_K}
		return func(A,B)
	return A
def sparta_90a3f63729(func):
	def A(webSocket,json_data,user_obj):
		B=user_obj;A=json_data
		if not sparta_03f96f382a(A,B):return{_A:-1,_D:_K}
		return func(webSocket,A,B)
	return A
def sparta_b53615b62f(repo,user_obj):
...
def sparta_19c80e73c5(webSocket,json_data,user_obj):
...
def sparta_43e24674ec(json_data,user_obj):
	...
	return{_A:1,'output':'\n'.join(F)}
def sparta_e06e1b9a0b(json_data,user_obj):
	F=user_obj;B=json_data;logger.debug('sqEditorGitAddRemoteOrigin json_data');logger.debug(B);C=B[_C];G=sparta_95ca5d2e2e(C)
	if G:
		H=B['remoteUrl'];I=B['remoteName'];A=git.Repo(C);sparta_b53615b62f(A,F);J=A.create_remote(I,url=H);A=git.Repo(C);sparta_b53615b62f(A,F)
		for D in A.remotes:D.fetch()
		for D in A.remotes:
			if J==D:
				E=D.refs
				if len(E)>0:K=E[len(E)-1];L=os.path.dirname(os.path.realpath(__file__));os.chdir(C);sparta_26d826d4af(K);os.chdir(L)
	return{_A:1}
def sparta_d6eb7aa6b6(json_data,user_obj):
	A=json_data;logger.debug('git_load_available_track_remote json_data');logger.debug(A);B=A[_C];F=sparta_95ca5d2e2e(B)
	if F:
		G=A[_H];C=git.Repo(B);sparta_b53615b62f(C,user_obj);D=[]
		for E in C.remotes:
			if G==E.config_reader.get(_J):
				for H in E.refs:D.append({_I:H.name})
	return{_A:1,'available_branches':D}
def sparta_08a2b2aa9b(json_data,user_obj):
	A=json_data;logger.debug('*******************************************');logger.debug('git_set_track_remote json_data');logger.debug(A);B=A[_C];D=sparta_95ca5d2e2e(B)
	if D:G=A[_H];E=A['remoteBranchToTrack'];C=git.Repo(B);sparta_b53615b62f(C,user_obj);H=C.head.ref.name;F=os.path.dirname(os.path.realpath(__file__));os.chdir(B);sparta_26d826d4af(E);os.chdir(F)
	return{_A:1}
@sparta_492b8a998f
def sparta_6efb3a9752(json_data,user_obj):return{_A:1}
def sparta_c54e29c878(json_data,user_obj):
	B=json_data[_C];C=sparta_95ca5d2e2e(B)
	if C:A=git.Repo(B);sparta_b53615b62f(A,user_obj);D=A.active_branch;E=D.tracking_branch().remote_name;F=A.remote(name=E);F.pull(allow_unrelated_histories=_B)
	return{_A:1}
def sparta_0b73085ad7(json_data,user_obj):
	B=json_data[_C];C=sparta_95ca5d2e2e(B)
	if C:A=git.Repo(B);sparta_b53615b62f(A,user_obj);D=A.active_branch;E=D.tracking_branch().remote_name;F=A.remote(name=f"{E}");F.push()
	return{_A:1}
def sparta_2da4841a85(json_data,user_obj):
	A=json_data[_C];C=sparta_95ca5d2e2e(A)
	if C:
		B=git.Repo(A);sparta_b53615b62f(B,user_obj)
		for D in B.remotes:D.fetch()
	return{_A:1}
def sparta_763b0a8c4d(json_data,user_obj):A=json_data[_C];B=sparta_95ca5d2e2e(A);return{_A:1,_L:B}
def sparta_e832a4c567(json_data,user_obj):
	...
	def B(folder_path):
		j='remotes_arr';i='commits_ahead_arr';h='commits_behind_arr';g='is_ahead';f='is_behind';W='branch';H=folder_path;P=sparta_95ca5d2e2e(H);logger.debug(f"is_git_repository > {H}");logger.debug(P)
		if P:
			A=git.Repo(H);F=[];Z=[]
...
def sparta_98eda057f8(json_data,user_obj):
	H='path';G='file';C=json_data[_C];I=sparta_95ca5d2e2e(C)
	if I:
		B=git.Repo(C);sparta_b53615b62f(B,user_obj);D=[]
	...
	return{_A:1}
def sparta_8efceff16b(json_data,user_obj):
	B=json_data;C=B[_C];D=B['gitMsg'];E=sparta_95ca5d2e2e(C)
	if E:A=git.Repo(C);sparta_b53615b62f(A,user_obj);A.git.add(all=_B);A.git.commit('-m',D)
	return{_A:1}
@sparta_492b8a998f
def sparta_9f48ef915e(json_data,user_obj):return{_A:1}
def sparta_765328f6f9(json_data,user_obj):
	A=json_data;logger.debug('Delete Remoete');logger.debug(A);C=A[_C];E=sparta_95ca5d2e2e(C)
	if E:
		B=git.Repo(C);sparta_b53615b62f(B,user_obj);F=A[_H]
		for D in B.remotes:
			if F==D.config_reader.get(_J):B.delete_remote(D);break
	return{_A:1}
def sparta_910752f738(json_data,user_obj):return{_A:1}
def sparta_d946f0dad8(json_data,user_obj):
	D=json_data;E=D[_C];C=D['newBranchName'];F=sparta_95ca5d2e2e(E)
	if F:
		A=git.Repo(E);sparta_b53615b62f(A,user_obj);B=[A for A in A.branches if A.name==C]
	...
	return{_A:-1,_D:_G}
def sparta_6879397454(json_data,user_obj):
	A=json_data;B=A[_C];D=A['branch2Checkout'];E=sparta_95ca5d2e2e(B)
	if E:
		C=git.Repo(B);sparta_b53615b62f(C,user_obj)
		try:C.git.checkout(D);return{_A:1}
		except Exception as F:return{_A:-1,_D:str(F)}
	return{_A:-1,_D:_G}
def sparta_5fc35595e6(json_data,user_obj):
	B=json_data;C=B[_C];D=B['branch2Merge'];F=sparta_95ca5d2e2e(C)
	if F:
		A=git.Repo(C);sparta_b53615b62f(A,user_obj);E=A.head.ref.name
	...
	return{_A:-1,_D:_G}
def sparta_a8a9ca5ddd(json_data,user_obj):
	D=json_data;A=D[_C];B=D['branch2Delete'];F=sparta_95ca5d2e2e(A)
	if F:
		E=git.Repo(A);sparta_b53615b62f(E,user_obj);G=E.head.ref.name
	...
	return{_A:-1,_D:_G}
def sparta_ac06914a8e(json_data,user_obj):
	F='diff_output';A=json_data;logger.debug('sqEditorGitLoadFilesDiff');logger.debug(A);B=A[_C];G=A['filePath'];H=A['fileType'];C=sparta_95ca5d2e2e(B);logger.debug(_L);logger.debug(C)
	if C:D=git.Repo(B);sparta_b53615b62f(D,user_obj);E=D.git.diff();logger.debug(F);logger.debug(E);return{_A:1,F:E}
	return{_A:-1,_D:_G}