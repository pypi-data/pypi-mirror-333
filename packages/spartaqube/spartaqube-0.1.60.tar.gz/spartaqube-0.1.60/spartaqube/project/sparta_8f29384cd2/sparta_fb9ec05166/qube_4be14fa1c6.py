...
def sparta_6faeb64289(user_obj):
	...
	return B
def sparta_bf979a4c92():B=sparta_21526a056d();A=os.path.join(B,'sq_venv');os.makedirs(A,exist_ok=_C);return A
def sparta_d515bc12df(env_name):
	A=env_name;B=sparta_bf979a4c92()
	if sys.platform=='win32':C=os.path.join(B,A,'Scripts','pip.exe')
	else:C=os.path.join(B,A,'bin','pip')
	return C
def sparta_0857ed96db(json_data,user_obj):A=sparta_bf979a4c92();B=[B for B in os.listdir(A)if os.path.isdir(os.path.join(A,B))];return{_A:1,'available_venvs':B}
def sparta_9624308c11(json_data,user_obj):
	B=sparta_bf979a4c92();A=json_data[_D];C=os.path.join(B,A)
	try:
		subprocess.run([sys.executable,'-m','venv',C],check=_C);D=['cloudpickle']
		for E in D:
			F=sparta_d515bc12df(A);G=f"pip install {E}";H=G.replace('pip',F);I=subprocess.Popen(H,stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=_C)
...
def sparta_24d5a1fd9c(json_data,user_obj):
	...
	L=qube_47219f2032.sparta_c699806bd7(C,B);return{_A:1}
def sparta_51161f9a84(json_data,user_obj):
	B=user_obj;E=json_data[_H];C=Dashboard.objects.filter(dashboard_id__startswith=E,is_delete=_B).all()
	if C.count()==1:
		A=C[C.count()-1];E=A.dashboard_id;F=sparta_6faeb64289(B)
	...
	return{_A:1}
def sparta_dc7d375192(json_data,user_obj):
	C=user_obj;A=json_data;D=A[_H];E=A[_J]
	if str(D)!=_F:sparta_51161f9a84(A,C)
	if str(E)!=_F:sparta_24eb26ee27(A,C)
	F=sparta_bf979a4c92();G=A[_D];H=os.path.join(F,G)
...
def sparta_6ceec3e50e(json_data,user_obj):
	B=json_data[_D];C=sparta_d515bc12df(B);A=[]
	try:
...
def sparta_f94019ab9c(env_name,project_path):
	A=os.path.join(project_path,'requirements.txt');B=sparta_d515bc12df(env_name)
	try:
		with open(A,'w')as C:subprocess.run([B,'freeze'],stdout=C)
		return{_A:1}
	except Exception as D:return{_A:-1,_E:str(D)}
def sparta_8e16165b4f(json_data,user_obj):
	...
	if D.count()==1:
		A=D[D.count()-1];F=A.developer_id;G=sparta_6faeb64289(C)
	...
	L=qube_47219f2032.sparta_c699806bd7(B,C);return{_A:1}
def sparta_24eb26ee27(json_data,user_obj):
	B=user_obj;E=json_data[_J];C=Developer.objects.filter(developer_id__startswith=E,is_delete=_B).all()
	if C.count()==1:
		A=C[C.count()-1];E=A.developer_id;F=sparta_6faeb64289(B)
	...
	return{_A:1}
def sparta_6967f08227(json_data,user_obj):
...
def sparta_dc7780ed58(json_data,user_obj):
	...
	return{_A:1}
def sparta_89351e9cc1(json_data,user_obj):
	C=json_data;B=user_obj;from project.sparta_8f29384cd2.sparta_2283960592 import qube_47219f2032 as J;F=C[_L];D=Kernel.objects.filter(kernel_manager_uuid__startswith=F,is_delete=_B).all()
	if D.count()==1:
		A=D[D.count()-1];F=A.kernel_manager_uuid;G=sparta_6faeb64289(B)
	...
	M=J.sparta_c699806bd7(C,B);return{_A:1}
def sparta_0b2e776df7(json_data,user_obj):
	B=user_obj;E=json_data[_L];C=Kernel.objects.filter(kernel_manager_uuid__startswith=E,is_delete=_B).all()
	if C.count()==1:
		A=C[C.count()-1];E=A.kernel_manager_uuid;F=sparta_6faeb64289(B)
	...
	return{_A:1}
def sparta_0aadfc023b(json_data,user_obj):
	B=user_obj;A=json_data;logger.debug(_O);logger.debug(A);D=A[_H];E=A[_J];F=A[_K];G=A[_L]
	if str(D)!=_F:return sparta_a9de9ba863(A,B)
	if str(E)!=_F:return sparta_bbbf29327a(A,B)
	if str(F)!=_F:return sparta_6c58d25cb8(A,B)
	if str(G)!=_F:return sparta_0ce4bd3a10(A,B)
	H=A[_D];C=A[_G]
	if A[_P]:C=os.path.join(C,_M)
	return sparta_f94019ab9c(H,C)
def sparta_a9de9ba863(json_data,user_obj):
	C=user_obj;B=json_data;F=B[_H];J=B[_D];K=B[_G];D=Dashboard.objects.filter(dashboard_id__startswith=F,is_delete=_B).all()
	if D.count()==1:
		A=D[D.count()-1];F=A.dashboard_id;G=sparta_6faeb64289(C)
		...
		if E.count()>0:
			...
			if I.is_admin or I.has_write_rights:H=_C
		if H:return sparta_f94019ab9c(J,K)
	return{_A:1}
def sparta_bbbf29327a(json_data,user_obj):
	...
	if E.count()==1:
		A=E[E.count()-1];G=A.developer_id;H=sparta_6faeb64289(C)
		...
		if I:return sparta_f94019ab9c(K,D)
	return{_A:1}
def sparta_6c58d25cb8(json_data,user_obj):
	...
	if D.count()==1:
		...
		if H:return sparta_f94019ab9c(J,K)
	return{_A:1}
def sparta_0ce4bd3a10(json_data,user_obj):
	A=json_data;from project.sparta_8f29384cd2.sparta_2283960592 import qube_47219f2032 as B;C=A[_L];D=A[_D];E=A[_G];F=B.sparta_8a41a7d617(user_obj,C)
	if F is _I:return sparta_f94019ab9c(D,E)
	return{_A:1}
def sparta_5f07e8560d(json_data,user_obj):
	B=json_data;logger.debug(_O);logger.debug(B);E=B[_D];C=os.path.join(sparta_bf979a4c92(),E);logger.debug('venv_path');logger.debug(C);A=sparta_85e1a912d2(B[_G])
