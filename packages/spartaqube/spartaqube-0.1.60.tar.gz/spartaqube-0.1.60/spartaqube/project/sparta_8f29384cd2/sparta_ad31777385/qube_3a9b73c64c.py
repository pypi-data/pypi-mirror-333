...
def sparta_85e1a912d2(path):A=Path(path).resolve();return str(A).replace('\\','\\\\')
def sparta_89f4391784():
	B='virtual_env_name';C=hasattr(sys,'real_prefix')or hasattr(sys,'base_prefix')and sys.base_prefix!=sys.prefix
	if C:A=sys.prefix;D=os.path.basename(A);return{_A:True,B:D,_B:sparta_85e1a912d2(A)}
	else:return{_A:False,B:None,_B:None}
def sparta_12a7b95afa(bat_file_path,shortcut_path,icon_path):
...
def sparta_d4a3515bbd(shell_script_path,shortcut_name,icon_path):
...
def sparta_c0f90e835c(shortcut_name,shell_script_path,icon_path):
...
def sparta_a1794bc1d6(json_data,user_obj):
	S='SpartaQube Launcher';R='SpartaQube.sh';Q='SpartaQube.lnk';L='errorMsg';F='res';M=qube_2d0baaf631.sparta_3a4fb7e403(json_data,user_obj)
	if M[F]==1:
		T=M['token'];B=sparta_f7d2e15968()['api'];A=sparta_85e1a912d2(os.path.join(B,'spartaqube_exec.py'))
		if os.path.exists(A):
			try:os.remove(A)
...
"""
		...
		G=sparta_85e1a912d2(os.path.join(B,'spartaqube_launcher.py'))
		if os.path.exists(G):
			try:os.remove(G)
			except Exception as D:return{F:-1,L:str(D)}
		I=sparta_89f4391784();logger.debug('env_info');logger.debug(I)
		if I[_A]:E=I[_B];V=sparta_85e1a912d2(os.path.join(f"{E}",'bin','python')if os.name!='nt'else os.path.join(f"{E}",'Scripts','python.exe'));N=sparta_85e1a912d2(os.path.join(f"{E}",'bin/activate')if os.name!='nt'else os.path.join(E,'Scripts/activate.bat'));O=f'''
import os
import sys
import subprocess
import platform
...
'''
		...
		W=sparta_f7d2e15968()['spartaqube_path'];J=sparta_85e1a912d2(os.path.join(W,'static','assets','images','Icon','favicon512.ico'));P=platform.system()
		if P==_C:
			C=sparta_85e1a912d2(os.path.join(B,'SpartaQube.bat'));K=sparta_85e1a912d2(os.path.join(B,Q))
			if os.path.exists(K):
				try:os.remove(K)
				except Exception as D:return{F:-1,L:str(D)}
			sparta_12a7b95afa(C,K,J);X=os.path.join(os.path.expanduser('~'),'Desktop');Y=os.path.join(X,Q);sparta_12a7b95afa(C,Y,J)
		elif P=='Darwin':C=sparta_85e1a912d2(os.path.join(B,R));sparta_d4a3515bbd(shell_script_path=C,shortcut_name=S,icon_path=J)
		else:C=sparta_85e1a912d2(os.path.join(B,R));sparta_c0f90e835c(shell_script_path=C,shortcut_name=S,icon_path='/path/to/your/icon.png')
	return{F:1}