...
from project.sparta_22da4d4238.sparta_0d12b1779d.qube_bd77c02cd4 import sparta_5c73022983,sparta_88a4fd3d4b,sparta_260ede11ee
from project.sparta_8f29384cd2.sparta_82d887a6fc.qube_83d9f0c2bd import sparta_0e339bc7d7
from project.logger_config import logger
def sparta_5d98c626f3(json_data,user_obj):
...
def sparta_144614bfd6(message,typeCase=0,companyName=_G):
	...
	if C.count()>0:
		...
		A.send()
def sparta_bd59b16ca3(json_data,userObj):
	...
	return B
def sparta_4b8e4de6bc(json_data,userObj):
	...
	return A
def sparta_ea106a177b(json_data,userObj):
	D=json_data;F=D['old_spartaqube_code'];G=D['new_spartaqube_code']
	if not sparta_0e339bc7d7(F):return{_A:-1,_B:'Invalid current code'}
...
def sparta_fbc08fd1d2():B='spartaqube-reset-password';A=B.encode(_D);A=hashlib.md5(A).hexdigest();A=base64.b64encode(A.encode(_D));return A.decode(_D)
def sparta_6dae6f66ab(json_data):
	A=json_data;C=A[_F];E=A[_H];B=A[_I];F=A[_R]
	if not sparta_260ede11ee(E):return{_A:-1,_B:_J}
...
def sparta_93e230712c(json_data):
	A=json_data;E=A[_E];B=A[_F];F=A[_H];G=sparta_5c73022983(E)
	if G[_A]!=1:return{_A:-1,_B:_T}
	if not sparta_88a4fd3d4b(F):return{_A:-1,_B:_J}
...
def sparta_d17acf8b70(request,json_data):
	A=json_data;F=A[_E];D=A[_F];G=A[_H];H=A[_U];E=A[_I];I=A[_R];J=sparta_5c73022983(F)
	if J[_A]!=1:return{_A:-1,_B:_T}
	if not sparta_88a4fd3d4b(G):return{_A:-1,_B:_J}
