...
from seavad.vad_analyzer import VADAnalyzer, VADParams
from seavad.config import settings
...
try:
    ONNX_SESSION = onnxruntime.InferenceSession(settings.VAD_MODEL_PATH, sess_options=opts)
    ONNX_CPU_SESSION = onnxruntime.InferenceSession(
        settings.VAD_MODEL_PATH, providers=["CPUExecutionProvider"], sess_options=opts
    )
except Exception as e:
    logger.warning(
        f"Failed to init global onnx session with {e}. Possibly VAD_MODEL_PATH environment variable not set or not found "
        f"at {repr(settings.VAD_MODEL_PATH)}. Set VAD_MODEL_PATH before importing seavad to share the same onnx session "
        "between multiple SeaVAD instances."
    )
    ONNX_SESSION = None
...
class SileroOnnxModel:
    def __init__(self, force_onnx_cpu: bool = True, vad_model_path: str = ""):
        global np
        import numpy as np
        try:
            if force_onnx_cpu and "CPUExecutionProvider" in onnxruntime.get_available_providers():
                if not ONNX_CPU_SESSION:
                    self.session = onnxruntime.InferenceSession(
                        vad_model_path, providers=["CPUExecutionProvider"], sess_options=opts
                    )
                else:
                    self.session = ONNX_CPU_SESSION
            else:
                if not ONNX_SESSION:
                    self.session = onnxruntime.InferenceSession(vad_model_path, sess_options=opts)
                else:
                    self.session = ONNX_SESSION
        except Exception as e:
            logger.error(f"{e} when init SileroOnnxModel", exc_info=True)
            raise FileNotFoundError(
                "Either set VAD_MODEL_PATH environment variable to reuse a global onnx session "
                "or provide vad_model_path when initializing each SeaVAD instance."
            )
        self.reset_states()
...
class SileroVADAnalyzer(VADAnalyzer):
    def __init__(self, *, sample_rate: int = 16000, params: VADParams = VADParams()):
        ...
        self._model = SileroOnnxModel(force_onnx_cpu=True)
