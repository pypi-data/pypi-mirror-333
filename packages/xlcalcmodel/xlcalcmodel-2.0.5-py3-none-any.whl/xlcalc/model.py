...
class Cell:
    def __init__(self, address_obj: CellAddress, value=None, formula=None):
...
##        local_key = CellAddress.from_string(local_key).to_string(include_sheet=False)
    ...
    def resolve_range(self, sheet_name: str, rng_str: str):
        ...
        if ':' not in rng_str:
            ...
            #print(f"DEBUG => single-cell range => {addr}")
            return [addr]
        ...
        for r in range(min_row, max_row + 1):
            for c in range(min_col, max_col + 1):
                ...
                addr_obj = CellAddress(
                    sheet=sheet_name,
                    column=col_letters,
                    row=r
                )
                addresses.append(addr_obj)
        #print(f"DEBUG => multi-cell => {addresses}")
        return addresses
