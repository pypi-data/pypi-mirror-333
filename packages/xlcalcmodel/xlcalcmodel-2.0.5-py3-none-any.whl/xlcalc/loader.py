    ...
    for sht in wb.sheets:
        ...
        for row in full_range.rows:
            for cell in row:
                ...
                # *** FIX: If the cell has a formula, do not preserve its computed value ***
                #THESE ARE NEEDED TO SEED THE ITERATIVE CALCULATIONS
                #if cell_formula is not None:
                #    cell_value = None

                # Only add a cell if it has a value or a formula.
                if cell_value is not None or cell_formula is not None:
