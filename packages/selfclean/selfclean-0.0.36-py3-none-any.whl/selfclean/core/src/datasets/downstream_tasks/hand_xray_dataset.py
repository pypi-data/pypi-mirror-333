...
class HandXrayDataset(BaseDataset):
    """Hand X-Ray Dataset from Kaggle: https://www.kaggle.com/datasets/antonbudnychuk/hand-xray."""
    ...
    def __init__(
        self,
        dataset_dir: Union[str, Path] = "data/Hand_XRay_Kaggle/",
        transform=None,
        val_transform=None,
        ...
        **kwargs,
        ...
        for img_name, img_path in imgs_path_dict.items():
            ...
            split = Path(img_path).parents[1].name
            s_item = pd.Series([img_name, img_path, label, split])
            self.meta_data = pd.concat([self.meta_data, pd.DataFrame(s_item).T])
        self.meta_data.columns = ["img_name", self.IMG_COL, self.LBL_COL, "split"]
