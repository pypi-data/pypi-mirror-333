...
# v10-3 - Cross-attention with modality-specific context layers added
class CrossAttention(nn.Module):
    ...
    def forward(self, image_features, text_features):
        ...
        image_ctx = self.image_context(image_norm)
        text_ctx = self.text_context(text_norm)

        # Image -> Text direction attention (using context)
        q_img = split_heads(self.to_q(image_ctx))
        k_txt = split_heads(self.to_k(text_ctx))
        v_txt = split_heads(self.to_v(text_ctx))
        ...
        # Text -> Image direction attention (using context)
        q_txt = split_heads(self.to_q(text_ctx))
        k_img = split_heads(self.to_k(image_ctx))
        v_img = split_heads(self.to_v(image_ctx))
