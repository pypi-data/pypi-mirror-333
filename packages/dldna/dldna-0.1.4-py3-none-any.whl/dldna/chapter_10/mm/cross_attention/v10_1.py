...
# v10_1 - Cross-attention with modality-specific Q/K/V projections added
class CrossAttention(nn.Module):
    def __init__(self, dim, num_heads=8, dropout=0.1, ff_dim=None):
        ...
        self.image_to_q = nn.Linear(dim, dim)
        self.image_to_k = nn.Linear(dim, dim)
        self.image_to_v = nn.Linear(dim, dim)

        self.text_to_q = nn.Linear(dim, dim)
        self.text_to_k = nn.Linear(dim, dim)
        self.text_to_v = nn.Linear(dim, dim)
    ...
    def forward(self, image_features, text_features):
        ...
        # Image -> Text direction attention (using modality-specific Q/K/V)
        q_img = split_heads(self.image_to_q(image_norm))
        k_txt = split_heads(self.text_to_k(text_norm))
        v_txt = split_heads(self.text_to_v(text_norm))
        ...
        # Text -> Image direction attention (using modality-specific Q/K/V)
        q_txt = split_heads(self.text_to_q(text_norm))
        k_img = split_heads(self.image_to_k(image_norm))
        v_img = split_heads(self.image_to_v(image_norm))
