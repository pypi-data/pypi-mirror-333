...
from datasets import load_dataset, load_dataset_builder  # load_dataset_builder 추가
import random
import math
...
#############################################
def generate_text(model, prompt, tokenizer, max_length, device, temperature=1.0, do_sample=True, top_k=0, top_p=0.9):
    model.eval()
...
def run_generation_examples(model, tokenizer, device):
    ...
    for prompt in prompts:
        generated = generate_text(model, prompt, tokenizer, max_length=100, device=device, temperature=0.8, top_k=50, top_p=0.95)
...
if __name__ == "__main__":
    ...
    # Load BookCorpus dataset (0.01% subset)
    # 1. Get dataset info to find out total number of samples
    builder = load_dataset_builder("bookcorpus", trust_remote_code=True)
    total_train_samples = builder.info.splits["train"].num_examples
    print(f"Total training samples: {total_train_samples}")

    # 2. Calculate the number of samples for 0.01%
    num_samples = int(total_train_samples * 0.0001)  # 0.01% = 0.0001

    # 3. Load the dataset with the calculated number of samples
    dataset = load_dataset("bookcorpus", split=f"train[:{num_samples}]", trust_remote_code=True)
    dataset = dataset.map(simple_clean, num_proc=24)
    texts = dataset["text"]
    print(f"Loaded {len(texts)} samples from BookCorpus (0.01% subset).")
