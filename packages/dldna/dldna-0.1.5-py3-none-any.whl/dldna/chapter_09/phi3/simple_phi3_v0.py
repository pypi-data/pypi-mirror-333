...
class PhiMiniAttention(nn.Module):
    def __init__(self, config):
        ...
        # Query, key, value projection layers (Phi-2 uses bias=True)
    ...
    #         # query (new token) is [past_len, past_len + q_len)
    #         position_ids_q = torch.arange(past_len, past_len + q_len, dtype=torch.long, device=query_states.device).unsqueeze(0)
    #         # key (whole) is [0, past_len + q_len)
    #         position_ids_k = torch.arange(0, past_len + q_len, dtype=torch.long, device=query_states.device).unsqueeze(0)
    ...
    #     if past_key_value is not None:
    #         for i, (past_kv, new_kv) in enumerate(zip(past_key_value, [key_states, value_states])):
    #             if past_kv.size(-1) != new_kv.size(-1):
    #                 raise RuntimeError(f"Size mismatch in layer {i}: past_kv.size(-1)={past_kv.size(-1)}, new_kv.size(-1)={new_kv.size(-1)}")

    #     # # # past_key_value processing: Combine existing cache and newly calculated key, value
    #     # if past_key_value is not None:
    ...
    #     # past_key_value handling: combine existing cache with newly computed key_states and value_states
    #     if past_key_value is not None:
    #         # ensure that the last dimension of the existing cache matches the last dimension of the newly computed tensors
    #         if past_key_value[0].size(-1) != key_states.size(-1):
    #             raise RuntimeError("Key tensor size mismatch between past_key_values and new key_states.")
    #         if past_key_value[1].size(-1) != value_states.size(-1):
    #             raise RuntimeError("Value tensor size mismatch between past_key_values and new value_states.")
    ...
    #             # Assume attention_mask is (B, q_len, kv_seq_len)
    #             combined_mask = (attention_mask + causal_mask.unsqueeze(0)).unsqueeze(1)  # shape: (B, 1, q_len, kv_seq_len)
    #         else:
    #             combined_mask = causal_mask.unsqueeze(0).unsqueeze(1)  # (1, 1, q_len, kv_seq_len)
    ...
    #             kv_seq_len_new = key_states.size(-2)
    #             causal_mask = torch.triu(torch.full((q_len_new, kv_seq_len_new), float('-inf')), diagonal=1).to(query_states.device)
    #             # Modification: Remove unsqueeze(1) so that the final shape of the mask is (B, q_len, kv_seq_len).
...
def apply_rotary_pos_emb_single(x, cos, sin, position_ids):
    # cos, sin: shape (1, 1, max_seq_len, dim)
    # x: shape (B, *, seq_len, dim)
    # position_ids: shape (B, seq_len)
    # Squeeze to (max_seq_len, dim), then index with position_ids to get (B, seq_len, dim),
    # and apply unsqueeze(1) to match dimensions.
