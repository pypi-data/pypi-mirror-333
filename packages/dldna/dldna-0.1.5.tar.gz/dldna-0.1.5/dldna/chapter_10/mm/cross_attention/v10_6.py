...
# v10-6: Cross-attention with a contrastive learning-based attention mechanism, applying contrastive learning only to the final output
class CrossAttention(nn.Module):
    ...
    def forward(self, image_features, text_features):
        ...
        image_contrast = self.contrast_proj(image_attended)
        text_contrast = self.contrast_proj(text_attended)

        # Normalized contrastive learning features
        image_norm = F.normalize(image_contrast, dim=-1)
        text_norm = F.normalize(text_contrast, dim=-1)

        # Apply contrastive learning similarity constraint
        similarity = torch.matmul(image_norm, text_norm.transpose(-2, -1)) * 0.1
        image_attended = image_attended + similarity
        text_attended = text_attended + similarity.transpose(-2, -1)
