...
# v10-2 - Cross-attention with added cross-gate mechanism
class CrossAttention(nn.Module):
    def __init__(self, dim, num_heads=8, dropout=0.1, ff_dim=None):
        ...
        # Gate normalization layer
        self.gate_norm = nn.LayerNorm(dim * 2)  # Adapted to the dimension of the concatenated features (dim * 2)
        self.gate_scale = nn.Parameter(torch.ones(1) * 0.1)  # Gate scale
    ...
    def forward(self, image_features, text_features):
        ...
        # Apply cross-gate
        image_gate_input = self.gate_norm(torch.cat([image_attended, image_features], dim=-1))
        text_gate_input = self.gate_norm(torch.cat([text_attended, text_features], dim=-1))

        image_gate_value = self.image_gate(image_gate_input)
        text_gate_value = self.text_gate(text_gate_input)


        image_attended = image_attended * image_gate_value
        text_attended = text_attended * text_gate_value
