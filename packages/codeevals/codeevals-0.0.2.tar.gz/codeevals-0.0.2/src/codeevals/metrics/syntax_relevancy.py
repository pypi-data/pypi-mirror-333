...
from codeevals.results.syntax_relevancy_result import SyntaxRelevancyResult
from codeevals.templates.syntax_relevancy_template import SyntaxRelevancyTemplate


class SyntaxRelevancy(BaseMetric, ABC):
    ...
    def compute(self, llm_inputs: List[LLMInput], *args, **kwargs) -> Any:
        eval_results: EvaluationResult = kwargs.get("eval_results")
        for llm_input in llm_inputs:
            _template = SyntaxRelevancyTemplate.generate_code_syntax_relevancy_percentage_template(ground_truth_code=llm_input.ground_truth,
            ...
            if self.model is not None and isinstance(self.model, CustomModel):
                ...
                if response.choices is not None and len(response.choices) > 0:
                    ...
                    syntax_relevancy_result = SyntaxRelevancyResult(score=resp["syntax_relevancy_score"], reason=resp["reason"])
                    eval_results.push(llm_result=syntax_relevancy_result)
            else:
                raise Exception("model cannot be None")

        return eval_results.show()