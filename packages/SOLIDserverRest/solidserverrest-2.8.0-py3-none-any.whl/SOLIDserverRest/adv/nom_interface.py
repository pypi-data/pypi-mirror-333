...
class NomPort(ClassParams):
    ...
    def __init__(self,
        ...
        if not name:
            raise SDSError(
                message=f"name is required for network port")
...
class NomInterface(ClassParams):
    ...
    # -------------------------------------
    def set_ip(self, ip: ipaddress):
        if isinstance(ip, str):
            try:
                ip = ipaddress.ip_address(ip)
            except ValueError:
                raise SDSError(
                    message=f"interface {self.name} requires a valid IP address, even as string")
    ...
    def create(self):
        ...
        for _tmpifid in list(self.port.aInterfaces):
            ...
            if _tmpif.vlan != self.vlan:
                ...
                if (_tmpif.name != self.name
                    or _tmpif.ipv4 != self.ipv4
                        or _tmpif.ipv6 != self.ipv6):
                    self.myid = _tmpifid
                    self.update()
                    return
        ...
        if self.ipv6:
            params['nomiface_hostaddr6'] = str(self.ipv6)
