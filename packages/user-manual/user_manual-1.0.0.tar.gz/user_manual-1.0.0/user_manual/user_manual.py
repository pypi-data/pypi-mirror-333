...
#             ]
    ...
    for num, input_text in enumerate(chat_history):
        ...
        if num <= 1:
            pyautogui.write(' Note: Please write the level 1 headings of user manual as: "number.number text". You can pick it from the first item of the list of OCR Content', interval=0.01)
        else:
            pyautogui.write(' "Note: Please dont write the level 1 headings again."', interval=0.01)
...
def get_headings(image_path, ocr):
    new_ocr = []
...
def create_user_manual():
    ...
    for idx, image_path in enumerate(images):
        if idx == 0:
            headings_ocr_content = get_headings(image_path, ocr)
            headings = headings_ocr_content[0]
