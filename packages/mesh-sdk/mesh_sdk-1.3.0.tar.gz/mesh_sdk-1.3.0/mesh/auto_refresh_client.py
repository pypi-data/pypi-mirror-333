...
class AutoRefreshMeshClient:
    ...
    def authenticate(self) -> bool:
        ...
        This ensures we exclusively use the backend-managed authentication flow
        where all sensitive operations happen securely on the backend.
        ...
        try:
            ...
            self.logger.info("Creating client with backend-managed authentication")
            self.client = MeshClient()  # No direct auth_token, will use backend auth
            
            # The first API call will trigger backend-driven authentication if needed
            self.logger.info("Authentication will happen automatically when needed")
            return True
    ...
    def create_client(self) -> None:
        """
        Create a MeshClient with the current token using the backend-managed authentication flow.
        
        Note: This method maintains backward compatibility but internally relies on the
        backend-managed authentication flow. The access token is still passed to maintain
        compatibility, but the MeshClient now uses it in a deprecated manner with proper warnings.
        """
        if self.token_data and "access_token" in self.token_data:
            # Create a new MeshClient with the access token
            # Note: The direct auth_token parameter is deprecated but maintained for compatibility
            # MeshClient will issue appropriate warnings when this is used
