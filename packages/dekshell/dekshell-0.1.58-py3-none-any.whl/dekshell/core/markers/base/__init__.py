...
class MarkerBase:
    ...
    trans_marker_cmd_call = cmd_call_prefix
    trans_marker_decode = '//'
    trans_marker_encode = '/'
    trans_marker_env = '$$'
    ...
    @classmethod
    def cmd_call_parse(cls, context, s):
        argv = cls.split(s)
        func = argv[0]
    ...
    @classmethod
    def cmd_call(cls, context, s):
        func, args, kwargs = cls.cmd_call_parse(context, s)
    ...
    def translate(self, context, s):
        ...
        def handler_core(expression, ignore, marker):
            ...
            elif key.startswith(self.trans_marker_cmd_call):
                return self.cmd_call(context, expression)
    ...
    def eval_mixin(cls, context, expression):
        if expression.startswith(cmd_call_prefix):
            return cls.cmd_call(context, expression[len(cmd_call_prefix):])
        else:
