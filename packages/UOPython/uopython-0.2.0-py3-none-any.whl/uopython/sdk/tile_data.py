...
if not TileData.loaded:
    TileData.load()
    print("TileData loaded")


def item_data(index):
    if index < 0 or index >= len(TileData.item_data):
        raise IndexError("Index out of range for item_data.")
    return TileData.item_data[index]


def land_data(index):
    if index < 0 or index >= len(TileData.land_data):
        raise IndexError("Index out of range for land_data.")
