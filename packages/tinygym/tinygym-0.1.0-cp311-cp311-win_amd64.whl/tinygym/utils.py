...
def play(env, model=None, playable=False, steps=None, fps=4, obs='obs', dump=False, with_data=True, idx=0, **kwargs):
    ...
    for i in range((10000 if playable else 64) if steps is None else steps):
        ...
        if model is not None:
            o = Tensor(env.obs)
            with Tensor.test(): acts, logp, entropy, val, state = model(o, state=state, with_entropy=True)
            data.update({'model act': acts[idx].numpy(), 'logp': logp[idx].numpy(), 'entropy': entropy[idx].numpy(),
                         'value': val[idx].numpy()})
            acts = acts.numpy()
