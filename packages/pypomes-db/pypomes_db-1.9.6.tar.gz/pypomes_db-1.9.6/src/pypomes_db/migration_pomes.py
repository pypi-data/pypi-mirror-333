...
def db_migrate_data(errors: list[str] | None,
    ...
    *batch_size_in* and *offset_count* are ignored.
    ...
    if curr_source_conn and curr_target_conn:
        ...
        # definite whether it is possible to retrieve tuples with offsets and in batches
        if not orderby_clause and (batch_size_in or offset_count):
            if source_engine == DbEngine.ORACLE:
                orderby_clause = "ROWID"
            else:
                batch_size_in = 0
                offset_count = 0
                if logger:
                    logger.warning(msg=f"No 'ORDER BY' defined for table '{source_table}', "
                                       "so 'batch_size_in' and 'offset_count' were disregarded")
