...
from .. import device
from ..sampling import sampling_pde
...
class NeuralGalerkin_x:
    def __init__(self, pde_x, sampler_x, sampler_mu, network, **kwargs):
        ...
        self.nb_params = len(torch.nn.utils.parameters_to_vector(self.network.parameters()))
        print("/////////////// Neural Galerkin method ///////////////")
        print("///// Time scheme: ",self.scheme)
        print("///// Type of initilization: ",self.type_init)
        print("///// Activation of the subsampling: ",self.subsample_bool," and activation of Least Square solver",self.ls_bool)
    ...
    def compute_initial_data(self, 
        ...
        if self.type_init == 1:
            ...
            self.projector = projector_training.Projector_x(
                self.network, self.sampler, w0=w0, learning_rate=lr_init
            )
    ...
    def compute_model(self):
        ...
        """
        self.m_jac = self.jacobian(self.x_no_grad, self.mu_no_grad)
        if not self.ls_bool:
            self.M = torch.einsum("bjs,bjr->sr", self.m_jac, self.m_jac) / self.N + self.eye_matrix
    ...
    def compute_error_in_time(self, time, sol_exact):
        ...
        err_abs = (torch.sum((w_pred["w"] - w_exact) ** 2,dim=0).detach().cpu()/ N)**0.5
    ...
    def euler_exp_ls(self, dt):
        ...
        if self.subsample_bool:
            ...
            J=J[:,S_t]
        delta_t =torch.linalg.lstsq(J,b,driver="gelsd").solution
        if self.subsample_bool:
    ...
    def time_loop(self, 
        ...
        while time < T:
            ...
            time = time + dt
            if sol_exact is not None and nt%5==0:
    ...
    def plot(self, n_visu=20000, sol_exact=None, mu=None):
        ...
        if self.pde.dimension_x == 1:
            if sol_exact is not None:
                ...
                print(
                    "erreur ",
                    (torch.sum((w_pred["w"] - w_exact) ** 2,dim=0).detach().cpu()
                    / n_visu)**0.5 /(torch.sum(w_exact ** 2,dim=0).detach().cpu()
                    / n_visu)**0.5,
                )
        ...
        if self.pde.dimension_x == 2:
            ...
            im = ax[1, 0].scatter(
                ...
                x2.detach().cpu().numpy(),
                s=10,
                c=w_pred["w"][:, 0].detach().cpu().numpy()-w_exact[:, 0].detach().cpu().numpy(),
                cmap="gist_ncar",
                label="error)",
            )
            ...
            if sol_exact is not None:
                ax[1,1].plot(self.errors)
                ax[1,1].set_title(" error dynamic")
                ax[1,1].legend()
            plt.show()
