...
def authenticate_browser(
    ...
    # Try a range of ports starting from the specified one
    for try_port in range(port, port + 10):
        try:
            server = socketserver.TCPServer(("localhost", try_port), CallbackHandler)
            ...
            if not CallbackHandler.server_ready.wait(timeout=5):
                logger.error(f"Failed to start authentication server on port {try_port}")
                server.shutdown()
                server.server_close()
                continue
            
            logger.info(f"Auth server started on port {try_port}")
    ...
    if server is None:
        logger.error("Failed to start authentication server on any port")
        return None
    ...
    try:
        ...
        if not browser_opened:
            logger.warning("Failed to open browser automatically. Please manually visit this URL to authenticate:")
...
def authenticate(
    ...
    client_secret = client_secret or auth0_config.get("client_secret") or AUTH0_CLIENT_SECRET
    audience = audience or auth0_config.get("audience") or AUTH0_AUDIENCE
    port = port or int(get_config("AUTH0_CALLBACK_PORT", "45678"))  # Use a less common port
