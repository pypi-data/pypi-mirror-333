...
def _get_client() -> MeshClient:
    ...
    if not token_data or not is_token_valid(token_data):
        ...
        # Try device code flow first (doesn't require a local server)
        try:
            logger.info("Attempting device code authentication")
            ...
            if not token_data:
                logger.warning("Device code authentication failed")
            ...
            print("\nAuthentication failed. Please run 'mesh-auth' from the command line.")
            print("If you're on a server without a browser, use: mesh-auth --headless")
            print("Make sure your Auth0 application is configured for device code flow.")
            print("See the README for configuration instructions.")
            raise RuntimeError(f"Authentication failed. Please run 'mesh-auth' from the command line.")
